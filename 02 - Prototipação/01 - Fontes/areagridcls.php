...
$area_grid = NULL; // Initialize page object first

class carea_grid extends carea {
	...
	// Page object name
	var $PageObjName = 'area_grid';

	// Grid form hidden field names
	var $FormName = 'fareagrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["area"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function Page_Main() {
		...
		// Load master record
		if ($this->CurrentMode <> "add" && $this->GetMasterFilter() <> "" && $this->getCurrentMasterTable() == "tparea") {
			global $tparea;
			$rsmaster = $tparea->LoadRs($this->DbMasterFilter);
			$this->MasterRecordExists = ($rsmaster && !$rsmaster->EOF);
			if (!$this->MasterRecordExists) {
				$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record found
				$this->Page_Terminate("tparealist.php"); // Return to master page
			} else {
				$tparea->LoadListRowValues($rsmaster);
				$tparea->RowType = EW_ROWTYPE_MASTER; // Master row
				$tparea->RenderListRow();
				$rsmaster->Close();
			}
		}
	...
	function SetupKeyValues($key) {
		...
		if (count($arrKeyFlds) >= 1) {
			$this->nu_area->setFormValue($arrKeyFlds[0]);
			if (!is_numeric($this->nu_area->FormValue))
				return FALSE;
		}
		return TRUE;
	}
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->nu_area->CurrentValue;
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_no_area") && $objForm->HasValue("o_no_area") && $this->no_area->CurrentValue <> $this->no_area->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_nu_tpArea") && $objForm->HasValue("o_nu_tpArea") && $this->nu_tpArea->CurrentValue <> $this->nu_tpArea->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_nu_pessoaResp") && $objForm->HasValue("o_nu_pessoaResp") && $this->nu_pessoaResp->CurrentValue <> $this->nu_pessoaResp->OldValue)
			return FALSE;
	...
	function LoadSortOrder() {
		...
		if ($sOrderBy == "") {
			if ($this->SqlOrderBy() <> "") {
				...
				$this->nu_tpArea->setSort("ASC");
				$this->no_area->setSort("ASC");
			}
		}
	}
	...
	function ResetCmd() {
		...
		if (substr($this->Command,0,5) == "reset") {
			...
			if ($this->Command == "resetall") {
				...
				$this->DbMasterFilter = "";
				$this->DbDetailFilter = "";
				$this->nu_tpArea->setSessionValue("");
				$this->nu_organizacao->setSessionValue("");
			}
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->nu_area->CurrentValue . "\">";
		}
	...
	function LoadFormValues() {
		...
		if (!$this->nu_area->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->nu_area->CurrentValue = strval($arKeys[0]); // nu_area
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function AddRow($rsold = NULL) {
		...
		if ($this->nu_organizacao->getSessionValue() <> "") {
			$sMasterFilter = str_replace("@nu_organizacao@", ew_AdjustSql($this->nu_organizacao->getSessionValue()), $sMasterFilter);
		} else {
			$bValidMasterRecord = FALSE;
		}
		if ($bValidMasterRecord) {
		...
		// nu_organizacao
		if ($this->nu_organizacao->getSessionValue() <> "") {
			$rsnew['nu_organizacao'] = $this->nu_organizacao->getSessionValue();
		}
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "tparea") {
			$this->nu_tpArea->Visible = FALSE;
			if ($GLOBALS["tparea"]->EventCancelled) $this->EventCancelled = TRUE;
		}
		if ($sMasterTblVar == "organizacao") {
			$this->nu_organizacao->Visible = FALSE;
