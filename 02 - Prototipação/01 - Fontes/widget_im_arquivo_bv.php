...
$widget_im_arquivo_blobview = NULL; // Initialize page object first

class cwidget_im_arquivo_blobview extends cwidget {
	...
	// Page object name
	var $PageObjName = 'widget_im_arquivo_blobview';
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel(CurrentProjectID() . 'widget');
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('widget', 'x_im_arquivo');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (!EW_DEBUG_ENABLED && ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('im_arquivo');
				...
				if (trim(strval($this->Recordset->fields('im_arquivo'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('im_arquivo') . "\"");
				}
...
// Create page object
if (!isset($widget_im_arquivo_blobview)) $widget_im_arquivo_blobview = new cwidget_im_arquivo_blobview();

// Page init
$widget_im_arquivo_blobview->Page_Init();

// Page main
$widget_im_arquivo_blobview->Page_Main();
?>
<?php
$widget_im_arquivo_blobview->Page_Terminate();
?>
