...
$tpElemento_view = NULL; // Initialize page object first

class ctpElemento_view extends ctpElemento {
	...
	// Page object name
	var $PageObjName = 'tpElemento_view';
	...
	function __construct() {
		...
		if (@$_GET["nu_tpElemento"] <> "") {
			$this->RecKey["nu_tpElemento"] = $_GET["nu_tpElemento"];
			$KeyUrl .= "&nu_tpElemento=" . urlencode($this->RecKey["nu_tpElemento"]);
		}
	...
	function Page_Init() {
		...
		if (@$_GET["nu_tpElemento"] <> "") {
			...
			$gsExportFile .= ew_StripSlashes($_GET["nu_tpElemento"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["nu_tpElemento"] <> "") {
				...
				$this->RecKey["nu_tpElemento"] = $this->nu_tpElemento->QueryStringValue;
			} else {
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					...
					if ($this->TotalRecs <= 0) { // No record found
						...
						$this->Page_Terminate("tpelementolist.php"); // Return to list page
						...
						while (!$this->Recordset->EOF) {
							if (strval($this->nu_tpElemento->CurrentValue) == strval($this->Recordset->fields('nu_tpElemento'))) {
					...
					if (!$bMatchRecord) {
						if ($this->getSuccessMessage() == "" && $this->getFailureMessage() == "")
							$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record message
						$sReturnUrl = "tpelementolist.php"; // No matching record, return to list
					} else {
			...
			$sReturnUrl = "tpelementolist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a id=\"emf_tpElemento\" href=\"javascript:void(0);\" class=\"ewExportLink ewEmail\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_tpElemento',hdr:ewLanguage.Phrase('ExportToEmail'),f:document.ftpElementoview,key:" . ew_ArrayToJsonAttr($this->RecKey) . ",sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
...
// Create page object
if (!isset($tpElemento_view)) $tpElemento_view = new ctpElemento_view();

// Page init
$tpElemento_view->Page_Init();

// Page main
$tpElemento_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$tpElemento_view->Page_Render();
?>
...
// Page object
var tpElemento_view = new ew_Page("tpElemento_view");
tpElemento_view.PageID = "view"; // Page ID
var EW_PAGE_ID = tpElemento_view.PageID; // For backward compatibility

// Form object
var ftpElementoview = new ew_Form("ftpElementoview");

// Form_CustomValidate event
ftpElementoview.Form_CustomValidate = 
...
ftpElementoview.ValidateRequired = true;
<?php } else { ?>
ftpElementoview.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ftpElementoview.Lists["x_nu_tpManutencao"] = {"LinkField":"x_nu_tpManutencao","Ajax":null,"AutoFill":false,"DisplayFields":["x_no_tpManutencao","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $tpElemento_view->ExportOptions->Render("body") ?>
<?php if (!$tpElemento_view->ExportOptions->UseDropDownButton) { ?>
</div>
<div class="ewViewOtherOptions">
<?php } ?>
<?php
	foreach ($tpElemento_view->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<?php } ?>
<?php $tpElemento_view->ShowPageHeader(); ?>
<?php
$tpElemento_view->ShowMessage();
?>
<form name="ftpElementoview" id="ftpElementoview" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="tpElemento">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_tpElementoview" class="table table-bordered table-striped">
<?php if ($tpElemento->nu_tpManutencao->Visible) { // nu_tpManutencao ?>
	<tr id="r_nu_tpManutencao">
		<td><span id="elh_tpElemento_nu_tpManutencao"><?php echo $tpElemento->nu_tpManutencao->FldCaption() ?></span></td>
		<td<?php echo $tpElemento->nu_tpManutencao->CellAttributes() ?>>
<span id="el_tpElemento_nu_tpManutencao" class="control-group">
...
<?php if ($tpElemento->no_tpElemento->Visible) { // no_tpElemento ?>
	<tr id="r_no_tpElemento">
		<td><span id="elh_tpElemento_no_tpElemento"><?php echo $tpElemento->no_tpElemento->FldCaption() ?></span></td>
...
<?php if ($tpElemento->ic_funcional->Visible) { // ic_funcional ?>
	<tr id="r_ic_funcional">
		<td><span id="elh_tpElemento_ic_funcional"><?php echo $tpElemento->ic_funcional->FldCaption() ?></span></td>
...
<span<?php echo $tpElemento->ds_helpTela->ViewAttributes() ?>>
<?php echo $tpElemento->ds_helpTela->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($tpElemento->ic_ativo->Visible) { // ic_ativo ?>
	<tr id="r_ic_ativo">
		<td><span id="elh_tpElemento_ic_ativo"><?php echo $tpElemento->ic_ativo->FldCaption() ?></span></td>
...
<table class="ewPager">
<tr><td>
<?php if (!isset($tpElemento_view->Pager)) $tpElemento_view->Pager = new cNumericPager($tpElemento_view->StartRec, $tpElemento_view->DisplayRecs, $tpElemento_view->TotalRecs, $tpElemento_view->RecRange) ?>
<?php if ($tpElemento_view->Pager->RecordCount > 0) { ?>
<table cellspacing="0" class="ewStdTable"><tbody><tr><td>
<div class="pagination"><ul>
	<?php if ($tpElemento_view->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $tpElemento_view->PageUrl() ?>start=<?php echo $tpElemento_view->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($tpElemento_view->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $tpElemento_view->PageUrl() ?>start=<?php echo $tpElemento_view->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($tpElemento_view->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $tpElemento_view->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($tpElemento_view->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $tpElemento_view->PageUrl() ?>start=<?php echo $tpElemento_view->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($tpElemento_view->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $tpElemento_view->PageUrl() ?>start=<?php echo $tpElemento_view->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</td>
</tr></tbody></table>
...
<script type="text/javascript">
ftpElementoview.Init();
</script>
<?php
$tpElemento_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($tpElemento->Export == "") { ?>
...
<?php
$tpElemento_view->Page_Terminate();
?>
