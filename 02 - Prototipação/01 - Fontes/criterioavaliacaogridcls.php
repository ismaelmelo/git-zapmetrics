...
$criterioavaliacao_grid = NULL; // Initialize page object first

class ccriterioavaliacao_grid extends ccriterioavaliacao {
	...
	// Page object name
	var $PageObjName = 'criterioavaliacao_grid';

	// Grid form hidden field names
	var $FormName = 'fcriterioavaliacaogrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["criterioavaliacao"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->nu_alternativaAvaliacao->CurrentValue;
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_no_alternativa") && $objForm->HasValue("o_no_alternativa") && $this->no_alternativa->CurrentValue <> $this->no_alternativa->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_vr_alternativa") && $objForm->HasValue("o_vr_alternativa") && $this->vr_alternativa->CurrentValue <> $this->vr_alternativa->OldValue)
			return FALSE;
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->nu_alternativaAvaliacao->CurrentValue . "\">";
		}
	...
	function SetRecordKey(&$key, $rs) {
		...
		$key .= $rs->fields('nu_alternativaAvaliacao');
	}
	...
	function LoadFormValues() {
		...
		$this->no_alternativa->setOldValue($objForm->GetValue("o_no_alternativa"));
		if (!$this->vr_alternativa->FldIsDetailKey) {
			$this->vr_alternativa->setFormValue($objForm->GetValue("x_vr_alternativa"));
		}
		$this->vr_alternativa->setOldValue($objForm->GetValue("o_vr_alternativa"));
		...
		$this->dt_manutencao->setOldValue($objForm->GetValue("o_dt_manutencao"));
		if (!$this->nu_usuarioAlterou->FldIsDetailKey) {
			$this->nu_usuarioAlterou->setFormValue($objForm->GetValue("x_nu_usuarioAlterou"));
		}
		$this->nu_usuarioAlterou->setOldValue($objForm->GetValue("o_nu_usuarioAlterou"));
		...
		if (!$this->nu_alternativaAvaliacao->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->nu_alternativaAvaliacao->CurrentValue = strval($arKeys[0]); // nu_alternativaAvaliacao
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			if (strval($this->vr_alternativa->EditValue) <> "" && is_numeric($this->vr_alternativa->EditValue)) {
			$this->vr_alternativa->EditValue = ew_FormatNumber($this->vr_alternativa->EditValue, -2, -2, -2, -2);
			$this->vr_alternativa->OldValue = $this->vr_alternativa->EditValue;
			}
	...
	function AddRow($rsold = NULL) {
		...
		if ($this->nu_criterioPrioridade->getSessionValue() <> "") {
			$rsnew['nu_criterioPrioridade'] = $this->nu_criterioPrioridade->getSessionValue();
		}
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "criterio") {
			$this->nu_criterioPrioridade->Visible = FALSE;
			if ($GLOBALS["criterio"]->EventCancelled) $this->EventCancelled = TRUE;
		}
