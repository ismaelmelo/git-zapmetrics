...
$metati_delete = NULL; // Initialize page object first

class cmetati_delete extends cmetati {
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("metatilist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in metati class, metatiinfo.php
...
// Create page object
if (!isset($metati_delete)) $metati_delete = new cmetati_delete();

// Page init
$metati_delete->Page_Init();

// Page main
$metati_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$metati_delete->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var metati_delete = new ew_Page("metati_delete");
metati_delete.PageID = "delete"; // Page ID
var EW_PAGE_ID = metati_delete.PageID; // For backward compatibility

// Form object
var fmetatidelete = new ew_Form("fmetatidelete");

// Form_CustomValidate event
fmetatidelete.Form_CustomValidate = 
...
fmetatidelete.ValidateRequired = true;
<?php } else { ?>
fmetatidelete.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fmetatidelete.Lists["x_nu_periodoPei"] = {"LinkField":"x_nu_periodoPei","Ajax":null,"AutoFill":false,"DisplayFields":["x_nu_anoInicio","x_nu_anoFim","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fmetatidelete.Lists["x_nu_periodoPdti"] = {"LinkField":"x_nu_periodo","Ajax":null,"AutoFill":false,"DisplayFields":["x_no_periodo","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fmetatidelete.Lists["x_nu_metaneg"] = {"LinkField":"x_nu_metaneg","Ajax":null,"AutoFill":false,"DisplayFields":["x_no_metaneg","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
if ($metati_delete->Recordset = $metati_delete->LoadRecordset())
	$metati_deleteTotalRecs = $metati_delete->Recordset->RecordCount(); // Get record count
if ($metati_deleteTotalRecs <= 0) { // No record found, exit
	if ($metati_delete->Recordset)
		$metati_delete->Recordset->Close();
	$metati_delete->Page_Terminate("metatilist.php"); // Return to list
}
?>
<?php $Breadcrumb->Render(); ?>
<?php $metati_delete->ShowPageHeader(); ?>
<?php
$metati_delete->ShowMessage();
?>
<form name="fmetatidelete" id="fmetatidelete" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="metati">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($metati_delete->RecKeys as $key) { ?>
<?php $keyvalue = is_array($key) ? implode($EW_COMPOSITE_KEY_SEPARATOR, $key) : $key; ?>
<input type="hidden" name="key_m[]" value="<?php echo ew_HtmlEncode($keyvalue) ?>">
<?php } ?>
<table cellspacing="0" class="ewGrid"><tr><td class="ewGridContent">
<div class="ewGridMiddlePanel">
<table id="tbl_metatidelete" class="ewTable ewTableSeparate">
<?php echo $metati->TableCustomInnerHtml ?>
	<thead>
	<tr class="ewTableHeader">
		<td><span id="elh_metati_nu_metati" class="metati_nu_metati"><?php echo $metati->nu_metati->FldCaption() ?></span></td>
		<td><span id="elh_metati_nu_periodoPei" class="metati_nu_periodoPei"><?php echo $metati->nu_periodoPei->FldCaption() ?></span></td>
		<td><span id="elh_metati_nu_periodoPdti" class="metati_nu_periodoPdti"><?php echo $metati->nu_periodoPdti->FldCaption() ?></span></td>
		<td><span id="elh_metati_nu_metaneg" class="metati_nu_metaneg"><?php echo $metati->nu_metaneg->FldCaption() ?></span></td>
		<td><span id="elh_metati_ic_perspectiva" class="metati_ic_perspectiva"><?php echo $metati->ic_perspectiva->FldCaption() ?></span></td>
		<td><span id="elh_metati_no_metati" class="metati_no_metati"><?php echo $metati->no_metati->FldCaption() ?></span></td>
		<td><span id="elh_metati_ic_situacao" class="metati_ic_situacao"><?php echo $metati->ic_situacao->FldCaption() ?></span></td>
	</tr>
	</thead>
	<tbody>
<?php
$metati_delete->RecCnt = 0;
$i = 0;
while (!$metati_delete->Recordset->EOF) {
	$metati_delete->RecCnt++;
	$metati_delete->RowCnt++;

	// Set row properties
	$metati->ResetAttrs();
	$metati->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$metati_delete->LoadRowValues($metati_delete->Recordset);

	// Render row
	$metati_delete->RenderRow();
?>
	<tr<?php echo $metati->RowAttributes() ?>>
		<td<?php echo $metati->nu_metati->CellAttributes() ?>>
<span id="el<?php echo $metati_delete->RowCnt ?>_metati_nu_metati" class="control-group metati_nu_metati">
...
<span id="el<?php echo $metati_delete->RowCnt ?>_metati_nu_periodoPei" class="control-group metati_nu_periodoPei">
...
<span id="el<?php echo $metati_delete->RowCnt ?>_metati_nu_periodoPdti" class="control-group metati_nu_periodoPdti">
...
<span id="el<?php echo $metati_delete->RowCnt ?>_metati_nu_metaneg" class="control-group metati_nu_metaneg">
...
<span id="el<?php echo $metati_delete->RowCnt ?>_metati_ic_perspectiva" class="control-group metati_ic_perspectiva">
...
<span id="el<?php echo $metati_delete->RowCnt ?>_metati_no_metati" class="control-group metati_no_metati">
...
<span id="el<?php echo $metati_delete->RowCnt ?>_metati_ic_situacao" class="control-group metati_ic_situacao">
...
<?php
	$metati_delete->Recordset->MoveNext();
}
$metati_delete->Recordset->Close();
?>
</tbody>
