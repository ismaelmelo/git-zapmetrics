...
$necstake_grid = NULL; // Initialize page object first

class cnecstake_grid extends cnecstake {
	...
	// Page object name
	var $PageObjName = 'necstake_grid';

	// Grid form hidden field names
	var $FormName = 'fnecstakegrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["necstake"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function Page_Main() {
		...
		// Load master record
		if ($this->CurrentMode <> "add" && $this->GetMasterFilter() <> "" && $this->getCurrentMasterTable() == "metaneg") {
			global $metaneg;
			$rsmaster = $metaneg->LoadRs($this->DbMasterFilter);
			$this->MasterRecordExists = ($rsmaster && !$rsmaster->EOF);
			if (!$this->MasterRecordExists) {
				$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record found
				$this->Page_Terminate("metaneglist.php"); // Return to master page
			} else {
				$metaneg->LoadListRowValues($rsmaster);
				$metaneg->RowType = EW_ROWTYPE_MASTER; // Master row
				$metaneg->RenderListRow();
				$rsmaster->Close();
			}
		}
	...
	function SetupKeyValues($key) {
		...
		if (count($arrKeyFlds) >= 1) {
			$this->nu_necessidade->setFormValue($arrKeyFlds[0]);
			if (!is_numeric($this->nu_necessidade->FormValue))
				return FALSE;
		}
		return TRUE;
	}
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->nu_necessidade->CurrentValue;
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_nu_periodoPei") && $objForm->HasValue("o_nu_periodoPei") && $this->nu_periodoPei->CurrentValue <> $this->nu_periodoPei->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_nu_motivador") && $objForm->HasValue("o_nu_motivador") && $this->nu_motivador->CurrentValue <> $this->nu_motivador->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_no_necessidade") && $objForm->HasValue("o_no_necessidade") && $this->no_necessidade->CurrentValue <> $this->no_necessidade->OldValue)
			return FALSE;
	...
	function ResetCmd() {
		...
		if (substr($this->Command,0,5) == "reset") {
			...
			if ($this->Command == "resetall") {
				...
				$this->DbMasterFilter = "";
				$this->DbDetailFilter = "";
				$this->nu_necessidade->setSessionValue("");
			}
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->nu_necessidade->CurrentValue . "\">";
		}
	...
	function LoadFormValues() {
		...
		if (!$this->nu_necessidade->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->nu_necessidade->CurrentValue = strval($arKeys[0]); // nu_necessidade
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function AddRow($rsold = NULL) {
		...
		// Set up foreign key field value from Session
			if ($this->getCurrentMasterTable() == "metaneg") {
				$this->nu_necessidade->CurrentValue = $this->nu_necessidade->getSessionValue();
			}
		if ($this->no_necessidade->CurrentValue <> "") { // Check field with unique index
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "metaneg") {
			$this->nu_necessidade->Visible = FALSE;
			if ($GLOBALS["metaneg"]->EventCancelled) $this->EventCancelled = TRUE;
		}
