...
$atividade_papel_grid = NULL; // Initialize page object first

class catividade_papel_grid extends catividade_papel {
	...
	// Page object name
	var $PageObjName = 'atividade_papel_grid';

	// Grid form hidden field names
	var $FormName = 'fatividade_papelgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["atividade_papel"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					if ($sKey <> "") $sKey .= $GLOBALS["EW_COMPOSITE_KEY_SEPARATOR"];
					$sKey .= $this->co_papel->CurrentValue;
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_nu_atividade") && $objForm->HasValue("o_nu_atividade") && $this->nu_atividade->CurrentValue <> $this->nu_atividade->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_co_papel") && $objForm->HasValue("o_co_papel") && $this->co_papel->CurrentValue <> $this->co_papel->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_ic_responsabilidade") && $objForm->HasValue("o_ic_responsabilidade") && $this->ic_responsabilidade->CurrentValue <> $this->ic_responsabilidade->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->nu_atividade->CurrentValue . $GLOBALS["EW_COMPOSITE_KEY_SEPARATOR"] . $this->co_papel->CurrentValue . "\">";
		}
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 2) {
			if (strval($arKeys[0]) <> "")
				...
				$bValidKey = FALSE;
			if (strval($arKeys[1]) <> "")
				$this->co_papel->CurrentValue = strval($arKeys[1]); // co_papel
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			$sSqlWrk .= " ORDER BY [no_atividade] ASC";
				$rswrk = $conn->Execute($sSqlWrk);
				if ($rswrk && !$rswrk->EOF) { // Lookup values found
					$this->nu_atividade->EditValue = $rswrk->fields('DispFld');
					$rswrk->Close();
				} else {
					$this->nu_atividade->EditValue = $this->nu_atividade->CurrentValue;
				}
			} else {
				$this->nu_atividade->EditValue = NULL;
			}
			$this->nu_atividade->ViewCustomAttributes = "";
			...
			$sSqlWrk .= " ORDER BY [no_papel] ASC";
				$rswrk = $conn->Execute($sSqlWrk);
				if ($rswrk && !$rswrk->EOF) { // Lookup values found
					$this->co_papel->EditValue = $rswrk->fields('DispFld');
					$rswrk->Close();
				} else {
					$this->co_papel->EditValue = $this->co_papel->CurrentValue;
				}
			} else {
				$this->co_papel->EditValue = NULL;
			}
			$this->co_papel->ViewCustomAttributes = "";
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			$this->ic_responsabilidade->SetDbValueDef($rsnew, $this->ic_responsabilidade->CurrentValue, "", $this->ic_responsabilidade->ReadOnly);

			// Check referential integrity for master table 'atividade'
			$bValidMasterRecord = TRUE;
			$sMasterFilter = $this->SqlMasterFilter_atividade();
			$KeyValue = isset($rsnew['nu_atividade']) ? $rsnew['nu_atividade'] : $rsold['nu_atividade'];
			if (strval($KeyValue) <> "") {
				$sMasterFilter = str_replace("@nu_atividade@", ew_AdjustSql($KeyValue), $sMasterFilter);
			} else {
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "atividade") {
			$this->nu_atividade->Visible = FALSE;
			if ($GLOBALS["atividade"]->EventCancelled) $this->EventCancelled = TRUE;
		}
