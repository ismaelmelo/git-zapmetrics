...
$uc_ator_grid = NULL; // Initialize page object first

class cuc_ator_grid extends cuc_ator {
	...
	// Page object name
	var $PageObjName = 'uc_ator_grid';

	// Grid form hidden field names
	var $FormName = 'fuc_atorgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["uc_ator"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_nu_ator") && $objForm->HasValue("o_nu_ator") && $this->nu_ator->CurrentValue <> $this->nu_ator->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->nu_uc->CurrentValue . $GLOBALS["EW_COMPOSITE_KEY_SEPARATOR"] . $this->nu_ator->CurrentValue . "\">";
		}
	...
	function LoadFormValues() {
		...
		$this->nu_ator->setOldValue($objForm->GetValue("o_nu_ator"));
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 2) {
			if (strval($arKeys[0]) <> "")
				...
				$bValidKey = FALSE;
			if (strval($arKeys[1]) <> "")
				$this->nu_ator->CurrentValue = strval($arKeys[1]); // nu_ator
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			$sSqlWrk .= " ORDER BY [no_ator] ASC";
				$rswrk = $conn->Execute($sSqlWrk);
				if ($rswrk && !$rswrk->EOF) { // Lookup values found
					$this->nu_ator->EditValue = $rswrk->fields('DispFld');
					$rswrk->Close();
				} else {
					$this->nu_ator->EditValue = $this->nu_ator->CurrentValue;
				}
			} else {
				$this->nu_ator->EditValue = NULL;
			}
			$this->nu_ator->ViewCustomAttributes = "";
