...
$ativroteiro_add = NULL; // Initialize page object first

class cativroteiro_add extends cativroteiro {
	...
	// Page object name
	var $PageObjName = 'ativroteiro_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["nu_ativRoteiro"] != "") {
				...
				$this->setKey("nu_ativRoteiro", $this->nu_ativRoteiro->CurrentValue); // Set up key
			} else {
				$this->setKey("nu_ativRoteiro", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "ativroteiroview.php")
	...
	function LoadDefaultValues() {
		$this->nu_faseRoteiro->CurrentValue = NULL;
		$this->nu_faseRoteiro->OldValue = $this->nu_faseRoteiro->CurrentValue;
		$this->no_ativRoteiro->CurrentValue = NULL;
		$this->no_ativRoteiro->OldValue = $this->no_ativRoteiro->CurrentValue;
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			if ($this->nu_faseRoteiro->getSessionValue() <> "") {
				$this->nu_faseRoteiro->CurrentValue = $this->nu_faseRoteiro->getSessionValue();
			if (strval($this->nu_faseRoteiro->CurrentValue) <> "") {
				$sFilterWrk = "[nu_roteiro]" . ew_SearchString("=", $this->nu_faseRoteiro->CurrentValue, EW_DATATYPE_NUMBER);
			$sSqlWrk = "SELECT [nu_roteiro], [no_roteiro] AS [DispFld], '' AS [Disp2Fld], '' AS [Disp3Fld], '' AS [Disp4Fld] FROM [dbo].[roteiro]";
			$sWhereWrk = "";
			...
			$this->nu_faseRoteiro->EditValue = $arwrk;
			}
	...
	function ValidateForm() {
		...
		if (!EW_SERVER_VALIDATE)
			return ($gsFormError == "");
		if (!$this->nu_faseRoteiro->FldIsDetailKey && !is_null($this->nu_faseRoteiro->FormValue) && $this->nu_faseRoteiro->FormValue == "") {
			ew_AddMessage($gsFormError, $Language->Phrase("EnterRequiredField") . " - " . $this->nu_faseRoteiro->FldCaption());
		}
	...
	function AddRow($rsold = NULL) {
		...
		if (strval($this->nu_faseRoteiro->CurrentValue) <> "") {
			$sMasterFilter = str_replace("@nu_faseRoteiro@", ew_AdjustSql($this->nu_faseRoteiro->CurrentValue), $sMasterFilter);
		} else {
		...
		// nu_faseRoteiro
		$this->nu_faseRoteiro->SetDbValueDef($rsnew, $this->nu_faseRoteiro->CurrentValue, 0, FALSE);

		// no_ativRoteiro
		$this->no_ativRoteiro->SetDbValueDef($rsnew, $this->no_ativRoteiro->CurrentValue, "", FALSE);
		...
		if ($AddRow) {
			$this->nu_ativRoteiro->setDbValue($conn->Insert_ID());
			$rsnew['nu_ativRoteiro'] = $this->nu_ativRoteiro->DbValue;
		}
		if ($AddRow) {
...
// Create page object
if (!isset($ativroteiro_add)) $ativroteiro_add = new cativroteiro_add();

// Page init
$ativroteiro_add->Page_Init();

// Page main
$ativroteiro_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$ativroteiro_add->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var ativroteiro_add = new ew_Page("ativroteiro_add");
ativroteiro_add.PageID = "add"; // Page ID
var EW_PAGE_ID = ativroteiro_add.PageID; // For backward compatibility

// Form object
var fativroteiroadd = new ew_Form("fativroteiroadd");

// Validate form
fativroteiroadd.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			elm = this.GetElements("x" + infix + "_nu_faseRoteiro");
			if (elm && !ew_HasValue(elm))
				return this.OnError(elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($ativroteiro->nu_faseRoteiro->FldCaption()) ?>");
...
fativroteiroadd.Form_CustomValidate = 
...
fativroteiroadd.ValidateRequired = true;
<?php } else { ?>
fativroteiroadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fativroteiroadd.Lists["x_nu_faseRoteiro"] = {"LinkField":"x_nu_roteiro","Ajax":null,"AutoFill":false,"DisplayFields":["x_no_roteiro","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $ativroteiro_add->ShowPageHeader(); ?>
<?php
$ativroteiro_add->ShowMessage();
?>
<form name="fativroteiroadd" id="fativroteiroadd" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="ativroteiro">
<input type="hidden" name="a_add" id="a_add" value="A">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_ativroteiroadd" class="table table-bordered table-striped">
<?php if ($ativroteiro->nu_faseRoteiro->Visible) { // nu_faseRoteiro ?>
	<tr id="r_nu_faseRoteiro">
		<td><span id="elh_ativroteiro_nu_faseRoteiro"><?php echo $ativroteiro->nu_faseRoteiro->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
		<td<?php echo $ativroteiro->nu_faseRoteiro->CellAttributes() ?>>
<?php if ($ativroteiro->nu_faseRoteiro->getSessionValue() <> "") { ?>
<span<?php echo $ativroteiro->nu_faseRoteiro->ViewAttributes() ?>>
<?php echo $ativroteiro->nu_faseRoteiro->ViewValue ?></span>
<input type="hidden" id="x_nu_faseRoteiro" name="x_nu_faseRoteiro" value="<?php echo ew_HtmlEncode($ativroteiro->nu_faseRoteiro->CurrentValue) ?>">
<?php } else { ?>
<select data-field="x_nu_faseRoteiro" id="x_nu_faseRoteiro" name="x_nu_faseRoteiro"<?php echo $ativroteiro->nu_faseRoteiro->EditAttributes() ?>>
<?php
if (is_array($ativroteiro->nu_faseRoteiro->EditValue)) {
	$arwrk = $ativroteiro->nu_faseRoteiro->EditValue;
	$rowswrk = count($arwrk);
	$emptywrk = TRUE;
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($ativroteiro->nu_faseRoteiro->CurrentValue) == strval($arwrk[$rowcntwrk][0])) ? " selected=\"selected\"" : "";
...
<script type="text/javascript">
fativroteiroadd.Lists["x_nu_faseRoteiro"].Options = <?php echo (is_array($ativroteiro->nu_faseRoteiro->EditValue)) ? ew_ArrayToJson($ativroteiro->nu_faseRoteiro->EditValue, 1) : "[]" ?>;
</script>
<?php } ?>
...
<script type="text/javascript">
fativroteiroadd.Init();
<?php if (EW_MOBILE_REFLOW && ew_IsMobile()) { ?>
ew_Reflow();
<?php } ?>
</script>
<?php
$ativroteiro_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$ativroteiro_add->Page_Terminate();
?>
