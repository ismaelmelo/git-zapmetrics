...
$fatura_os_grid = NULL; // Initialize page object first

class cfatura_os_grid extends cfatura_os {
	...
	// Page object name
	var $PageObjName = 'fatura_os_grid';

	// Grid form hidden field names
	var $FormName = 'ffatura_osgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["fatura_os"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->nu_fatura->CurrentValue;
					if ($sKey <> "") $sKey .= $GLOBALS["EW_COMPOSITE_KEY_SEPARATOR"];
					$sKey .= $this->nu_os->CurrentValue;
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_nu_os") && $objForm->HasValue("o_nu_os") && $this->nu_os->CurrentValue <> $this->nu_os->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_ic_pagIntegralOs") && $objForm->HasValue("o_ic_pagIntegralOs") && $this->ic_pagIntegralOs->CurrentValue <> $this->ic_pagIntegralOs->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_vr_pagoOsFatura") && $objForm->HasValue("o_vr_pagoOsFatura") && $this->vr_pagoOsFatura->CurrentValue <> $this->vr_pagoOsFatura->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->nu_fatura->CurrentValue . $GLOBALS["EW_COMPOSITE_KEY_SEPARATOR"] . $this->nu_os->CurrentValue . "\">";
		}
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 2) {
			if (strval($arKeys[0]) <> "")
				$this->nu_fatura->CurrentValue = strval($arKeys[0]); // nu_fatura
			else
				$bValidKey = FALSE;
			if (strval($arKeys[1]) <> "")
				$this->nu_os->CurrentValue = strval($arKeys[1]); // nu_os
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			if (strval($this->vr_pagoOsFatura->EditValue) <> "" && is_numeric($this->vr_pagoOsFatura->EditValue)) {
			$this->vr_pagoOsFatura->EditValue = ew_FormatNumber($this->vr_pagoOsFatura->EditValue, -2, -1, -2, 0);
			$this->vr_pagoOsFatura->OldValue = $this->vr_pagoOsFatura->EditValue;
			}
			...
			$sSqlWrk .= " ORDER BY [nu_os] ASC";
				$rswrk = $conn->Execute($sSqlWrk);
				if ($rswrk && !$rswrk->EOF) { // Lookup values found
					$this->nu_os->EditValue = $rswrk->fields('DispFld');
					$rswrk->Close();
				} else {
					$this->nu_os->EditValue = $this->nu_os->CurrentValue;
				}
			} else {
				$this->nu_os->EditValue = NULL;
			}
			$this->nu_os->ViewCustomAttributes = "";
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// ic_pagIntegralOs
			$this->ic_pagIntegralOs->SetDbValueDef($rsnew, $this->ic_pagIntegralOs->CurrentValue, NULL, $this->ic_pagIntegralOs->ReadOnly);

			// vr_pagoOsFatura
			$this->vr_pagoOsFatura->SetDbValueDef($rsnew, $this->vr_pagoOsFatura->CurrentValue, 0, $this->vr_pagoOsFatura->ReadOnly);

			// Check referential integrity for master table 'fatura'
			$bValidMasterRecord = TRUE;
			$sMasterFilter = $this->SqlMasterFilter_fatura();
			$KeyValue = isset($rsnew['nu_fatura']) ? $rsnew['nu_fatura'] : $rsold['nu_fatura'];
			if (strval($KeyValue) <> "") {
				$sMasterFilter = str_replace("@nu_fatura@", ew_AdjustSql($KeyValue), $sMasterFilter);
			} else {
	...
	function AddRow($rsold = NULL) {
		...
		if ($this->nu_fatura->getSessionValue() <> "") {
			$sMasterFilter = str_replace("@nu_fatura@", ew_AdjustSql($this->nu_fatura->getSessionValue()), $sMasterFilter);
		} else {
			$bValidMasterRecord = FALSE;
		}
		if ($bValidMasterRecord) {
		...
		// nu_fatura
		if ($this->nu_fatura->getSessionValue() <> "") {
			$rsnew['nu_fatura'] = $this->nu_fatura->getSessionValue();
		}
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "fatura") {
			$this->nu_fatura->Visible = FALSE;
			if ($GLOBALS["fatura"]->EventCancelled) $this->EventCancelled = TRUE;
		}
