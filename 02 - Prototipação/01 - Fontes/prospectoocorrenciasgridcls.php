...
$prospectoocorrencias_grid = NULL; // Initialize page object first

class cprospectoocorrencias_grid extends cprospectoocorrencias {
	...
	// Page object name
	var $PageObjName = 'prospectoocorrencias_grid';

	// Grid form hidden field names
	var $FormName = 'fprospectoocorrenciasgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["prospectoocorrencias"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->nu_ocorrencia->CurrentValue;
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_no_assuntoOcor") && $objForm->HasValue("o_no_assuntoOcor") && $this->no_assuntoOcor->CurrentValue <> $this->no_assuntoOcor->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_dh_inclusao") && $objForm->HasValue("o_dh_inclusao") && $this->dh_inclusao->CurrentValue <> $this->dh_inclusao->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->nu_ocorrencia->CurrentValue . "\">";
		}
	...
	function LoadDefaultValues() {
		$this->nu_ocorrencia->CurrentValue = NULL;
		$this->nu_ocorrencia->OldValue = $this->nu_ocorrencia->CurrentValue;
	...
	function LoadFormValues() {
		...
		// Load from form
		global $objForm;
		if (!$this->nu_ocorrencia->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
		...
		$this->no_assuntoOcor->setOldValue($objForm->GetValue("o_no_assuntoOcor"));
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->nu_ocorrencia->CurrentValue = strval($arKeys[0]); // nu_ocorrencia
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "prospecto") {
			...
			if ($GLOBALS["prospecto"]->EventCancelled) $this->EventCancelled = TRUE;
		}
