...
$gc_membro_add = NULL; // Initialize page object first

class cgc_membro_add extends cgc_membro {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["nu_membro"] != "") {
				...
				$this->setKey("nu_membro", $this->nu_membro->CurrentValue); // Set up key
			} else {
				$this->setKey("nu_membro", ""); // Clear key
	...
	function LoadDefaultValues() {
		...
		$this->nu_papel->CurrentValue = NULL;
		$this->nu_papel->OldValue = $this->nu_papel->CurrentValue;
	...
	function AddRow($rsold = NULL) {
		...
		// nu_pessoa
		$this->nu_pessoa->SetDbValueDef($rsnew, $this->nu_pessoa->CurrentValue, 0, FALSE);

		// nu_papel
		$this->nu_papel->SetDbValueDef($rsnew, $this->nu_papel->CurrentValue, 0, FALSE);
		...
		if ($AddRow) {
			$this->nu_membro->setDbValue($conn->Insert_ID());
			$rsnew['nu_membro'] = $this->nu_membro->DbValue;
		}
		if ($AddRow) {
...
// Create page object
if (!isset($gc_membro_add)) $gc_membro_add = new cgc_membro_add();

// Page init
$gc_membro_add->Page_Init();

// Page main
$gc_membro_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$gc_membro_add->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var gc_membro_add = new ew_Page("gc_membro_add");
gc_membro_add.PageID = "add"; // Page ID
var EW_PAGE_ID = gc_membro_add.PageID; // For backward compatibility

// Form object
var fgc_membroadd = new ew_Form("fgc_membroadd");

// Validate form
fgc_membroadd.Validate = function() {
...
fgc_membroadd.Form_CustomValidate = 
...
fgc_membroadd.ValidateRequired = true;
<?php } else { ?>
fgc_membroadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fgc_membroadd.Lists["x_nu_grupoOuComite"] = {"LinkField":"x_nu_gpComite","Ajax":null,"AutoFill":false,"DisplayFields":["x_no_gpComite","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fgc_membroadd.Lists["x_nu_pessoa"] = {"LinkField":"x_nu_pessoa","Ajax":null,"AutoFill":false,"DisplayFields":["x_no_pessoa","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fgc_membroadd.Lists["x_nu_papel"] = {"LinkField":"x_co_papel","Ajax":null,"AutoFill":false,"DisplayFields":["x_no_papel","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $gc_membro_add->ShowPageHeader(); ?>
<?php
$gc_membro_add->ShowMessage();
?>
<form name="fgc_membroadd" id="fgc_membroadd" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="gc_membro">
<input type="hidden" name="a_add" id="a_add" value="A">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_gc_membroadd" class="table table-bordered table-striped">
...
<script type="text/javascript">
fgc_membroadd.Lists["x_nu_grupoOuComite"].Options = <?php echo (is_array($gc_membro->nu_grupoOuComite->EditValue)) ? ew_ArrayToJson($gc_membro->nu_grupoOuComite->EditValue, 1) : "[]" ?>;
</script>
</span>
...
<?php } ?>
<script type="text/javascript">
fgc_membroadd.Lists["x_nu_pessoa"].Options = <?php echo (is_array($gc_membro->nu_pessoa->EditValue)) ? ew_ArrayToJson($gc_membro->nu_pessoa->EditValue, 1) : "[]" ?>;
</script>
</span>
...
<?php } ?>
<script type="text/javascript">
fgc_membroadd.Lists["x_nu_papel"].Options = <?php echo (is_array($gc_membro->nu_papel->EditValue)) ? ew_ArrayToJson($gc_membro->nu_papel->EditValue, 1) : "[]" ?>;
</script>
</span>
...
ew_CreateCalendar("fgc_membroadd", "x_dt_inicio", "%d/%m/%Y");
</script>
<?php } ?>
</span>
...
ew_CreateCalendar("fgc_membroadd", "x_dt_fim", "%d/%m/%Y");
</script>
<?php } ?>
</span>
