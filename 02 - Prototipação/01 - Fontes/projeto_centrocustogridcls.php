...
$projeto_centrocusto_grid = NULL; // Initialize page object first

class cprojeto_centrocusto_grid extends cprojeto_centrocusto {
	...
	// Page object name
	var $PageObjName = 'projeto_centrocusto_grid';

	// Grid form hidden field names
	var $FormName = 'fprojeto_centrocustogrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["projeto_centrocusto"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->nu_projeto->CurrentValue;
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_nu_centroCusto_") && $objForm->HasValue("o_nu_centroCusto_") && $this->nu_centroCusto_->CurrentValue <> $this->nu_centroCusto_->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_pc_participacao") && $objForm->HasValue("o_pc_participacao") && $this->pc_participacao->CurrentValue <> $this->pc_participacao->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->nu_projeto->CurrentValue . "\">";
		}
	...
	function LoadDefaultValues() {
		...
		$this->pc_participacao->OldValue = $this->pc_participacao->CurrentValue;
	}
	...
	function LoadFormValues() {
		...
		$this->nu_centroCusto_->setOldValue($objForm->GetValue("o_nu_centroCusto_"));
		if (!$this->pc_participacao->FldIsDetailKey) {
			$this->pc_participacao->setFormValue($objForm->GetValue("x_pc_participacao"));
		}
		$this->pc_participacao->setOldValue($objForm->GetValue("o_pc_participacao"));
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->nu_projeto->CurrentValue = strval($arKeys[0]); // nu_projeto
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			if (strval($this->pc_participacao->EditValue) <> "" && is_numeric($this->pc_participacao->EditValue)) {
			$this->pc_participacao->EditValue = ew_FormatNumber($this->pc_participacao->EditValue, -2, -1, -2, 0);
			$this->pc_participacao->OldValue = $this->pc_participacao->EditValue;
			}
	...
	function AddRow($rsold = NULL) {
		...
		if ($this->nu_projeto->getSessionValue() <> "") {
			$sMasterFilter = str_replace("@nu_projeto@", ew_AdjustSql($this->nu_projeto->getSessionValue()), $sMasterFilter);
		} else {
			$bValidMasterRecord = FALSE;
		}
		if ($bValidMasterRecord) {
		...
		// nu_projeto
		if ($this->nu_projeto->getSessionValue() <> "") {
			$rsnew['nu_projeto'] = $this->nu_projeto->getSessionValue();
		}
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "centrocusto") {
			$this->nu_centroCusto_->Visible = FALSE;
			if ($GLOBALS["centrocusto"]->EventCancelled) $this->EventCancelled = TRUE;
		}
