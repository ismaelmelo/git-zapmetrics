...
$centrocusto_delete = NULL; // Initialize page object first

class ccentrocusto_delete extends ccentrocusto {
	...
	// Page object name
	var $PageObjName = 'centrocusto_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("centrocustolist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in centrocusto class, centrocustoinfo.php
...
// Create page object
if (!isset($centrocusto_delete)) $centrocusto_delete = new ccentrocusto_delete();

// Page init
$centrocusto_delete->Page_Init();

// Page main
$centrocusto_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$centrocusto_delete->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var centrocusto_delete = new ew_Page("centrocusto_delete");
centrocusto_delete.PageID = "delete"; // Page ID
var EW_PAGE_ID = centrocusto_delete.PageID; // For backward compatibility

// Form object
var fcentrocustodelete = new ew_Form("fcentrocustodelete");

// Form_CustomValidate event
fcentrocustodelete.Form_CustomValidate = 
...
fcentrocustodelete.ValidateRequired = true;
<?php } else { ?>
fcentrocustodelete.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fcentrocustodelete.Lists["x_nu_area"] = {"LinkField":"x_nu_area","Ajax":null,"AutoFill":false,"DisplayFields":["x_no_area","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
if ($centrocusto_delete->Recordset = $centrocusto_delete->LoadRecordset())
	$centrocusto_deleteTotalRecs = $centrocusto_delete->Recordset->RecordCount(); // Get record count
if ($centrocusto_deleteTotalRecs <= 0) { // No record found, exit
	if ($centrocusto_delete->Recordset)
		$centrocusto_delete->Recordset->Close();
	$centrocusto_delete->Page_Terminate("centrocustolist.php"); // Return to list
}
?>
<?php $Breadcrumb->Render(); ?>
<?php $centrocusto_delete->ShowPageHeader(); ?>
<?php
$centrocusto_delete->ShowMessage();
?>
<form name="fcentrocustodelete" id="fcentrocustodelete" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="centrocusto">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($centrocusto_delete->RecKeys as $key) { ?>
<?php $keyvalue = is_array($key) ? implode($EW_COMPOSITE_KEY_SEPARATOR, $key) : $key; ?>
<input type="hidden" name="key_m[]" value="<?php echo ew_HtmlEncode($keyvalue) ?>">
<?php } ?>
<table cellspacing="0" class="ewGrid"><tr><td class="ewGridContent">
<div class="ewGridMiddlePanel">
<table id="tbl_centrocustodelete" class="ewTable ewTableSeparate">
<?php echo $centrocusto->TableCustomInnerHtml ?>
	<thead>
	<tr class="ewTableHeader">
		<td><span id="elh_centrocusto_nu_area" class="centrocusto_nu_area"><?php echo $centrocusto->nu_area->FldCaption() ?></span></td>
		<td><span id="elh_centrocusto_co_alternativo" class="centrocusto_co_alternativo"><?php echo $centrocusto->co_alternativo->FldCaption() ?></span></td>
		<td><span id="elh_centrocusto_no_centroCusto" class="centrocusto_no_centroCusto"><?php echo $centrocusto->no_centroCusto->FldCaption() ?></span></td>
		<td><span id="elh_centrocusto_ic_ativo" class="centrocusto_ic_ativo"><?php echo $centrocusto->ic_ativo->FldCaption() ?></span></td>
	</tr>
	</thead>
	<tbody>
<?php
$centrocusto_delete->RecCnt = 0;
$i = 0;
while (!$centrocusto_delete->Recordset->EOF) {
	$centrocusto_delete->RecCnt++;
	$centrocusto_delete->RowCnt++;

	// Set row properties
	$centrocusto->ResetAttrs();
	$centrocusto->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$centrocusto_delete->LoadRowValues($centrocusto_delete->Recordset);

	// Render row
	$centrocusto_delete->RenderRow();
?>
	<tr<?php echo $centrocusto->RowAttributes() ?>>
		<td<?php echo $centrocusto->nu_area->CellAttributes() ?>>
<span id="el<?php echo $centrocusto_delete->RowCnt ?>_centrocusto_nu_area" class="control-group centrocusto_nu_area">
...
<span id="el<?php echo $centrocusto_delete->RowCnt ?>_centrocusto_co_alternativo" class="control-group centrocusto_co_alternativo">
...
<span id="el<?php echo $centrocusto_delete->RowCnt ?>_centrocusto_no_centroCusto" class="control-group centrocusto_no_centroCusto">
...
<span id="el<?php echo $centrocusto_delete->RowCnt ?>_centrocusto_ic_ativo" class="control-group centrocusto_ic_ativo">
...
<?php
	$centrocusto_delete->Recordset->MoveNext();
}
$centrocusto_delete->Recordset->Close();
?>
</tbody>
...
<script type="text/javascript">
fcentrocustodelete.Init();
</script>
<?php
$centrocusto_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$centrocusto_delete->Page_Terminate();
?>
