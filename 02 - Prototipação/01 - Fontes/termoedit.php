...
$termo_edit = NULL; // Initialize page object first

class ctermo_edit extends ctermo {
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->nu_termo->CurrentValue == "")
			$this->Page_Terminate("termolist.php"); // Invalid key, return to list
	...
	function LoadFormValues() {
		...
		if (!$this->nu_termo->FldIsDetailKey)
			$this->nu_termo->setFormValue($objForm->GetValue("x_nu_termo"));
	}
	...
	function RestoreFormValues() {
		global $objForm;
		$this->LoadRow();
		$this->nu_termo->CurrentValue = $this->nu_termo->FormValue;
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// no_termo
			$this->no_termo->SetDbValueDef($rsnew, $this->no_termo->CurrentValue, "", $this->no_termo->ReadOnly);

			// ds_termo
			$this->ds_termo->SetDbValueDef($rsnew, $this->ds_termo->CurrentValue, "", $this->ds_termo->ReadOnly);

			// ic_tpTermo
			$this->ic_tpTermo->SetDbValueDef($rsnew, $this->ic_tpTermo->CurrentValue, "", $this->ic_tpTermo->ReadOnly);

			// dt_emissao
			$this->dt_emissao->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->dt_emissao->CurrentValue, 7), NULL, $this->dt_emissao->ReadOnly);
...
// Create page object
if (!isset($termo_edit)) $termo_edit = new ctermo_edit();

// Page init
$termo_edit->Page_Init();

// Page main
$termo_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$termo_edit->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var termo_edit = new ew_Page("termo_edit");
termo_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = termo_edit.PageID; // For backward compatibility

// Form object
var ftermoedit = new ew_Form("ftermoedit");

// Validate form
ftermoedit.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_HasValue(elm))
				return this.OnError(elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($termo->ic_situacao->FldCaption()) ?>");
...
ftermoedit.Form_CustomValidate = 
...
ftermoedit.ValidateRequired = true;
<?php } else { ?>
ftermoedit.ValidateRequired = false; 
<?php } ?>
...
<?php $termo_edit->ShowPageHeader(); ?>
<?php
$termo_edit->ShowMessage();
?>
<form name="ftermoedit" id="ftermoedit" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="termo">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_termoedit" class="table table-bordered table-striped">
...
ew_CreateCalendar("ftermoedit", "x_dt_emissao", "%d/%m/%Y");
</script>
<?php } ?>
</span>
...
<?php if ($termo->ic_situacao->Visible) { // ic_situacao ?>
	<tr id="r_ic_situacao">
		<td><span id="elh_termo_ic_situacao"><?php echo $termo->ic_situacao->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
		<td<?php echo $termo->ic_situacao->CellAttributes() ?>>
<span id="el_termo_ic_situacao" class="control-group">
<select data-field="x_ic_situacao" id="x_ic_situacao" name="x_ic_situacao"<?php echo $termo->ic_situacao->EditAttributes() ?>>
<?php
if (is_array($termo->ic_situacao->EditValue)) {
	$arwrk = $termo->ic_situacao->EditValue;
	$rowswrk = count($arwrk);
	$emptywrk = TRUE;
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($termo->ic_situacao->CurrentValue) == strval($arwrk[$rowcntwrk][0])) ? " selected=\"selected\"" : "";
		if ($selwrk <> "") $emptywrk = FALSE;
?>
<option value="<?php echo ew_HtmlEncode($arwrk[$rowcntwrk][0]) ?>"<?php echo $selwrk ?>>
<?php echo $arwrk[$rowcntwrk][1] ?>
...
<?php echo $termo->ic_situacao->CustomMsg ?></td>
	</tr>
<?php } ?>
</table>
</td></tr></table>
<input type="hidden" data-field="x_nu_termo" name="x_nu_termo" id="x_nu_termo" value="<?php echo ew_HtmlEncode($termo->nu_termo->CurrentValue) ?>">
