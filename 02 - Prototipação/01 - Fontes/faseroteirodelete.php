...
$faseroteiro_delete = NULL; // Initialize page object first

class cfaseroteiro_delete extends cfaseroteiro {
	...
	// Page object name
	var $PageObjName = 'faseroteiro_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("faseroteirolist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in faseroteiro class, faseroteiroinfo.php
...
// Create page object
if (!isset($faseroteiro_delete)) $faseroteiro_delete = new cfaseroteiro_delete();

// Page init
$faseroteiro_delete->Page_Init();

// Page main
$faseroteiro_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$faseroteiro_delete->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var faseroteiro_delete = new ew_Page("faseroteiro_delete");
faseroteiro_delete.PageID = "delete"; // Page ID
var EW_PAGE_ID = faseroteiro_delete.PageID; // For backward compatibility

// Form object
var ffaseroteirodelete = new ew_Form("ffaseroteirodelete");

// Form_CustomValidate event
ffaseroteirodelete.Form_CustomValidate = 
...
ffaseroteirodelete.ValidateRequired = true;
<?php } else { ?>
ffaseroteirodelete.ValidateRequired = false; 
<?php } ?>
...
if ($faseroteiro_delete->Recordset = $faseroteiro_delete->LoadRecordset())
	$faseroteiro_deleteTotalRecs = $faseroteiro_delete->Recordset->RecordCount(); // Get record count
if ($faseroteiro_deleteTotalRecs <= 0) { // No record found, exit
	if ($faseroteiro_delete->Recordset)
		$faseroteiro_delete->Recordset->Close();
	$faseroteiro_delete->Page_Terminate("faseroteirolist.php"); // Return to list
}
?>
<?php $Breadcrumb->Render(); ?>
<?php $faseroteiro_delete->ShowPageHeader(); ?>
<?php
$faseroteiro_delete->ShowMessage();
?>
<form name="ffaseroteirodelete" id="ffaseroteirodelete" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="faseroteiro">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($faseroteiro_delete->RecKeys as $key) { ?>
<?php $keyvalue = is_array($key) ? implode($EW_COMPOSITE_KEY_SEPARATOR, $key) : $key; ?>
<input type="hidden" name="key_m[]" value="<?php echo ew_HtmlEncode($keyvalue) ?>">
<?php } ?>
<table cellspacing="0" class="ewGrid"><tr><td class="ewGridContent">
<div class="ewGridMiddlePanel">
<table id="tbl_faseroteirodelete" class="ewTable ewTableSeparate">
<?php echo $faseroteiro->TableCustomInnerHtml ?>
	<thead>
	<tr class="ewTableHeader">
		<td><span id="elh_faseroteiro_no_faseRoteiro" class="faseroteiro_no_faseRoteiro"><?php echo $faseroteiro->no_faseRoteiro->FldCaption() ?></span></td>
		<td><span id="elh_faseroteiro_pc_distribuicao" class="faseroteiro_pc_distribuicao"><?php echo $faseroteiro->pc_distribuicao->FldCaption() ?></span></td>
		<td><span id="elh_faseroteiro_ic_ativo" class="faseroteiro_ic_ativo"><?php echo $faseroteiro->ic_ativo->FldCaption() ?></span></td>
		<td><span id="elh_faseroteiro_nu_ordem" class="faseroteiro_nu_ordem"><?php echo $faseroteiro->nu_ordem->FldCaption() ?></span></td>
	</tr>
	</thead>
	<tbody>
<?php
$faseroteiro_delete->RecCnt = 0;
$i = 0;
while (!$faseroteiro_delete->Recordset->EOF) {
	$faseroteiro_delete->RecCnt++;
	$faseroteiro_delete->RowCnt++;

	// Set row properties
	$faseroteiro->ResetAttrs();
	$faseroteiro->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$faseroteiro_delete->LoadRowValues($faseroteiro_delete->Recordset);

	// Render row
	$faseroteiro_delete->RenderRow();
?>
	<tr<?php echo $faseroteiro->RowAttributes() ?>>
		<td<?php echo $faseroteiro->no_faseRoteiro->CellAttributes() ?>>
<span id="el<?php echo $faseroteiro_delete->RowCnt ?>_faseroteiro_no_faseRoteiro" class="control-group faseroteiro_no_faseRoteiro">
...
<span id="el<?php echo $faseroteiro_delete->RowCnt ?>_faseroteiro_pc_distribuicao" class="control-group faseroteiro_pc_distribuicao">
...
<span id="el<?php echo $faseroteiro_delete->RowCnt ?>_faseroteiro_ic_ativo" class="control-group faseroteiro_ic_ativo">
...
<span id="el<?php echo $faseroteiro_delete->RowCnt ?>_faseroteiro_nu_ordem" class="control-group faseroteiro_nu_ordem">
...
<?php
	$faseroteiro_delete->Recordset->MoveNext();
}
$faseroteiro_delete->Recordset->Close();
?>
</tbody>
...
<script type="text/javascript">
ffaseroteirodelete.Init();
</script>
<?php
$faseroteiro_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$faseroteiro_delete->Page_Terminate();
?>
