...
$mensagem_addopt = NULL; // Initialize page object first

class cmensagem_addopt extends cmensagem {
	...
	// Page object name
	var $PageObjName = 'mensagem_addopt';
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow()) { // Add successful
					$row = array();
					$row["x_nu_mensagem"] = $this->nu_mensagem->DbValue;
					$row["x_co_alternativo"] = $this->co_alternativo->DbValue;
					$row["x_no_mensagem"] = $this->no_mensagem->DbValue;
					$row["x_ds_mensagem"] = $this->ds_mensagem->DbValue;
					$row["x_ds_ajuda"] = $this->ds_ajuda->DbValue;
					$row["x_nu_stMensagem"] = $this->nu_stMensagem->DbValue;
					if (!EW_DEBUG_ENABLED && ob_get_length())
						ob_end_clean();
	...
	function LoadFormValues() {
		...
		if (!$this->no_mensagem->FldIsDetailKey) {
			$this->no_mensagem->setFormValue(ew_ConvertFromUtf8($objForm->GetValue("x_no_mensagem")));
		}
		if (!$this->ds_mensagem->FldIsDetailKey) {
			$this->ds_mensagem->setFormValue(ew_ConvertFromUtf8($objForm->GetValue("x_ds_mensagem")));
		}
		if (!$this->ds_ajuda->FldIsDetailKey) {
			$this->ds_ajuda->setFormValue(ew_ConvertFromUtf8($objForm->GetValue("x_ds_ajuda")));
		}
		if (!$this->nu_stMensagem->FldIsDetailKey) {
			$this->nu_stMensagem->setFormValue(ew_ConvertFromUtf8($objForm->GetValue("x_nu_stMensagem")));
		}
	}
	...
	function RestoreFormValues() {
		...
		$this->no_mensagem->CurrentValue = ew_ConvertToUtf8($this->no_mensagem->FormValue);
		$this->ds_mensagem->CurrentValue = ew_ConvertToUtf8($this->ds_mensagem->FormValue);
		$this->ds_ajuda->CurrentValue = ew_ConvertToUtf8($this->ds_ajuda->FormValue);
		$this->nu_stMensagem->CurrentValue = ew_ConvertToUtf8($this->nu_stMensagem->FormValue);
	}
...
// Create page object
if (!isset($mensagem_addopt)) $mensagem_addopt = new cmensagem_addopt();

// Page init
$mensagem_addopt->Page_Init();

// Page main
$mensagem_addopt->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$mensagem_addopt->Page_Render();
?>
<script type="text/javascript">

// Page object
var mensagem_addopt = new ew_Page("mensagem_addopt");
mensagem_addopt.PageID = "addopt"; // Page ID
var EW_PAGE_ID = mensagem_addopt.PageID; // For backward compatibility

// Form object
var fmensagemaddopt = new ew_Form("fmensagemaddopt");

// Validate form
fmensagemaddopt.Validate = function() {
...
fmensagemaddopt.Form_CustomValidate = 
...
fmensagemaddopt.ValidateRequired = true;
<?php } else { ?>
fmensagemaddopt.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fmensagemaddopt.Lists["x_nu_stMensagem"] = {"LinkField":"x_nu_stMensagem","Ajax":null,"AutoFill":false,"DisplayFields":["x_no_stMensagem","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php
$mensagem_addopt->ShowMessage();
?>
<form name="fmensagemaddopt" id="fmensagemaddopt" class="ewForm form-horizontal" action="mensagemaddopt.php" method="post">
<input type="hidden" name="t" value="mensagem">
<input type="hidden" name="a_addopt" id="a_addopt" value="A">
<div id="tbl_mensagemaddopt">
	<div class="control-group">
		<label class="control-label" for="x_co_alternativo"><?php echo $mensagem->co_alternativo->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="controls">
<input type="text" data-field="x_co_alternativo" name="x_co_alternativo" id="x_co_alternativo" size="30" maxlength="20" placeholder="<?php echo $mensagem->co_alternativo->PlaceHolder ?>" value="<?php echo $mensagem->co_alternativo->EditValue ?>"<?php echo $mensagem->co_alternativo->EditAttributes() ?>>
	...
	<div class="control-group">
		<label class="control-label" for="x_no_mensagem"><?php echo $mensagem->no_mensagem->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="controls">
<input type="text" data-field="x_no_mensagem" name="x_no_mensagem" id="x_no_mensagem" size="30" maxlength="120" placeholder="<?php echo $mensagem->no_mensagem->PlaceHolder ?>" value="<?php echo $mensagem->no_mensagem->EditValue ?>"<?php echo $mensagem->no_mensagem->EditAttributes() ?>>
	...
	<div class="control-group">
		<label class="control-label" for="x_ds_mensagem"><?php echo $mensagem->ds_mensagem->FldCaption() ?></label>
		<div class="controls">
<textarea data-field="x_ds_mensagem" name="x_ds_mensagem" id="x_ds_mensagem" cols="35" rows="4" placeholder="<?php echo $mensagem->ds_mensagem->PlaceHolder ?>"<?php echo $mensagem->ds_mensagem->EditAttributes() ?>><?php echo $mensagem->ds_mensagem->EditValue ?></textarea>
	...
	<div class="control-group">
		<label class="control-label" for="x_ds_ajuda"><?php echo $mensagem->ds_ajuda->FldCaption() ?></label>
		<div class="controls">
<textarea data-field="x_ds_ajuda" name="x_ds_ajuda" id="x_ds_ajuda" cols="35" rows="4" placeholder="<?php echo $mensagem->ds_ajuda->PlaceHolder ?>"<?php echo $mensagem->ds_ajuda->EditAttributes() ?>><?php echo $mensagem->ds_ajuda->EditValue ?></textarea>
	...
	<div class="control-group">
		<label class="control-label" for="x_nu_stMensagem"><?php echo $mensagem->nu_stMensagem->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="controls">
...
<script type="text/javascript">
fmensagemaddopt.Lists["x_nu_stMensagem"].Options = <?php echo (is_array($mensagem->nu_stMensagem->EditValue)) ? ew_ArrayToJson($mensagem->nu_stMensagem->EditValue, 1) : "[]" ?>;
</script>
</div>
	</div>
</div>
...
<?php
$mensagem_addopt->Page_Terminate();
?>
