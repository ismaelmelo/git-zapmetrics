...
$ciialternativa_grid = NULL; // Initialize page object first

class cciialternativa_grid extends cciialternativa {
	...
	// Page object name
	var $PageObjName = 'ciialternativa_grid';

	// Grid form hidden field names
	var $FormName = 'fciialternativagrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["ciialternativa"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->nu_alternativa->CurrentValue;
	...
	function EmptyRow() {
		...
		if ($objForm->HasValue("x_vr_alternativa") && $objForm->HasValue("o_vr_alternativa") && $this->vr_alternativa->CurrentValue <> $this->vr_alternativa->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_nu_peso") && $objForm->HasValue("o_nu_peso") && $this->nu_peso->CurrentValue <> $this->nu_peso->OldValue)
			return FALSE;
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->nu_alternativa->CurrentValue . "\">";
		}
	...
	function LoadFormValues() {
		...
		$this->nu_peso->setOldValue($objForm->GetValue("o_nu_peso"));
		...
		if (!$this->nu_alternativa->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->nu_alternativa->CurrentValue = strval($arKeys[0]); // nu_alternativa
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			$this->vr_alternativa->EditValue = ew_FormatNumber($this->vr_alternativa->EditValue, -2, -1, -2, 0);
	...
	function AddRow($rsold = NULL) {
		...
		if ($this->co_questao->getSessionValue() <> "") {
			$rsnew['co_questao'] = $this->co_questao->getSessionValue();
		}
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "ciiquestao") {
			$this->co_questao->Visible = FALSE;
			if ($GLOBALS["ciiquestao"]->EventCancelled) $this->EventCancelled = TRUE;
		}
