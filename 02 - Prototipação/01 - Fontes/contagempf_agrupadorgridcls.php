...
$contagempf_agrupador_grid = NULL; // Initialize page object first

class ccontagempf_agrupador_grid extends ccontagempf_agrupador {
	...
	// Page object name
	var $PageObjName = 'contagempf_agrupador_grid';

	// Grid form hidden field names
	var $FormName = 'fcontagempf_agrupadorgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["contagempf_agrupador"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function SetupKeyValues($key) {
		...
		if (count($arrKeyFlds) >= 2) {
			...
			if (!is_numeric($this->nu_contagem->FormValue))
				return FALSE;
			$this->nu_agrupador->setFormValue($arrKeyFlds[1]);
			if (!is_numeric($this->nu_agrupador->FormValue))
				return FALSE;
		}
		return TRUE;
	}
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					if ($sKey <> "") $sKey .= $GLOBALS["EW_COMPOSITE_KEY_SEPARATOR"];
					$sKey .= $this->nu_agrupador->CurrentValue;
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_no_agrupador") && $objForm->HasValue("o_no_agrupador") && $this->no_agrupador->CurrentValue <> $this->no_agrupador->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->nu_contagem->CurrentValue . $GLOBALS["EW_COMPOSITE_KEY_SEPARATOR"] . $this->nu_agrupador->CurrentValue . "\">";
		}
	...
	function LoadFormValues() {
		...
		$this->no_agrupador->setOldValue($objForm->GetValue("o_no_agrupador"));
		if (!$this->nu_contagem->FldIsDetailKey)
			$this->nu_contagem->setFormValue($objForm->GetValue("x_nu_contagem"));
		if (!$this->nu_agrupador->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 2) {
			if (strval($arKeys[0]) <> "")
				...
				$bValidKey = FALSE;
			if (strval($arKeys[1]) <> "")
				$this->nu_agrupador->CurrentValue = strval($arKeys[1]); // nu_agrupador
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// no_agrupador
			$this->no_agrupador->SetDbValueDef($rsnew, $this->no_agrupador->CurrentValue, NULL, $this->no_agrupador->ReadOnly);
