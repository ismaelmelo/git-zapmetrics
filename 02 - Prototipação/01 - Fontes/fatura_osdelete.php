...
$fatura_os_delete = NULL; // Initialize page object first

class cfatura_os_delete extends cfatura_os {
	...
	// Page object name
	var $PageObjName = 'fatura_os_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("fatura_oslist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in fatura_os class, fatura_osinfo.php
...
// Create page object
if (!isset($fatura_os_delete)) $fatura_os_delete = new cfatura_os_delete();

// Page init
$fatura_os_delete->Page_Init();

// Page main
$fatura_os_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$fatura_os_delete->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var fatura_os_delete = new ew_Page("fatura_os_delete");
fatura_os_delete.PageID = "delete"; // Page ID
var EW_PAGE_ID = fatura_os_delete.PageID; // For backward compatibility

// Form object
var ffatura_osdelete = new ew_Form("ffatura_osdelete");

// Form_CustomValidate event
ffatura_osdelete.Form_CustomValidate = 
...
ffatura_osdelete.ValidateRequired = true;
<?php } else { ?>
ffatura_osdelete.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ffatura_osdelete.Lists["x_nu_os"] = {"LinkField":"x_nu_os","Ajax":null,"AutoFill":false,"DisplayFields":["x_no_titulo","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
if ($fatura_os_delete->Recordset = $fatura_os_delete->LoadRecordset())
	$fatura_os_deleteTotalRecs = $fatura_os_delete->Recordset->RecordCount(); // Get record count
if ($fatura_os_deleteTotalRecs <= 0) { // No record found, exit
	if ($fatura_os_delete->Recordset)
		$fatura_os_delete->Recordset->Close();
	$fatura_os_delete->Page_Terminate("fatura_oslist.php"); // Return to list
}
?>
<?php $Breadcrumb->Render(); ?>
<?php $fatura_os_delete->ShowPageHeader(); ?>
<?php
$fatura_os_delete->ShowMessage();
?>
<form name="ffatura_osdelete" id="ffatura_osdelete" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="fatura_os">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($fatura_os_delete->RecKeys as $key) { ?>
<?php $keyvalue = is_array($key) ? implode($EW_COMPOSITE_KEY_SEPARATOR, $key) : $key; ?>
<input type="hidden" name="key_m[]" value="<?php echo ew_HtmlEncode($keyvalue) ?>">
<?php } ?>
<table cellspacing="0" class="ewGrid"><tr><td class="ewGridContent">
<div class="ewGridMiddlePanel">
<table id="tbl_fatura_osdelete" class="ewTable ewTableSeparate">
<?php echo $fatura_os->TableCustomInnerHtml ?>
	<thead>
	<tr class="ewTableHeader">
		<td><span id="elh_fatura_os_nu_os" class="fatura_os_nu_os"><?php echo $fatura_os->nu_os->FldCaption() ?></span></td>
		<td><span id="elh_fatura_os_ic_pagIntegralOs" class="fatura_os_ic_pagIntegralOs"><?php echo $fatura_os->ic_pagIntegralOs->FldCaption() ?></span></td>
		<td><span id="elh_fatura_os_vr_pagoOsFatura" class="fatura_os_vr_pagoOsFatura"><?php echo $fatura_os->vr_pagoOsFatura->FldCaption() ?></span></td>
	</tr>
	</thead>
	<tbody>
<?php
$fatura_os_delete->RecCnt = 0;
$i = 0;
while (!$fatura_os_delete->Recordset->EOF) {
	$fatura_os_delete->RecCnt++;
	$fatura_os_delete->RowCnt++;

	// Set row properties
	$fatura_os->ResetAttrs();
	$fatura_os->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$fatura_os_delete->LoadRowValues($fatura_os_delete->Recordset);

	// Render row
	$fatura_os_delete->RenderRow();
?>
	<tr<?php echo $fatura_os->RowAttributes() ?>>
		<td<?php echo $fatura_os->nu_os->CellAttributes() ?>>
<span id="el<?php echo $fatura_os_delete->RowCnt ?>_fatura_os_nu_os" class="control-group fatura_os_nu_os">
...
<span id="el<?php echo $fatura_os_delete->RowCnt ?>_fatura_os_ic_pagIntegralOs" class="control-group fatura_os_ic_pagIntegralOs">
...
<span id="el<?php echo $fatura_os_delete->RowCnt ?>_fatura_os_vr_pagoOsFatura" class="control-group fatura_os_vr_pagoOsFatura">
...
<?php
	$fatura_os_delete->Recordset->MoveNext();
}
$fatura_os_delete->Recordset->Close();
?>
</tbody>
...
<script type="text/javascript">
ffatura_osdelete.Init();
</script>
<?php
$fatura_os_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$fatura_os_delete->Page_Terminate();
?>
