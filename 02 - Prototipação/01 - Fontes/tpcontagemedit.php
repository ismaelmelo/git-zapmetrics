...
$tpcontagem_edit = NULL; // Initialize page object first

class ctpcontagem_edit extends ctpcontagem {
	...
	// Page object name
	var $PageObjName = 'tpcontagem_edit';
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->nu_tpContagem->CurrentValue == "")
			$this->Page_Terminate("tpcontagemlist.php"); // Invalid key, return to list
	...
	function ValidateForm() {
		...
		if (in_array("proposito", $DetailTblVar) && $GLOBALS["proposito"]->DetailEdit) {
		...
		if (in_array("tpmanutencao", $DetailTblVar) && $GLOBALS["tpmanutencao"]->DetailEdit) {
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// no_tpContagem
			$this->no_tpContagem->SetDbValueDef($rsnew, $this->no_tpContagem->CurrentValue, "", $this->no_tpContagem->ReadOnly);
			...
			if ($bUpdateRow) {
				...
				if ($EditRow) {
					...
					if (in_array("proposito", $DetailTblVar) && $GLOBALS["proposito"]->DetailEdit) {
						if (!isset($GLOBALS["proposito_grid"])) $GLOBALS["proposito_grid"] = new cproposito_grid(); // Get detail page object
						$EditRow = $GLOBALS["proposito_grid"]->GridUpdate();
					}
					if (in_array("tpmanutencao", $DetailTblVar) && $GLOBALS["tpmanutencao"]->DetailEdit) {
						if (!isset($GLOBALS["tpmanutencao_grid"])) $GLOBALS["tpmanutencao_grid"] = new ctpmanutencao_grid(); // Get detail page object
						$EditRow = $GLOBALS["tpmanutencao_grid"]->GridUpdate();
					}
				}
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("proposito", $DetailTblVar)) {
				...
				if ($GLOBALS["proposito_grid"]->DetailEdit) {
					$GLOBALS["proposito_grid"]->CurrentMode = "edit";
					$GLOBALS["proposito_grid"]->CurrentAction = "gridedit";
			...
			if (in_array("tpmanutencao", $DetailTblVar)) {
				...
				if ($GLOBALS["tpmanutencao_grid"]->DetailEdit) {
					$GLOBALS["tpmanutencao_grid"]->CurrentMode = "edit";
					$GLOBALS["tpmanutencao_grid"]->CurrentAction = "gridedit";
...
// Create page object
if (!isset($tpcontagem_edit)) $tpcontagem_edit = new ctpcontagem_edit();

// Page init
$tpcontagem_edit->Page_Init();

// Page main
$tpcontagem_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$tpcontagem_edit->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var tpcontagem_edit = new ew_Page("tpcontagem_edit");
tpcontagem_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = tpcontagem_edit.PageID; // For backward compatibility

// Form object
var ftpcontagemedit = new ew_Form("ftpcontagemedit");

// Validate form
ftpcontagemedit.Validate = function() {
...
ftpcontagemedit.Form_CustomValidate = 
...
ftpcontagemedit.ValidateRequired = true;
<?php } else { ?>
ftpcontagemedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ftpcontagemedit.Lists["x_nu_tpMetrica"] = {"LinkField":"x_nu_tpMetrica","Ajax":null,"AutoFill":false,"DisplayFields":["x_no_tpMetrica","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $tpcontagem_edit->ShowPageHeader(); ?>
<?php
$tpcontagem_edit->ShowMessage();
?>
<form name="ftpcontagemedit" id="ftpcontagemedit" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="tpcontagem">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_tpcontagemedit" class="table table-bordered table-striped">
...
<?php echo $tpcontagem->nu_tpMetrica->EditValue ?></span>
</span>
<input type="hidden" data-field="x_nu_tpMetrica" name="x_nu_tpMetrica" id="x_nu_tpMetrica" value="<?php echo ew_HtmlEncode($tpcontagem->nu_tpMetrica->CurrentValue) ?>">
...
<input type="hidden" data-field="x_nu_tpContagem" name="x_nu_tpContagem" id="x_nu_tpContagem" value="<?php echo ew_HtmlEncode($tpcontagem->nu_tpContagem->CurrentValue) ?>">
<?php
	if (in_array("proposito", explode(",", $tpcontagem->getCurrentDetailTable())) && $proposito->DetailEdit) {
?>
<?php include_once "propositogrid.php" ?>
<?php } ?>
<?php
	if (in_array("tpmanutencao", explode(",", $tpcontagem->getCurrentDetailTable())) && $tpmanutencao->DetailEdit) {
?>
...
<script type="text/javascript">
ftpcontagemedit.Init();
<?php if (EW_MOBILE_REFLOW && ew_IsMobile()) { ?>
ew_Reflow();
<?php } ?>
</script>
<?php
$tpcontagem_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$tpcontagem_edit->Page_Terminate();
?>
