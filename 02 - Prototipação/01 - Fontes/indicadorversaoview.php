...
$indicadorversao_view = NULL; // Initialize page object first

class cindicadorversao_view extends cindicadorversao {
	...
	// Page object name
	var $PageObjName = 'indicadorversao_view';
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["nu_indicador"] <> "") {
				...
				$sReturnUrl = "indicadorversaolist.php"; // Return to list
			}
			if (@$_GET["nu_versao"] <> "") {
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "indicadorversaolist.php"; // No matching record, return to list
					}
			}
			...
			$sReturnUrl = "indicadorversaolist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupOtherOptions() {
		...
		// Detail table 'indicador'
		$body = $Language->TablePhrase("indicador", "TblCaption");
		$body = "<a class=\"ewAction ewDetailList\" href=\"" . ew_HtmlEncode("indicadorlist.php?" . EW_TABLE_SHOW_MASTER . "=indicadorversao&nu_indicador=" . strval($this->nu_indicador->CurrentValue) . "") . "\">" . $body . "</a>";
		$item = &$option->Add("detail_indicador");
		$item->Body = $body;
		$item->Visible = $Security->AllowList(CurrentProjectID() . 'indicador');
		if ($item->Visible) {
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a id=\"emf_indicadorversao\" href=\"javascript:void(0);\" class=\"ewExportLink ewEmail\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_indicadorversao',hdr:ewLanguage.Phrase('ExportToEmail'),f:document.findicadorversaoview,key:" . ew_ArrayToJsonAttr($this->RecKey) . ",sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("indicador", $DetailTblVar)) {
				...
				if ($GLOBALS["indicador_grid"]->DetailView) {
					$GLOBALS["indicador_grid"]->CurrentMode = "view";
...
// Create page object
if (!isset($indicadorversao_view)) $indicadorversao_view = new cindicadorversao_view();

// Page init
$indicadorversao_view->Page_Init();

// Page main
$indicadorversao_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$indicadorversao_view->Page_Render();
?>
...
// Page object
var indicadorversao_view = new ew_Page("indicadorversao_view");
indicadorversao_view.PageID = "view"; // Page ID
var EW_PAGE_ID = indicadorversao_view.PageID; // For backward compatibility

// Form object
var findicadorversaoview = new ew_Form("findicadorversaoview");

// Form_CustomValidate event
findicadorversaoview.Form_CustomValidate = 
...
findicadorversaoview.ValidateRequired = true;
<?php } else { ?>
findicadorversaoview.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
findicadorversaoview.Lists["x_nu_indicador"] = {"LinkField":"x_nu_indicador","Ajax":null,"AutoFill":false,"DisplayFields":["x_no_indicador","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $indicadorversao_view->ExportOptions->Render("body") ?>
<?php if (!$indicadorversao_view->ExportOptions->UseDropDownButton) { ?>
</div>
<div class="ewViewOtherOptions">
<?php } ?>
<?php
	foreach ($indicadorversao_view->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<?php } ?>
<?php $indicadorversao_view->ShowPageHeader(); ?>
<?php
$indicadorversao_view->ShowMessage();
?>
<form name="findicadorversaoview" id="findicadorversaoview" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="indicadorversao">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_indicadorversaoview" class="table table-bordered table-striped">
<?php if ($indicadorversao->nu_indicador->Visible) { // nu_indicador ?>
	<tr id="r_nu_indicador">
		<td><span id="elh_indicadorversao_nu_indicador"><?php echo $indicadorversao->nu_indicador->FldCaption() ?></span></td>
...
<?php if ($indicadorversao->nu_versao->Visible) { // nu_versao ?>
	<tr id="r_nu_versao">
		<td><span id="elh_indicadorversao_nu_versao"><?php echo $indicadorversao->nu_versao->FldCaption() ?></span></td>
...
<?php if ($indicadorversao->ic_periodicidadeGeracao->Visible) { // ic_periodicidadeGeracao ?>
	<tr id="r_ic_periodicidadeGeracao">
		<td><span id="elh_indicadorversao_ic_periodicidadeGeracao"><?php echo $indicadorversao->ic_periodicidadeGeracao->FldCaption() ?></span></td>
...
<span<?php echo $indicadorversao->ds_origemIndicador->ViewAttributes() ?>>
<?php echo $indicadorversao->ds_origemIndicador->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($indicadorversao->ic_reponsavelColetaCtrl->Visible) { // ic_reponsavelColetaCtrl ?>
	<tr id="r_ic_reponsavelColetaCtrl">
		<td><span id="elh_indicadorversao_ic_reponsavelColetaCtrl"><?php echo $indicadorversao->ic_reponsavelColetaCtrl->FldCaption() ?></span></td>
...
<span<?php echo $indicadorversao->ds_codigoSql->ViewAttributes() ?>>
<?php echo $indicadorversao->ds_codigoSql->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($indicadorversao->dh_versao->Visible) { // dh_versao ?>
	<tr id="r_dh_versao">
		<td><span id="elh_indicadorversao_dh_versao"><?php echo $indicadorversao->dh_versao->FldCaption() ?></span></td>
...
<?php
	if (in_array("indicador", explode(",", $indicadorversao->getCurrentDetailTable())) && $indicador->DetailView) {
?>
<?php include_once "indicadorgrid.php" ?>
<?php } ?>
</form>
<script type="text/javascript">
findicadorversaoview.Init();
</script>
<?php
$indicadorversao_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($indicadorversao->Export == "") { ?>
...
<?php
$indicadorversao_view->Page_Terminate();
?>
