...
$indicador_view = NULL; // Initialize page object first

class cindicador_view extends cindicador {
	...
	// Page object name
	var $PageObjName = 'indicador_view';
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["nu_indicador"] <> "") {
				...
				$sReturnUrl = "indicadorlist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "indicadorlist.php"; // No matching record, return to list
					}
			}
			...
			$sReturnUrl = "indicadorlist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a id=\"emf_indicador\" href=\"javascript:void(0);\" class=\"ewExportLink ewEmail\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_indicador',hdr:ewLanguage.Phrase('ExportToEmail'),f:document.findicadorview,key:" . ew_ArrayToJsonAttr($this->RecKey) . ",sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
...
// Create page object
if (!isset($indicador_view)) $indicador_view = new cindicador_view();

// Page init
$indicador_view->Page_Init();

// Page main
$indicador_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$indicador_view->Page_Render();
?>
...
// Page object
var indicador_view = new ew_Page("indicador_view");
indicador_view.PageID = "view"; // Page ID
var EW_PAGE_ID = indicador_view.PageID; // For backward compatibility

// Form object
var findicadorview = new ew_Form("findicadorview");

// Form_CustomValidate event
findicadorview.Form_CustomValidate = 
...
findicadorview.ValidateRequired = true;
<?php } else { ?>
findicadorview.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
findicadorview.Lists["x_nu_processoCobit5"] = {"LinkField":"x_nu_processo","Ajax":null,"AutoFill":false,"DisplayFields":["x_co_alternativo","x_no_processo","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $indicador_view->ExportOptions->Render("body") ?>
<?php if (!$indicador_view->ExportOptions->UseDropDownButton) { ?>
</div>
<div class="ewViewOtherOptions">
<?php } ?>
<?php
	foreach ($indicador_view->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<?php } ?>
<?php $indicador_view->ShowPageHeader(); ?>
<?php
$indicador_view->ShowMessage();
?>
<form name="findicadorview" id="findicadorview" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="indicador">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_indicadorview" class="table table-bordered table-striped">
<?php if ($indicador->no_indicador->Visible) { // no_indicador ?>
	<tr id="r_no_indicador">
		<td><span id="elh_indicador_no_indicador"><?php echo $indicador->no_indicador->FldCaption() ?></span></td>
...
<?php if ($indicador->ds_indicador->Visible) { // ds_indicador ?>
	<tr id="r_ds_indicador">
		<td><span id="elh_indicador_ds_indicador"><?php echo $indicador->ds_indicador->FldCaption() ?></span></td>
...
<?php if ($indicador->ic_tpIndicador->Visible) { // ic_tpIndicador ?>
	<tr id="r_ic_tpIndicador">
		<td><span id="elh_indicador_ic_tpIndicador"><?php echo $indicador->ic_tpIndicador->FldCaption() ?></span></td>
...
<?php if ($indicador->ic_ativo->Visible) { // ic_ativo ?>
	<tr id="r_ic_ativo">
		<td><span id="elh_indicador_ic_ativo"><?php echo $indicador->ic_ativo->FldCaption() ?></span></td>
