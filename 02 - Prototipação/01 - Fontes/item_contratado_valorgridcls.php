...
$Item_contratado_valor_grid = NULL; // Initialize page object first

class cItem_contratado_valor_grid extends cItem_contratado_valor {
	...
	// Page object name
	var $PageObjName = 'Item_contratado_valor_grid';

	// Grid form hidden field names
	var $FormName = 'fItem_contratado_valorgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["Item_contratado_valor"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->nu_valor->CurrentValue;
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_vr_item") && $objForm->HasValue("o_vr_item") && $this->vr_item->CurrentValue <> $this->vr_item->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_dt_valor") && $objForm->HasValue("o_dt_valor") && $this->dt_valor->CurrentValue <> $this->dt_valor->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->nu_valor->CurrentValue . "\">";
		}
	...
	function LoadFormValues() {
		...
		$this->vr_item->setOldValue($objForm->GetValue("o_vr_item"));
		...
		$this->dt_valor->setOldValue($objForm->GetValue("o_dt_valor"));
		if (!$this->nu_valor->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->nu_valor->CurrentValue = strval($arKeys[0]); // nu_valor
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			if (strval($this->vr_item->EditValue) <> "" && is_numeric($this->vr_item->EditValue)) {
			$this->vr_item->EditValue = ew_FormatNumber($this->vr_item->EditValue, -2, -1, -2, 0);
			$this->vr_item->OldValue = $this->vr_item->EditValue;
			}
	...
	function AddRow($rsold = NULL) {
		...
		if ($this->nu_itemContratado->getSessionValue() <> "") {
			$sMasterFilter = str_replace("@nu_itemContratado@", ew_AdjustSql($this->nu_itemContratado->getSessionValue()), $sMasterFilter);
		} else {
			$bValidMasterRecord = FALSE;
		}
		if ($bValidMasterRecord) {
		...
		// nu_itemContratado
		if ($this->nu_itemContratado->getSessionValue() <> "") {
			$rsnew['nu_itemContratado'] = $this->nu_itemContratado->getSessionValue();
		}
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "item_contratado") {
			$this->nu_itemContratado->Visible = FALSE;
			if ($GLOBALS["item_contratado"]->EventCancelled) $this->EventCancelled = TRUE;
		}
