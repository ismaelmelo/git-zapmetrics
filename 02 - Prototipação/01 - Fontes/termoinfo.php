...
//
// Table class for termo
//
class ctermo extends cTable {
	var $nu_termo;
	var $no_termo;
	var $ds_termo;
	var $ic_tpTermo;
	...
	function __construct() {
		...
		// nu_termo
		$this->nu_termo = new cField('termo', 'termo', 'x_nu_termo', 'nu_termo', '[nu_termo]', 'CAST([nu_termo] AS NVARCHAR)', 3, -1, FALSE, '[nu_termo]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->nu_termo->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['nu_termo'] = &$this->nu_termo;

		// no_termo
		$this->no_termo = new cField('termo', 'termo', 'x_no_termo', 'no_termo', '[no_termo]', '[no_termo]', 200, -1, FALSE, '[no_termo]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['no_termo'] = &$this->no_termo;

		// ds_termo
		$this->ds_termo = new cField('termo', 'termo', 'x_ds_termo', 'ds_termo', '[ds_termo]', '[ds_termo]', 201, -1, FALSE, '[ds_termo]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['ds_termo'] = &$this->ds_termo;

		// ic_tpTermo
		$this->ic_tpTermo = new cField('termo', 'termo', 'x_ic_tpTermo', 'ic_tpTermo', '[ic_tpTermo]', '[ic_tpTermo]', 129, -1, FALSE, '[ic_tpTermo]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['ic_tpTermo'] = &$this->ic_tpTermo;

		// dt_emissao
		$this->dt_emissao = new cField('termo', 'termo', 'x_dt_emissao', 'dt_emissao', '[dt_emissao]', '(REPLACE(STR(DAY([dt_emissao]),2,0),\' \',\'0\') + \'/\' + REPLACE(STR(MONTH([dt_emissao]),2,0),\' \',\'0\') + \'/\' + STR(YEAR([dt_emissao]),4,0))', 135, 7, FALSE, '[dt_emissao]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->dt_emissao->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateDMY"));
		$this->fields['dt_emissao'] = &$this->dt_emissao;

		// im_anexo
		$this->im_anexo = new cField('termo', 'termo', 'x_im_anexo', 'im_anexo', '[im_anexo]', '[im_anexo]', 200, -1, TRUE, '[im_anexo]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->im_anexo->UploadMultiple = TRUE;
		$this->fields['im_anexo'] = &$this->im_anexo;

		// ic_situacao
		$this->ic_situacao = new cField('termo', 'termo', 'x_ic_situacao', 'ic_situacao', '[ic_situacao]', '[ic_situacao]', 129, -1, FALSE, '[ic_situacao]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['ic_situacao'] = &$this->ic_situacao;

		// nu_usuario
		$this->nu_usuario = new cField('termo', 'termo', 'x_nu_usuario', 'nu_usuario', '[nu_usuario]', 'CAST([nu_usuario] AS NVARCHAR)', 3, -1, FALSE, '[nu_usuario]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->nu_usuario->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['nu_usuario'] = &$this->nu_usuario;

		// ts_datahora
		$this->ts_datahora = new cField('termo', 'termo', 'x_ts_datahora', 'ts_datahora', '[ts_datahora]', '(REPLACE(STR(DAY([ts_datahora]),2,0),\' \',\'0\') + \'/\' + REPLACE(STR(MONTH([ts_datahora]),2,0),\' \',\'0\') + \'/\' + STR(YEAR([ts_datahora]),4,0))', 135, 7, FALSE, '[ts_datahora]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			if (array_key_exists('nu_termo', $rs))
				ew_AddFilter($where, ew_QuotedName('nu_termo') . '=' . ew_QuotedValue($rs['nu_termo'], $this->nu_termo->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function SqlKeyFilter() {
		return "[nu_termo] = @nu_termo@";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		if (!is_numeric($this->nu_termo->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@nu_termo@", ew_AdjustSql($this->nu_termo->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("termoview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("termoview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}
	...
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("termoedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("termoadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("termodelete.php", $this->UrlParm());
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->nu_termo->CurrentValue)) {
			$sUrl .= "nu_termo=" . urlencode($this->nu_termo->CurrentValue);
		} else {
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->nu_termo->CurrentValue = $key;
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				if ($this->nu_termo->Exportable) $Doc->ExportCaption($this->nu_termo);
				if ($this->no_termo->Exportable) $Doc->ExportCaption($this->no_termo);
				if ($this->ds_termo->Exportable) $Doc->ExportCaption($this->ds_termo);
				if ($this->ic_tpTermo->Exportable) $Doc->ExportCaption($this->ic_tpTermo);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					if ($this->nu_termo->Exportable) $Doc->ExportField($this->nu_termo);
					if ($this->no_termo->Exportable) $Doc->ExportField($this->no_termo);
					if ($this->ds_termo->Exportable) $Doc->ExportField($this->ds_termo);
					if ($this->ic_tpTermo->Exportable) $Doc->ExportField($this->ic_tpTermo);
