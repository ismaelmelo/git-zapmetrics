...
$uc_grid = NULL; // Initialize page object first

class cuc_grid extends cuc {
	...
	// Page object name
	var $PageObjName = 'uc_grid';

	// Grid form hidden field names
	var $FormName = 'fucgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["uc"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_co_alternativo") && $objForm->HasValue("o_co_alternativo") && $this->co_alternativo->CurrentValue <> $this->co_alternativo->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_no_uc") && $objForm->HasValue("o_no_uc") && $this->no_uc->CurrentValue <> $this->no_uc->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_nu_stUc") && $objForm->HasValue("o_nu_stUc") && $this->nu_stUc->CurrentValue <> $this->nu_stUc->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->nu_uc->CurrentValue . "\">";
		}
	...
	function LoadFormValues() {
		...
		$this->co_alternativo->setOldValue($objForm->GetValue("o_co_alternativo"));
		if (!$this->no_uc->FldIsDetailKey) {
			$this->no_uc->setFormValue($objForm->GetValue("x_no_uc"));
		}
		$this->no_uc->setOldValue($objForm->GetValue("o_no_uc"));
		if (!$this->nu_stUc->FldIsDetailKey) {
			$this->nu_stUc->setFormValue($objForm->GetValue("x_nu_stUc"));
		}
		$this->nu_stUc->setOldValue($objForm->GetValue("o_nu_stUc"));
		if (!$this->nu_uc->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
	...
	function DeleteRows() {
		...
		// Check if records exist for detail table 'uc_ator'
		$rows = ($rs) ? $rs->GetRows() : array();
		if (!isset($GLOBALS["uc_ator"])) $GLOBALS["uc_ator"] = new cuc_ator();
		foreach ($rows as $row) {
			$rsdetail = $GLOBALS["uc_ator"]->LoadRs("[nu_uc] = " . ew_QuotedValue($row['nu_uc'], EW_DATATYPE_NUMBER));
			if ($rsdetail && !$rsdetail->EOF) {
				$sRelatedRecordMsg = str_replace("%t", "uc_ator", $Language->Phrase("RelatedRecordExists"));
				$this->setFailureMessage($sRelatedRecordMsg);
				return FALSE;
			}
		}

		// Check if records exist for detail table 'uc_mensagem'
		$rows = ($rs) ? $rs->GetRows() : array();
		if (!isset($GLOBALS["uc_mensagem"])) $GLOBALS["uc_mensagem"] = new cuc_mensagem();
		foreach ($rows as $row) {
			$rsdetail = $GLOBALS["uc_mensagem"]->LoadRs("[nu_uc] = " . ew_QuotedValue($row['nu_uc'], EW_DATATYPE_NUMBER));
			if ($rsdetail && !$rsdetail->EOF) {
				$sRelatedRecordMsg = str_replace("%t", "uc_mensagem", $Language->Phrase("RelatedRecordExists"));
				$this->setFailureMessage($sRelatedRecordMsg);
				return FALSE;
			}
		}

		// Check if records exist for detail table 'uc_regranegocio'
		$rows = ($rs) ? $rs->GetRows() : array();
		if (!isset($GLOBALS["uc_regranegocio"])) $GLOBALS["uc_regranegocio"] = new cuc_regranegocio();
		foreach ($rows as $row) {
			$rsdetail = $GLOBALS["uc_regranegocio"]->LoadRs("[nu_uc] = " . ew_QuotedValue($row['nu_uc'], EW_DATATYPE_NUMBER));
			if ($rsdetail && !$rsdetail->EOF) {
				$sRelatedRecordMsg = str_replace("%t", "uc_regranegocio", $Language->Phrase("RelatedRecordExists"));
				$this->setFailureMessage($sRelatedRecordMsg);
				return FALSE;
			}
		}
