...
$contagempf_grid = NULL; // Initialize page object first

class ccontagempf_grid extends ccontagempf {
	...
	// Page object name
	var $PageObjName = 'contagempf_grid';

	// Grid form hidden field names
	var $FormName = 'fcontagempfgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["contagempf"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->nu_contagem->CurrentValue;
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_nu_tpMetrica") && $objForm->HasValue("o_nu_tpMetrica") && $this->nu_tpMetrica->CurrentValue <> $this->nu_tpMetrica->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_nu_tpContagem") && $objForm->HasValue("o_nu_tpContagem") && $this->nu_tpContagem->CurrentValue <> $this->nu_tpContagem->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_nu_sistema") && $objForm->HasValue("o_nu_sistema") && $this->nu_sistema->CurrentValue <> $this->nu_sistema->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_nu_faseMedida") && $objForm->HasValue("o_nu_faseMedida") && $this->nu_faseMedida->CurrentValue <> $this->nu_faseMedida->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_ic_stContagem") && $objForm->HasValue("o_ic_stContagem") && $this->ic_stContagem->CurrentValue <> $this->ic_stContagem->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_pc_varFasesRoteiro") && $objForm->HasValue("o_pc_varFasesRoteiro") && $this->pc_varFasesRoteiro->CurrentValue <> $this->pc_varFasesRoteiro->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_vr_pfFaturamento") && $objForm->HasValue("o_vr_pfFaturamento") && $this->vr_pfFaturamento->CurrentValue <> $this->vr_pfFaturamento->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->nu_contagem->CurrentValue . "\">";
		}
	...
	function LoadDefaultValues() {
		...
		$this->pc_varFasesRoteiro->CurrentValue = NULL;
		$this->pc_varFasesRoteiro->OldValue = $this->pc_varFasesRoteiro->CurrentValue;
		$this->vr_pfFaturamento->CurrentValue = "0";
		$this->vr_pfFaturamento->OldValue = $this->vr_pfFaturamento->CurrentValue;
	}
	...
	function LoadFormValues() {
		...
		// Load from form
		global $objForm;
		if (!$this->nu_contagem->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
		...
		$this->nu_tpMetrica->setOldValue($objForm->GetValue("o_nu_tpMetrica"));
		if (!$this->nu_tpContagem->FldIsDetailKey) {
			$this->nu_tpContagem->setFormValue($objForm->GetValue("x_nu_tpContagem"));
		}
		$this->nu_tpContagem->setOldValue($objForm->GetValue("o_nu_tpContagem"));
		...
		$this->vr_pfFaturamento->setOldValue($objForm->GetValue("o_vr_pfFaturamento"));
	}
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->nu_contagem->CurrentValue = strval($arKeys[0]); // nu_contagem
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			$this->vr_pfFaturamento->EditValue = ew_HtmlEncode($this->vr_pfFaturamento->CurrentValue);
			$this->vr_pfFaturamento->PlaceHolder = ew_HtmlEncode(ew_RemoveHtml($this->vr_pfFaturamento->FldCaption()));
			if (strval($this->vr_pfFaturamento->EditValue) <> "" && is_numeric($this->vr_pfFaturamento->EditValue)) {
			$this->vr_pfFaturamento->EditValue = ew_FormatNumber($this->vr_pfFaturamento->EditValue, -2, -1, -2, 0);
			$this->vr_pfFaturamento->OldValue = $this->vr_pfFaturamento->EditValue;
			}
	...
	function AddRow($rsold = NULL) {
		...
		// vr_pfFaturamento
		$this->vr_pfFaturamento->SetDbValueDef($rsnew, $this->vr_pfFaturamento->CurrentValue, NULL, FALSE);
