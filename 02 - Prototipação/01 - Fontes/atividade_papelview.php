...
$atividade_papel_view = NULL; // Initialize page object first

class catividade_papel_view extends catividade_papel {
	...
	// Page object name
	var $PageObjName = 'atividade_papel_view';
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					...
					if ($this->TotalRecs <= 0) { // No record found
						...
						$this->Page_Terminate("atividade_papellist.php"); // Return to list page
						...
						while (!$this->Recordset->EOF) {
							if (strval($this->nu_atividade->CurrentValue) == strval($this->Recordset->fields('nu_atividade')) && strval($this->co_papel->CurrentValue) == strval($this->Recordset->fields('co_papel'))) {
					...
					if (!$bMatchRecord) {
						if ($this->getSuccessMessage() == "" && $this->getFailureMessage() == "")
							$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record message
						$sReturnUrl = "atividade_papellist.php"; // No matching record, return to list
					} else {
			...
			$sReturnUrl = "atividade_papellist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a id=\"emf_atividade_papel\" href=\"javascript:void(0);\" class=\"ewExportLink ewEmail\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_atividade_papel',hdr:ewLanguage.Phrase('ExportToEmail'),f:document.fatividade_papelview,key:" . ew_ArrayToJsonAttr($this->RecKey) . ",sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
...
// Create page object
if (!isset($atividade_papel_view)) $atividade_papel_view = new catividade_papel_view();

// Page init
$atividade_papel_view->Page_Init();

// Page main
$atividade_papel_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$atividade_papel_view->Page_Render();
?>
...
// Page object
var atividade_papel_view = new ew_Page("atividade_papel_view");
atividade_papel_view.PageID = "view"; // Page ID
var EW_PAGE_ID = atividade_papel_view.PageID; // For backward compatibility

// Form object
var fatividade_papelview = new ew_Form("fatividade_papelview");

// Form_CustomValidate event
fatividade_papelview.Form_CustomValidate = 
...
fatividade_papelview.ValidateRequired = true;
<?php } else { ?>
fatividade_papelview.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fatividade_papelview.Lists["x_nu_atividade"] = {"LinkField":"x_nu_atividade","Ajax":null,"AutoFill":false,"DisplayFields":["x_no_atividade","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fatividade_papelview.Lists["x_co_papel"] = {"LinkField":"x_co_papel","Ajax":null,"AutoFill":false,"DisplayFields":["x_no_papel","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $atividade_papel_view->ExportOptions->Render("body") ?>
<?php if (!$atividade_papel_view->ExportOptions->UseDropDownButton) { ?>
</div>
<div class="ewViewOtherOptions">
<?php } ?>
<?php
	foreach ($atividade_papel_view->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<?php } ?>
<?php $atividade_papel_view->ShowPageHeader(); ?>
<?php
$atividade_papel_view->ShowMessage();
?>
<form name="fatividade_papelview" id="fatividade_papelview" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="atividade_papel">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_atividade_papelview" class="table table-bordered table-striped">
<?php if ($atividade_papel->nu_atividade->Visible) { // nu_atividade ?>
	<tr id="r_nu_atividade">
		<td><span id="elh_atividade_papel_nu_atividade"><?php echo $atividade_papel->nu_atividade->FldCaption() ?></span></td>
		<td<?php echo $atividade_papel->nu_atividade->CellAttributes() ?>>
<span id="el_atividade_papel_nu_atividade" class="control-group">
...
<?php if ($atividade_papel->co_papel->Visible) { // co_papel ?>
	<tr id="r_co_papel">
		<td><span id="elh_atividade_papel_co_papel"><?php echo $atividade_papel->co_papel->FldCaption() ?></span></td>
...
<?php if ($atividade_papel->ic_responsabilidade->Visible) { // ic_responsabilidade ?>
	<tr id="r_ic_responsabilidade">
		<td><span id="elh_atividade_papel_ic_responsabilidade"><?php echo $atividade_papel->ic_responsabilidade->FldCaption() ?></span></td>
...
<table class="ewPager">
<tr><td>
<?php if (!isset($atividade_papel_view->Pager)) $atividade_papel_view->Pager = new cNumericPager($atividade_papel_view->StartRec, $atividade_papel_view->DisplayRecs, $atividade_papel_view->TotalRecs, $atividade_papel_view->RecRange) ?>
<?php if ($atividade_papel_view->Pager->RecordCount > 0) { ?>
<table cellspacing="0" class="ewStdTable"><tbody><tr><td>
<div class="pagination"><ul>
	<?php if ($atividade_papel_view->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $atividade_papel_view->PageUrl() ?>start=<?php echo $atividade_papel_view->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($atividade_papel_view->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $atividade_papel_view->PageUrl() ?>start=<?php echo $atividade_papel_view->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($atividade_papel_view->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $atividade_papel_view->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($atividade_papel_view->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $atividade_papel_view->PageUrl() ?>start=<?php echo $atividade_papel_view->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($atividade_papel_view->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $atividade_papel_view->PageUrl() ?>start=<?php echo $atividade_papel_view->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</td>
</tr></tbody></table>
...
<script type="text/javascript">
fatividade_papelview.Init();
</script>
<?php
$atividade_papel_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($atividade_papel->Export == "") { ?>
...
<?php
$atividade_papel_view->Page_Terminate();
?>
