...
$indicadorvalor_delete = NULL; // Initialize page object first

class cindicadorvalor_delete extends cindicadorvalor {
	...
	// Page object name
	var $PageObjName = 'indicadorvalor_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("indicadorvalorlist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in indicadorvalor class, indicadorvalorinfo.php
...
// Create page object
if (!isset($indicadorvalor_delete)) $indicadorvalor_delete = new cindicadorvalor_delete();

// Page init
$indicadorvalor_delete->Page_Init();

// Page main
$indicadorvalor_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$indicadorvalor_delete->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var indicadorvalor_delete = new ew_Page("indicadorvalor_delete");
indicadorvalor_delete.PageID = "delete"; // Page ID
var EW_PAGE_ID = indicadorvalor_delete.PageID; // For backward compatibility

// Form object
var findicadorvalordelete = new ew_Form("findicadorvalordelete");

// Form_CustomValidate event
findicadorvalordelete.Form_CustomValidate = 
...
findicadorvalordelete.ValidateRequired = true;
<?php } else { ?>
findicadorvalordelete.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
findicadorvalordelete.Lists["x_nu_indicador"] = {"LinkField":"x_nu_indicador","Ajax":null,"AutoFill":false,"DisplayFields":["x_no_indicador","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
findicadorvalordelete.Lists["x_nu_versao"] = {"LinkField":"x_nu_versao","Ajax":null,"AutoFill":false,"DisplayFields":["x_nu_versao","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
if ($indicadorvalor_delete->Recordset = $indicadorvalor_delete->LoadRecordset())
	$indicadorvalor_deleteTotalRecs = $indicadorvalor_delete->Recordset->RecordCount(); // Get record count
if ($indicadorvalor_deleteTotalRecs <= 0) { // No record found, exit
	if ($indicadorvalor_delete->Recordset)
		$indicadorvalor_delete->Recordset->Close();
	$indicadorvalor_delete->Page_Terminate("indicadorvalorlist.php"); // Return to list
}
?>
<?php $Breadcrumb->Render(); ?>
<?php $indicadorvalor_delete->ShowPageHeader(); ?>
<?php
$indicadorvalor_delete->ShowMessage();
?>
<form name="findicadorvalordelete" id="findicadorvalordelete" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="indicadorvalor">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($indicadorvalor_delete->RecKeys as $key) { ?>
<?php $keyvalue = is_array($key) ? implode($EW_COMPOSITE_KEY_SEPARATOR, $key) : $key; ?>
<input type="hidden" name="key_m[]" value="<?php echo ew_HtmlEncode($keyvalue) ?>">
<?php } ?>
<table cellspacing="0" class="ewGrid"><tr><td class="ewGridContent">
<div class="ewGridMiddlePanel">
<table id="tbl_indicadorvalordelete" class="ewTable ewTableSeparate">
<?php echo $indicadorvalor->TableCustomInnerHtml ?>
	<thead>
	<tr class="ewTableHeader">
		<td><span id="elh_indicadorvalor_nu_indicador" class="indicadorvalor_nu_indicador"><?php echo $indicadorvalor->nu_indicador->FldCaption() ?></span></td>
		<td><span id="elh_indicadorvalor_nu_versao" class="indicadorvalor_nu_versao"><?php echo $indicadorvalor->nu_versao->FldCaption() ?></span></td>
		<td><span id="elh_indicadorvalor_dh_geracao" class="indicadorvalor_dh_geracao"><?php echo $indicadorvalor->dh_geracao->FldCaption() ?></span></td>
		<td><span id="elh_indicadorvalor_vr_indicadorNumerico" class="indicadorvalor_vr_indicadorNumerico"><?php echo $indicadorvalor->vr_indicadorNumerico->FldCaption() ?></span></td>
		<td><span id="elh_indicadorvalor_vr_indicadorTexto" class="indicadorvalor_vr_indicadorTexto"><?php echo $indicadorvalor->vr_indicadorTexto->FldCaption() ?></span></td>
	</tr>
	</thead>
	<tbody>
<?php
$indicadorvalor_delete->RecCnt = 0;
$i = 0;
while (!$indicadorvalor_delete->Recordset->EOF) {
	$indicadorvalor_delete->RecCnt++;
	$indicadorvalor_delete->RowCnt++;

	// Set row properties
	$indicadorvalor->ResetAttrs();
	$indicadorvalor->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$indicadorvalor_delete->LoadRowValues($indicadorvalor_delete->Recordset);

	// Render row
	$indicadorvalor_delete->RenderRow();
?>
	<tr<?php echo $indicadorvalor->RowAttributes() ?>>
		<td<?php echo $indicadorvalor->nu_indicador->CellAttributes() ?>>
<span id="el<?php echo $indicadorvalor_delete->RowCnt ?>_indicadorvalor_nu_indicador" class="control-group indicadorvalor_nu_indicador">
...
<span id="el<?php echo $indicadorvalor_delete->RowCnt ?>_indicadorvalor_nu_versao" class="control-group indicadorvalor_nu_versao">
...
<span id="el<?php echo $indicadorvalor_delete->RowCnt ?>_indicadorvalor_dh_geracao" class="control-group indicadorvalor_dh_geracao">
...
<span id="el<?php echo $indicadorvalor_delete->RowCnt ?>_indicadorvalor_vr_indicadorNumerico" class="control-group indicadorvalor_vr_indicadorNumerico">
...
<span id="el<?php echo $indicadorvalor_delete->RowCnt ?>_indicadorvalor_vr_indicadorTexto" class="control-group indicadorvalor_vr_indicadorTexto">
...
<?php
	$indicadorvalor_delete->Recordset->MoveNext();
}
$indicadorvalor_delete->Recordset->Close();
?>
</tbody>
...
<script type="text/javascript">
findicadorvalordelete.Init();
</script>
<?php
$indicadorvalor_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$indicadorvalor_delete->Page_Terminate();
?>
