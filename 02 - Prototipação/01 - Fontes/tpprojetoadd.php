...
$tpprojeto_add = NULL; // Initialize page object first

class ctpprojeto_add extends ctpprojeto {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["nu_tpProjeto"] != "") {
				...
				$this->setKey("nu_tpProjeto", $this->nu_tpProjeto->CurrentValue); // Set up key
			} else {
				$this->setKey("nu_tpProjeto", ""); // Clear key
	...
	function LoadDefaultValues() {
		$this->ic_tpProjDem->CurrentValue = NULL;
		$this->ic_tpProjDem->OldValue = $this->ic_tpProjDem->CurrentValue;
		$this->no_tpProjeto->CurrentValue = NULL;
		$this->no_tpProjeto->OldValue = $this->no_tpProjeto->CurrentValue;
	...
	function ValidateForm() {
		...
		if (!EW_SERVER_VALIDATE)
			return ($gsFormError == "");
		if ($this->ic_tpProjDem->FormValue == "") {
			ew_AddMessage($gsFormError, $Language->Phrase("EnterRequiredField") . " - " . $this->ic_tpProjDem->FldCaption());
		}
	...
	function AddRow($rsold = NULL) {
		...
		// ic_tpProjDem
		$this->ic_tpProjDem->SetDbValueDef($rsnew, $this->ic_tpProjDem->CurrentValue, NULL, FALSE);

		// no_tpProjeto
		$this->no_tpProjeto->SetDbValueDef($rsnew, $this->no_tpProjeto->CurrentValue, "", FALSE);
		...
		if ($AddRow) {
			$this->nu_tpProjeto->setDbValue($conn->Insert_ID());
			$rsnew['nu_tpProjeto'] = $this->nu_tpProjeto->DbValue;
		}
		if ($AddRow) {
...
// Create page object
if (!isset($tpprojeto_add)) $tpprojeto_add = new ctpprojeto_add();

// Page init
$tpprojeto_add->Page_Init();

// Page main
$tpprojeto_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$tpprojeto_add->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var tpprojeto_add = new ew_Page("tpprojeto_add");
tpprojeto_add.PageID = "add"; // Page ID
var EW_PAGE_ID = tpprojeto_add.PageID; // For backward compatibility

// Form object
var ftpprojetoadd = new ew_Form("ftpprojetoadd");

// Validate form
ftpprojetoadd.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			elm = this.GetElements("x" + infix + "_ic_tpProjDem");
			if (elm && !ew_HasValue(elm))
				return this.OnError(elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($tpprojeto->ic_tpProjDem->FldCaption()) ?>");
...
ftpprojetoadd.Form_CustomValidate = 
...
ftpprojetoadd.ValidateRequired = true;
<?php } else { ?>
ftpprojetoadd.ValidateRequired = false; 
<?php } ?>
...
<?php $tpprojeto_add->ShowPageHeader(); ?>
<?php
$tpprojeto_add->ShowMessage();
?>
<form name="ftpprojetoadd" id="ftpprojetoadd" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="tpprojeto">
<input type="hidden" name="a_add" id="a_add" value="A">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_tpprojetoadd" class="table table-bordered table-striped">
<?php if ($tpprojeto->ic_tpProjDem->Visible) { // ic_tpProjDem ?>
	<tr id="r_ic_tpProjDem">
		<td><span id="elh_tpprojeto_ic_tpProjDem"><?php echo $tpprojeto->ic_tpProjDem->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
...
if (is_array($arwrk)) {
	...
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($tpprojeto->ic_tpProjDem->CurrentValue) == strval($arwrk[$rowcntwrk][0])) ? " checked=\"checked\"" : "";
