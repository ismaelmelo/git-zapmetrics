...
//
// Table class for gc_ata
//
class cgc_ata extends cTable {
	var $nu_ata;
	var $ds_ata;
	...
	function __construct() {
		...
		$this->TableVar = 'gc_ata';
		$this->TableName = 'gc_ata';
		...
		// nu_ata
		$this->nu_ata = new cField('gc_ata', 'gc_ata', 'x_nu_ata', 'nu_ata', '[nu_ata]', 'CAST([nu_ata] AS NVARCHAR)', 3, -1, FALSE, '[nu_ata]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->nu_ata->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['nu_ata'] = &$this->nu_ata;

		// ds_ata
		$this->ds_ata = new cField('gc_ata', 'gc_ata', 'x_ds_ata', 'ds_ata', '[ds_ata]', '[ds_ata]', 201, -1, FALSE, '[ds_ata]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['ds_ata'] = &$this->ds_ata;

		// nu_grupoOuComite
		$this->nu_grupoOuComite = new cField('gc_ata', 'gc_ata', 'x_nu_grupoOuComite', 'nu_grupoOuComite', '[nu_grupoOuComite]', 'CAST([nu_grupoOuComite] AS NVARCHAR)', 3, -1, FALSE, '[nu_grupoOuComite]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		...
		// nu_reuniao
		$this->nu_reuniao = new cField('gc_ata', 'gc_ata', 'x_nu_reuniao', 'nu_reuniao', '[nu_reuniao]', 'CAST([nu_reuniao] AS NVARCHAR)', 3, -1, FALSE, '[nu_reuniao]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->nu_reuniao->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['nu_reuniao'] = &$this->nu_reuniao;

		// dt_publicacao
		$this->dt_publicacao = new cField('gc_ata', 'gc_ata', 'x_dt_publicacao', 'dt_publicacao', '[dt_publicacao]', '(REPLACE(STR(DAY([dt_publicacao]),2,0),\' \',\'0\') + \'/\' + REPLACE(STR(MONTH([dt_publicacao]),2,0),\' \',\'0\') + \'/\' + STR(YEAR([dt_publicacao]),4,0))', 135, 7, FALSE, '[dt_publicacao]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		...
		// ic_situacao
		$this->ic_situacao = new cField('gc_ata', 'gc_ata', 'x_ic_situacao', 'ic_situacao', '[ic_situacao]', '[ic_situacao]', 129, -1, FALSE, '[ic_situacao]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['ic_situacao'] = &$this->ic_situacao;

		// no_local
		$this->no_local = new cField('gc_ata', 'gc_ata', 'x_no_local', 'no_local', '[no_local]', '[no_local]', 200, -1, FALSE, '[no_local]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['no_local'] = &$this->no_local;

		// im_anexo
		$this->im_anexo = new cField('gc_ata', 'gc_ata', 'x_im_anexo', 'im_anexo', '[im_anexo]', '[im_anexo]', 200, -1, TRUE, '[im_anexo]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->im_anexo->UploadMultiple = TRUE;
		$this->fields['im_anexo'] = &$this->im_anexo;

		// nu_usuario
		$this->nu_usuario = new cField('gc_ata', 'gc_ata', 'x_nu_usuario', 'nu_usuario', '[nu_usuario]', 'CAST([nu_usuario] AS NVARCHAR)', 3, -1, FALSE, '[nu_usuario]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->nu_usuario->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['nu_usuario'] = &$this->nu_usuario;

		// ts_datahora
		$this->ts_datahora = new cField('gc_ata', 'gc_ata', 'x_ts_datahora', 'ts_datahora', '[ts_datahora]', '(REPLACE(STR(DAY([ts_datahora]),2,0),\' \',\'0\') + \'/\' + REPLACE(STR(MONTH([ts_datahora]),2,0),\' \',\'0\') + \'/\' + STR(YEAR([ts_datahora]),4,0))', 135, 7, FALSE, '[ts_datahora]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			if (array_key_exists('nu_ata', $rs))
				ew_AddFilter($where, ew_QuotedName('nu_ata') . '=' . ew_QuotedValue($rs['nu_ata'], $this->nu_ata->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function SqlKeyFilter() {
		return "[nu_ata] = @nu_ata@";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		if (!is_numeric($this->nu_ata->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@nu_ata@", ew_AdjustSql($this->nu_ata->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("gc_ataview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("gc_ataview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl() {
		return "gc_ataadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("gc_ataedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("gc_ataadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("gc_atadelete.php", $this->UrlParm());
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->nu_ata->CurrentValue)) {
			$sUrl .= "nu_ata=" . urlencode($this->nu_ata->CurrentValue);
		} else {
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				if ($this->nu_ata->Exportable) $Doc->ExportCaption($this->nu_ata);
				if ($this->ds_ata->Exportable) $Doc->ExportCaption($this->ds_ata);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					if ($this->nu_ata->Exportable) $Doc->ExportField($this->nu_ata);
					if ($this->ds_ata->Exportable) $Doc->ExportField($this->ds_ata);
