...
$planoestrategico_im_anexo_blobview = NULL; // Initialize page object first

class cplanoestrategico_im_anexo_blobview extends cplanoestrategico {
	...
	// Page object name
	var $PageObjName = 'planoestrategico_im_anexo_blobview';
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel(CurrentProjectID() . 'planoestrategico');
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('planoestrategico', 'x_im_anexo');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (!EW_DEBUG_ENABLED && ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('im_anexo');
				...
				if (trim(strval($this->Recordset->fields('im_anexo'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('im_anexo') . "\"");
				}
...
// Create page object
if (!isset($planoestrategico_im_anexo_blobview)) $planoestrategico_im_anexo_blobview = new cplanoestrategico_im_anexo_blobview();

// Page init
$planoestrategico_im_anexo_blobview->Page_Init();

// Page main
$planoestrategico_im_anexo_blobview->Page_Main();
?>
<?php
$planoestrategico_im_anexo_blobview->Page_Terminate();
?>
