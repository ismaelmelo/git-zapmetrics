...
$atividade_grid = NULL; // Initialize page object first

class catividade_grid extends catividade {
	...
	// Page object name
	var $PageObjName = 'atividade_grid';

	// Grid form hidden field names
	var $FormName = 'fatividadegrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["atividade"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->nu_atividade->CurrentValue;
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_no_atividade") && $objForm->HasValue("o_no_atividade") && $this->no_atividade->CurrentValue <> $this->no_atividade->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_vr_duracao") && $objForm->HasValue("o_vr_duracao") && $this->vr_duracao->CurrentValue <> $this->vr_duracao->OldValue)
			return FALSE;
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->nu_atividade->CurrentValue . "\">";
		}
	...
	function LoadDefaultValues() {
		...
		$this->vr_duracao->OldValue = $this->vr_duracao->CurrentValue;
	...
	function LoadFormValues() {
		...
		$this->no_atividade->setOldValue($objForm->GetValue("o_no_atividade"));
		if (!$this->vr_duracao->FldIsDetailKey) {
			$this->vr_duracao->setFormValue($objForm->GetValue("x_vr_duracao"));
		}
		$this->vr_duracao->setOldValue($objForm->GetValue("o_vr_duracao"));
		...
		if (!$this->nu_atividade->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->nu_atividade->CurrentValue = strval($arKeys[0]); // nu_atividade
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			if (strval($this->vr_duracao->EditValue) <> "" && is_numeric($this->vr_duracao->EditValue)) {
			$this->vr_duracao->EditValue = ew_FormatNumber($this->vr_duracao->EditValue, -2, -2, -2, -2);
			$this->vr_duracao->OldValue = $this->vr_duracao->EditValue;
			}
