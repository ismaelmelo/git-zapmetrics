...
$prospectoocorrencias = NULL;

//
// Table class for prospectoocorrencias
//
class cprospectoocorrencias extends cTable {
	var $nu_prospecto;
	var $nu_ocorrencia;
	var $no_assuntoOcor;
	var $ds_ocorrencia;
	...
	function __construct() {
		...
		$this->TableVar = 'prospectoocorrencias';
		$this->TableName = 'prospectoocorrencias';
		...
		// nu_prospecto
		$this->nu_prospecto = new cField('prospectoocorrencias', 'prospectoocorrencias', 'x_nu_prospecto', 'nu_prospecto', '[nu_prospecto]', 'CAST([nu_prospecto] AS NVARCHAR)', 3, -1, FALSE, '[nu_prospecto]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->nu_prospecto->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['nu_prospecto'] = &$this->nu_prospecto;

		// nu_ocorrencia
		$this->nu_ocorrencia = new cField('prospectoocorrencias', 'prospectoocorrencias', 'x_nu_ocorrencia', 'nu_ocorrencia', '[nu_ocorrencia]', 'CAST([nu_ocorrencia] AS NVARCHAR)', 3, -1, FALSE, '[nu_ocorrencia]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->nu_ocorrencia->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['nu_ocorrencia'] = &$this->nu_ocorrencia;

		// no_assuntoOcor
		$this->no_assuntoOcor = new cField('prospectoocorrencias', 'prospectoocorrencias', 'x_no_assuntoOcor', 'no_assuntoOcor', '[no_assuntoOcor]', '[no_assuntoOcor]', 200, -1, FALSE, '[no_assuntoOcor]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['no_assuntoOcor'] = &$this->no_assuntoOcor;

		// ds_ocorrencia
		$this->ds_ocorrencia = new cField('prospectoocorrencias', 'prospectoocorrencias', 'x_ds_ocorrencia', 'ds_ocorrencia', '[ds_ocorrencia]', '[ds_ocorrencia]', 201, -1, FALSE, '[ds_ocorrencia]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['ds_ocorrencia'] = &$this->ds_ocorrencia;

		// dh_inclusao
		$this->dh_inclusao = new cField('prospectoocorrencias', 'prospectoocorrencias', 'x_dh_inclusao', 'dh_inclusao', '[dh_inclusao]', '(REPLACE(STR(DAY([dh_inclusao]),2,0),\' \',\'0\') + \'/\' + REPLACE(STR(MONTH([dh_inclusao]),2,0),\' \',\'0\') + \'/\' + STR(YEAR([dh_inclusao]),4,0))', 135, 11, FALSE, '[dh_inclusao]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function GetMasterFilter() {
		...
		// Master filter
		$sMasterFilter = "";
		if ($this->getCurrentMasterTable() == "prospecto") {
	...
	// Master filter
	function SqlMasterFilter_prospecto() {
		return "[nu_prospecto]=@nu_prospecto@";
	}

	// Detail filter
	function SqlDetailFilter_prospecto() {
		return "[nu_prospecto]=@nu_prospecto@";
	}

	// Table level SQL
	function SqlFrom() { // From
		return "[dbo].[prospectoocorrencias]";
	}
	...
	// Update Table
	var $UpdateTable = "[dbo].[prospectoocorrencias]";
	...
	function SqlKeyFilter() {
		return "[nu_ocorrencia] = @nu_ocorrencia@";
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "prospectoocorrenciaslist.php";
		}
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("prospectoocorrenciasview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("prospectoocorrenciasview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl() {
		return "prospectoocorrenciasadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("prospectoocorrenciasedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("prospectoocorrenciasadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("prospectoocorrenciasdelete.php", $this->UrlParm());
	}
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKeys[] = @$_GET["nu_ocorrencia"]; // nu_ocorrencia
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->nu_ocorrencia->CurrentValue = $key;
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				...
				if ($this->no_assuntoOcor->Exportable) $Doc->ExportCaption($this->no_assuntoOcor);
				if ($this->ds_ocorrencia->Exportable) $Doc->ExportCaption($this->ds_ocorrencia);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					...
					if ($this->no_assuntoOcor->Exportable) $Doc->ExportField($this->no_assuntoOcor);
					if ($this->ds_ocorrencia->Exportable) $Doc->ExportField($this->ds_ocorrencia);
