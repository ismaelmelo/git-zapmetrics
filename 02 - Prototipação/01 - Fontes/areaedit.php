...
class carea_edit extends carea {
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			$sSqlWrk .= " ORDER BY [no_organizacao] ASC";
				$rswrk = $conn->Execute($sSqlWrk);
				if ($rswrk && !$rswrk->EOF) { // Lookup values found
					$this->nu_organizacao->EditValue = $rswrk->fields('DispFld');
					$rswrk->Close();
				} else {
					$this->nu_organizacao->EditValue = $this->nu_organizacao->CurrentValue;
				}
			} else {
				$this->nu_organizacao->EditValue = NULL;
			}
			$this->nu_organizacao->ViewCustomAttributes = "";
	...
	function ValidateForm() {
		...
		if (in_array("divisao", $DetailTblVar) && $GLOBALS["divisao"]->DetailEdit) {
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// no_area
			$this->no_area->SetDbValueDef($rsnew, $this->no_area->CurrentValue, "", $this->no_area->ReadOnly);

			// ds_area
			$this->ds_area->SetDbValueDef($rsnew, $this->ds_area->CurrentValue, NULL, $this->ds_area->ReadOnly);

			// nu_tpArea
			$this->nu_tpArea->SetDbValueDef($rsnew, $this->nu_tpArea->CurrentValue, 0, $this->nu_tpArea->ReadOnly);
			...
			$KeyValue = isset($rsnew['nu_tpArea']) ? $rsnew['nu_tpArea'] : $rsold['nu_tpArea'];
			if (strval($KeyValue) <> "") {
				$sMasterFilter = str_replace("@nu_tpArea@", ew_AdjustSql($KeyValue), $sMasterFilter);
			} else {
			...
			$KeyValue = isset($rsnew['nu_organizacao']) ? $rsnew['nu_organizacao'] : $rsold['nu_organizacao'];
			if (strval($KeyValue) <> "") {
			...
			if ($bUpdateRow) {
				...
				if ($EditRow) {
					...
					if (in_array("divisao", $DetailTblVar) && $GLOBALS["divisao"]->DetailEdit) {
						if (!isset($GLOBALS["divisao_grid"])) $GLOBALS["divisao_grid"] = new cdivisao_grid(); // Get detail page object
						$EditRow = $GLOBALS["divisao_grid"]->GridUpdate();
					}
				}
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("divisao", $DetailTblVar)) {
				...
				if ($GLOBALS["divisao_grid"]->DetailEdit) {
					$GLOBALS["divisao_grid"]->CurrentMode = "edit";
					$GLOBALS["divisao_grid"]->CurrentAction = "gridedit";
...
// Form object
var fareaedit = new ew_Form("fareaedit");
...
fareaedit.ValidateRequired = true;
<?php } else { ?>
fareaedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fareaedit.Lists["x_nu_organizacao"] = {"LinkField":"x_nu_organizacao","Ajax":null,"AutoFill":false,"DisplayFields":["x_no_organizacao","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fareaedit.Lists["x_nu_tpArea"] = {"LinkField":"x_nu_tpArea","Ajax":null,"AutoFill":false,"DisplayFields":["x_no_tpArea","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fareaedit.Lists["x_nu_pessoaResp"] = {"LinkField":"x_nu_pessoa","Ajax":null,"AutoFill":false,"DisplayFields":["x_no_pessoa","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
?>
<form name="fareaedit" id="fareaedit" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="area">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_areaedit" class="table table-bordered table-striped">
...
<?php echo $area->nu_organizacao->EditValue ?></span>
</span>
<input type="hidden" data-field="x_nu_organizacao" name="x_nu_organizacao" id="x_nu_organizacao" value="<?php echo ew_HtmlEncode($area->nu_organizacao->CurrentValue) ?>">
...
<?php } ?>
<script type="text/javascript">
fareaedit.Lists["x_nu_tpArea"].Options = <?php echo (is_array($area->nu_tpArea->EditValue)) ? ew_ArrayToJson($area->nu_tpArea->EditValue, 1) : "[]" ?>;
</script>
<?php } ?>
...
<script type="text/javascript">
fareaedit.Lists["x_nu_pessoaResp"].Options = <?php echo (is_array($area->nu_pessoaResp->EditValue)) ? ew_ArrayToJson($area->nu_pessoaResp->EditValue, 1) : "[]" ?>;
</script>
</span>
...
<input type="hidden" data-field="x_nu_area" name="x_nu_area" id="x_nu_area" value="<?php echo ew_HtmlEncode($area->nu_area->CurrentValue) ?>">
<?php
	if (in_array("divisao", explode(",", $area->getCurrentDetailTable())) && $divisao->DetailEdit) {
?>
