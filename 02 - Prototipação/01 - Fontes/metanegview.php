...
$metaneg_view = NULL; // Initialize page object first

class cmetaneg_view extends cmetaneg {
	...
	function __construct() {
		...
		$KeyUrl = "";
		if (@$_GET["nu_metaneg"] <> "") {
			$this->RecKey["nu_metaneg"] = $_GET["nu_metaneg"];
			$KeyUrl .= "&nu_metaneg=" . urlencode($this->RecKey["nu_metaneg"]);
		}
	...
	function Page_Init() {
		...
		if (@$_GET["nu_metaneg"] <> "") {
			...
			$gsExportFile .= ew_StripSlashes($_GET["nu_metaneg"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["nu_metaneg"] <> "") {
				...
				$this->RecKey["nu_metaneg"] = $this->nu_metaneg->QueryStringValue;
			} else {
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					...
					if ($this->TotalRecs <= 0) { // No record found
						...
						$this->Page_Terminate("metaneglist.php"); // Return to list page
						...
						while (!$this->Recordset->EOF) {
							if (strval($this->nu_metaneg->CurrentValue) == strval($this->Recordset->fields('nu_metaneg'))) {
					...
					if (!$bMatchRecord) {
						if ($this->getSuccessMessage() == "" && $this->getFailureMessage() == "")
							$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record message
						$sReturnUrl = "metaneglist.php"; // No matching record, return to list
					} else {
			...
			$sReturnUrl = "metaneglist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a id=\"emf_metaneg\" href=\"javascript:void(0);\" class=\"ewExportLink ewEmail\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_metaneg',hdr:ewLanguage.Phrase('ExportToEmail'),f:document.fmetanegview,key:" . ew_ArrayToJsonAttr($this->RecKey) . ",sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
...
// Create page object
if (!isset($metaneg_view)) $metaneg_view = new cmetaneg_view();

// Page init
$metaneg_view->Page_Init();

// Page main
$metaneg_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$metaneg_view->Page_Render();
?>
...
// Page object
var metaneg_view = new ew_Page("metaneg_view");
metaneg_view.PageID = "view"; // Page ID
var EW_PAGE_ID = metaneg_view.PageID; // For backward compatibility

// Form object
var fmetanegview = new ew_Form("fmetanegview");

// Form_CustomValidate event
fmetanegview.Form_CustomValidate = 
...
fmetanegview.ValidateRequired = true;
<?php } else { ?>
fmetanegview.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fmetanegview.Lists["x_nu_periodoPei"] = {"LinkField":"x_nu_periodoPei","Ajax":null,"AutoFill":false,"DisplayFields":["x_nu_anoInicio","x_nu_anoFim","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $metaneg_view->ExportOptions->Render("body") ?>
<?php if (!$metaneg_view->ExportOptions->UseDropDownButton) { ?>
</div>
<div class="ewViewOtherOptions">
<?php } ?>
<?php
	foreach ($metaneg_view->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<?php } ?>
<?php $metaneg_view->ShowPageHeader(); ?>
<?php
$metaneg_view->ShowMessage();
?>
<form name="fmetanegview" id="fmetanegview" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="metaneg">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_metanegview" class="table table-bordered table-striped">
...
<?php echo $metaneg->nu_metaneg->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($metaneg->nu_periodoPei->Visible) { // nu_periodoPei ?>
	<tr id="r_nu_periodoPei">
		<td><span id="elh_metaneg_nu_periodoPei"><?php echo $metaneg->nu_periodoPei->FldCaption() ?></span></td>
...
<?php echo $metaneg->nu_periodoPei->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php echo $metaneg->nu_necessidade->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($metaneg->ic_perspectiva->Visible) { // ic_perspectiva ?>
	<tr id="r_ic_perspectiva">
		<td><span id="elh_metaneg_ic_perspectiva"><?php echo $metaneg->ic_perspectiva->FldCaption() ?></span></td>
...
<?php echo $metaneg->ic_perspectiva->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php echo $metaneg->no_metaneg->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<span<?php echo $metaneg->ds_metaneg->ViewAttributes() ?>>
<?php echo $metaneg->ds_metaneg->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($metaneg->ic_situacao->Visible) { // ic_situacao ?>
	<tr id="r_ic_situacao">
		<td><span id="elh_metaneg_ic_situacao"><?php echo $metaneg->ic_situacao->FldCaption() ?></span></td>
...
<?php echo $metaneg->ic_situacao->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<table class="ewPager">
<tr><td>
<?php if (!isset($metaneg_view->Pager)) $metaneg_view->Pager = new cNumericPager($metaneg_view->StartRec, $metaneg_view->DisplayRecs, $metaneg_view->TotalRecs, $metaneg_view->RecRange) ?>
<?php if ($metaneg_view->Pager->RecordCount > 0) { ?>
<table cellspacing="0" class="ewStdTable"><tbody><tr><td>
<div class="pagination"><ul>
	<?php if ($metaneg_view->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $metaneg_view->PageUrl() ?>start=<?php echo $metaneg_view->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($metaneg_view->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $metaneg_view->PageUrl() ?>start=<?php echo $metaneg_view->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($metaneg_view->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $metaneg_view->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($metaneg_view->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $metaneg_view->PageUrl() ?>start=<?php echo $metaneg_view->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($metaneg_view->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $metaneg_view->PageUrl() ?>start=<?php echo $metaneg_view->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</td>
</tr></tbody></table>
