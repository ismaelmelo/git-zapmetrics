...
$regranegocio_list = NULL; // Initialize page object first

class cregranegocio_list extends cregranegocio {
	...
	// Page object name
	var $PageObjName = 'regranegocio_list';

	// Grid form hidden field names
	var $FormName = 'fregranegociolist';
	...
	function __construct() {
		...
		$this->AddUrl = "regranegocioadd.php";
		$this->InlineAddUrl = $this->PageUrl() . "a=add";
		$this->GridAddUrl = $this->PageUrl() . "a=gridadd";
		$this->GridEditUrl = $this->PageUrl() . "a=gridedit";
		$this->MultiDeleteUrl = "regranegociodelete.php";
		$this->MultiUpdateUrl = "regranegocioupdate.php";
	...
	function SetupKeyValues($key) {
		...
		if (count($arrKeyFlds) >= 2) {
			$this->co_alternativo->setFormValue($arrKeyFlds[0]);
	...
	function ResetCmd() {
		...
		if (substr($this->Command,0,5) == "reset") {
			...
			if ($this->Command == "resetall") {
				...
				$this->DbMasterFilter = "";
				$this->DbDetailFilter = "";
				$this->co_alternativo->setSessionValue("");
			}
	...
	function RenderOtherOptions() {
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->CustomActions as $action => $name) {
				...
				$item->Body = "<a class=\"ewAction ewCustomAction\" href=\"\" onclick=\"ew_SubmitSelected(document.fregranegociolist, '" . ew_CurrentUrl() . "', null, '" . $action . "');return false;\">" . $name . "</a>";
			}
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a id=\"emf_regranegocio\" href=\"javascript:void(0);\" class=\"ewExportLink ewEmail\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_regranegocio',hdr:ewLanguage.Phrase('ExportToEmail'),f:document.fregranegociolist,sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
...
// Create page object
if (!isset($regranegocio_list)) $regranegocio_list = new cregranegocio_list();

// Page init
$regranegocio_list->Page_Init();

// Page main
$regranegocio_list->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$regranegocio_list->Page_Render();
?>
...
// Page object
var regranegocio_list = new ew_Page("regranegocio_list");
regranegocio_list.PageID = "list"; // Page ID
var EW_PAGE_ID = regranegocio_list.PageID; // For backward compatibility

// Form object
var fregranegociolist = new ew_Form("fregranegociolist");
fregranegociolist.FormKeyCountName = '<?php echo $regranegocio_list->FormKeyCountName ?>';

// Form_CustomValidate event
fregranegociolist.Form_CustomValidate = 
...
fregranegociolist.ValidateRequired = true;
<?php } else { ?>
fregranegociolist.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fregranegociolist.Lists["x_nu_projeto"] = {"LinkField":"x_nu_projeto","Ajax":null,"AutoFill":false,"DisplayFields":["x_no_projeto","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fregranegociolist.Lists["x_nu_stRegraNegocio"] = {"LinkField":"x_nu_stRegraNegocio","Ajax":null,"AutoFill":false,"DisplayFields":["x_no_stRegraNegocio","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fregranegociolist.Lists["x_nu_usuario"] = {"LinkField":"x_nu_usuario","Ajax":true,"AutoFill":false,"DisplayFields":["x_no_usuario","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php } ?>
<?php if ($regranegocio->getCurrentMasterTable() == "" && $regranegocio_list->ExportOptions->Visible()) { ?>
<div class="ewListExportOptions"><?php $regranegocio_list->ExportOptions->Render("body") ?></div>
<?php } ?>
<?php if (($regranegocio->Export == "") || (EW_EXPORT_MASTER_RECORD && $regranegocio->Export == "print")) { ?>
<?php
$gsMasterReturnUrl = "cornlist.php";
if ($regranegocio_list->DbMasterFilter <> "" && $regranegocio->getCurrentMasterTable() == "corn") {
	if ($regranegocio_list->MasterRecordExists) {
		if ($regranegocio->getCurrentMasterTable() == $regranegocio->TableVar) $gsMasterReturnUrl .= "?" . EW_TABLE_SHOW_MASTER . "=";
?>
<?php if ($regranegocio_list->ExportOptions->Visible()) { ?>
<div class="ewListExportOptions"><?php $regranegocio_list->ExportOptions->Render("body") ?></div>
<?php } ?>
...
<?php
	$bSelectLimit = EW_SELECT_LIMIT;
	if ($bSelectLimit) {
		$regranegocio_list->TotalRecs = $regranegocio->SelectRecordCount();
	} else {
		if ($regranegocio_list->Recordset = $regranegocio_list->LoadRecordset())
			$regranegocio_list->TotalRecs = $regranegocio_list->Recordset->RecordCount();
	}
	$regranegocio_list->StartRec = 1;
	if ($regranegocio_list->DisplayRecs <= 0 || ($regranegocio->Export <> "" && $regranegocio->ExportAll)) // Display all records
		$regranegocio_list->DisplayRecs = $regranegocio_list->TotalRecs;
	if (!($regranegocio->Export <> "" && $regranegocio->ExportAll))
		$regranegocio_list->SetUpStartRec(); // Set up start record position
	if ($bSelectLimit)
		$regranegocio_list->Recordset = $regranegocio_list->LoadRecordset($regranegocio_list->StartRec-1, $regranegocio_list->DisplayRecs);
$regranegocio_list->RenderOtherOptions();
?>
<?php $regranegocio_list->ShowPageHeader(); ?>
<?php
$regranegocio_list->ShowMessage();
?>
<table cellspacing="0" class="ewGrid"><tr><td class="ewGridContent">
<form name="fregranegociolist" id="fregranegociolist" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="regranegocio">
<div id="gmp_regranegocio" class="ewGridMiddlePanel">
<?php if ($regranegocio_list->TotalRecs > 0) { ?>
<table id="tbl_regranegociolist" class="ewTable ewTableSeparate">
...
// Render list options
$regranegocio_list->RenderListOptions();

// Render list options (header, left)
$regranegocio_list->ListOptions->Render("header", "left");
?>
<?php if ($regranegocio->nu_versao->Visible) { // nu_versao ?>
	...
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $regranegocio->SortUrl($regranegocio->nu_versao) ?>',2);"><div id="elh_regranegocio_nu_versao" class="regranegocio_nu_versao">
...
<?php if ($regranegocio->no_regraNegocio->Visible) { // no_regraNegocio ?>
	...
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $regranegocio->SortUrl($regranegocio->no_regraNegocio) ?>',2);"><div id="elh_regranegocio_no_regraNegocio" class="regranegocio_no_regraNegocio">
...
<?php if ($regranegocio->nu_projeto->Visible) { // nu_projeto ?>
	...
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $regranegocio->SortUrl($regranegocio->nu_projeto) ?>',2);"><div id="elh_regranegocio_nu_projeto" class="regranegocio_nu_projeto">
...
<?php if ($regranegocio->nu_stRegraNegocio->Visible) { // nu_stRegraNegocio ?>
	...
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $regranegocio->SortUrl($regranegocio->nu_stRegraNegocio) ?>',2);"><div id="elh_regranegocio_nu_stRegraNegocio" class="regranegocio_nu_stRegraNegocio">
...
<?php if ($regranegocio->nu_usuario->Visible) { // nu_usuario ?>
	...
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $regranegocio->SortUrl($regranegocio->nu_usuario) ?>',2);"><div id="elh_regranegocio_nu_usuario" class="regranegocio_nu_usuario">
...
<?php if ($regranegocio->dt_versao->Visible) { // dt_versao ?>
	...
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $regranegocio->SortUrl($regranegocio->dt_versao) ?>',2);"><div id="elh_regranegocio_dt_versao" class="regranegocio_dt_versao">
...
$regranegocio_list->ListOptions->Render("header", "right");
?>
	</tr>
</thead>
<tbody>
<?php
if ($regranegocio->ExportAll && $regranegocio->Export <> "") {
	$regranegocio_list->StopRec = $regranegocio_list->TotalRecs;
} else {

	// Set the last record to display
	if ($regranegocio_list->TotalRecs > $regranegocio_list->StartRec + $regranegocio_list->DisplayRecs - 1)
		$regranegocio_list->StopRec = $regranegocio_list->StartRec + $regranegocio_list->DisplayRecs - 1;
	else
		$regranegocio_list->StopRec = $regranegocio_list->TotalRecs;
}
$regranegocio_list->RecCnt = $regranegocio_list->StartRec - 1;
if ($regranegocio_list->Recordset && !$regranegocio_list->Recordset->EOF) {
	$regranegocio_list->Recordset->MoveFirst();
	if (!$bSelectLimit && $regranegocio_list->StartRec > 1)
		$regranegocio_list->Recordset->Move($regranegocio_list->StartRec - 1);
} elseif (!$regranegocio->AllowAddDeleteRow && $regranegocio_list->StopRec == 0) {
	$regranegocio_list->StopRec = $regranegocio->GridAddRowCount;
}

// Initialize aggregate
$regranegocio->RowType = EW_ROWTYPE_AGGREGATEINIT;
$regranegocio->ResetAttrs();
$regranegocio_list->RenderRow();
while ($regranegocio_list->RecCnt < $regranegocio_list->StopRec) {
	$regranegocio_list->RecCnt++;
	if (intval($regranegocio_list->RecCnt) >= intval($regranegocio_list->StartRec)) {
		$regranegocio_list->RowCnt++;

		// Set up key count
		$regranegocio_list->KeyCount = $regranegocio_list->RowIndex;
		...
		if ($regranegocio->CurrentAction == "gridadd") {
			...
			$regranegocio_list->LoadRowValues($regranegocio_list->Recordset); // Load row values
		}
		$regranegocio->RowType = EW_ROWTYPE_VIEW; // Render view

		// Set up row id / data-rowindex
		$regranegocio->RowAttrs = array_merge($regranegocio->RowAttrs, array('data-rowindex'=>$regranegocio_list->RowCnt, 'id'=>'r' . $regranegocio_list->RowCnt . '_regranegocio', 'data-rowtype'=>$regranegocio->RowType));
...
$regranegocio_list->ListOptions->Render("body", "left", $regranegocio_list->RowCnt);
?>
	<?php if ($regranegocio->nu_versao->Visible) { // nu_versao ?>
...
<a id="<?php echo $regranegocio_list->PageObjName . "_row_" . $regranegocio_list->RowCnt ?>"></a></td>
	<?php } ?>
	<?php if ($regranegocio->no_regraNegocio->Visible) { // no_regraNegocio ?>
...
$regranegocio_list->ListOptions->Render("body", "right", $regranegocio_list->RowCnt);
?>
	</tr>
<?php
	}
	if ($regranegocio->CurrentAction <> "gridadd")
		$regranegocio_list->Recordset->MoveNext();
}
?>
</tbody>
</table>
<?php } ?>
<?php if ($regranegocio->CurrentAction == "") { ?>
...
// Close recordset
if ($regranegocio_list->Recordset)
	$regranegocio_list->Recordset->Close();
?>
<?php if ($regranegocio->Export == "") { ?>
<div class="ewGridLowerPanel">
<?php if ($regranegocio->CurrentAction <> "gridadd" && $regranegocio->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>">
<table class="ewPager">
<tr><td>
<?php if (!isset($regranegocio_list->Pager)) $regranegocio_list->Pager = new cNumericPager($regranegocio_list->StartRec, $regranegocio_list->DisplayRecs, $regranegocio_list->TotalRecs, $regranegocio_list->RecRange) ?>
<?php if ($regranegocio_list->Pager->RecordCount > 0) { ?>
<table cellspacing="0" class="ewStdTable"><tbody><tr><td>
<div class="pagination"><ul>
	<?php if ($regranegocio_list->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $regranegocio_list->PageUrl() ?>start=<?php echo $regranegocio_list->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($regranegocio_list->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $regranegocio_list->PageUrl() ?>start=<?php echo $regranegocio_list->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($regranegocio_list->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $regranegocio_list->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($regranegocio_list->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $regranegocio_list->PageUrl() ?>start=<?php echo $regranegocio_list->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($regranegocio_list->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $regranegocio_list->PageUrl() ?>start=<?php echo $regranegocio_list->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</td>
<td>
	<?php if ($regranegocio_list->Pager->ButtonCount > 0) { ?>&nbsp;&nbsp;&nbsp;&nbsp;<?php } ?>
	<?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $regranegocio_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $regranegocio_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $regranegocio_list->Pager->RecordCount ?>
</td>
</tr></tbody></table>
<?php } else { ?>
	<?php if ($Security->CanList()) { ?>
	<?php if ($regranegocio_list->SearchWhere == "0=101") { ?>
...
<?php
	foreach ($regranegocio_list->OtherOptions as &$option)
...
<?php } ?>
<?php
$regranegocio_list->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($regranegocio->Export == "") { ?>
...
<?php
$regranegocio_list->Page_Terminate();
?>
