...
$indicador_add = NULL; // Initialize page object first

class cindicador_add extends cindicador {
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "indicadorview.php")
	...
	function LoadDefaultValues() {
		$this->no_indicador->CurrentValue = NULL;
		$this->no_indicador->OldValue = $this->no_indicador->CurrentValue;
		$this->ds_indicador->CurrentValue = NULL;
		$this->ds_indicador->OldValue = $this->ds_indicador->CurrentValue;
		$this->ic_tpIndicador->CurrentValue = NULL;
		$this->ic_tpIndicador->OldValue = $this->ic_tpIndicador->CurrentValue;
	...
	function AddRow($rsold = NULL) {
		...
		// no_indicador
		$this->no_indicador->SetDbValueDef($rsnew, $this->no_indicador->CurrentValue, "", FALSE);

		// ds_indicador
		$this->ds_indicador->SetDbValueDef($rsnew, $this->ds_indicador->CurrentValue, NULL, FALSE);

		// ic_tpIndicador
		$this->ic_tpIndicador->SetDbValueDef($rsnew, $this->ic_tpIndicador->CurrentValue, NULL, FALSE);
		...
		if ($this->nu_indicador->getSessionValue() <> "") {
			$rsnew['nu_indicador'] = $this->nu_indicador->getSessionValue();
		}
		...
		if ($AddRow) {
			$this->nu_indicador->setDbValue($conn->Insert_ID());
			$rsnew['nu_indicador'] = $this->nu_indicador->DbValue;
		}
		if ($AddRow) {
...
// Create page object
if (!isset($indicador_add)) $indicador_add = new cindicador_add();

// Page init
$indicador_add->Page_Init();

// Page main
$indicador_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$indicador_add->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var indicador_add = new ew_Page("indicador_add");
indicador_add.PageID = "add"; // Page ID
var EW_PAGE_ID = indicador_add.PageID; // For backward compatibility

// Form object
var findicadoradd = new ew_Form("findicadoradd");

// Validate form
findicadoradd.Validate = function() {
...
findicadoradd.Form_CustomValidate = 
...
findicadoradd.ValidateRequired = true;
<?php } else { ?>
findicadoradd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
findicadoradd.Lists["x_nu_processoCobit5"] = {"LinkField":"x_nu_processo","Ajax":null,"AutoFill":false,"DisplayFields":["x_co_alternativo","x_no_processo","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $indicador_add->ShowPageHeader(); ?>
<?php
$indicador_add->ShowMessage();
?>
<form name="findicadoradd" id="findicadoradd" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="indicador">
<input type="hidden" name="a_add" id="a_add" value="A">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_indicadoradd" class="table table-bordered table-striped">
...
<script type="text/javascript">
findicadoradd.Lists["x_nu_processoCobit5"].Options = <?php echo (is_array($indicador->nu_processoCobit5->EditValue)) ? ew_ArrayToJson($indicador->nu_processoCobit5->EditValue, 1) : "[]" ?>;
</script>
</span>
...
<?php if (strval($indicador->nu_indicador->getSessionValue()) <> "") { ?>
<input type="hidden" name="x_nu_indicador" id="x_nu_indicador" value="<?php echo ew_HtmlEncode(strval($indicador->nu_indicador->getSessionValue())) ?>">
<?php } ?>
