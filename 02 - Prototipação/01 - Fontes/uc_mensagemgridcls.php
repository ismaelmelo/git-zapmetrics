...
$uc_mensagem_grid = NULL; // Initialize page object first

class cuc_mensagem_grid extends cuc_mensagem {
	...
	// Page object name
	var $PageObjName = 'uc_mensagem_grid';

	// Grid form hidden field names
	var $FormName = 'fuc_mensagemgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["uc_mensagem"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					if ($sKey <> "") $sKey .= $GLOBALS["EW_COMPOSITE_KEY_SEPARATOR"];
					$sKey .= $this->nu_mensagem->CurrentValue;
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_nu_mensagem") && $objForm->HasValue("o_nu_mensagem") && $this->nu_mensagem->CurrentValue <> $this->nu_mensagem->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->nu_uc->CurrentValue . $GLOBALS["EW_COMPOSITE_KEY_SEPARATOR"] . $this->nu_mensagem->CurrentValue . "\">";
		}
	...
	function LoadFormValues() {
		...
		$this->nu_mensagem->setOldValue($objForm->GetValue("o_nu_mensagem"));
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 2) {
			if (strval($arKeys[0]) <> "")
				...
				$bValidKey = FALSE;
			if (strval($arKeys[1]) <> "")
				$this->nu_mensagem->CurrentValue = strval($arKeys[1]); // nu_mensagem
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
