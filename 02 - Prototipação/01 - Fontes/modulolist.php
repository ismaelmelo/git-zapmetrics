...
class cmodulo_list extends cmodulo {
	...
	function SetupKeyValues($key) {
		...
		if (count($arrKeyFlds) >= 1) {
			$this->nu_modulo->setFormValue($arrKeyFlds[0]);
			if (!is_numeric($this->nu_modulo->FormValue))
				return FALSE;
		}
		return TRUE;
	}
	...
	function SetUpSortOrder() {
		...
		if (@$_GET["order"] <> "") {
			...
			$this->UpdateSort($this->no_modulo, $bCtrl); // no_modulo
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a id=\"emf_modulo\" href=\"javascript:void(0);\" class=\"ewExportLink ewEmail\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_modulo',hdr:ewLanguage.Phrase('ExportToEmail'),f:document.fmodulolist,sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
...
fmodulolist.Lists["x_nu_stSistema"] = {"LinkField":"x_nu_stSistema","Ajax":null,"AutoFill":false,"DisplayFields":["x_no_stSistema","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php } ?>
<?php if ($modulo->getCurrentMasterTable() == "" && $modulo_list->ExportOptions->Visible()) { ?>
<div class="ewListExportOptions"><?php $modulo_list->ExportOptions->Render("body") ?></div>
<?php } ?>
<?php if (($modulo->Export == "") || (EW_EXPORT_MASTER_RECORD && $modulo->Export == "print")) { ?>
<?php
$gsMasterReturnUrl = "sistemalist.php";
if ($modulo_list->DbMasterFilter <> "" && $modulo->getCurrentMasterTable() == "sistema") {
	if ($modulo_list->MasterRecordExists) {
		if ($modulo->getCurrentMasterTable() == $modulo->TableVar) $gsMasterReturnUrl .= "?" . EW_TABLE_SHOW_MASTER . "=";
?>
<?php if ($modulo_list->ExportOptions->Visible()) { ?>
<div class="ewListExportOptions"><?php $modulo_list->ExportOptions->Render("body") ?></div>
<?php } ?>
...
<form name="fmodulolist" id="fmodulolist" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="modulo">
<div id="gmp_modulo" class="ewGridMiddlePanel">
<?php if ($modulo_list->TotalRecs > 0) { ?>
<table id="tbl_modulolist" class="ewTable ewTableSeparate">
...
<?php if ($modulo->no_modulo->Visible) { // no_modulo ?>
	...
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $modulo->SortUrl($modulo->no_modulo) ?>',2);"><div id="elh_modulo_no_modulo" class="modulo_no_modulo">
...
<?php if ($modulo->nu_stSistema->Visible) { // nu_stSistema ?>
	...
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $modulo->SortUrl($modulo->nu_stSistema) ?>',2);"><div id="elh_modulo_nu_stSistema" class="modulo_nu_stSistema">
...
<?php if ($modulo->nu_ordem->Visible) { // nu_ordem ?>
	...
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $modulo->SortUrl($modulo->nu_ordem) ?>',2);"><div id="elh_modulo_nu_ordem" class="modulo_nu_ordem">
...
<table class="ewPager">
<tr><td>
<?php if (!isset($modulo_list->Pager)) $modulo_list->Pager = new cNumericPager($modulo_list->StartRec, $modulo_list->DisplayRecs, $modulo_list->TotalRecs, $modulo_list->RecRange) ?>
<?php if ($modulo_list->Pager->RecordCount > 0) { ?>
<table cellspacing="0" class="ewStdTable"><tbody><tr><td>
<div class="pagination"><ul>
	<?php if ($modulo_list->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $modulo_list->PageUrl() ?>start=<?php echo $modulo_list->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($modulo_list->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $modulo_list->PageUrl() ?>start=<?php echo $modulo_list->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($modulo_list->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $modulo_list->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($modulo_list->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $modulo_list->PageUrl() ?>start=<?php echo $modulo_list->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($modulo_list->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $modulo_list->PageUrl() ?>start=<?php echo $modulo_list->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</td>
<td>
	<?php if ($modulo_list->Pager->ButtonCount > 0) { ?>&nbsp;&nbsp;&nbsp;&nbsp;<?php } ?>
	<?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $modulo_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $modulo_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $modulo_list->Pager->RecordCount ?>
</td>
</tr></tbody></table>
<?php } else { ?>
	<?php if ($Security->CanList()) { ?>
	<?php if ($modulo_list->SearchWhere == "0=101") { ?>
