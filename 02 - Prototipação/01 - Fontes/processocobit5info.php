...
//
// Table class for processocobit5
//
class cprocessocobit5 extends cTable {
	var $ic_dominio;
	var $nu_processo;
	var $co_alternativo;
	var $no_processo;
	...
	function __construct() {
		...
		$this->TableVar = 'processocobit5';
		$this->TableName = 'processocobit5';
		...
		// ic_dominio
		$this->ic_dominio = new cField('processocobit5', 'processocobit5', 'x_ic_dominio', 'ic_dominio', '[ic_dominio]', '[ic_dominio]', 129, -1, FALSE, '[ic_dominio]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['ic_dominio'] = &$this->ic_dominio;

		// nu_processo
		$this->nu_processo = new cField('processocobit5', 'processocobit5', 'x_nu_processo', 'nu_processo', '[nu_processo]', 'CAST([nu_processo] AS NVARCHAR)', 3, -1, FALSE, '[nu_processo]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->nu_processo->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['nu_processo'] = &$this->nu_processo;

		// co_alternativo
		$this->co_alternativo = new cField('processocobit5', 'processocobit5', 'x_co_alternativo', 'co_alternativo', '[co_alternativo]', '[co_alternativo]', 200, -1, FALSE, '[co_alternativo]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['co_alternativo'] = &$this->co_alternativo;

		// no_processo
		$this->no_processo = new cField('processocobit5', 'processocobit5', 'x_no_processo', 'no_processo', '[no_processo]', '[no_processo]', 200, -1, FALSE, '[no_processo]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['no_processo'] = &$this->no_processo;

		// ds_dominio
		$this->ds_dominio = new cField('processocobit5', 'processocobit5', 'x_ds_dominio', 'ds_dominio', '[ds_dominio]', '[ds_dominio]', 201, -1, FALSE, '[ds_dominio]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['ds_dominio'] = &$this->ds_dominio;
	}
	...
	// Update Table
	var $UpdateTable = "[dbo].[processocobit5]";
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("processocobit5view.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("processocobit5view.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl() {
		return "processocobit5add.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("processocobit5edit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("processocobit5add.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("processocobit5delete.php", $this->UrlParm());
	}
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				if ($this->ic_dominio->Exportable) $Doc->ExportCaption($this->ic_dominio);
				...
				if ($this->ds_dominio->Exportable) $Doc->ExportCaption($this->ds_dominio);
			} else {
				if ($this->ic_dominio->Exportable) $Doc->ExportCaption($this->ic_dominio);
				if ($this->co_alternativo->Exportable) $Doc->ExportCaption($this->co_alternativo);
				if ($this->no_processo->Exportable) $Doc->ExportCaption($this->no_processo);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					if ($this->ic_dominio->Exportable) $Doc->ExportField($this->ic_dominio);
					...
					if ($this->ds_dominio->Exportable) $Doc->ExportField($this->ds_dominio);
				} else {
					if ($this->ic_dominio->Exportable) $Doc->ExportField($this->ic_dominio);
					if ($this->co_alternativo->Exportable) $Doc->ExportField($this->co_alternativo);
					if ($this->no_processo->Exportable) $Doc->ExportField($this->no_processo);
