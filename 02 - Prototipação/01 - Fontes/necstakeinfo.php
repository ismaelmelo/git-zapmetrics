...
//
// Table class for necstake
//
class cnecstake extends cTable {
	var $nu_necessidade;
	var $nu_periodoPei;
	var $nu_motivador;
	var $no_necessidade;
	...
	function __construct() {
		...
		$this->TableVar = 'necstake';
		$this->TableName = 'necstake';
		...
		// nu_necessidade
		$this->nu_necessidade = new cField('necstake', 'necstake', 'x_nu_necessidade', 'nu_necessidade', '[nu_necessidade]', 'CAST([nu_necessidade] AS NVARCHAR)', 3, -1, FALSE, '[nu_necessidade]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->nu_necessidade->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['nu_necessidade'] = &$this->nu_necessidade;

		// nu_periodoPei
		$this->nu_periodoPei = new cField('necstake', 'necstake', 'x_nu_periodoPei', 'nu_periodoPei', '[nu_periodoPei]', 'CAST([nu_periodoPei] AS NVARCHAR)', 3, -1, FALSE, '[EV__nu_periodoPei]', TRUE, TRUE, TRUE, 'FORMATTED TEXT');
		$this->nu_periodoPei->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['nu_periodoPei'] = &$this->nu_periodoPei;

		// nu_motivador
		$this->nu_motivador = new cField('necstake', 'necstake', 'x_nu_motivador', 'nu_motivador', '[nu_motivador]', 'CAST([nu_motivador] AS NVARCHAR)', 3, -1, FALSE, '[nu_motivador]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->nu_motivador->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['nu_motivador'] = &$this->nu_motivador;

		// no_necessidade
		$this->no_necessidade = new cField('necstake', 'necstake', 'x_no_necessidade', 'no_necessidade', '[no_necessidade]', '[no_necessidade]', 200, -1, FALSE, '[no_necessidade]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['no_necessidade'] = &$this->no_necessidade;

		// ds_necessidade
		$this->ds_necessidade = new cField('necstake', 'necstake', 'x_ds_necessidade', 'ds_necessidade', '[ds_necessidade]', '[ds_necessidade]', 201, -1, FALSE, '[ds_necessidade]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['ds_necessidade'] = &$this->ds_necessidade;

		// ic_situacao
		$this->ic_situacao = new cField('necstake', 'necstake', 'x_ic_situacao', 'ic_situacao', '[ic_situacao]', '[ic_situacao]', 129, -1, FALSE, '[ic_situacao]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function GetMasterFilter() {
		...
		// Master filter
		$sMasterFilter = "";
		if ($this->getCurrentMasterTable() == "metaneg") {
			if ($this->nu_necessidade->getSessionValue() <> "")
				$sMasterFilter .= "[nu_necessidade]=" . ew_QuotedValue($this->nu_necessidade->getSessionValue(), EW_DATATYPE_NUMBER);
			else
				return "";
		}
	...
	function GetDetailFilter() {
		...
		if ($this->getCurrentMasterTable() == "metaneg") {
			if ($this->nu_necessidade->getSessionValue() <> "")
				$sDetailFilter .= "[nu_necessidade]=" . ew_QuotedValue($this->nu_necessidade->getSessionValue(), EW_DATATYPE_NUMBER);
			else
				return "";
		}
	...
	// Master filter
	function SqlMasterFilter_metaneg() {
		return "[nu_necessidade]=@nu_necessidade@";
	}

	// Detail filter
	function SqlDetailFilter_metaneg() {
		return "[nu_necessidade]=@nu_necessidade@";
	}
	...
	function SqlSelectList() { // Select for List page
		return "SELECT * FROM (" .
			"SELECT *, (SELECT TOP 1 [nu_anoInicio] + '" . ew_ValueSeparator(1, $this->nu_periodoPei) . "' + [nu_anoFim] FROM [periodopei] [EW_TMP_LOOKUPTABLE] WHERE [EW_TMP_LOOKUPTABLE].[nu_periodoPei] = [necstake].[nu_periodoPei]) AS [EV__nu_periodoPei] FROM [dbo].[necstake]" .
			") [EW_TMP_TABLE]";
	}
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			if (array_key_exists('nu_necessidade', $rs))
				ew_AddFilter($where, ew_QuotedName('nu_necessidade') . '=' . ew_QuotedValue($rs['nu_necessidade'], $this->nu_necessidade->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->nu_necessidade->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@nu_necessidade@", ew_AdjustSql($this->nu_necessidade->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("necstakeview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("necstakeview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl() {
		return "necstakeadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("necstakeedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("necstakeadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("necstakedelete.php", $this->UrlParm());
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->nu_necessidade->CurrentValue)) {
			$sUrl .= "nu_necessidade=" . urlencode($this->nu_necessidade->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKeys[] = @$_GET["nu_necessidade"]; // nu_necessidade
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->nu_necessidade->CurrentValue = $key;
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				...
				if ($this->nu_motivador->Exportable) $Doc->ExportCaption($this->nu_motivador);
				if ($this->no_necessidade->Exportable) $Doc->ExportCaption($this->no_necessidade);
				if ($this->ds_necessidade->Exportable) $Doc->ExportCaption($this->ds_necessidade);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					...
					if ($this->nu_motivador->Exportable) $Doc->ExportField($this->nu_motivador);
					if ($this->no_necessidade->Exportable) $Doc->ExportField($this->no_necessidade);
					if ($this->ds_necessidade->Exportable) $Doc->ExportField($this->ds_necessidade);
