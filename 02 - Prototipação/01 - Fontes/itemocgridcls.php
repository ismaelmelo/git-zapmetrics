...
$itemoc_grid = NULL; // Initialize page object first

class citemoc_grid extends citemoc {
	...
	// Page object name
	var $PageObjName = 'itemoc_grid';

	// Grid form hidden field names
	var $FormName = 'fitemocgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["itemoc"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->nu_itemOc->CurrentValue;
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_nu_tpItem") && $objForm->HasValue("o_nu_tpItem") && $this->nu_tpItem->CurrentValue <> $this->nu_tpItem->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_no_itemOc") && $objForm->HasValue("o_no_itemOc") && $this->no_itemOc->CurrentValue <> $this->no_itemOc->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->nu_itemOc->CurrentValue . "\">";
		}
	...
	function LoadFormValues() {
		...
		$this->nu_tpItem->setOldValue($objForm->GetValue("o_nu_tpItem"));
		if (!$this->no_itemOc->FldIsDetailKey) {
			$this->no_itemOc->setFormValue($objForm->GetValue("x_no_itemOc"));
		}
		$this->no_itemOc->setOldValue($objForm->GetValue("o_no_itemOc"));
		if (!$this->nu_itemOc->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->nu_itemOc->CurrentValue = strval($arKeys[0]); // nu_itemOc
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function AddRow($rsold = NULL) {
		...
		if ($this->nu_oc->getSessionValue() <> "") {
			$rsnew['nu_oc'] = $this->nu_oc->getSessionValue();
		}
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "oc") {
			$this->nu_oc->Visible = FALSE;
			if ($GLOBALS["oc"]->EventCancelled) $this->EventCancelled = TRUE;
		}
