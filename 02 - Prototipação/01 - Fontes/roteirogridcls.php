...
$roteiro_grid = NULL; // Initialize page object first

class croteiro_grid extends croteiro {
	...
	// Page object name
	var $PageObjName = 'roteiro_grid';

	// Grid form hidden field names
	var $FormName = 'froteirogrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["roteiro"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->nu_roteiro->CurrentValue;
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_nu_metodologia") && $objForm->HasValue("o_nu_metodologia") && $this->nu_metodologia->CurrentValue <> $this->nu_metodologia->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_no_roteiro") && $objForm->HasValue("o_no_roteiro") && $this->no_roteiro->CurrentValue <> $this->no_roteiro->OldValue)
			return FALSE;
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->nu_roteiro->CurrentValue . "\">";
		}
	...
	function LoadFormValues() {
		...
		$this->nu_metodologia->setOldValue($objForm->GetValue("o_nu_metodologia"));
		if (!$this->no_roteiro->FldIsDetailKey) {
			$this->no_roteiro->setFormValue($objForm->GetValue("x_no_roteiro"));
		}
		$this->no_roteiro->setOldValue($objForm->GetValue("o_no_roteiro"));
		...
		if (!$this->nu_roteiro->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->nu_roteiro->CurrentValue = strval($arKeys[0]); // nu_roteiro
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "metodologia") {
			$this->nu_metodologia->Visible = FALSE;
			if ($GLOBALS["metodologia"]->EventCancelled) $this->EventCancelled = TRUE;
		}
