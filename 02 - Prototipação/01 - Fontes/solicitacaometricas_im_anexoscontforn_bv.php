...
$solicitacaoMetricas_im_anexosContForn_blobview = NULL; // Initialize page object first

class csolicitacaoMetricas_im_anexosContForn_blobview extends csolicitacaoMetricas {
	...
	// Page object name
	var $PageObjName = 'solicitacaoMetricas_im_anexosContForn_blobview';
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel(CurrentProjectID() . 'solicitacaoMetricas');
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('solicitacaoMetricas', 'x_im_anexosContForn');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (!EW_DEBUG_ENABLED && ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('im_anexosContForn');
				...
				if (trim(strval($this->Recordset->fields('im_anexosContForn'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('im_anexosContForn') . "\"");
				}
...
// Create page object
if (!isset($solicitacaoMetricas_im_anexosContForn_blobview)) $solicitacaoMetricas_im_anexosContForn_blobview = new csolicitacaoMetricas_im_anexosContForn_blobview();

// Page init
$solicitacaoMetricas_im_anexosContForn_blobview->Page_Init();

// Page main
$solicitacaoMetricas_im_anexosContForn_blobview->Page_Main();
?>
<?php
$solicitacaoMetricas_im_anexosContForn_blobview->Page_Terminate();
?>
