...
$atividade_papel = NULL;

//
// Table class for atividade_papel
//
class catividade_papel extends cTable {
	var $nu_atividade;
	var $co_papel;
	var $ic_responsabilidade;
	...
	function __construct() {
		...
		$this->TableVar = 'atividade_papel';
		$this->TableName = 'atividade_papel';
		...
		// nu_atividade
		$this->nu_atividade = new cField('atividade_papel', 'atividade_papel', 'x_nu_atividade', 'nu_atividade', '[nu_atividade]', 'CAST([nu_atividade] AS NVARCHAR)', 3, -1, FALSE, '[nu_atividade]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->nu_atividade->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['nu_atividade'] = &$this->nu_atividade;

		// co_papel
		$this->co_papel = new cField('atividade_papel', 'atividade_papel', 'x_co_papel', 'co_papel', '[co_papel]', 'CAST([co_papel] AS NVARCHAR)', 3, -1, FALSE, '[co_papel]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->co_papel->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['co_papel'] = &$this->co_papel;

		// ic_responsabilidade
		$this->ic_responsabilidade = new cField('atividade_papel', 'atividade_papel', 'x_ic_responsabilidade', 'ic_responsabilidade', '[ic_responsabilidade]', '[ic_responsabilidade]', 129, -1, FALSE, '[ic_responsabilidade]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['ic_responsabilidade'] = &$this->ic_responsabilidade;
	}
	...
	function GetMasterFilter() {
		...
		// Master filter
		$sMasterFilter = "";
		if ($this->getCurrentMasterTable() == "atividade") {
			if ($this->nu_atividade->getSessionValue() <> "")
				$sMasterFilter .= "[nu_atividade]=" . ew_QuotedValue($this->nu_atividade->getSessionValue(), EW_DATATYPE_NUMBER);
			else
				return "";
		}
	...
	function GetDetailFilter() {
		...
		if ($this->getCurrentMasterTable() == "atividade") {
			if ($this->nu_atividade->getSessionValue() <> "")
				$sDetailFilter .= "[nu_atividade]=" . ew_QuotedValue($this->nu_atividade->getSessionValue(), EW_DATATYPE_NUMBER);
			else
				return "";
		}
	...
	// Master filter
	function SqlMasterFilter_atividade() {
		return "[nu_atividade]=@nu_atividade@";
	}

	// Detail filter
	function SqlDetailFilter_atividade() {
		return "[nu_atividade]=@nu_atividade@";
	}

	// Table level SQL
	function SqlFrom() { // From
		return "[dbo].[atividade_papel]";
	}
	...
	function SqlOrderBy() { // Order By
		return "[nu_atividade] ASC,[co_papel] ASC";
	}
	...
	// Update Table
	var $UpdateTable = "[dbo].[atividade_papel]";
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			...
			if (array_key_exists('co_papel', $rs))
				ew_AddFilter($where, ew_QuotedName('co_papel') . '=' . ew_QuotedValue($rs['co_papel'], $this->co_papel->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function SqlKeyFilter() {
		return "[nu_atividade] = @nu_atividade@ AND [co_papel] = @co_papel@";
	}
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->co_papel->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@co_papel@", ew_AdjustSql($this->co_papel->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "atividade_papellist.php";
		}
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("atividade_papelview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("atividade_papelview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl() {
		return "atividade_papeladd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("atividade_papeledit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("atividade_papeladd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("atividade_papeldelete.php", $this->UrlParm());
	}
	...
	function KeyUrl($url, $parm = "") {
		...
		if (!is_null($this->co_papel->CurrentValue)) {
			$sUrl .= "&co_papel=" . urlencode($this->co_papel->CurrentValue);
		} else {
			return "javascript:alert(ewLanguage.Phrase('InvalidRecord'));";
		}
		return $sUrl;
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKey[] = @$_GET["nu_atividade"]; // nu_atividade
			$arKey[] = @$_GET["co_papel"]; // co_papel
			$arKeys[] = $arKey;
		...
		foreach ($arKeys as $key) {
			...
			if (!is_numeric($key[0])) // nu_atividade
				continue;
			if (!is_numeric($key[1])) // co_papel
				continue;
			$ar[] = $key;
		}
		return $ar;
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->nu_atividade->CurrentValue = $key[0];
			$this->co_papel->CurrentValue = $key[1];
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				if ($this->nu_atividade->Exportable) $Doc->ExportCaption($this->nu_atividade);
				if ($this->co_papel->Exportable) $Doc->ExportCaption($this->co_papel);
				if ($this->ic_responsabilidade->Exportable) $Doc->ExportCaption($this->ic_responsabilidade);
			} else {
				if ($this->nu_atividade->Exportable) $Doc->ExportCaption($this->nu_atividade);
				if ($this->co_papel->Exportable) $Doc->ExportCaption($this->co_papel);
				if ($this->ic_responsabilidade->Exportable) $Doc->ExportCaption($this->ic_responsabilidade);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					if ($this->nu_atividade->Exportable) $Doc->ExportField($this->nu_atividade);
					if ($this->co_papel->Exportable) $Doc->ExportField($this->co_papel);
					if ($this->ic_responsabilidade->Exportable) $Doc->ExportField($this->ic_responsabilidade);
				} else {
					if ($this->nu_atividade->Exportable) $Doc->ExportField($this->nu_atividade);
					if ($this->co_papel->Exportable) $Doc->ExportField($this->co_papel);
					if ($this->ic_responsabilidade->Exportable) $Doc->ExportField($this->ic_responsabilidade);
