...
$fatura_os_view = NULL; // Initialize page object first

class cfatura_os_view extends cfatura_os {
	...
	// Page object name
	var $PageObjName = 'fatura_os_view';
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					...
					if ($this->TotalRecs <= 0) { // No record found
						...
						$this->Page_Terminate("fatura_oslist.php"); // Return to list page
						...
						while (!$this->Recordset->EOF) {
							if (strval($this->nu_fatura->CurrentValue) == strval($this->Recordset->fields('nu_fatura')) && strval($this->nu_os->CurrentValue) == strval($this->Recordset->fields('nu_os'))) {
					...
					if (!$bMatchRecord) {
						if ($this->getSuccessMessage() == "" && $this->getFailureMessage() == "")
							$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record message
						$sReturnUrl = "fatura_oslist.php"; // No matching record, return to list
					} else {
			...
			$sReturnUrl = "fatura_oslist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a id=\"emf_fatura_os\" href=\"javascript:void(0);\" class=\"ewExportLink ewEmail\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_fatura_os',hdr:ewLanguage.Phrase('ExportToEmail'),f:document.ffatura_osview,key:" . ew_ArrayToJsonAttr($this->RecKey) . ",sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
...
// Create page object
if (!isset($fatura_os_view)) $fatura_os_view = new cfatura_os_view();

// Page init
$fatura_os_view->Page_Init();

// Page main
$fatura_os_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$fatura_os_view->Page_Render();
?>
...
// Page object
var fatura_os_view = new ew_Page("fatura_os_view");
fatura_os_view.PageID = "view"; // Page ID
var EW_PAGE_ID = fatura_os_view.PageID; // For backward compatibility

// Form object
var ffatura_osview = new ew_Form("ffatura_osview");

// Form_CustomValidate event
ffatura_osview.Form_CustomValidate = 
...
ffatura_osview.ValidateRequired = true;
<?php } else { ?>
ffatura_osview.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ffatura_osview.Lists["x_nu_fatura"] = {"LinkField":"x_nu_fatura","Ajax":null,"AutoFill":false,"DisplayFields":["x_nu_fatura","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
ffatura_osview.Lists["x_nu_os"] = {"LinkField":"x_nu_os","Ajax":null,"AutoFill":false,"DisplayFields":["x_no_titulo","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $fatura_os_view->ExportOptions->Render("body") ?>
<?php if (!$fatura_os_view->ExportOptions->UseDropDownButton) { ?>
</div>
<div class="ewViewOtherOptions">
<?php } ?>
<?php
	foreach ($fatura_os_view->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<?php } ?>
<?php $fatura_os_view->ShowPageHeader(); ?>
<?php
$fatura_os_view->ShowMessage();
?>
<form name="ffatura_osview" id="ffatura_osview" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="fatura_os">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_fatura_osview" class="table table-bordered table-striped">
<?php if ($fatura_os->nu_fatura->Visible) { // nu_fatura ?>
	<tr id="r_nu_fatura">
		<td><span id="elh_fatura_os_nu_fatura"><?php echo $fatura_os->nu_fatura->FldCaption() ?></span></td>
		<td<?php echo $fatura_os->nu_fatura->CellAttributes() ?>>
<span id="el_fatura_os_nu_fatura" class="control-group">
...
<?php if ($fatura_os->nu_os->Visible) { // nu_os ?>
	<tr id="r_nu_os">
		<td><span id="elh_fatura_os_nu_os"><?php echo $fatura_os->nu_os->FldCaption() ?></span></td>
...
<?php if ($fatura_os->ic_pagIntegralOs->Visible) { // ic_pagIntegralOs ?>
	<tr id="r_ic_pagIntegralOs">
		<td><span id="elh_fatura_os_ic_pagIntegralOs"><?php echo $fatura_os->ic_pagIntegralOs->FldCaption() ?></span></td>
...
<?php if ($fatura_os->vr_pagoOsFatura->Visible) { // vr_pagoOsFatura ?>
	<tr id="r_vr_pagoOsFatura">
		<td><span id="elh_fatura_os_vr_pagoOsFatura"><?php echo $fatura_os->vr_pagoOsFatura->FldCaption() ?></span></td>
...
<table class="ewPager">
<tr><td>
<?php if (!isset($fatura_os_view->Pager)) $fatura_os_view->Pager = new cNumericPager($fatura_os_view->StartRec, $fatura_os_view->DisplayRecs, $fatura_os_view->TotalRecs, $fatura_os_view->RecRange) ?>
<?php if ($fatura_os_view->Pager->RecordCount > 0) { ?>
<table cellspacing="0" class="ewStdTable"><tbody><tr><td>
<div class="pagination"><ul>
	<?php if ($fatura_os_view->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $fatura_os_view->PageUrl() ?>start=<?php echo $fatura_os_view->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($fatura_os_view->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $fatura_os_view->PageUrl() ?>start=<?php echo $fatura_os_view->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($fatura_os_view->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $fatura_os_view->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($fatura_os_view->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $fatura_os_view->PageUrl() ?>start=<?php echo $fatura_os_view->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($fatura_os_view->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $fatura_os_view->PageUrl() ?>start=<?php echo $fatura_os_view->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</td>
</tr></tbody></table>
