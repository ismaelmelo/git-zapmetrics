...
class cuserpriv extends cusuario_permissoes {
	...
	function __construct() {
		...
		if (!isset($GLOBALS["usuario_permissoes"])) $GLOBALS["usuario_permissoes"] = &$this;
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel(CurrentProjectID() . 'usuario_permissoes');
	...
	function Page_Main() {
		...
		$Breadcrumb->Add("list", "<span>" . $Language->TablePhrase("usuario_permissoes", "TblCaption") . "</span>", "usuario_permissoeslist.php", "usuario_permissoes");
		...
		if (@$_POST["a_edit"] == "") {
			...
			if (@$_GET["nu_level"] <> "") {
				...
				$this->Page_Terminate("usuario_permissoeslist.php"); // Return to list
			}
			if ($this->nu_level->QueryStringValue == "-1") {
			...
			$this->nu_level->setFormValue($_POST["x_nu_level"]);
	...
	function EditRow() {
		...
		for ($i = 0; $i < $this->TableNameCount; $i++) {
			$Sql = "SELECT * FROM " . EW_USER_LEVEL_PRIV_TABLE . " WHERE " . 
				...
				EW_USER_LEVEL_PRIV_USER_LEVEL_ID_FIELD . " = " . $this->nu_level->CurrentValue;
			$rs = $conn->Execute($Sql);
			if ($rs && !$rs->EOF) {
				...
				$Sql = "INSERT INTO " . EW_USER_LEVEL_PRIV_TABLE . " (" . EW_USER_LEVEL_PRIV_TABLE_NAME_FIELD . ", " . EW_USER_LEVEL_PRIV_USER_LEVEL_ID_FIELD . ", " . EW_USER_LEVEL_PRIV_PRIV_FIELD . ") VALUES ('" . ew_AdjustSql($this->TableList[$i][4] . $this->TableList[$i][0]) . "', " . $this->nu_level->CurrentValue . ", " . $this->Privileges[$i] . ")";
				$conn->Execute($Sql);
			}
			if ($rs)
				$rs->Close();
...
<p><?php echo $Language->Phrase("UserLevel") ?><?php echo $Security->GetUserLevelName($usuario_permissoes->nu_level->CurrentValue) ?>(<?php echo $usuario_permissoes->nu_level->CurrentValue ?>)</p>
<?php
...
<input type="hidden" name="x_nu_level" id="x_nu_level" value="<?php echo $usuario_permissoes->nu_level->CurrentValue ?>">
...
for ($i = 0; $i < $userpriv->TableNameCount; $i++) {
	$userpriv->TempPriv = $Security->GetUserLevelPrivEx($userpriv->TableList[$i][4] . $userpriv->TableList[$i][0], $usuario_permissoes->nu_level->CurrentValue);
