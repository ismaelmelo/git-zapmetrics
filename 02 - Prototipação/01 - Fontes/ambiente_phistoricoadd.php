...
$ambiente_phistorico_add = NULL; // Initialize page object first

class cambiente_phistorico_add extends cambiente_phistorico {
	...
	// Page object name
	var $PageObjName = 'ambiente_phistorico_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["nu_projhist"] != "") {
				...
				$this->setKey("nu_projhist", $this->nu_projhist->CurrentValue); // Set up key
			} else {
				$this->setKey("nu_projhist", ""); // Clear key
	...
	function LoadDefaultValues() {
		...
		$this->ds_projeto->CurrentValue = NULL;
		$this->ds_projeto->OldValue = $this->ds_projeto->CurrentValue;
	...
	function AddRow($rsold = NULL) {
		...
		// ds_projeto
		$this->ds_projeto->SetDbValueDef($rsnew, $this->ds_projeto->CurrentValue, NULL, FALSE);
...
// Create page object
if (!isset($ambiente_phistorico_add)) $ambiente_phistorico_add = new cambiente_phistorico_add();

// Page init
$ambiente_phistorico_add->Page_Init();

// Page main
$ambiente_phistorico_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$ambiente_phistorico_add->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var ambiente_phistorico_add = new ew_Page("ambiente_phistorico_add");
ambiente_phistorico_add.PageID = "add"; // Page ID
var EW_PAGE_ID = ambiente_phistorico_add.PageID; // For backward compatibility

// Form object
var fambiente_phistoricoadd = new ew_Form("fambiente_phistoricoadd");

// Validate form
fambiente_phistoricoadd.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_HasValue(elm))
				return this.OnError(elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($ambiente_phistorico->nu_ambiente->FldCaption()) ?>");
...
fambiente_phistoricoadd.Form_CustomValidate = 
...
fambiente_phistoricoadd.ValidateRequired = true;
<?php } else { ?>
fambiente_phistoricoadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fambiente_phistoricoadd.Lists["x_nu_ambiente"] = {"LinkField":"x_nu_ambiente","Ajax":null,"AutoFill":false,"DisplayFields":["x_no_ambiente","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fambiente_phistoricoadd.Lists["x_nu_usuarioInc"] = {"LinkField":"x_nu_usuario","Ajax":null,"AutoFill":false,"DisplayFields":["x_no_usuario","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fambiente_phistoricoadd.Lists["x_nu_usuarioAlt"] = {"LinkField":"x_nu_usuario","Ajax":true,"AutoFill":false,"DisplayFields":["x_no_usuario","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $ambiente_phistorico_add->ShowPageHeader(); ?>
<?php
$ambiente_phistorico_add->ShowMessage();
?>
<form name="fambiente_phistoricoadd" id="fambiente_phistoricoadd" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="ambiente_phistorico">
<input type="hidden" name="a_add" id="a_add" value="A">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_ambiente_phistoricoadd" class="table table-bordered table-striped">
<?php if ($ambiente_phistorico->nu_ambiente->Visible) { // nu_ambiente ?>
	<tr id="r_nu_ambiente">
		<td><span id="elh_ambiente_phistorico_nu_ambiente"><?php echo $ambiente_phistorico->nu_ambiente->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
		<td<?php echo $ambiente_phistorico->nu_ambiente->CellAttributes() ?>>
<?php if ($ambiente_phistorico->nu_ambiente->getSessionValue() <> "") { ?>
<span<?php echo $ambiente_phistorico->nu_ambiente->ViewAttributes() ?>>
<?php echo $ambiente_phistorico->nu_ambiente->ViewValue ?></span>
<input type="hidden" id="x_nu_ambiente" name="x_nu_ambiente" value="<?php echo ew_HtmlEncode($ambiente_phistorico->nu_ambiente->CurrentValue) ?>">
<?php } else { ?>
<select data-field="x_nu_ambiente" id="x_nu_ambiente" name="x_nu_ambiente"<?php echo $ambiente_phistorico->nu_ambiente->EditAttributes() ?>>
<?php
if (is_array($ambiente_phistorico->nu_ambiente->EditValue)) {
	$arwrk = $ambiente_phistorico->nu_ambiente->EditValue;
	$rowswrk = count($arwrk);
	$emptywrk = TRUE;
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($ambiente_phistorico->nu_ambiente->CurrentValue) == strval($arwrk[$rowcntwrk][0])) ? " selected=\"selected\"" : "";
...
<script type="text/javascript">
fambiente_phistoricoadd.Lists["x_nu_ambiente"].Options = <?php echo (is_array($ambiente_phistorico->nu_ambiente->EditValue)) ? ew_ArrayToJson($ambiente_phistorico->nu_ambiente->EditValue, 1) : "[]" ?>;
</script>
<?php } ?>
<?php echo $ambiente_phistorico->nu_ambiente->CustomMsg ?></td>
	</tr>
<?php } ?>
...
<script type="text/javascript">
fambiente_phistoricoadd.Init();
<?php if (EW_MOBILE_REFLOW && ew_IsMobile()) { ?>
ew_Reflow();
<?php } ?>
</script>
<?php
$ambiente_phistorico_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$ambiente_phistorico_add->Page_Terminate();
?>
