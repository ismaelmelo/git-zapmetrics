...
$acaorisco_add = NULL; // Initialize page object first

class cacaorisco_add extends cacaorisco {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["nu_acaoRisco"] != "") {
				...
				$this->setKey("nu_acaoRisco", $this->nu_acaoRisco->CurrentValue); // Set up key
			} else {
				$this->setKey("nu_acaoRisco", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "C": // Copy an existing record
				if (!$this->LoadRow()) { // Load record based on key
					...
					$this->Page_Terminate("acaoriscolist.php"); // No matching record, return to list
				}
				break;
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "acaoriscoview.php")
	...
	function LoadDefaultValues() {
		$this->no_acaoRisco->CurrentValue = NULL;
		$this->no_acaoRisco->OldValue = $this->no_acaoRisco->CurrentValue;
	...
	function LoadFormValues() {
		...
		// Load from form
		global $objForm;
		if (!$this->no_acaoRisco->FldIsDetailKey) {
			$this->no_acaoRisco->setFormValue($objForm->GetValue("x_no_acaoRisco"));
		}
	...
	function RestoreFormValues() {
		global $objForm;
		$this->LoadOldRecord();
		$this->no_acaoRisco->CurrentValue = $this->no_acaoRisco->FormValue;
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			$this->no_acaoRisco->EditValue = ew_HtmlEncode($this->no_acaoRisco->CurrentValue);
	...
	function ValidateForm() {
		...
		if (!EW_SERVER_VALIDATE)
			return ($gsFormError == "");
		if (!$this->no_acaoRisco->FldIsDetailKey && !is_null($this->no_acaoRisco->FormValue) && $this->no_acaoRisco->FormValue == "") {
			ew_AddMessage($gsFormError, $Language->Phrase("EnterRequiredField") . " - " . $this->no_acaoRisco->FldCaption());
		}
	...
	function AddRow($rsold = NULL) {
		...
		// no_acaoRisco
		$this->no_acaoRisco->SetDbValueDef($rsnew, $this->no_acaoRisco->CurrentValue, "", FALSE);

		// ic_tpRisco
		$this->ic_tpRisco->SetDbValueDef($rsnew, $this->ic_tpRisco->CurrentValue, NULL, FALSE);
		...
		if ($AddRow) {
			$this->nu_acaoRisco->setDbValue($conn->Insert_ID());
			$rsnew['nu_acaoRisco'] = $this->nu_acaoRisco->DbValue;
		}
		if ($AddRow) {
...
// Create page object
if (!isset($acaorisco_add)) $acaorisco_add = new cacaorisco_add();

// Page init
$acaorisco_add->Page_Init();

// Page main
$acaorisco_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$acaorisco_add->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var acaorisco_add = new ew_Page("acaorisco_add");
acaorisco_add.PageID = "add"; // Page ID
var EW_PAGE_ID = acaorisco_add.PageID; // For backward compatibility

// Form object
var facaoriscoadd = new ew_Form("facaoriscoadd");

// Validate form
facaoriscoadd.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			elm = this.GetElements("x" + infix + "_no_acaoRisco");
			if (elm && !ew_HasValue(elm))
				return this.OnError(elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($acaorisco->no_acaoRisco->FldCaption()) ?>");
			elm = this.GetElements("x" + infix + "_ic_tpRisco");
			if (elm && !ew_HasValue(elm))
				return this.OnError(elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($acaorisco->ic_tpRisco->FldCaption()) ?>");
...
facaoriscoadd.Form_CustomValidate = 
...
facaoriscoadd.ValidateRequired = true;
<?php } else { ?>
facaoriscoadd.ValidateRequired = false; 
<?php } ?>
...
<?php $acaorisco_add->ShowPageHeader(); ?>
<?php
$acaorisco_add->ShowMessage();
?>
<form name="facaoriscoadd" id="facaoriscoadd" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="acaorisco">
<input type="hidden" name="a_add" id="a_add" value="A">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_acaoriscoadd" class="table table-bordered table-striped">
<?php if ($acaorisco->no_acaoRisco->Visible) { // no_acaoRisco ?>
	<tr id="r_no_acaoRisco">
		<td><span id="elh_acaorisco_no_acaoRisco"><?php echo $acaorisco->no_acaoRisco->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
...
<?php echo $acaorisco->no_acaoRisco->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($acaorisco->ic_tpRisco->Visible) { // ic_tpRisco ?>
	<tr id="r_ic_tpRisco">
		<td><span id="elh_acaorisco_ic_tpRisco"><?php echo $acaorisco->ic_tpRisco->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
...
if (is_array($acaorisco->ic_tpRisco->EditValue)) {
	...
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($acaorisco->ic_tpRisco->CurrentValue) == strval($arwrk[$rowcntwrk][0])) ? " selected=\"selected\"" : "";
...
<?php echo $acaorisco->ic_tpRisco->CustomMsg ?></td>
	</tr>
<?php } ?>
</table>
</td></tr></table>
