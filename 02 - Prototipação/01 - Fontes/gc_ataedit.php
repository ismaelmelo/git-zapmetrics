...
$gc_ata_edit = NULL; // Initialize page object first

class cgc_ata_edit extends cgc_ata {
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->nu_ata->CurrentValue == "")
			$this->Page_Terminate("gc_atalist.php"); // Invalid key, return to list
	...
	function LoadFormValues() {
		...
		if (!$this->nu_ata->FldIsDetailKey)
			$this->nu_ata->setFormValue($objForm->GetValue("x_nu_ata"));
	}
	...
	function RestoreFormValues() {
		global $objForm;
		$this->LoadRow();
		$this->nu_ata->CurrentValue = $this->nu_ata->FormValue;
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// ds_ata
			$this->ds_ata->SetDbValueDef($rsnew, $this->ds_ata->CurrentValue, "", $this->ds_ata->ReadOnly);

			// nu_grupoOuComite
			$this->nu_grupoOuComite->SetDbValueDef($rsnew, $this->nu_grupoOuComite->CurrentValue, NULL, $this->nu_grupoOuComite->ReadOnly);

			// nu_reuniao
			$this->nu_reuniao->SetDbValueDef($rsnew, $this->nu_reuniao->CurrentValue, NULL, $this->nu_reuniao->ReadOnly);
...
// Create page object
if (!isset($gc_ata_edit)) $gc_ata_edit = new cgc_ata_edit();

// Page init
$gc_ata_edit->Page_Init();

// Page main
$gc_ata_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$gc_ata_edit->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var gc_ata_edit = new ew_Page("gc_ata_edit");
gc_ata_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = gc_ata_edit.PageID; // For backward compatibility

// Form object
var fgc_ataedit = new ew_Form("fgc_ataedit");

// Validate form
fgc_ataedit.Validate = function() {
...
fgc_ataedit.Form_CustomValidate = 
...
fgc_ataedit.ValidateRequired = true;
<?php } else { ?>
fgc_ataedit.ValidateRequired = false; 
<?php } ?>

// Multi-Page properties
fgc_ataedit.MultiPage = new ew_MultiPage("fgc_ataedit",
	[["x_ds_ata",1],["x_nu_grupoOuComite",1],["x_nu_reuniao",1],["x_dt_publicacao",1],["x_ic_situacao",1],["x_no_local",2],["x_im_anexo",2]]
);

// Dynamic selection lists
fgc_ataedit.Lists["x_nu_grupoOuComite"] = {"LinkField":"x_nu_gpComite","Ajax":null,"AutoFill":false,"DisplayFields":["x_no_gpComite","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fgc_ataedit.Lists["x_nu_reuniao"] = {"LinkField":"x_nu_reuniao","Ajax":null,"AutoFill":false,"DisplayFields":["x_nu_reuniao","x_dt_reuniao","x_no_local","x_hh_inicio"],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $gc_ata_edit->ShowPageHeader(); ?>
<?php
$gc_ata_edit->ShowMessage();
?>
<form name="fgc_ataedit" id="fgc_ataedit" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="gc_ata">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<table class="ewStdTable"><tbody><tr><td>
<div class="tabbable" id="gc_ata_edit">
...
<table id="tbl_gc_ataedit1" class="table table-bordered table-striped">
...
<script type="text/javascript">
fgc_ataedit.Lists["x_nu_grupoOuComite"].Options = <?php echo (is_array($gc_ata->nu_grupoOuComite->EditValue)) ? ew_ArrayToJson($gc_ata->nu_grupoOuComite->EditValue, 1) : "[]" ?>;
</script>
</span>
...
<script type="text/javascript">
fgc_ataedit.Lists["x_nu_reuniao"].Options = <?php echo (is_array($gc_ata->nu_reuniao->EditValue)) ? ew_ArrayToJson($gc_ata->nu_reuniao->EditValue, 1) : "[]" ?>;
</script>
</span>
...
ew_CreateCalendar("fgc_ataedit", "x_dt_publicacao", "%d/%m/%Y");
</script>
<?php } ?>
</span>
...
<table cellspacing="0" class="ewGrid" style="width: 100%"><tr><td>
<table id="tbl_gc_ataedit2" class="table table-bordered table-striped">
...
<input type="hidden" data-field="x_nu_ata" name="x_nu_ata" id="x_nu_ata" value="<?php echo ew_HtmlEncode($gc_ata->nu_ata->CurrentValue) ?>">
