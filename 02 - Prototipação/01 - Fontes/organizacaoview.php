...
$organizacao_view = NULL; // Initialize page object first

class corganizacao_view extends corganizacao {
	...
	// Page object name
	var $PageObjName = 'organizacao_view';
	...
	function __construct() {
		...
		if (@$_GET["nu_organizacao"] <> "") {
			$this->RecKey["nu_organizacao"] = $_GET["nu_organizacao"];
			$KeyUrl .= "&nu_organizacao=" . urlencode($this->RecKey["nu_organizacao"]);
		}
	...
	function Page_Init() {
		...
		if (@$_GET["nu_organizacao"] <> "") {
			...
			$gsExportFile .= ew_StripSlashes($_GET["nu_organizacao"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["nu_organizacao"] <> "") {
				...
				$this->RecKey["nu_organizacao"] = $this->nu_organizacao->QueryStringValue;
			} else {
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					...
					if ($this->TotalRecs <= 0) { // No record found
						...
						$this->Page_Terminate("organizacaolist.php"); // Return to list page
						...
						while (!$this->Recordset->EOF) {
							if (strval($this->nu_organizacao->CurrentValue) == strval($this->Recordset->fields('nu_organizacao'))) {
					...
					if (!$bMatchRecord) {
						if ($this->getSuccessMessage() == "" && $this->getFailureMessage() == "")
							$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record message
						$sReturnUrl = "organizacaolist.php"; // No matching record, return to list
					} else {
			...
			$sReturnUrl = "organizacaolist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupOtherOptions() {
		...
		// Detail table 'area'
		$body = $Language->TablePhrase("area", "TblCaption");
		$body = "<a class=\"ewAction ewDetailList\" href=\"" . ew_HtmlEncode("arealist.php?" . EW_TABLE_SHOW_MASTER . "=organizacao&nu_organizacao=" . strval($this->nu_organizacao->CurrentValue) . "") . "\">" . $body . "</a>";
		$item = &$option->Add("detail_area");
		$item->Body = $body;
		$item->Visible = $Security->AllowList(CurrentProjectID() . 'area');
		if ($item->Visible) {
			if ($DetailTableLink <> "") $DetailTableLink .= ",";
			$DetailTableLink .= "area";
		}

		// Detail table 'pargerais'
		$body = $Language->TablePhrase("pargerais", "TblCaption");
		$body = "<a class=\"ewAction ewDetailList\" href=\"" . ew_HtmlEncode("pargeraislist.php?" . EW_TABLE_SHOW_MASTER . "=organizacao&nu_organizacao=" . strval($this->nu_organizacao->CurrentValue) . "") . "\">" . $body . "</a>";
		$item = &$option->Add("detail_pargerais");
		$item->Body = $body;
		$item->Visible = $Security->AllowList(CurrentProjectID() . 'pargerais');
		if ($item->Visible) {
			if ($DetailTableLink <> "") $DetailTableLink .= ",";
			$DetailTableLink .= "pargerais";
		}
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a id=\"emf_organizacao\" href=\"javascript:void(0);\" class=\"ewExportLink ewEmail\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_organizacao',hdr:ewLanguage.Phrase('ExportToEmail'),f:document.forganizacaoview,key:" . ew_ArrayToJsonAttr($this->RecKey) . ",sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("area", $DetailTblVar)) {
				...
				if ($GLOBALS["area_grid"]->DetailView) {
					$GLOBALS["area_grid"]->CurrentMode = "view";
			...
			if (in_array("pargerais", $DetailTblVar)) {
				...
				if ($GLOBALS["pargerais_grid"]->DetailView) {
					$GLOBALS["pargerais_grid"]->CurrentMode = "view";
...
// Create page object
if (!isset($organizacao_view)) $organizacao_view = new corganizacao_view();

// Page init
$organizacao_view->Page_Init();

// Page main
$organizacao_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$organizacao_view->Page_Render();
?>
<?php include_once "header.php" ?>
<?php if ($organizacao->Export == "") { ?>
<script type="text/javascript">

// Page object
var organizacao_view = new ew_Page("organizacao_view");
organizacao_view.PageID = "view"; // Page ID
var EW_PAGE_ID = organizacao_view.PageID; // For backward compatibility

// Form object
var forganizacaoview = new ew_Form("forganizacaoview");

// Form_CustomValidate event
forganizacaoview.Form_CustomValidate = 
...
forganizacaoview.ValidateRequired = true;
<?php } else { ?>
forganizacaoview.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
forganizacaoview.Lists["x_nu_verticalNegocio"] = {"LinkField":"x_nu_verticalNegocio","Ajax":null,"AutoFill":false,"DisplayFields":["x_no_vertical","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $organizacao_view->ExportOptions->Render("body") ?>
<?php if (!$organizacao_view->ExportOptions->UseDropDownButton) { ?>
</div>
<div class="ewViewOtherOptions">
<?php } ?>
<?php
	foreach ($organizacao_view->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<?php } ?>
<?php $organizacao_view->ShowPageHeader(); ?>
<?php
$organizacao_view->ShowMessage();
?>
<form name="forganizacaoview" id="forganizacaoview" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="organizacao">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_organizacaoview" class="table table-bordered table-striped">
<?php if ($organizacao->no_organizacao->Visible) { // no_organizacao ?>
	<tr id="r_no_organizacao">
		<td><span id="elh_organizacao_no_organizacao"><?php echo $organizacao->no_organizacao->FldCaption() ?></span></td>
...
<?php echo $organizacao->no_organizacao->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<span<?php echo $organizacao->ds_organizacao->ViewAttributes() ?>>
<?php echo $organizacao->ds_organizacao->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($organizacao->nu_verticalNegocio->Visible) { // nu_verticalNegocio ?>
	<tr id="r_nu_verticalNegocio">
		<td><span id="elh_organizacao_nu_verticalNegocio"><?php echo $organizacao->nu_verticalNegocio->FldCaption() ?></span></td>
...
<?php echo $organizacao->nu_verticalNegocio->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($organizacao->ic_ativo->Visible) { // ic_ativo ?>
	<tr id="r_ic_ativo">
		<td><span id="elh_organizacao_ic_ativo"><?php echo $organizacao->ic_ativo->FldCaption() ?></span></td>
...
<?php echo $organizacao->ic_ativo->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($organizacao->ic_padrao->Visible) { // ic_padrao ?>
	<tr id="r_ic_padrao">
		<td><span id="elh_organizacao_ic_padrao"><?php echo $organizacao->ic_padrao->FldCaption() ?></span></td>
		<td<?php echo $organizacao->ic_padrao->CellAttributes() ?>>
<span id="el_organizacao_ic_padrao" class="control-group">
<span<?php echo $organizacao->ic_padrao->ViewAttributes() ?>>
<?php echo $organizacao->ic_padrao->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<table class="ewPager">
<tr><td>
<?php if (!isset($organizacao_view->Pager)) $organizacao_view->Pager = new cNumericPager($organizacao_view->StartRec, $organizacao_view->DisplayRecs, $organizacao_view->TotalRecs, $organizacao_view->RecRange) ?>
<?php if ($organizacao_view->Pager->RecordCount > 0) { ?>
<table cellspacing="0" class="ewStdTable"><tbody><tr><td>
<div class="pagination"><ul>
	<?php if ($organizacao_view->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $organizacao_view->PageUrl() ?>start=<?php echo $organizacao_view->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($organizacao_view->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $organizacao_view->PageUrl() ?>start=<?php echo $organizacao_view->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($organizacao_view->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $organizacao_view->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($organizacao_view->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $organizacao_view->PageUrl() ?>start=<?php echo $organizacao_view->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($organizacao_view->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $organizacao_view->PageUrl() ?>start=<?php echo $organizacao_view->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</td>
</tr></tbody></table>
...
<?php } ?>
<?php
	if (in_array("area", explode(",", $organizacao->getCurrentDetailTable())) && $area->DetailView) {
?>
<?php include_once "areagrid.php" ?>
<?php } ?>
<?php
	if (in_array("pargerais", explode(",", $organizacao->getCurrentDetailTable())) && $pargerais->DetailView) {
?>
<?php include_once "pargeraisgrid.php" ?>
<?php } ?>
</form>
<script type="text/javascript">
forganizacaoview.Init();
</script>
<?php
$organizacao_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($organizacao->Export == "") { ?>
...
<?php
$organizacao_view->Page_Terminate();
?>
