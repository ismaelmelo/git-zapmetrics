...
$roteiro_add = NULL; // Initialize page object first

class croteiro_add extends croteiro {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["nu_roteiro"] != "") {
				...
				$this->setKey("nu_roteiro", $this->nu_roteiro->CurrentValue); // Set up key
			} else {
				$this->setKey("nu_roteiro", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "roteiroview.php")
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			if ($this->nu_metodologia->getSessionValue() <> "") {
				$this->nu_metodologia->CurrentValue = $this->nu_metodologia->getSessionValue();
			if (strval($this->nu_metodologia->CurrentValue) <> "") {
				$sFilterWrk = "[nu_metodologia]" . ew_SearchString("=", $this->nu_metodologia->CurrentValue, EW_DATATYPE_NUMBER);
			$sSqlWrk = "SELECT [nu_metodologia], [no_metodologia] AS [DispFld], '' AS [Disp2Fld], '' AS [Disp3Fld], '' AS [Disp4Fld] FROM [dbo].[metodologia]";
			$sWhereWrk = "";
	...
	function ValidateForm() {
		...
		if (in_array("faseroteiro", $DetailTblVar) && $GLOBALS["faseroteiro"]->DetailAdd) {
	...
	function AddRow($rsold = NULL) {
		...
		if ($AddRow) {
			$DetailTblVar = explode(",", $this->getCurrentDetailTable());
			if (in_array("faseroteiro", $DetailTblVar) && $GLOBALS["faseroteiro"]->DetailAdd) {
				$GLOBALS["faseroteiro"]->nu_roteiro->setSessionValue($this->nu_roteiro->CurrentValue); // Set master key
				if (!isset($GLOBALS["faseroteiro_grid"])) $GLOBALS["faseroteiro_grid"] = new cfaseroteiro_grid(); // Get detail page object
				$AddRow = $GLOBALS["faseroteiro_grid"]->GridInsert();
				if (!$AddRow)
					$GLOBALS["faseroteiro"]->nu_roteiro->setSessionValue(""); // Clear master key if insert failed
			}
		}
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("faseroteiro", $DetailTblVar)) {
				...
				if ($GLOBALS["faseroteiro_grid"]->DetailAdd) {
					if ($this->CopyRecord)
						$GLOBALS["faseroteiro_grid"]->CurrentMode = "copy";
					else
						$GLOBALS["faseroteiro_grid"]->CurrentMode = "add";
					$GLOBALS["faseroteiro_grid"]->CurrentAction = "gridadd";
...
// Create page object
if (!isset($roteiro_add)) $roteiro_add = new croteiro_add();

// Page init
$roteiro_add->Page_Init();

// Page main
$roteiro_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$roteiro_add->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var roteiro_add = new ew_Page("roteiro_add");
roteiro_add.PageID = "add"; // Page ID
var EW_PAGE_ID = roteiro_add.PageID; // For backward compatibility

// Form object
var froteiroadd = new ew_Form("froteiroadd");

// Validate form
froteiroadd.Validate = function() {
...
froteiroadd.Form_CustomValidate = 
...
froteiroadd.ValidateRequired = true;
<?php } else { ?>
froteiroadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
froteiroadd.Lists["x_nu_metodologia"] = {"LinkField":"x_nu_metodologia","Ajax":null,"AutoFill":false,"DisplayFields":["x_no_metodologia","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $roteiro_add->ShowPageHeader(); ?>
<?php
$roteiro_add->ShowMessage();
?>
<form name="froteiroadd" id="froteiroadd" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="roteiro">
<input type="hidden" name="a_add" id="a_add" value="A">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_roteiroadd" class="table table-bordered table-striped">
<?php if ($roteiro->nu_metodologia->Visible) { // nu_metodologia ?>
	<tr id="r_nu_metodologia">
		<td><span id="elh_roteiro_nu_metodologia"><?php echo $roteiro->nu_metodologia->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
...
<input type="hidden" id="x_nu_metodologia" name="x_nu_metodologia" value="<?php echo ew_HtmlEncode($roteiro->nu_metodologia->CurrentValue) ?>">
<?php } else { ?>
...
<script type="text/javascript">
froteiroadd.Lists["x_nu_metodologia"].Options = <?php echo (is_array($roteiro->nu_metodologia->EditValue)) ? ew_ArrayToJson($roteiro->nu_metodologia->EditValue, 1) : "[]" ?>;
</script>
<?php } ?>
...
<?php
	if (in_array("faseroteiro", explode(",", $roteiro->getCurrentDetailTable())) && $faseroteiro->DetailAdd) {
?>
