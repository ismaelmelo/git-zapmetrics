...
$prospecto_itempdti_grid = NULL; // Initialize page object first

class cprospecto_itempdti_grid extends cprospecto_itempdti {
	...
	// Page object name
	var $PageObjName = 'prospecto_itempdti_grid';

	// Grid form hidden field names
	var $FormName = 'fprospecto_itempdtigrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["prospecto_itempdti"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					if ($sKey <> "") $sKey .= $GLOBALS["EW_COMPOSITE_KEY_SEPARATOR"];
					$sKey .= $this->nu_itemPdti->CurrentValue;
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_nu_itemPdti") && $objForm->HasValue("o_nu_itemPdti") && $this->nu_itemPdti->CurrentValue <> $this->nu_itemPdti->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->nu_prospecto->CurrentValue . $GLOBALS["EW_COMPOSITE_KEY_SEPARATOR"] . $this->nu_itemPdti->CurrentValue . "\">";
		}
	...
	function LoadFormValues() {
		...
		$this->nu_itemPdti->setOldValue($objForm->GetValue("o_nu_itemPdti"));
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 2) {
			if (strval($arKeys[0]) <> "")
				...
				$bValidKey = FALSE;
			if (strval($arKeys[1]) <> "")
				$this->nu_itemPdti->CurrentValue = strval($arKeys[1]); // nu_itemPdti
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
