...
$solicitacao_uc_grid = NULL; // Initialize page object first

class csolicitacao_uc_grid extends csolicitacao_uc {
	...
	// Page object name
	var $PageObjName = 'solicitacao_uc_grid';

	// Grid form hidden field names
	var $FormName = 'fsolicitacao_ucgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["solicitacao_uc"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function EmptyRow() {
		...
		if ($objForm->HasValue("x_nu_uc") && $objForm->HasValue("o_nu_uc") && $this->nu_uc->CurrentValue <> $this->nu_uc->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_ic_impacto") && $objForm->HasValue("o_ic_impacto") && $this->ic_impacto->CurrentValue <> $this->ic_impacto->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->nu_solicitacao->CurrentValue . $GLOBALS["EW_COMPOSITE_KEY_SEPARATOR"] . $this->nu_uc->CurrentValue . "\">";
		}
	...
	function LoadFormValues() {
		...
		$this->ic_impacto->setOldValue($objForm->GetValue("o_ic_impacto"));
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 2) {
			if (strval($arKeys[0]) <> "")
				...
				$bValidKey = FALSE;
			if (strval($arKeys[1]) <> "")
				$this->nu_uc->CurrentValue = strval($arKeys[1]); // nu_uc
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			$this->ic_impacto->SetDbValueDef($rsnew, $this->ic_impacto->CurrentValue, NULL, $this->ic_impacto->ReadOnly);
