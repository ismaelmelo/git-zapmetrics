...
//
// Table class for pdti
//
class cpdti extends cTable {
	var $nu_periodo;
	var $no_tituloCapa;
	var $ds_apresentacao;
	var $ds_introducao;
	var $no_localArquivo;
	var $im_planoTrabalho;
	...
	function __construct() {
		...
		// nu_periodo
		$this->nu_periodo = new cField('pdti', 'pdti', 'x_nu_periodo', 'nu_periodo', '[nu_periodo]', 'CAST([nu_periodo] AS NVARCHAR)', 3, -1, FALSE, '[nu_periodo]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->nu_periodo->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['nu_periodo'] = &$this->nu_periodo;

		// no_tituloCapa
		$this->no_tituloCapa = new cField('pdti', 'pdti', 'x_no_tituloCapa', 'no_tituloCapa', '[no_tituloCapa]', '[no_tituloCapa]', 200, -1, FALSE, '[no_tituloCapa]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['no_tituloCapa'] = &$this->no_tituloCapa;

		// ds_apresentacao
		$this->ds_apresentacao = new cField('pdti', 'pdti', 'x_ds_apresentacao', 'ds_apresentacao', '[ds_apresentacao]', '[ds_apresentacao]', 201, -1, FALSE, '[ds_apresentacao]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['ds_apresentacao'] = &$this->ds_apresentacao;

		// ds_introducao
		$this->ds_introducao = new cField('pdti', 'pdti', 'x_ds_introducao', 'ds_introducao', '[ds_introducao]', '[ds_introducao]', 201, -1, FALSE, '[ds_introducao]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['ds_introducao'] = &$this->ds_introducao;

		// no_localArquivo
		$this->no_localArquivo = new cField('pdti', 'pdti', 'x_no_localArquivo', 'no_localArquivo', '[no_localArquivo]', '[no_localArquivo]', 200, -1, FALSE, '[no_localArquivo]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['no_localArquivo'] = &$this->no_localArquivo;

		// im_planoTrabalho
		$this->im_planoTrabalho = new cField('pdti', 'pdti', 'x_im_planoTrabalho', 'im_planoTrabalho', '[im_planoTrabalho]', '[im_planoTrabalho]', 201, -1, TRUE, '[im_planoTrabalho]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->im_planoTrabalho->UploadMultiple = TRUE;
		$this->fields['im_planoTrabalho'] = &$this->im_planoTrabalho;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("pdtiview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("pdtiview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}
	...
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("pdtiedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("pdtiadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("pdtidelete.php", $this->UrlParm());
	}
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				...
				if ($this->no_tituloCapa->Exportable) $Doc->ExportCaption($this->no_tituloCapa);
				if ($this->ds_apresentacao->Exportable) $Doc->ExportCaption($this->ds_apresentacao);
				...
				if ($this->im_planoTrabalho->Exportable) $Doc->ExportCaption($this->im_planoTrabalho);
			} else {
				if ($this->nu_periodo->Exportable) $Doc->ExportCaption($this->nu_periodo);
				if ($this->no_tituloCapa->Exportable) $Doc->ExportCaption($this->no_tituloCapa);
				if ($this->no_localArquivo->Exportable) $Doc->ExportCaption($this->no_localArquivo);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					...
					if ($this->no_tituloCapa->Exportable) $Doc->ExportField($this->no_tituloCapa);
					if ($this->ds_apresentacao->Exportable) $Doc->ExportField($this->ds_apresentacao);
					...
					if ($this->im_planoTrabalho->Exportable) $Doc->ExportField($this->im_planoTrabalho);
				} else {
					if ($this->nu_periodo->Exportable) $Doc->ExportField($this->nu_periodo);
					if ($this->no_tituloCapa->Exportable) $Doc->ExportField($this->no_tituloCapa);
					if ($this->no_localArquivo->Exportable) $Doc->ExportField($this->no_localArquivo);
