...
$projeto_centrocusto_view = NULL; // Initialize page object first

class cprojeto_centrocusto_view extends cprojeto_centrocusto {
	...
	// Page object name
	var $PageObjName = 'projeto_centrocusto_view';
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					...
					if ($this->TotalRecs <= 0) { // No record found
						...
						$this->Page_Terminate("projeto_centrocustolist.php"); // Return to list page
					...
					if (!$bMatchRecord) {
						if ($this->getSuccessMessage() == "" && $this->getFailureMessage() == "")
							$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record message
						$sReturnUrl = "projeto_centrocustolist.php"; // No matching record, return to list
					} else {
			...
			$sReturnUrl = "projeto_centrocustolist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($projeto_centrocusto_view)) $projeto_centrocusto_view = new cprojeto_centrocusto_view();

// Page init
$projeto_centrocusto_view->Page_Init();

// Page main
$projeto_centrocusto_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$projeto_centrocusto_view->Page_Render();
?>
...
// Page object
var projeto_centrocusto_view = new ew_Page("projeto_centrocusto_view");
projeto_centrocusto_view.PageID = "view"; // Page ID
var EW_PAGE_ID = projeto_centrocusto_view.PageID; // For backward compatibility

// Form object
var fprojeto_centrocustoview = new ew_Form("fprojeto_centrocustoview");

// Form_CustomValidate event
fprojeto_centrocustoview.Form_CustomValidate = 
...
fprojeto_centrocustoview.ValidateRequired = true;
<?php } else { ?>
fprojeto_centrocustoview.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fprojeto_centrocustoview.Lists["x_nu_projeto"] = {"LinkField":"x_nu_projeto","Ajax":null,"AutoFill":false,"DisplayFields":["x_no_projeto","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fprojeto_centrocustoview.Lists["x_nu_centroCusto_"] = {"LinkField":"x_nu_centroCusto","Ajax":null,"AutoFill":false,"DisplayFields":["x_co_alternativo","x_no_centroCusto","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $projeto_centrocusto_view->ExportOptions->Render("body") ?>
<?php if (!$projeto_centrocusto_view->ExportOptions->UseDropDownButton) { ?>
</div>
<div class="ewViewOtherOptions">
<?php } ?>
<?php
	foreach ($projeto_centrocusto_view->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<?php } ?>
<?php $projeto_centrocusto_view->ShowPageHeader(); ?>
<?php
$projeto_centrocusto_view->ShowMessage();
?>
<form name="fprojeto_centrocustoview" id="fprojeto_centrocustoview" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="projeto_centrocusto">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_projeto_centrocustoview" class="table table-bordered table-striped">
...
<?php if ($projeto_centrocusto->pc_participacao->Visible) { // pc_participacao ?>
	<tr id="r_pc_participacao">
		<td><span id="elh_projeto_centrocusto_pc_participacao"><?php echo $projeto_centrocusto->pc_participacao->FldCaption() ?></span></td>
...
<table class="ewPager">
<tr><td>
<?php if (!isset($projeto_centrocusto_view->Pager)) $projeto_centrocusto_view->Pager = new cNumericPager($projeto_centrocusto_view->StartRec, $projeto_centrocusto_view->DisplayRecs, $projeto_centrocusto_view->TotalRecs, $projeto_centrocusto_view->RecRange) ?>
<?php if ($projeto_centrocusto_view->Pager->RecordCount > 0) { ?>
<table cellspacing="0" class="ewStdTable"><tbody><tr><td>
<div class="pagination"><ul>
	<?php if ($projeto_centrocusto_view->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $projeto_centrocusto_view->PageUrl() ?>start=<?php echo $projeto_centrocusto_view->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($projeto_centrocusto_view->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $projeto_centrocusto_view->PageUrl() ?>start=<?php echo $projeto_centrocusto_view->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($projeto_centrocusto_view->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $projeto_centrocusto_view->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($projeto_centrocusto_view->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $projeto_centrocusto_view->PageUrl() ?>start=<?php echo $projeto_centrocusto_view->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($projeto_centrocusto_view->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $projeto_centrocusto_view->PageUrl() ?>start=<?php echo $projeto_centrocusto_view->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</td>
</tr></tbody></table>
...
<script type="text/javascript">
fprojeto_centrocustoview.Init();
</script>
<?php
$projeto_centrocusto_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($projeto_centrocusto->Export == "") { ?>
...
<?php
$projeto_centrocusto_view->Page_Terminate();
?>
