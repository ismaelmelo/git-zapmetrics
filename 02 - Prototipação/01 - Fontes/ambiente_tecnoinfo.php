...
//
// Table class for ambiente_tecno
//
class cambiente_tecno extends cTable {
	var $nu_ambiente;
	var $nu_tecnoDesenv;
	...
	function __construct() {
		...
		$this->TableVar = 'ambiente_tecno';
		$this->TableName = 'ambiente_tecno';
		...
		// nu_ambiente
		$this->nu_ambiente = new cField('ambiente_tecno', 'ambiente_tecno', 'x_nu_ambiente', 'nu_ambiente', '[nu_ambiente]', 'CAST([nu_ambiente] AS NVARCHAR)', 3, -1, FALSE, '[nu_ambiente]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->nu_ambiente->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['nu_ambiente'] = &$this->nu_ambiente;

		// nu_tecnoDesenv
		$this->nu_tecnoDesenv = new cField('ambiente_tecno', 'ambiente_tecno', 'x_nu_tecnoDesenv', 'nu_tecnoDesenv', '[nu_tecnoDesenv]', 'CAST([nu_tecnoDesenv] AS NVARCHAR)', 3, -1, FALSE, '[nu_tecnoDesenv]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->nu_tecnoDesenv->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->nu_tecnoDesenv->AdvancedSearch->SearchValueDefault = "S";
		$this->nu_tecnoDesenv->AdvancedSearch->SearchOperatorDefault = "=";
		$this->nu_tecnoDesenv->AdvancedSearch->SearchOperatorDefault2 = "";
		$this->nu_tecnoDesenv->AdvancedSearch->SearchConditionDefault = "AND";
	...
	function SqlOrderBy() { // Order By
		return "[nu_ambiente] ASC,[nu_tecnoDesenv] ASC";
	}
	...
	// Update Table
	var $UpdateTable = "[dbo].[ambiente_tecno]";
	...
	function SqlKeyFilter() {
		return "[nu_ambiente] = @nu_ambiente@ AND [nu_tecnoDesenv] = @nu_tecnoDesenv@";
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("ambiente_tecnoview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("ambiente_tecnoview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl() {
		return "ambiente_tecnoadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("ambiente_tecnoedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("ambiente_tecnoadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("ambiente_tecnodelete.php", $this->UrlParm());
	}
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKey[] = @$_GET["nu_tecnoDesenv"]; // nu_tecnoDesenv
			$arKeys[] = $arKey;
		...
		foreach ($arKeys as $key) {
			...
			if (!is_numeric($key[0])) // nu_ambiente
				continue;
			if (!is_numeric($key[1])) // nu_tecnoDesenv
				continue;
			$ar[] = $key;
		}
		return $ar;
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->nu_tecnoDesenv->CurrentValue = $key[1];
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				...
				if ($this->nu_tecnoDesenv->Exportable) $Doc->ExportCaption($this->nu_tecnoDesenv);
			} else {
				if ($this->nu_ambiente->Exportable) $Doc->ExportCaption($this->nu_ambiente);
				if ($this->nu_tecnoDesenv->Exportable) $Doc->ExportCaption($this->nu_tecnoDesenv);
			}
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					...
					if ($this->nu_tecnoDesenv->Exportable) $Doc->ExportField($this->nu_tecnoDesenv);
				} else {
					if ($this->nu_ambiente->Exportable) $Doc->ExportField($this->nu_ambiente);
					if ($this->nu_tecnoDesenv->Exportable) $Doc->ExportField($this->nu_tecnoDesenv);
				}
