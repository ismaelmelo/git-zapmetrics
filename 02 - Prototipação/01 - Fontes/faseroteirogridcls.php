...
$faseroteiro_grid = NULL; // Initialize page object first

class cfaseroteiro_grid extends cfaseroteiro {
	...
	// Page object name
	var $PageObjName = 'faseroteiro_grid';

	// Grid form hidden field names
	var $FormName = 'ffaseroteirogrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["faseroteiro"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->nu_faseRoteiro->CurrentValue;
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_no_faseRoteiro") && $objForm->HasValue("o_no_faseRoteiro") && $this->no_faseRoteiro->CurrentValue <> $this->no_faseRoteiro->OldValue)
			return FALSE;
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->nu_faseRoteiro->CurrentValue . "\">";
		}
	...
	function LoadFormValues() {
		...
		$this->no_faseRoteiro->setOldValue($objForm->GetValue("o_no_faseRoteiro"));
		...
		if (!$this->nu_faseRoteiro->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->nu_faseRoteiro->CurrentValue = strval($arKeys[0]); // nu_faseRoteiro
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function AddRow($rsold = NULL) {
		...
		if ($this->nu_roteiro->getSessionValue() <> "") {
			$rsnew['nu_roteiro'] = $this->nu_roteiro->getSessionValue();
		}
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "roteiro") {
			$this->nu_roteiro->Visible = FALSE;
			if ($GLOBALS["roteiro"]->EventCancelled) $this->EventCancelled = TRUE;
		}
