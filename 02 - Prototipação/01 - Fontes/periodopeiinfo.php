...
//
// Table class for periodopei
//
class cperiodopei extends cTable {
	var $nu_periodoPei;
	var $nu_anoInicio;
	var $nu_anoFim;
	var $no_periodo;
	...
	function __construct() {
		...
		$this->TableVar = 'periodopei';
		$this->TableName = 'periodopei';
		...
		// nu_periodoPei
		$this->nu_periodoPei = new cField('periodopei', 'periodopei', 'x_nu_periodoPei', 'nu_periodoPei', '[nu_periodoPei]', 'CAST([nu_periodoPei] AS NVARCHAR)', 3, -1, FALSE, '[nu_periodoPei]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->nu_periodoPei->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['nu_periodoPei'] = &$this->nu_periodoPei;

		// nu_anoInicio
		$this->nu_anoInicio = new cField('periodopei', 'periodopei', 'x_nu_anoInicio', 'nu_anoInicio', '[nu_anoInicio]', 'CAST([nu_anoInicio] AS NVARCHAR)', 3, -1, FALSE, '[nu_anoInicio]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->nu_anoInicio->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['nu_anoInicio'] = &$this->nu_anoInicio;

		// nu_anoFim
		$this->nu_anoFim = new cField('periodopei', 'periodopei', 'x_nu_anoFim', 'nu_anoFim', '[nu_anoFim]', 'CAST([nu_anoFim] AS NVARCHAR)', 3, -1, FALSE, '[nu_anoFim]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->nu_anoFim->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['nu_anoFim'] = &$this->nu_anoFim;

		// no_periodo
		$this->no_periodo = new cField('periodopei', 'periodopei', 'x_no_periodo', 'no_periodo', '[no_periodo]', '[no_periodo]', 200, -1, FALSE, '[no_periodo]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['no_periodo'] = &$this->no_periodo;

		// ic_situacao
		$this->ic_situacao = new cField('periodopei', 'periodopei', 'x_ic_situacao', 'ic_situacao', '[ic_situacao]', '[ic_situacao]', 129, -1, FALSE, '[ic_situacao]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	// Update Table
	var $UpdateTable = "[dbo].[periodopei]";
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			if (array_key_exists('nu_periodoPei', $rs))
				ew_AddFilter($where, ew_QuotedName('nu_periodoPei') . '=' . ew_QuotedValue($rs['nu_periodoPei'], $this->nu_periodoPei->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function SqlKeyFilter() {
		return "[nu_periodoPei] = @nu_periodoPei@";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		if (!is_numeric($this->nu_periodoPei->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@nu_periodoPei@", ew_AdjustSql($this->nu_periodoPei->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("periodopeiview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("periodopeiview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl() {
		return "periodopeiadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("periodopeiedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("periodopeiadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("periodopeidelete.php", $this->UrlParm());
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->nu_periodoPei->CurrentValue)) {
			$sUrl .= "nu_periodoPei=" . urlencode($this->nu_periodoPei->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKeys[] = @$_GET["nu_periodoPei"]; // nu_periodoPei
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->nu_periodoPei->CurrentValue = $key;
