...
$contrato_im_contrato_blobview = NULL; // Initialize page object first

class ccontrato_im_contrato_blobview extends ccontrato {
	...
	// Page object name
	var $PageObjName = 'contrato_im_contrato_blobview';
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel(CurrentProjectID() . 'contrato');
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('contrato', 'x_im_contrato');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (!EW_DEBUG_ENABLED && ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('im_contrato');
...
// Create page object
if (!isset($contrato_im_contrato_blobview)) $contrato_im_contrato_blobview = new ccontrato_im_contrato_blobview();

// Page init
$contrato_im_contrato_blobview->Page_Init();

// Page main
$contrato_im_contrato_blobview->Page_Main();
?>
<?php
$contrato_im_contrato_blobview->Page_Terminate();
?>
