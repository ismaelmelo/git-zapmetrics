...
$solicitacao_ocorrencia_list = NULL; // Initialize page object first

class csolicitacao_ocorrencia_list extends csolicitacao_ocorrencia {
	...
	// Page object name
	var $PageObjName = 'solicitacao_ocorrencia_list';

	// Grid form hidden field names
	var $FormName = 'fsolicitacao_ocorrencialist';
	...
	function __construct() {
		...
		$this->AddUrl = "solicitacao_ocorrenciaadd.php";
		$this->InlineAddUrl = $this->PageUrl() . "a=add";
		$this->GridAddUrl = $this->PageUrl() . "a=gridadd";
		$this->GridEditUrl = $this->PageUrl() . "a=gridedit";
		$this->MultiDeleteUrl = "solicitacao_ocorrenciadelete.php";
		$this->MultiUpdateUrl = "solicitacao_ocorrenciaupdate.php";
	...
	function Page_Init() {
		...
		$this->nu_usuarioInc->Visible = !$this->IsAddOrEdit();
	...
	function SetupKeyValues($key) {
		...
		if (count($arrKeyFlds) >= 2) {
			...
			if (!is_numeric($this->nu_solicitacao->FormValue))
				return FALSE;
			$this->nu_ocorrencia->setFormValue($arrKeyFlds[1]);
			if (!is_numeric($this->nu_ocorrencia->FormValue))
				return FALSE;
		}
		return TRUE;
	}
	...
	function SetUpSortOrder() {
		...
		if (@$_GET["order"] <> "") {
			...
			$this->UpdateSort($this->ic_tpOcorrencia, $bCtrl); // ic_tpOcorrencia
			$this->UpdateSort($this->ic_exibirNoLaudo, $bCtrl); // ic_exibirNoLaudo
			$this->UpdateSort($this->nu_usuarioInc, $bCtrl); // nu_usuarioInc
	...
	function ResetCmd() {
		...
		if (substr($this->Command,0,5) == "reset") {
			...
			if ($this->Command == "resetsort") {
				$sOrderBy = "";
				$this->setSessionOrderBy($sOrderBy);
				$this->ic_tpOcorrencia->setSort("");
				$this->ic_exibirNoLaudo->setSort("");
				$this->nu_usuarioInc->setSort("");
	...
	function RenderOtherOptions() {
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->CustomActions as $action => $name) {
				...
				$item->Body = "<a class=\"ewAction ewCustomAction\" href=\"\" onclick=\"ew_SubmitSelected(document.fsolicitacao_ocorrencialist, '" . ew_CurrentUrl() . "', null, '" . $action . "');return false;\">" . $name . "</a>";
			}
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a id=\"emf_solicitacao_ocorrencia\" href=\"javascript:void(0);\" class=\"ewExportLink ewEmail\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_solicitacao_ocorrencia',hdr:ewLanguage.Phrase('ExportToEmail'),f:document.fsolicitacao_ocorrencialist,sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
...
// Create page object
if (!isset($solicitacao_ocorrencia_list)) $solicitacao_ocorrencia_list = new csolicitacao_ocorrencia_list();

// Page init
$solicitacao_ocorrencia_list->Page_Init();

// Page main
$solicitacao_ocorrencia_list->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$solicitacao_ocorrencia_list->Page_Render();
?>
...
// Page object
var solicitacao_ocorrencia_list = new ew_Page("solicitacao_ocorrencia_list");
solicitacao_ocorrencia_list.PageID = "list"; // Page ID
var EW_PAGE_ID = solicitacao_ocorrencia_list.PageID; // For backward compatibility

// Form object
var fsolicitacao_ocorrencialist = new ew_Form("fsolicitacao_ocorrencialist");
fsolicitacao_ocorrencialist.FormKeyCountName = '<?php echo $solicitacao_ocorrencia_list->FormKeyCountName ?>';

// Form_CustomValidate event
fsolicitacao_ocorrencialist.Form_CustomValidate = 
...
fsolicitacao_ocorrencialist.ValidateRequired = true;
<?php } else { ?>
fsolicitacao_ocorrencialist.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fsolicitacao_ocorrencialist.Lists["x_nu_usuarioInc"] = {"LinkField":"x_nu_usuario","Ajax":null,"AutoFill":false,"DisplayFields":["x_no_usuario","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php } ?>
<?php if ($solicitacao_ocorrencia->getCurrentMasterTable() == "" && $solicitacao_ocorrencia_list->ExportOptions->Visible()) { ?>
<div class="ewListExportOptions"><?php $solicitacao_ocorrencia_list->ExportOptions->Render("body") ?></div>
<?php } ?>
<?php if (($solicitacao_ocorrencia->Export == "") || (EW_EXPORT_MASTER_RECORD && $solicitacao_ocorrencia->Export == "print")) { ?>
<?php
$gsMasterReturnUrl = "solicitacaometricaslist.php";
if ($solicitacao_ocorrencia_list->DbMasterFilter <> "" && $solicitacao_ocorrencia->getCurrentMasterTable() == "solicitacaoMetricas") {
	if ($solicitacao_ocorrencia_list->MasterRecordExists) {
		if ($solicitacao_ocorrencia->getCurrentMasterTable() == $solicitacao_ocorrencia->TableVar) $gsMasterReturnUrl .= "?" . EW_TABLE_SHOW_MASTER . "=";
?>
<?php if ($solicitacao_ocorrencia_list->ExportOptions->Visible()) { ?>
<div class="ewListExportOptions"><?php $solicitacao_ocorrencia_list->ExportOptions->Render("body") ?></div>
<?php } ?>
...
<?php
	$bSelectLimit = EW_SELECT_LIMIT;
	if ($bSelectLimit) {
		$solicitacao_ocorrencia_list->TotalRecs = $solicitacao_ocorrencia->SelectRecordCount();
	} else {
		if ($solicitacao_ocorrencia_list->Recordset = $solicitacao_ocorrencia_list->LoadRecordset())
			$solicitacao_ocorrencia_list->TotalRecs = $solicitacao_ocorrencia_list->Recordset->RecordCount();
	}
	$solicitacao_ocorrencia_list->StartRec = 1;
	if ($solicitacao_ocorrencia_list->DisplayRecs <= 0 || ($solicitacao_ocorrencia->Export <> "" && $solicitacao_ocorrencia->ExportAll)) // Display all records
		$solicitacao_ocorrencia_list->DisplayRecs = $solicitacao_ocorrencia_list->TotalRecs;
	if (!($solicitacao_ocorrencia->Export <> "" && $solicitacao_ocorrencia->ExportAll))
		$solicitacao_ocorrencia_list->SetUpStartRec(); // Set up start record position
	if ($bSelectLimit)
		$solicitacao_ocorrencia_list->Recordset = $solicitacao_ocorrencia_list->LoadRecordset($solicitacao_ocorrencia_list->StartRec-1, $solicitacao_ocorrencia_list->DisplayRecs);
$solicitacao_ocorrencia_list->RenderOtherOptions();
?>
<?php $solicitacao_ocorrencia_list->ShowPageHeader(); ?>
<?php
$solicitacao_ocorrencia_list->ShowMessage();
?>
<table cellspacing="0" class="ewGrid"><tr><td class="ewGridContent">
<form name="fsolicitacao_ocorrencialist" id="fsolicitacao_ocorrencialist" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="solicitacao_ocorrencia">
<div id="gmp_solicitacao_ocorrencia" class="ewGridMiddlePanel">
<?php if ($solicitacao_ocorrencia_list->TotalRecs > 0) { ?>
<table id="tbl_solicitacao_ocorrencialist" class="ewTable ewTableSeparate">
...
// Render list options
$solicitacao_ocorrencia_list->RenderListOptions();

// Render list options (header, left)
$solicitacao_ocorrencia_list->ListOptions->Render("header", "left");
?>
<?php if ($solicitacao_ocorrencia->ic_tpOcorrencia->Visible) { // ic_tpOcorrencia ?>
	...
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $solicitacao_ocorrencia->SortUrl($solicitacao_ocorrencia->ic_tpOcorrencia) ?>',2);"><div id="elh_solicitacao_ocorrencia_ic_tpOcorrencia" class="solicitacao_ocorrencia_ic_tpOcorrencia">
...
<?php if ($solicitacao_ocorrencia->ic_exibirNoLaudo->Visible) { // ic_exibirNoLaudo ?>
	...
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $solicitacao_ocorrencia->SortUrl($solicitacao_ocorrencia->ic_exibirNoLaudo) ?>',2);"><div id="elh_solicitacao_ocorrencia_ic_exibirNoLaudo" class="solicitacao_ocorrencia_ic_exibirNoLaudo">
...
<?php if ($solicitacao_ocorrencia->nu_usuarioInc->Visible) { // nu_usuarioInc ?>
	...
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $solicitacao_ocorrencia->SortUrl($solicitacao_ocorrencia->nu_usuarioInc) ?>',2);"><div id="elh_solicitacao_ocorrencia_nu_usuarioInc" class="solicitacao_ocorrencia_nu_usuarioInc">
...
<?php if ($solicitacao_ocorrencia->dh_inclusao->Visible) { // dh_inclusao ?>
	...
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $solicitacao_ocorrencia->SortUrl($solicitacao_ocorrencia->dh_inclusao) ?>',2);"><div id="elh_solicitacao_ocorrencia_dh_inclusao" class="solicitacao_ocorrencia_dh_inclusao">
...
$solicitacao_ocorrencia_list->ListOptions->Render("header", "right");
?>
	</tr>
</thead>
<tbody>
<?php
if ($solicitacao_ocorrencia->ExportAll && $solicitacao_ocorrencia->Export <> "") {
	$solicitacao_ocorrencia_list->StopRec = $solicitacao_ocorrencia_list->TotalRecs;
} else {

	// Set the last record to display
	if ($solicitacao_ocorrencia_list->TotalRecs > $solicitacao_ocorrencia_list->StartRec + $solicitacao_ocorrencia_list->DisplayRecs - 1)
		$solicitacao_ocorrencia_list->StopRec = $solicitacao_ocorrencia_list->StartRec + $solicitacao_ocorrencia_list->DisplayRecs - 1;
	else
		$solicitacao_ocorrencia_list->StopRec = $solicitacao_ocorrencia_list->TotalRecs;
}
$solicitacao_ocorrencia_list->RecCnt = $solicitacao_ocorrencia_list->StartRec - 1;
if ($solicitacao_ocorrencia_list->Recordset && !$solicitacao_ocorrencia_list->Recordset->EOF) {
	$solicitacao_ocorrencia_list->Recordset->MoveFirst();
	if (!$bSelectLimit && $solicitacao_ocorrencia_list->StartRec > 1)
		$solicitacao_ocorrencia_list->Recordset->Move($solicitacao_ocorrencia_list->StartRec - 1);
} elseif (!$solicitacao_ocorrencia->AllowAddDeleteRow && $solicitacao_ocorrencia_list->StopRec == 0) {
	$solicitacao_ocorrencia_list->StopRec = $solicitacao_ocorrencia->GridAddRowCount;
}

// Initialize aggregate
$solicitacao_ocorrencia->RowType = EW_ROWTYPE_AGGREGATEINIT;
$solicitacao_ocorrencia->ResetAttrs();
$solicitacao_ocorrencia_list->RenderRow();
while ($solicitacao_ocorrencia_list->RecCnt < $solicitacao_ocorrencia_list->StopRec) {
	$solicitacao_ocorrencia_list->RecCnt++;
	if (intval($solicitacao_ocorrencia_list->RecCnt) >= intval($solicitacao_ocorrencia_list->StartRec)) {
		$solicitacao_ocorrencia_list->RowCnt++;

		// Set up key count
		$solicitacao_ocorrencia_list->KeyCount = $solicitacao_ocorrencia_list->RowIndex;
		...
		if ($solicitacao_ocorrencia->CurrentAction == "gridadd") {
			...
			$solicitacao_ocorrencia_list->LoadRowValues($solicitacao_ocorrencia_list->Recordset); // Load row values
		}
		$solicitacao_ocorrencia->RowType = EW_ROWTYPE_VIEW; // Render view

		// Set up row id / data-rowindex
		$solicitacao_ocorrencia->RowAttrs = array_merge($solicitacao_ocorrencia->RowAttrs, array('data-rowindex'=>$solicitacao_ocorrencia_list->RowCnt, 'id'=>'r' . $solicitacao_ocorrencia_list->RowCnt . '_solicitacao_ocorrencia', 'data-rowtype'=>$solicitacao_ocorrencia->RowType));
...
$solicitacao_ocorrencia_list->ListOptions->Render("body", "left", $solicitacao_ocorrencia_list->RowCnt);
?>
	<?php if ($solicitacao_ocorrencia->ic_tpOcorrencia->Visible) { // ic_tpOcorrencia ?>
...
<a id="<?php echo $solicitacao_ocorrencia_list->PageObjName . "_row_" . $solicitacao_ocorrencia_list->RowCnt ?>"></a></td>
	<?php } ?>
	<?php if ($solicitacao_ocorrencia->ic_exibirNoLaudo->Visible) { // ic_exibirNoLaudo ?>
...
$solicitacao_ocorrencia_list->ListOptions->Render("body", "right", $solicitacao_ocorrencia_list->RowCnt);
?>
	</tr>
<?php
	}
	if ($solicitacao_ocorrencia->CurrentAction <> "gridadd")
		$solicitacao_ocorrencia_list->Recordset->MoveNext();
}
?>
</tbody>
</table>
<?php } ?>
<?php if ($solicitacao_ocorrencia->CurrentAction == "") { ?>
...
// Close recordset
if ($solicitacao_ocorrencia_list->Recordset)
	$solicitacao_ocorrencia_list->Recordset->Close();
?>
<?php if ($solicitacao_ocorrencia->Export == "") { ?>
<div class="ewGridLowerPanel">
<?php if ($solicitacao_ocorrencia->CurrentAction <> "gridadd" && $solicitacao_ocorrencia->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>">
<table class="ewPager">
<tr><td>
<?php if (!isset($solicitacao_ocorrencia_list->Pager)) $solicitacao_ocorrencia_list->Pager = new cNumericPager($solicitacao_ocorrencia_list->StartRec, $solicitacao_ocorrencia_list->DisplayRecs, $solicitacao_ocorrencia_list->TotalRecs, $solicitacao_ocorrencia_list->RecRange) ?>
<?php if ($solicitacao_ocorrencia_list->Pager->RecordCount > 0) { ?>
<table cellspacing="0" class="ewStdTable"><tbody><tr><td>
<div class="pagination"><ul>
	<?php if ($solicitacao_ocorrencia_list->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $solicitacao_ocorrencia_list->PageUrl() ?>start=<?php echo $solicitacao_ocorrencia_list->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($solicitacao_ocorrencia_list->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $solicitacao_ocorrencia_list->PageUrl() ?>start=<?php echo $solicitacao_ocorrencia_list->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($solicitacao_ocorrencia_list->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $solicitacao_ocorrencia_list->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($solicitacao_ocorrencia_list->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $solicitacao_ocorrencia_list->PageUrl() ?>start=<?php echo $solicitacao_ocorrencia_list->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($solicitacao_ocorrencia_list->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $solicitacao_ocorrencia_list->PageUrl() ?>start=<?php echo $solicitacao_ocorrencia_list->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</td>
<td>
	<?php if ($solicitacao_ocorrencia_list->Pager->ButtonCount > 0) { ?>&nbsp;&nbsp;&nbsp;&nbsp;<?php } ?>
	<?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $solicitacao_ocorrencia_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $solicitacao_ocorrencia_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $solicitacao_ocorrencia_list->Pager->RecordCount ?>
</td>
</tr></tbody></table>
<?php } else { ?>
	<?php if ($Security->CanList()) { ?>
	<?php if ($solicitacao_ocorrencia_list->SearchWhere == "0=101") { ?>
...
<?php
	foreach ($solicitacao_ocorrencia_list->OtherOptions as &$option)
...
<?php if ($solicitacao_ocorrencia->Export == "") { ?>
<script type="text/javascript">
fsolicitacao_ocorrencialist.Init();
...
<?php } ?>
<?php
$solicitacao_ocorrencia_list->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($solicitacao_ocorrencia->Export == "") { ?>
...
<?php
$solicitacao_ocorrencia_list->Page_Terminate();
?>
