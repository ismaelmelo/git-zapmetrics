...
$tpmetrica_edit = NULL; // Initialize page object first

class ctpmetrica_edit extends ctpmetrica {
	...
	// Page object name
	var $PageObjName = 'tpmetrica_edit';
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->nu_tpMetrica->CurrentValue == "")
			$this->Page_Terminate("tpmetricalist.php"); // Invalid key, return to list
	...
	function ValidateForm() {
		...
		if (in_array("tpcontagem", $DetailTblVar) && $GLOBALS["tpcontagem"]->DetailEdit) {
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// no_tpMetrica
			$this->no_tpMetrica->SetDbValueDef($rsnew, $this->no_tpMetrica->CurrentValue, "", $this->no_tpMetrica->ReadOnly);

			// ic_tpMetrica
			$this->ic_tpMetrica->SetDbValueDef($rsnew, $this->ic_tpMetrica->CurrentValue, "", $this->ic_tpMetrica->ReadOnly);

			// ic_tpAplicacao
			$this->ic_tpAplicacao->SetDbValueDef($rsnew, $this->ic_tpAplicacao->CurrentValue, NULL, $this->ic_tpAplicacao->ReadOnly);
			...
			// ic_metodoEsforco
			$this->ic_metodoEsforco->SetDbValueDef($rsnew, $this->ic_metodoEsforco->CurrentValue, NULL, $this->ic_metodoEsforco->ReadOnly);

			// ic_metodoPrazo
			$this->ic_metodoPrazo->SetDbValueDef($rsnew, $this->ic_metodoPrazo->CurrentValue, NULL, $this->ic_metodoPrazo->ReadOnly);

			// ic_metodoCusto
			$this->ic_metodoCusto->SetDbValueDef($rsnew, $this->ic_metodoCusto->CurrentValue, NULL, $this->ic_metodoCusto->ReadOnly);

			// ic_metodoRecursos
			$this->ic_metodoRecursos->SetDbValueDef($rsnew, $this->ic_metodoRecursos->CurrentValue, NULL, $this->ic_metodoRecursos->ReadOnly);
			...
			if ($bUpdateRow) {
				...
				if ($EditRow) {
					...
					if (in_array("tpcontagem", $DetailTblVar) && $GLOBALS["tpcontagem"]->DetailEdit) {
						if (!isset($GLOBALS["tpcontagem_grid"])) $GLOBALS["tpcontagem_grid"] = new ctpcontagem_grid(); // Get detail page object
						$EditRow = $GLOBALS["tpcontagem_grid"]->GridUpdate();
					}
				}
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("tpcontagem", $DetailTblVar)) {
				...
				if ($GLOBALS["tpcontagem_grid"]->DetailEdit) {
					$GLOBALS["tpcontagem_grid"]->CurrentMode = "edit";
					$GLOBALS["tpcontagem_grid"]->CurrentAction = "gridedit";
...
// Create page object
if (!isset($tpmetrica_edit)) $tpmetrica_edit = new ctpmetrica_edit();

// Page init
$tpmetrica_edit->Page_Init();

// Page main
$tpmetrica_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$tpmetrica_edit->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var tpmetrica_edit = new ew_Page("tpmetrica_edit");
tpmetrica_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = tpmetrica_edit.PageID; // For backward compatibility

// Form object
var ftpmetricaedit = new ew_Form("ftpmetricaedit");

// Validate form
ftpmetricaedit.Validate = function() {
...
ftpmetricaedit.Form_CustomValidate = 
...
ftpmetricaedit.ValidateRequired = true;
<?php } else { ?>
ftpmetricaedit.ValidateRequired = false; 
<?php } ?>

// Multi-Page properties
ftpmetricaedit.MultiPage = new ew_MultiPage("ftpmetricaedit",
...
<?php $tpmetrica_edit->ShowPageHeader(); ?>
<?php
$tpmetrica_edit->ShowMessage();
?>
<form name="ftpmetricaedit" id="ftpmetricaedit" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="tpmetrica">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<table class="ewStdTable"><tbody><tr><td>
<div class="tabbable" id="tpmetrica_edit">
...
<table id="tbl_tpmetricaedit1" class="table table-bordered table-striped">
...
<table cellspacing="0" class="ewGrid" style="width: 100%"><tr><td>
<table id="tbl_tpmetricaedit2" class="table table-bordered table-striped">
...
<input type="hidden" data-field="x_nu_tpMetrica" name="x_nu_tpMetrica" id="x_nu_tpMetrica" value="<?php echo ew_HtmlEncode($tpmetrica->nu_tpMetrica->CurrentValue) ?>">
<?php
	if (in_array("tpcontagem", explode(",", $tpmetrica->getCurrentDetailTable())) && $tpcontagem->DetailEdit) {
?>
