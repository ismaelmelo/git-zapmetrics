...
$sistema_edit = NULL; // Initialize page object first

class csistema_edit extends csistema {
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->nu_sistema->CurrentValue == "")
			$this->Page_Terminate("sistemalist.php"); // Invalid key, return to list
	...
	function ValidateForm() {
		...
		if (in_array("uc", $DetailTblVar) && $GLOBALS["uc"]->DetailEdit) {
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// no_sistema
			$this->no_sistema->SetDbValueDef($rsnew, $this->no_sistema->CurrentValue, "", $this->no_sistema->ReadOnly);

			// ds_sistema
			$this->ds_sistema->SetDbValueDef($rsnew, $this->ds_sistema->CurrentValue, NULL, $this->ds_sistema->ReadOnly);

			// nu_fornecedor
			$this->nu_fornecedor->SetDbValueDef($rsnew, $this->nu_fornecedor->CurrentValue, NULL, $this->nu_fornecedor->ReadOnly);

			// ds_contatos
			$this->ds_contatos->SetDbValueDef($rsnew, $this->ds_contatos->CurrentValue, NULL, $this->ds_contatos->ReadOnly);

			// ds_comentarios
			$this->ds_comentarios->SetDbValueDef($rsnew, $this->ds_comentarios->CurrentValue, NULL, $this->ds_comentarios->ReadOnly);
			...
			if ($bUpdateRow) {
				...
				if ($EditRow) {
					...
					if (in_array("ator", $DetailTblVar) && $GLOBALS["ator"]->DetailEdit) {
						if (!isset($GLOBALS["ator_grid"])) $GLOBALS["ator_grid"] = new cator_grid(); // Get detail page object
						$EditRow = $GLOBALS["ator_grid"]->GridUpdate();
					}
					if (in_array("modulo", $DetailTblVar) && $GLOBALS["modulo"]->DetailEdit) {
						if (!isset($GLOBALS["modulo_grid"])) $GLOBALS["modulo_grid"] = new cmodulo_grid(); // Get detail page object
						$EditRow = $GLOBALS["modulo_grid"]->GridUpdate();
					}
					if (in_array("uc", $DetailTblVar) && $GLOBALS["uc"]->DetailEdit) {
						if (!isset($GLOBALS["uc_grid"])) $GLOBALS["uc_grid"] = new cuc_grid(); // Get detail page object
						$EditRow = $GLOBALS["uc_grid"]->GridUpdate();
					}
				}
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("ator", $DetailTblVar)) {
				...
				if ($GLOBALS["ator_grid"]->DetailEdit) {
					$GLOBALS["ator_grid"]->CurrentMode = "edit";
					$GLOBALS["ator_grid"]->CurrentAction = "gridedit";
			...
			if (in_array("modulo", $DetailTblVar)) {
				...
				if ($GLOBALS["modulo_grid"]->DetailEdit) {
					$GLOBALS["modulo_grid"]->CurrentMode = "edit";
					$GLOBALS["modulo_grid"]->CurrentAction = "gridedit";
			...
			if (in_array("uc", $DetailTblVar)) {
				...
				if ($GLOBALS["uc_grid"]->DetailEdit) {
					$GLOBALS["uc_grid"]->CurrentMode = "edit";
					$GLOBALS["uc_grid"]->CurrentAction = "gridedit";
...
// Create page object
if (!isset($sistema_edit)) $sistema_edit = new csistema_edit();

// Page init
$sistema_edit->Page_Init();

// Page main
$sistema_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$sistema_edit->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var sistema_edit = new ew_Page("sistema_edit");
sistema_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = sistema_edit.PageID; // For backward compatibility

// Form object
var fsistemaedit = new ew_Form("fsistemaedit");

// Validate form
fsistemaedit.Validate = function() {
...
fsistemaedit.Form_CustomValidate = 
...
fsistemaedit.ValidateRequired = true;
<?php } else { ?>
fsistemaedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fsistemaedit.Lists["x_nu_fornecedor"] = {"LinkField":"x_nu_fornecedor","Ajax":null,"AutoFill":false,"DisplayFields":["x_no_fornecedor","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fsistemaedit.Lists["x_nu_stSistema"] = {"LinkField":"x_nu_stSistema","Ajax":null,"AutoFill":false,"DisplayFields":["x_no_stSistema","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $sistema_edit->ShowPageHeader(); ?>
<?php
$sistema_edit->ShowMessage();
?>
<form name="fsistemaedit" id="fsistemaedit" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="sistema">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_sistemaedit" class="table table-bordered table-striped">
...
<script type="text/javascript">
fsistemaedit.Lists["x_nu_fornecedor"].Options = <?php echo (is_array($sistema->nu_fornecedor->EditValue)) ? ew_ArrayToJson($sistema->nu_fornecedor->EditValue, 1) : "[]" ?>;
</script>
</span>
...
<script type="text/javascript">
fsistemaedit.Lists["x_nu_stSistema"].Options = <?php echo (is_array($sistema->nu_stSistema->EditValue)) ? ew_ArrayToJson($sistema->nu_stSistema->EditValue, 1) : "[]" ?>;
</script>
</span>
...
<input type="hidden" data-field="x_nu_sistema" name="x_nu_sistema" id="x_nu_sistema" value="<?php echo ew_HtmlEncode($sistema->nu_sistema->CurrentValue) ?>">
<?php
	if (in_array("ator", explode(",", $sistema->getCurrentDetailTable())) && $ator->DetailEdit) {
?>
<?php include_once "atorgrid.php" ?>
<?php } ?>
<?php
	if (in_array("modulo", explode(",", $sistema->getCurrentDetailTable())) && $modulo->DetailEdit) {
?>
<?php include_once "modulogrid.php" ?>
<?php } ?>
<?php
	if (in_array("uc", explode(",", $sistema->getCurrentDetailTable())) && $uc->DetailEdit) {
?>
