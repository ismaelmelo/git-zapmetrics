...
$ambiente_tecno_grid = NULL; // Initialize page object first

class cambiente_tecno_grid extends cambiente_tecno {
	...
	// Page object name
	var $PageObjName = 'ambiente_tecno_grid';

	// Grid form hidden field names
	var $FormName = 'fambiente_tecnogrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["ambiente_tecno"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function SetupKeyValues($key) {
		...
		if (count($arrKeyFlds) >= 2) {
			...
			if (!is_numeric($this->nu_ambiente->FormValue))
				return FALSE;
			$this->nu_tecnoDesenv->setFormValue($arrKeyFlds[1]);
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					if ($sKey <> "") $sKey .= $GLOBALS["EW_COMPOSITE_KEY_SEPARATOR"];
					$sKey .= $this->nu_tecnoDesenv->CurrentValue;
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_nu_tecnoDesenv") && $objForm->HasValue("o_nu_tecnoDesenv") && $this->nu_tecnoDesenv->CurrentValue <> $this->nu_tecnoDesenv->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function LoadSortOrder() {
		...
		if ($sOrderBy == "") {
			if ($this->SqlOrderBy() <> "") {
				...
				$this->nu_tecnoDesenv->setSort("ASC");
			}
		}
	}
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->nu_ambiente->CurrentValue . $GLOBALS["EW_COMPOSITE_KEY_SEPARATOR"] . $this->nu_tecnoDesenv->CurrentValue . "\">";
		}
	...
	function LoadDefaultValues() {
		$this->nu_tecnoDesenv->CurrentValue = NULL;
		$this->nu_tecnoDesenv->OldValue = $this->nu_tecnoDesenv->CurrentValue;
	}
	...
	function LoadFormValues() {
		...
		$this->nu_tecnoDesenv->setOldValue($objForm->GetValue("o_nu_tecnoDesenv"));
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 2) {
			if (strval($arKeys[0]) <> "")
				...
				$bValidKey = FALSE;
			if (strval($arKeys[1]) <> "")
				$this->nu_tecnoDesenv->CurrentValue = strval($arKeys[1]); // nu_tecnoDesenv
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			// nu_tecnoDesenv
			$this->nu_tecnoDesenv->EditCustomAttributes = "";
			$sFilterWrk = "";
			$sSqlWrk = "SELECT [nu_tecnoDesenv], [no_tecDesenv] AS [DispFld], '' AS [Disp2Fld], '' AS [Disp3Fld], '' AS [Disp4Fld], '' AS [SelectFilterFld], '' AS [SelectFilterFld2], '' AS [SelectFilterFld3], '' AS [SelectFilterFld4] FROM [dbo].[tecdesenvsis]";
			$sWhereWrk = "";
			$lookuptblfilter = "[ic_ativo]='S'";
			if (strval($lookuptblfilter) <> "") {
				ew_AddFilter($sWhereWrk, $lookuptblfilter);
			...
			$this->nu_tecnoDesenv->EditValue = $arwrk;
			...
			$sSqlWrk .= " ORDER BY [nu_ordem] ASC";
				$rswrk = $conn->Execute($sSqlWrk);
				if ($rswrk && !$rswrk->EOF) { // Lookup values found
					$this->nu_tecnoDesenv->EditValue = $rswrk->fields('DispFld');
					$rswrk->Close();
				} else {
					$this->nu_tecnoDesenv->EditValue = $this->nu_tecnoDesenv->CurrentValue;
				}
			} else {
				$this->nu_tecnoDesenv->EditValue = NULL;
			}
			$this->nu_tecnoDesenv->ViewCustomAttributes = "";
	...
	function ValidateForm() {
		...
		if (!EW_SERVER_VALIDATE)
			return ($gsFormError == "");
		if (!$this->nu_tecnoDesenv->FldIsDetailKey && !is_null($this->nu_tecnoDesenv->FormValue) && $this->nu_tecnoDesenv->FormValue == "") {
			ew_AddMessage($gsFormError, $Language->Phrase("EnterRequiredField") . " - " . $this->nu_tecnoDesenv->FldCaption());
		}
	...
	function AddRow($rsold = NULL) {
		...
		// nu_tecnoDesenv
		$this->nu_tecnoDesenv->SetDbValueDef($rsnew, $this->nu_tecnoDesenv->CurrentValue, 0, FALSE);
		...
		// Check if key value entered
		if ($bInsertRow && $this->ValidateKey && $this->nu_tecnoDesenv->CurrentValue == "" && $this->nu_tecnoDesenv->getSessionValue() == "") {
			$this->setFailureMessage($Language->Phrase("InvalidKeyValue"));
			$bInsertRow = FALSE;
		}
