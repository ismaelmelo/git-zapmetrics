...
$planoti_add = NULL; // Initialize page object first

class cplanoti_add extends cplanoti {
	...
	function LoadDefaultValues() {
		...
		$this->nu_planoEstrategico->CurrentValue = NULL;
		$this->nu_planoEstrategico->OldValue = $this->nu_planoEstrategico->CurrentValue;
	...
	function AddRow($rsold = NULL) {
		...
		// nu_planoEstrategico
		$this->nu_planoEstrategico->SetDbValueDef($rsnew, $this->nu_planoEstrategico->CurrentValue, 0, FALSE);
...
// Create page object
if (!isset($planoti_add)) $planoti_add = new cplanoti_add();

// Page init
$planoti_add->Page_Init();

// Page main
$planoti_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$planoti_add->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var planoti_add = new ew_Page("planoti_add");
planoti_add.PageID = "add"; // Page ID
var EW_PAGE_ID = planoti_add.PageID; // For backward compatibility

// Form object
var fplanotiadd = new ew_Form("fplanotiadd");

// Validate form
fplanotiadd.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_HasValue(elm))
				return this.OnError(elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($planoti->nu_anoInicio->FldCaption()) ?>");
			elm = this.GetElements("x" + infix + "_nu_anoInicio");
			...
			if (elm && !ew_HasValue(elm))
				return this.OnError(elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($planoti->nu_anoFim->FldCaption()) ?>");
			elm = this.GetElements("x" + infix + "_nu_anoFim");
			if (elm && !ew_CheckInteger(elm.value))
...
fplanotiadd.Form_CustomValidate = 
...
fplanotiadd.ValidateRequired = true;
<?php } else { ?>
fplanotiadd.ValidateRequired = false; 
<?php } ?>

// Multi-Page properties
fplanotiadd.MultiPage = new ew_MultiPage("fplanotiadd",
...
fplanotiadd.Lists["x_nu_anoFim"] = {"LinkField":"x_nu_ano","Ajax":true,"AutoFill":false,"DisplayFields":["x_nu_ano","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fplanotiadd.Lists["x_nu_planoEstrategico"] = {"LinkField":"x_nu_plano","Ajax":null,"AutoFill":false,"DisplayFields":["x_no_plano","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $planoti_add->ShowPageHeader(); ?>
<?php
$planoti_add->ShowMessage();
?>
<form name="fplanotiadd" id="fplanotiadd" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="planoti">
<input type="hidden" name="a_add" id="a_add" value="A">
<table class="ewStdTable"><tbody><tr><td>
<div class="tabbable" id="planoti_add">
...
<table id="tbl_planotiadd1" class="table table-bordered table-striped">
<?php if ($planoti->nu_anoInicio->Visible) { // nu_anoInicio ?>
	<tr id="r_nu_anoInicio">
		<td><span id="elh_planoti_nu_anoInicio"><?php echo $planoti->nu_anoInicio->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
...
<?php if ($planoti->nu_anoFim->Visible) { // nu_anoFim ?>
	<tr id="r_nu_anoFim">
		<td><span id="elh_planoti_nu_anoFim"><?php echo $planoti->nu_anoFim->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
...
<input type="hidden" data-field="x_nu_anoFim" name="x_nu_anoFim" id="x_nu_anoFim" value="<?php echo $planoti->nu_anoFim->CurrentValue ?>"<?php echo $wrkonchange ?>>
<?php
...
<script type="text/javascript">
var oas = new ew_AutoSuggest("x_nu_anoFim", fplanotiadd, false, EW_AUTO_SUGGEST_MAX_ENTRIES);
fplanotiadd.AutoSuggests["x_nu_anoFim"] = oas;
</script>
</span>
...
<?php } ?>
<script type="text/javascript">
fplanotiadd.Lists["x_nu_planoEstrategico"].Options = <?php echo (is_array($planoti->nu_planoEstrategico->EditValue)) ? ew_ArrayToJson($planoti->nu_planoEstrategico->EditValue, 1) : "[]" ?>;
</script>
</span>
...
<table cellspacing="0" class="ewGrid" style="width: 100%"><tr><td>
<table id="tbl_planotiadd2" class="table table-bordered table-striped">
