...
$divisao_add = NULL; // Initialize page object first

class cdivisao_add extends cdivisao {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["nu_divisao"] != "") {
				...
				$this->setKey("nu_divisao", $this->nu_divisao->CurrentValue); // Set up key
			} else {
				$this->setKey("nu_divisao", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "divisaoview.php")
	...
	function LoadDefaultValues() {
		...
		$this->no_divisao->CurrentValue = NULL;
		$this->no_divisao->OldValue = $this->no_divisao->CurrentValue;
		$this->ds_divisao->CurrentValue = NULL;
		$this->ds_divisao->OldValue = $this->ds_divisao->CurrentValue;
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			if ($this->nu_area->getSessionValue() <> "") {
				$this->nu_area->CurrentValue = $this->nu_area->getSessionValue();
			if (strval($this->nu_area->CurrentValue) <> "") {
				$sFilterWrk = "[nu_area]" . ew_SearchString("=", $this->nu_area->CurrentValue, EW_DATATYPE_NUMBER);
			$sSqlWrk = "SELECT [nu_area], [no_area] AS [DispFld], '' AS [Disp2Fld], '' AS [Disp3Fld], '' AS [Disp4Fld] FROM [dbo].[area]";
			$sWhereWrk = "";
	...
	function AddRow($rsold = NULL) {
		...
		if (strval($this->nu_area->CurrentValue) <> "") {
			$sMasterFilter = str_replace("@nu_area@", ew_AdjustSql($this->nu_area->CurrentValue), $sMasterFilter);
		} else {
		...
		// no_divisao
		$this->no_divisao->SetDbValueDef($rsnew, $this->no_divisao->CurrentValue, "", FALSE);

		// ds_divisao
		$this->ds_divisao->SetDbValueDef($rsnew, $this->ds_divisao->CurrentValue, NULL, FALSE);
		...
		if ($AddRow) {
			$this->nu_divisao->setDbValue($conn->Insert_ID());
			$rsnew['nu_divisao'] = $this->nu_divisao->DbValue;
		}
		if ($AddRow) {
...
// Create page object
if (!isset($divisao_add)) $divisao_add = new cdivisao_add();

// Page init
$divisao_add->Page_Init();

// Page main
$divisao_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$divisao_add->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var divisao_add = new ew_Page("divisao_add");
divisao_add.PageID = "add"; // Page ID
var EW_PAGE_ID = divisao_add.PageID; // For backward compatibility

// Form object
var fdivisaoadd = new ew_Form("fdivisaoadd");

// Validate form
fdivisaoadd.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_HasValue(elm))
				return this.OnError(elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($divisao->nu_area->FldCaption()) ?>");
...
fdivisaoadd.Form_CustomValidate = 
...
fdivisaoadd.ValidateRequired = true;
<?php } else { ?>
fdivisaoadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fdivisaoadd.Lists["x_nu_area"] = {"LinkField":"x_nu_area","Ajax":null,"AutoFill":false,"DisplayFields":["x_no_area","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $divisao_add->ShowPageHeader(); ?>
<?php
$divisao_add->ShowMessage();
?>
<form name="fdivisaoadd" id="fdivisaoadd" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="divisao">
<input type="hidden" name="a_add" id="a_add" value="A">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_divisaoadd" class="table table-bordered table-striped">
<?php if ($divisao->nu_area->Visible) { // nu_area ?>
	<tr id="r_nu_area">
		<td><span id="elh_divisao_nu_area"><?php echo $divisao->nu_area->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
		<td<?php echo $divisao->nu_area->CellAttributes() ?>>
<?php if ($divisao->nu_area->getSessionValue() <> "") { ?>
<span<?php echo $divisao->nu_area->ViewAttributes() ?>>
<?php echo $divisao->nu_area->ViewValue ?></span>
<input type="hidden" id="x_nu_area" name="x_nu_area" value="<?php echo ew_HtmlEncode($divisao->nu_area->CurrentValue) ?>">
<?php } else { ?>
<select data-field="x_nu_area" id="x_nu_area" name="x_nu_area"<?php echo $divisao->nu_area->EditAttributes() ?>>
<?php
if (is_array($divisao->nu_area->EditValue)) {
	$arwrk = $divisao->nu_area->EditValue;
	$rowswrk = count($arwrk);
	$emptywrk = TRUE;
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($divisao->nu_area->CurrentValue) == strval($arwrk[$rowcntwrk][0])) ? " selected=\"selected\"" : "";
...
<script type="text/javascript">
fdivisaoadd.Lists["x_nu_area"].Options = <?php echo (is_array($divisao->nu_area->EditValue)) ? ew_ArrayToJson($divisao->nu_area->EditValue, 1) : "[]" ?>;
</script>
<?php } ?>
