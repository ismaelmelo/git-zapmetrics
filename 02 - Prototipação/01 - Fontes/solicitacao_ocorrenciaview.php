...
$solicitacao_ocorrencia_view = NULL; // Initialize page object first

class csolicitacao_ocorrencia_view extends csolicitacao_ocorrencia {
	...
	// Page object name
	var $PageObjName = 'solicitacao_ocorrencia_view';
	...
	function __construct() {
		...
		if (@$_GET["nu_ocorrencia"] <> "") {
			$this->RecKey["nu_ocorrencia"] = $_GET["nu_ocorrencia"];
			$KeyUrl .= "&nu_ocorrencia=" . urlencode($this->RecKey["nu_ocorrencia"]);
		}
	...
	function Page_Init() {
		...
		if (@$_GET["nu_ocorrencia"] <> "") {
			if ($gsExportFile <> "") $gsExportFile .= "_";
			$gsExportFile .= ew_StripSlashes($_GET["nu_ocorrencia"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			...
			if (@$_GET["nu_ocorrencia"] <> "") {
				...
				$this->RecKey["nu_ocorrencia"] = $this->nu_ocorrencia->QueryStringValue;
			} else {
				$bLoadCurrentRecord = TRUE;
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					...
					if ($this->TotalRecs <= 0) { // No record found
						...
						$this->Page_Terminate("solicitacao_ocorrencialist.php"); // Return to list page
						...
						while (!$this->Recordset->EOF) {
							if (strval($this->nu_solicitacao->CurrentValue) == strval($this->Recordset->fields('nu_solicitacao')) && strval($this->nu_ocorrencia->CurrentValue) == strval($this->Recordset->fields('nu_ocorrencia'))) {
					...
					if (!$bMatchRecord) {
						if ($this->getSuccessMessage() == "" && $this->getFailureMessage() == "")
							$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record message
						$sReturnUrl = "solicitacao_ocorrencialist.php"; // No matching record, return to list
					} else {
			...
			$sReturnUrl = "solicitacao_ocorrencialist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($solicitacao_ocorrencia_view)) $solicitacao_ocorrencia_view = new csolicitacao_ocorrencia_view();

// Page init
$solicitacao_ocorrencia_view->Page_Init();

// Page main
$solicitacao_ocorrencia_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$solicitacao_ocorrencia_view->Page_Render();
?>
...
// Page object
var solicitacao_ocorrencia_view = new ew_Page("solicitacao_ocorrencia_view");
solicitacao_ocorrencia_view.PageID = "view"; // Page ID
var EW_PAGE_ID = solicitacao_ocorrencia_view.PageID; // For backward compatibility

// Form object
var fsolicitacao_ocorrenciaview = new ew_Form("fsolicitacao_ocorrenciaview");

// Form_CustomValidate event
fsolicitacao_ocorrenciaview.Form_CustomValidate = 
...
fsolicitacao_ocorrenciaview.ValidateRequired = true;
<?php } else { ?>
fsolicitacao_ocorrenciaview.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fsolicitacao_ocorrenciaview.Lists["x_nu_usuarioInc"] = {"LinkField":"x_nu_usuario","Ajax":null,"AutoFill":false,"DisplayFields":["x_no_usuario","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $solicitacao_ocorrencia_view->ExportOptions->Render("body") ?>
<?php if (!$solicitacao_ocorrencia_view->ExportOptions->UseDropDownButton) { ?>
</div>
<div class="ewViewOtherOptions">
<?php } ?>
<?php
	foreach ($solicitacao_ocorrencia_view->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<?php } ?>
<?php $solicitacao_ocorrencia_view->ShowPageHeader(); ?>
<?php
$solicitacao_ocorrencia_view->ShowMessage();
?>
<form name="fsolicitacao_ocorrenciaview" id="fsolicitacao_ocorrenciaview" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="solicitacao_ocorrencia">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_solicitacao_ocorrenciaview" class="table table-bordered table-striped">
<?php if ($solicitacao_ocorrencia->nu_solicitacao->Visible) { // nu_solicitacao ?>
	<tr id="r_nu_solicitacao">
		<td><span id="elh_solicitacao_ocorrencia_nu_solicitacao"><?php echo $solicitacao_ocorrencia->nu_solicitacao->FldCaption() ?></span></td>
		<td<?php echo $solicitacao_ocorrencia->nu_solicitacao->CellAttributes() ?>>
<span id="el_solicitacao_ocorrencia_nu_solicitacao" class="control-group">
...
<?php } ?>
<?php if ($solicitacao_ocorrencia->nu_ocorrencia->Visible) { // nu_ocorrencia ?>
	<tr id="r_nu_ocorrencia">
		<td><span id="elh_solicitacao_ocorrencia_nu_ocorrencia"><?php echo $solicitacao_ocorrencia->nu_ocorrencia->FldCaption() ?></span></td>
		<td<?php echo $solicitacao_ocorrencia->nu_ocorrencia->CellAttributes() ?>>
<span id="el_solicitacao_ocorrencia_nu_ocorrencia" class="control-group">
<span<?php echo $solicitacao_ocorrencia->nu_ocorrencia->ViewAttributes() ?>>
<?php echo $solicitacao_ocorrencia->nu_ocorrencia->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($solicitacao_ocorrencia->ic_tpOcorrencia->Visible) { // ic_tpOcorrencia ?>
	<tr id="r_ic_tpOcorrencia">
		<td><span id="elh_solicitacao_ocorrencia_ic_tpOcorrencia"><?php echo $solicitacao_ocorrencia->ic_tpOcorrencia->FldCaption() ?></span></td>
...
<?php if ($solicitacao_ocorrencia->ic_exibirNoLaudo->Visible) { // ic_exibirNoLaudo ?>
	<tr id="r_ic_exibirNoLaudo">
		<td><span id="elh_solicitacao_ocorrencia_ic_exibirNoLaudo"><?php echo $solicitacao_ocorrencia->ic_exibirNoLaudo->FldCaption() ?></span></td>
...
<span<?php echo $solicitacao_ocorrencia->ds_observacao->ViewAttributes() ?>>
<?php echo $solicitacao_ocorrencia->ds_observacao->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($solicitacao_ocorrencia->nu_usuarioInc->Visible) { // nu_usuarioInc ?>
	<tr id="r_nu_usuarioInc">
		<td><span id="elh_solicitacao_ocorrencia_nu_usuarioInc"><?php echo $solicitacao_ocorrencia->nu_usuarioInc->FldCaption() ?></span></td>
		<td<?php echo $solicitacao_ocorrencia->nu_usuarioInc->CellAttributes() ?>>
<span id="el_solicitacao_ocorrencia_nu_usuarioInc" class="control-group">
...
<?php if ($solicitacao_ocorrencia->dh_inclusao->Visible) { // dh_inclusao ?>
	<tr id="r_dh_inclusao">
		<td><span id="elh_solicitacao_ocorrencia_dh_inclusao"><?php echo $solicitacao_ocorrencia->dh_inclusao->FldCaption() ?></span></td>
		<td<?php echo $solicitacao_ocorrencia->dh_inclusao->CellAttributes() ?>>
<span id="el_solicitacao_ocorrencia_dh_inclusao" class="control-group">
...
<table class="ewPager">
<tr><td>
<?php if (!isset($solicitacao_ocorrencia_view->Pager)) $solicitacao_ocorrencia_view->Pager = new cNumericPager($solicitacao_ocorrencia_view->StartRec, $solicitacao_ocorrencia_view->DisplayRecs, $solicitacao_ocorrencia_view->TotalRecs, $solicitacao_ocorrencia_view->RecRange) ?>
<?php if ($solicitacao_ocorrencia_view->Pager->RecordCount > 0) { ?>
<table cellspacing="0" class="ewStdTable"><tbody><tr><td>
<div class="pagination"><ul>
	<?php if ($solicitacao_ocorrencia_view->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $solicitacao_ocorrencia_view->PageUrl() ?>start=<?php echo $solicitacao_ocorrencia_view->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($solicitacao_ocorrencia_view->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $solicitacao_ocorrencia_view->PageUrl() ?>start=<?php echo $solicitacao_ocorrencia_view->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($solicitacao_ocorrencia_view->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $solicitacao_ocorrencia_view->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($solicitacao_ocorrencia_view->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $solicitacao_ocorrencia_view->PageUrl() ?>start=<?php echo $solicitacao_ocorrencia_view->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($solicitacao_ocorrencia_view->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $solicitacao_ocorrencia_view->PageUrl() ?>start=<?php echo $solicitacao_ocorrencia_view->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</td>
</tr></tbody></table>
...
<script type="text/javascript">
fsolicitacao_ocorrenciaview.Init();
</script>
<?php
$solicitacao_ocorrencia_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($solicitacao_ocorrencia->Export == "") { ?>
...
<?php
$solicitacao_ocorrencia_view->Page_Terminate();
?>
