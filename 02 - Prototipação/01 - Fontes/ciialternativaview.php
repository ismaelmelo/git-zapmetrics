...
$ciialternativa_view = NULL; // Initialize page object first

class cciialternativa_view extends cciialternativa {
	...
	// Page object name
	var $PageObjName = 'ciialternativa_view';
	...
	function __construct() {
		...
		if (@$_GET["nu_alternativa"] <> "") {
			$this->RecKey["nu_alternativa"] = $_GET["nu_alternativa"];
			$KeyUrl .= "&nu_alternativa=" . urlencode($this->RecKey["nu_alternativa"]);
		}
	...
	function Page_Init() {
		...
		if (@$_GET["nu_alternativa"] <> "") {
			...
			$gsExportFile .= ew_StripSlashes($_GET["nu_alternativa"]);
		}
		$this->CurrentAction = (@$_GET["a"] <> "") ? $_GET["a"] : @$_POST["a_list"]; // Set up curent action

		// Setup export options
		$this->SetupExportOptions();
		$this->nu_alternativa->Visible = !$this->IsAdd() && !$this->IsCopy() && !$this->IsGridAdd();
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["nu_alternativa"] <> "") {
				...
				$this->RecKey["nu_alternativa"] = $this->nu_alternativa->QueryStringValue;
			} else {
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					...
					if ($this->TotalRecs <= 0) { // No record found
						...
						$this->Page_Terminate("ciialternativalist.php"); // Return to list page
						...
						while (!$this->Recordset->EOF) {
							if (strval($this->nu_alternativa->CurrentValue) == strval($this->Recordset->fields('nu_alternativa'))) {
					...
					if (!$bMatchRecord) {
						if ($this->getSuccessMessage() == "" && $this->getFailureMessage() == "")
							$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record message
						$sReturnUrl = "ciialternativalist.php"; // No matching record, return to list
					} else {
			...
			$sReturnUrl = "ciialternativalist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			// nu_alternativa
			$this->nu_alternativa->LinkCustomAttributes = "";
			$this->nu_alternativa->HrefValue = "";
			$this->nu_alternativa->TooltipValue = "";
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a id=\"emf_ciialternativa\" href=\"javascript:void(0);\" class=\"ewExportLink ewEmail\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_ciialternativa',hdr:ewLanguage.Phrase('ExportToEmail'),f:document.fciialternativaview,key:" . ew_ArrayToJsonAttr($this->RecKey) . ",sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
...
// Create page object
if (!isset($ciialternativa_view)) $ciialternativa_view = new cciialternativa_view();

// Page init
$ciialternativa_view->Page_Init();

// Page main
$ciialternativa_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$ciialternativa_view->Page_Render();
?>
...
// Page object
var ciialternativa_view = new ew_Page("ciialternativa_view");
ciialternativa_view.PageID = "view"; // Page ID
var EW_PAGE_ID = ciialternativa_view.PageID; // For backward compatibility

// Form object
var fciialternativaview = new ew_Form("fciialternativaview");

// Form_CustomValidate event
fciialternativaview.Form_CustomValidate = 
...
fciialternativaview.ValidateRequired = true;
<?php } else { ?>
fciialternativaview.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fciialternativaview.Lists["x_co_questao"] = {"LinkField":"x_co_questao","Ajax":null,"AutoFill":false,"DisplayFields":["x_co_questao","x_no_questao","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $ciialternativa_view->ExportOptions->Render("body") ?>
<?php if (!$ciialternativa_view->ExportOptions->UseDropDownButton) { ?>
</div>
<div class="ewViewOtherOptions">
<?php } ?>
<?php
	foreach ($ciialternativa_view->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<?php } ?>
<?php $ciialternativa_view->ShowPageHeader(); ?>
<?php
$ciialternativa_view->ShowMessage();
?>
<form name="fciialternativaview" id="fciialternativaview" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="ciialternativa">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_ciialternativaview" class="table table-bordered table-striped">
<?php if ($ciialternativa->nu_alternativa->Visible) { // nu_alternativa ?>
	<tr id="r_nu_alternativa">
		<td><span id="elh_ciialternativa_nu_alternativa"><?php echo $ciialternativa->nu_alternativa->FldCaption() ?></span></td>
		<td<?php echo $ciialternativa->nu_alternativa->CellAttributes() ?>>
<span id="el_ciialternativa_nu_alternativa" class="control-group">
<span<?php echo $ciialternativa->nu_alternativa->ViewAttributes() ?>>
<?php echo $ciialternativa->nu_alternativa->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($ciialternativa->co_questao->Visible) { // co_questao ?>
	<tr id="r_co_questao">
		<td><span id="elh_ciialternativa_co_questao"><?php echo $ciialternativa->co_questao->FldCaption() ?></span></td>
		<td<?php echo $ciialternativa->co_questao->CellAttributes() ?>>
<span id="el_ciialternativa_co_questao" class="control-group">
...
<?php if ($ciialternativa->no_alternativa->Visible) { // no_alternativa ?>
	<tr id="r_no_alternativa">
		<td><span id="elh_ciialternativa_no_alternativa"><?php echo $ciialternativa->no_alternativa->FldCaption() ?></span></td>
...
<span<?php echo $ciialternativa->ds_alternativa->ViewAttributes() ?>>
<?php echo $ciialternativa->ds_alternativa->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($ciialternativa->vr_alternativa->Visible) { // vr_alternativa ?>
	<tr id="r_vr_alternativa">
		<td><span id="elh_ciialternativa_vr_alternativa"><?php echo $ciialternativa->vr_alternativa->FldCaption() ?></span></td>
...
<?php if ($ciialternativa->ic_ativo->Visible) { // ic_ativo ?>
	<tr id="r_ic_ativo">
		<td><span id="elh_ciialternativa_ic_ativo"><?php echo $ciialternativa->ic_ativo->FldCaption() ?></span></td>
...
<table class="ewPager">
<tr><td>
<?php if (!isset($ciialternativa_view->Pager)) $ciialternativa_view->Pager = new cNumericPager($ciialternativa_view->StartRec, $ciialternativa_view->DisplayRecs, $ciialternativa_view->TotalRecs, $ciialternativa_view->RecRange) ?>
<?php if ($ciialternativa_view->Pager->RecordCount > 0) { ?>
<table cellspacing="0" class="ewStdTable"><tbody><tr><td>
<div class="pagination"><ul>
	<?php if ($ciialternativa_view->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $ciialternativa_view->PageUrl() ?>start=<?php echo $ciialternativa_view->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($ciialternativa_view->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $ciialternativa_view->PageUrl() ?>start=<?php echo $ciialternativa_view->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($ciialternativa_view->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $ciialternativa_view->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($ciialternativa_view->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $ciialternativa_view->PageUrl() ?>start=<?php echo $ciialternativa_view->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($ciialternativa_view->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $ciialternativa_view->PageUrl() ?>start=<?php echo $ciialternativa_view->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</td>
</tr></tbody></table>
...
<script type="text/javascript">
fciialternativaview.Init();
</script>
<?php
$ciialternativa_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($ciialternativa->Export == "") { ?>
...
<?php
$ciialternativa_view->Page_Terminate();
?>
