...
$ator_grid = NULL; // Initialize page object first

class cator_grid extends cator {
	...
	// Page object name
	var $PageObjName = 'ator_grid';

	// Grid form hidden field names
	var $FormName = 'fatorgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["ator"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->nu_ator->CurrentValue;
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_no_ator") && $objForm->HasValue("o_no_ator") && $this->no_ator->CurrentValue <> $this->no_ator->OldValue)
			return FALSE;
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->nu_ator->CurrentValue . "\">";
		}
	...
	function LoadFormValues() {
		...
		$this->no_ator->setOldValue($objForm->GetValue("o_no_ator"));
		...
		if (!$this->nu_ator->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->nu_ator->CurrentValue = strval($arKeys[0]); // nu_ator
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function AddRow($rsold = NULL) {
		...
		if ($this->nu_sistema->getSessionValue() <> "") {
			$sMasterFilter = str_replace("@nu_sistema@", ew_AdjustSql($this->nu_sistema->getSessionValue()), $sMasterFilter);
		} else {
			$bValidMasterRecord = FALSE;
		}
		if ($bValidMasterRecord) {
		...
		// nu_sistema
		if ($this->nu_sistema->getSessionValue() <> "") {
			$rsnew['nu_sistema'] = $this->nu_sistema->getSessionValue();
		}
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "sistema") {
			$this->nu_sistema->Visible = FALSE;
			if ($GLOBALS["sistema"]->EventCancelled) $this->EventCancelled = TRUE;
		}
