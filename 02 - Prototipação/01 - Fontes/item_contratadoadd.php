...
$item_contratado_add = NULL; // Initialize page object first

class citem_contratado_add extends citem_contratado {
	...
	// Page object name
	var $PageObjName = 'item_contratado_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["nu_itemContratado"] != "") {
				...
				$this->setKey("nu_itemContratado", $this->nu_itemContratado->CurrentValue); // Set up key
			} else {
				$this->setKey("nu_itemContratado", ""); // Clear key
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			$sFilterWrk = "";
			$sSqlWrk = "SELECT [nu_contrato], [no_contrato] AS [DispFld], '' AS [Disp2Fld], '' AS [Disp3Fld], '' AS [Disp4Fld], '' AS [SelectFilterFld], '' AS [SelectFilterFld2], '' AS [SelectFilterFld3], '' AS [SelectFilterFld4] FROM [dbo].[contrato]";
			$sWhereWrk = "";
			if ($sFilterWrk <> "") {
				ew_AddFilter($sWhereWrk, $sFilterWrk);
			}
	...
	function ValidateForm() {
		...
		if (!EW_SERVER_VALIDATE)
			return ($gsFormError == "");
		if (!$this->nu_contrato->FldIsDetailKey && !is_null($this->nu_contrato->FormValue) && $this->nu_contrato->FormValue == "") {
			ew_AddMessage($gsFormError, $Language->Phrase("EnterRequiredField") . " - " . $this->nu_contrato->FldCaption());
		}
		...
		if (in_array("Item_contratado_valor", $DetailTblVar) && $GLOBALS["Item_contratado_valor"]->DetailAdd) {
	...
	function AddRow($rsold = NULL) {
		...
		if (strval($this->nu_contrato->CurrentValue) <> "") {
			$sMasterFilter = str_replace("@nu_contrato@", ew_AdjustSql($this->nu_contrato->CurrentValue), $sMasterFilter);
		} else {
		...
		// nu_contrato
		$this->nu_contrato->SetDbValueDef($rsnew, $this->nu_contrato->CurrentValue, 0, FALSE);
		...
		if ($AddRow) {
			$DetailTblVar = explode(",", $this->getCurrentDetailTable());
			if (in_array("Item_contratado_valor", $DetailTblVar) && $GLOBALS["Item_contratado_valor"]->DetailAdd) {
				$GLOBALS["Item_contratado_valor"]->nu_itemContratado->setSessionValue($this->nu_itemContratado->CurrentValue); // Set master key
				if (!isset($GLOBALS["Item_contratado_valor_grid"])) $GLOBALS["Item_contratado_valor_grid"] = new cItem_contratado_valor_grid(); // Get detail page object
				$AddRow = $GLOBALS["Item_contratado_valor_grid"]->GridInsert();
				if (!$AddRow)
					$GLOBALS["Item_contratado_valor"]->nu_itemContratado->setSessionValue(""); // Clear master key if insert failed
			}
		}
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("Item_contratado_valor", $DetailTblVar)) {
				...
				if ($GLOBALS["Item_contratado_valor_grid"]->DetailAdd) {
					if ($this->CopyRecord)
						$GLOBALS["Item_contratado_valor_grid"]->CurrentMode = "copy";
					else
						$GLOBALS["Item_contratado_valor_grid"]->CurrentMode = "add";
					$GLOBALS["Item_contratado_valor_grid"]->CurrentAction = "gridadd";
...
// Create page object
if (!isset($item_contratado_add)) $item_contratado_add = new citem_contratado_add();

// Page init
$item_contratado_add->Page_Init();

// Page main
$item_contratado_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$item_contratado_add->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var item_contratado_add = new ew_Page("item_contratado_add");
item_contratado_add.PageID = "add"; // Page ID
var EW_PAGE_ID = item_contratado_add.PageID; // For backward compatibility

// Form object
var fitem_contratadoadd = new ew_Form("fitem_contratadoadd");

// Validate form
fitem_contratadoadd.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			elm = this.GetElements("x" + infix + "_nu_contrato");
			if (elm && !ew_HasValue(elm))
				return this.OnError(elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($item_contratado->nu_contrato->FldCaption()) ?>");
...
fitem_contratadoadd.Form_CustomValidate = 
...
fitem_contratadoadd.ValidateRequired = true;
<?php } else { ?>
fitem_contratadoadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fitem_contratadoadd.Lists["x_nu_contrato"] = {"LinkField":"x_nu_contrato","Ajax":null,"AutoFill":false,"DisplayFields":["x_no_contrato","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fitem_contratadoadd.Lists["x_nu_itemOc"] = {"LinkField":"x_nu_itemOc","Ajax":null,"AutoFill":false,"DisplayFields":["x_no_itemOc","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fitem_contratadoadd.Lists["x_nu_unidade"] = {"LinkField":"x_nu_unidade","Ajax":null,"AutoFill":false,"DisplayFields":["x_no_unidade","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $item_contratado_add->ShowPageHeader(); ?>
<?php
$item_contratado_add->ShowMessage();
?>
<form name="fitem_contratadoadd" id="fitem_contratadoadd" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="item_contratado">
<input type="hidden" name="a_add" id="a_add" value="A">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_item_contratadoadd" class="table table-bordered table-striped">
<?php if ($item_contratado->nu_contrato->Visible) { // nu_contrato ?>
	<tr id="r_nu_contrato">
		<td><span id="elh_item_contratado_nu_contrato"><?php echo $item_contratado->nu_contrato->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
		<td<?php echo $item_contratado->nu_contrato->CellAttributes() ?>>
<?php if ($item_contratado->nu_contrato->getSessionValue() <> "") { ?>
<span<?php echo $item_contratado->nu_contrato->ViewAttributes() ?>>
<?php echo $item_contratado->nu_contrato->ViewValue ?></span>
<input type="hidden" id="x_nu_contrato" name="x_nu_contrato" value="<?php echo ew_HtmlEncode($item_contratado->nu_contrato->CurrentValue) ?>">
<?php } else { ?>
<select data-field="x_nu_contrato" id="x_nu_contrato" name="x_nu_contrato"<?php echo $item_contratado->nu_contrato->EditAttributes() ?>>
<?php
if (is_array($item_contratado->nu_contrato->EditValue)) {
	$arwrk = $item_contratado->nu_contrato->EditValue;
	$rowswrk = count($arwrk);
	$emptywrk = TRUE;
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($item_contratado->nu_contrato->CurrentValue) == strval($arwrk[$rowcntwrk][0])) ? " selected=\"selected\"" : "";
...
<script type="text/javascript">
fitem_contratadoadd.Lists["x_nu_contrato"].Options = <?php echo (is_array($item_contratado->nu_contrato->EditValue)) ? ew_ArrayToJson($item_contratado->nu_contrato->EditValue, 1) : "[]" ?>;
</script>
<?php } ?>
...
<script type="text/javascript">
fitem_contratadoadd.Lists["x_nu_itemOc"].Options = <?php echo (is_array($item_contratado->nu_itemOc->EditValue)) ? ew_ArrayToJson($item_contratado->nu_itemOc->EditValue, 1) : "[]" ?>;
</script>
</span>
...
<script type="text/javascript">
fitem_contratadoadd.Lists["x_nu_unidade"].Options = <?php echo (is_array($item_contratado->nu_unidade->EditValue)) ? ew_ArrayToJson($item_contratado->nu_unidade->EditValue, 1) : "[]" ?>;
</script>
</span>
...
<?php
	if (in_array("Item_contratado_valor", explode(",", $item_contratado->getCurrentDetailTable())) && $Item_contratado_valor->DetailAdd) {
?>
...
<script type="text/javascript">
fitem_contratadoadd.Init();
<?php if (EW_MOBILE_REFLOW && ew_IsMobile()) { ?>
ew_Reflow();
<?php } ?>
</script>
<?php
$item_contratado_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$item_contratado_add->Page_Terminate();
?>
