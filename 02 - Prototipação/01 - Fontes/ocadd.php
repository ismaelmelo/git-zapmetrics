...
$oc_add = NULL; // Initialize page object first

class coc_add extends coc {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["nu_oc"] != "") {
				...
				$this->setKey("nu_oc", $this->nu_oc->CurrentValue); // Set up key
			} else {
				$this->setKey("nu_oc", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "ocview.php")
	...
	function LoadDefaultValues() {
		$this->ic_tpOc->CurrentValue = NULL;
		$this->ic_tpOc->OldValue = $this->ic_tpOc->CurrentValue;
		$this->co_alternativo->CurrentValue = NULL;
		$this->co_alternativo->OldValue = $this->co_alternativo->CurrentValue;
		$this->ds_oc->CurrentValue = NULL;
		$this->ds_oc->OldValue = $this->ds_oc->CurrentValue;
		$this->dt_oc->CurrentValue = NULL;
		$this->dt_oc->OldValue = $this->dt_oc->CurrentValue;
		$this->nu_stOc->CurrentValue = NULL;
		$this->nu_stOc->OldValue = $this->nu_stOc->CurrentValue;
	...
	function ValidateForm() {
		...
		if (in_array("itemoc", $DetailTblVar) && $GLOBALS["itemoc"]->DetailAdd) {
	...
	function AddRow($rsold = NULL) {
		...
		// ic_tpOc
		$this->ic_tpOc->SetDbValueDef($rsnew, $this->ic_tpOc->CurrentValue, "", FALSE);

		// co_alternativo
		$this->co_alternativo->SetDbValueDef($rsnew, $this->co_alternativo->CurrentValue, "", FALSE);

		// ds_oc
		$this->ds_oc->SetDbValueDef($rsnew, $this->ds_oc->CurrentValue, NULL, FALSE);

		// dt_oc
		$this->dt_oc->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->dt_oc->CurrentValue, 7), ew_CurrentDate(), FALSE);

		// nu_stOc
		$this->nu_stOc->SetDbValueDef($rsnew, $this->nu_stOc->CurrentValue, NULL, FALSE);
		...
		if ($AddRow) {
			$this->nu_oc->setDbValue($conn->Insert_ID());
			$rsnew['nu_oc'] = $this->nu_oc->DbValue;
		}
		...
		if ($AddRow) {
			$DetailTblVar = explode(",", $this->getCurrentDetailTable());
			if (in_array("itemoc", $DetailTblVar) && $GLOBALS["itemoc"]->DetailAdd) {
				$GLOBALS["itemoc"]->nu_oc->setSessionValue($this->nu_oc->CurrentValue); // Set master key
				if (!isset($GLOBALS["itemoc_grid"])) $GLOBALS["itemoc_grid"] = new citemoc_grid(); // Get detail page object
				$AddRow = $GLOBALS["itemoc_grid"]->GridInsert();
				if (!$AddRow)
					$GLOBALS["itemoc"]->nu_oc->setSessionValue(""); // Clear master key if insert failed
			}
		}
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("itemoc", $DetailTblVar)) {
				...
				if ($GLOBALS["itemoc_grid"]->DetailAdd) {
					if ($this->CopyRecord)
						$GLOBALS["itemoc_grid"]->CurrentMode = "copy";
					else
						$GLOBALS["itemoc_grid"]->CurrentMode = "add";
					$GLOBALS["itemoc_grid"]->CurrentAction = "gridadd";
...
// Create page object
if (!isset($oc_add)) $oc_add = new coc_add();
...
// Page object
var oc_add = new ew_Page("oc_add");
oc_add.PageID = "add"; // Page ID
var EW_PAGE_ID = oc_add.PageID; // For backward compatibility

// Form object
var focadd = new ew_Form("focadd");

// Validate form
focadd.Validate = function() {
...
focadd.ValidateRequired = true;
<?php } else { ?>
focadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
focadd.Lists["x_nu_stOc"] = {"LinkField":"x_nu_stOc","Ajax":null,"AutoFill":false,"DisplayFields":["x_no_stOc","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $oc_add->ShowPageHeader(); ?>
<?php
$oc_add->ShowMessage();
?>
<form name="focadd" id="focadd" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="oc">
<input type="hidden" name="a_add" id="a_add" value="A">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_ocadd" class="table table-bordered table-striped">
...
ew_CreateCalendar("focadd", "x_dt_oc", "%d/%m/%Y");
</script>
<?php } ?>
</span>
...
<script type="text/javascript">
focadd.Lists["x_nu_stOc"].Options = <?php echo (is_array($oc->nu_stOc->EditValue)) ? ew_ArrayToJson($oc->nu_stOc->EditValue, 1) : "[]" ?>;
</script>
</span>
...
<?php
	if (in_array("itemoc", explode(",", $oc->getCurrentDetailTable())) && $itemoc->DetailAdd) {
?>
