...
$ativroteiro_delete = NULL; // Initialize page object first

class cativroteiro_delete extends cativroteiro {
	...
	// Page object name
	var $PageObjName = 'ativroteiro_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("ativroteirolist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in ativroteiro class, ativroteiroinfo.php
...
// Create page object
if (!isset($ativroteiro_delete)) $ativroteiro_delete = new cativroteiro_delete();

// Page init
$ativroteiro_delete->Page_Init();

// Page main
$ativroteiro_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$ativroteiro_delete->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var ativroteiro_delete = new ew_Page("ativroteiro_delete");
ativroteiro_delete.PageID = "delete"; // Page ID
var EW_PAGE_ID = ativroteiro_delete.PageID; // For backward compatibility

// Form object
var fativroteirodelete = new ew_Form("fativroteirodelete");

// Form_CustomValidate event
fativroteirodelete.Form_CustomValidate = 
...
fativroteirodelete.ValidateRequired = true;
<?php } else { ?>
fativroteirodelete.ValidateRequired = false; 
<?php } ?>
...
if ($ativroteiro_delete->Recordset = $ativroteiro_delete->LoadRecordset())
	$ativroteiro_deleteTotalRecs = $ativroteiro_delete->Recordset->RecordCount(); // Get record count
if ($ativroteiro_deleteTotalRecs <= 0) { // No record found, exit
	if ($ativroteiro_delete->Recordset)
		$ativroteiro_delete->Recordset->Close();
	$ativroteiro_delete->Page_Terminate("ativroteirolist.php"); // Return to list
}
?>
<?php $Breadcrumb->Render(); ?>
<?php $ativroteiro_delete->ShowPageHeader(); ?>
<?php
$ativroteiro_delete->ShowMessage();
?>
<form name="fativroteirodelete" id="fativroteirodelete" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="ativroteiro">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($ativroteiro_delete->RecKeys as $key) { ?>
<?php $keyvalue = is_array($key) ? implode($EW_COMPOSITE_KEY_SEPARATOR, $key) : $key; ?>
<input type="hidden" name="key_m[]" value="<?php echo ew_HtmlEncode($keyvalue) ?>">
<?php } ?>
<table cellspacing="0" class="ewGrid"><tr><td class="ewGridContent">
<div class="ewGridMiddlePanel">
<table id="tbl_ativroteirodelete" class="ewTable ewTableSeparate">
<?php echo $ativroteiro->TableCustomInnerHtml ?>
	<thead>
	<tr class="ewTableHeader">
		<td><span id="elh_ativroteiro_no_ativRoteiro" class="ativroteiro_no_ativRoteiro"><?php echo $ativroteiro->no_ativRoteiro->FldCaption() ?></span></td>
		<td><span id="elh_ativroteiro_pc_distribuicao" class="ativroteiro_pc_distribuicao"><?php echo $ativroteiro->pc_distribuicao->FldCaption() ?></span></td>
		<td><span id="elh_ativroteiro_ic_ativo" class="ativroteiro_ic_ativo"><?php echo $ativroteiro->ic_ativo->FldCaption() ?></span></td>
		<td><span id="elh_ativroteiro_nu_ordem" class="ativroteiro_nu_ordem"><?php echo $ativroteiro->nu_ordem->FldCaption() ?></span></td>
	</tr>
	</thead>
	<tbody>
<?php
$ativroteiro_delete->RecCnt = 0;
$i = 0;
while (!$ativroteiro_delete->Recordset->EOF) {
	$ativroteiro_delete->RecCnt++;
	$ativroteiro_delete->RowCnt++;

	// Set row properties
	$ativroteiro->ResetAttrs();
	$ativroteiro->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$ativroteiro_delete->LoadRowValues($ativroteiro_delete->Recordset);

	// Render row
	$ativroteiro_delete->RenderRow();
?>
	<tr<?php echo $ativroteiro->RowAttributes() ?>>
		<td<?php echo $ativroteiro->no_ativRoteiro->CellAttributes() ?>>
<span id="el<?php echo $ativroteiro_delete->RowCnt ?>_ativroteiro_no_ativRoteiro" class="control-group ativroteiro_no_ativRoteiro">
...
<span id="el<?php echo $ativroteiro_delete->RowCnt ?>_ativroteiro_pc_distribuicao" class="control-group ativroteiro_pc_distribuicao">
...
<span id="el<?php echo $ativroteiro_delete->RowCnt ?>_ativroteiro_ic_ativo" class="control-group ativroteiro_ic_ativo">
...
<span id="el<?php echo $ativroteiro_delete->RowCnt ?>_ativroteiro_nu_ordem" class="control-group ativroteiro_nu_ordem">
...
<?php
	$ativroteiro_delete->Recordset->MoveNext();
}
$ativroteiro_delete->Recordset->Close();
?>
</tbody>
...
<script type="text/javascript">
fativroteirodelete.Init();
</script>
<?php
$ativroteiro_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$ativroteiro_delete->Page_Terminate();
?>
