...
$tpElemento_grid = NULL; // Initialize page object first

class ctpElemento_grid extends ctpElemento {
	...
	// Page object name
	var $PageObjName = 'tpElemento_grid';

	// Grid form hidden field names
	var $FormName = 'ftpElementogrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["tpElemento"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->nu_tpElemento->CurrentValue;
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_no_tpElemento") && $objForm->HasValue("o_no_tpElemento") && $this->no_tpElemento->CurrentValue <> $this->no_tpElemento->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_ic_funcional") && $objForm->HasValue("o_ic_funcional") && $this->ic_funcional->CurrentValue <> $this->ic_funcional->OldValue)
			return FALSE;
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->nu_tpElemento->CurrentValue . "\">";
		}
	...
	function LoadDefaultValues() {
		...
		$this->ic_funcional->OldValue = $this->ic_funcional->CurrentValue;
	...
	function LoadFormValues() {
		...
		$this->no_tpElemento->setOldValue($objForm->GetValue("o_no_tpElemento"));
		if (!$this->ic_funcional->FldIsDetailKey) {
			$this->ic_funcional->setFormValue($objForm->GetValue("x_ic_funcional"));
		}
		$this->ic_funcional->setOldValue($objForm->GetValue("o_ic_funcional"));
		...
		if (!$this->nu_tpElemento->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->nu_tpElemento->CurrentValue = strval($arKeys[0]); // nu_tpElemento
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function AddRow($rsold = NULL) {
		...
		if ($this->nu_tpManutencao->getSessionValue() <> "") {
			$rsnew['nu_tpManutencao'] = $this->nu_tpManutencao->getSessionValue();
		}
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "tpmanutencao") {
			$this->nu_tpManutencao->Visible = FALSE;
			if ($GLOBALS["tpmanutencao"]->EventCancelled) $this->EventCancelled = TRUE;
		}
