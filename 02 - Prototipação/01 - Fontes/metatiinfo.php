...
//
// Table class for metati
//
class cmetati extends cTable {
	var $nu_metati;
	var $nu_periodoPei;
	var $nu_periodoPdti;
	var $nu_metaneg;
	...
	function __construct() {
		...
		$this->TableVar = 'metati';
		$this->TableName = 'metati';
		...
		// nu_metati
		$this->nu_metati = new cField('metati', 'metati', 'x_nu_metati', 'nu_metati', '[nu_metati]', 'CAST([nu_metati] AS NVARCHAR)', 3, -1, FALSE, '[nu_metati]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->nu_metati->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['nu_metati'] = &$this->nu_metati;

		// nu_periodoPei
		$this->nu_periodoPei = new cField('metati', 'metati', 'x_nu_periodoPei', 'nu_periodoPei', '[nu_periodoPei]', 'CAST([nu_periodoPei] AS NVARCHAR)', 3, -1, FALSE, '[nu_periodoPei]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->nu_periodoPei->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['nu_periodoPei'] = &$this->nu_periodoPei;

		// nu_periodoPdti
		$this->nu_periodoPdti = new cField('metati', 'metati', 'x_nu_periodoPdti', 'nu_periodoPdti', '[nu_periodoPdti]', 'CAST([nu_periodoPdti] AS NVARCHAR)', 3, -1, FALSE, '[nu_periodoPdti]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->nu_periodoPdti->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['nu_periodoPdti'] = &$this->nu_periodoPdti;

		// nu_metaneg
		$this->nu_metaneg = new cField('metati', 'metati', 'x_nu_metaneg', 'nu_metaneg', '[nu_metaneg]', 'CAST([nu_metaneg] AS NVARCHAR)', 3, -1, FALSE, '[nu_metaneg]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->nu_metaneg->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['nu_metaneg'] = &$this->nu_metaneg;

		// ic_perspectiva
		$this->ic_perspectiva = new cField('metati', 'metati', 'x_ic_perspectiva', 'ic_perspectiva', '[ic_perspectiva]', '[ic_perspectiva]', 129, -1, FALSE, '[ic_perspectiva]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['ic_perspectiva'] = &$this->ic_perspectiva;

		// no_metati
		$this->no_metati = new cField('metati', 'metati', 'x_no_metati', 'no_metati', '[no_metati]', '[no_metati]', 200, -1, FALSE, '[no_metati]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['no_metati'] = &$this->no_metati;

		// ds_metati
		$this->ds_metati = new cField('metati', 'metati', 'x_ds_metati', 'ds_metati', '[ds_metati]', '[ds_metati]', 201, -1, FALSE, '[ds_metati]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['ds_metati'] = &$this->ds_metati;

		// ic_situacao
		$this->ic_situacao = new cField('metati', 'metati', 'x_ic_situacao', 'ic_situacao', '[ic_situacao]', '[ic_situacao]', 129, -1, FALSE, '[ic_situacao]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			if (array_key_exists('nu_metati', $rs))
				ew_AddFilter($where, ew_QuotedName('nu_metati') . '=' . ew_QuotedValue($rs['nu_metati'], $this->nu_metati->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function SqlKeyFilter() {
		return "[nu_metati] = @nu_metati@";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		if (!is_numeric($this->nu_metati->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@nu_metati@", ew_AdjustSql($this->nu_metati->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("metatiview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("metatiview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl() {
		return "metatiadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("metatiedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("metatiadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("metatidelete.php", $this->UrlParm());
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->nu_metati->CurrentValue)) {
			$sUrl .= "nu_metati=" . urlencode($this->nu_metati->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKeys[] = @$_GET["nu_metati"]; // nu_metati
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->nu_metati->CurrentValue = $key;
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				if ($this->nu_metati->Exportable) $Doc->ExportCaption($this->nu_metati);
				...
				if ($this->no_metati->Exportable) $Doc->ExportCaption($this->no_metati);
				if ($this->ds_metati->Exportable) $Doc->ExportCaption($this->ds_metati);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					if ($this->nu_metati->Exportable) $Doc->ExportField($this->nu_metati);
					...
					if ($this->no_metati->Exportable) $Doc->ExportField($this->no_metati);
					if ($this->ds_metati->Exportable) $Doc->ExportField($this->ds_metati);
