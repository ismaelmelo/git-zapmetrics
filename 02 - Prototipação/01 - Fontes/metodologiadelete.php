...
$metodologia_delete = NULL; // Initialize page object first

class cmetodologia_delete extends cmetodologia {
	...
	// Page object name
	var $PageObjName = 'metodologia_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("metodologialist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in metodologia class, metodologiainfo.php
...
// Create page object
if (!isset($metodologia_delete)) $metodologia_delete = new cmetodologia_delete();

// Page init
$metodologia_delete->Page_Init();

// Page main
$metodologia_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$metodologia_delete->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var metodologia_delete = new ew_Page("metodologia_delete");
metodologia_delete.PageID = "delete"; // Page ID
var EW_PAGE_ID = metodologia_delete.PageID; // For backward compatibility

// Form object
var fmetodologiadelete = new ew_Form("fmetodologiadelete");

// Form_CustomValidate event
fmetodologiadelete.Form_CustomValidate = 
...
fmetodologiadelete.ValidateRequired = true;
<?php } else { ?>
fmetodologiadelete.ValidateRequired = false; 
<?php } ?>
...
if ($metodologia_delete->Recordset = $metodologia_delete->LoadRecordset())
	$metodologia_deleteTotalRecs = $metodologia_delete->Recordset->RecordCount(); // Get record count
if ($metodologia_deleteTotalRecs <= 0) { // No record found, exit
	if ($metodologia_delete->Recordset)
		$metodologia_delete->Recordset->Close();
	$metodologia_delete->Page_Terminate("metodologialist.php"); // Return to list
}
?>
<?php $Breadcrumb->Render(); ?>
<?php $metodologia_delete->ShowPageHeader(); ?>
<?php
$metodologia_delete->ShowMessage();
?>
<form name="fmetodologiadelete" id="fmetodologiadelete" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="metodologia">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($metodologia_delete->RecKeys as $key) { ?>
<?php $keyvalue = is_array($key) ? implode($EW_COMPOSITE_KEY_SEPARATOR, $key) : $key; ?>
<input type="hidden" name="key_m[]" value="<?php echo ew_HtmlEncode($keyvalue) ?>">
<?php } ?>
<table cellspacing="0" class="ewGrid"><tr><td class="ewGridContent">
<div class="ewGridMiddlePanel">
<table id="tbl_metodologiadelete" class="ewTable ewTableSeparate">
<?php echo $metodologia->TableCustomInnerHtml ?>
	<thead>
	<tr class="ewTableHeader">
		<td><span id="elh_metodologia_no_metodologia" class="metodologia_no_metodologia"><?php echo $metodologia->no_metodologia->FldCaption() ?></span></td>
		<td><span id="elh_metodologia_ic_tpModeloDev" class="metodologia_ic_tpModeloDev"><?php echo $metodologia->ic_tpModeloDev->FldCaption() ?></span></td>
		<td><span id="elh_metodologia_ic_ativo" class="metodologia_ic_ativo"><?php echo $metodologia->ic_ativo->FldCaption() ?></span></td>
		<td><span id="elh_metodologia_nu_ordem" class="metodologia_nu_ordem"><?php echo $metodologia->nu_ordem->FldCaption() ?></span></td>
	</tr>
	</thead>
	<tbody>
<?php
$metodologia_delete->RecCnt = 0;
$i = 0;
while (!$metodologia_delete->Recordset->EOF) {
	$metodologia_delete->RecCnt++;
	$metodologia_delete->RowCnt++;

	// Set row properties
	$metodologia->ResetAttrs();
	$metodologia->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$metodologia_delete->LoadRowValues($metodologia_delete->Recordset);

	// Render row
	$metodologia_delete->RenderRow();
?>
	<tr<?php echo $metodologia->RowAttributes() ?>>
		<td<?php echo $metodologia->no_metodologia->CellAttributes() ?>>
<span id="el<?php echo $metodologia_delete->RowCnt ?>_metodologia_no_metodologia" class="control-group metodologia_no_metodologia">
...
<span id="el<?php echo $metodologia_delete->RowCnt ?>_metodologia_ic_tpModeloDev" class="control-group metodologia_ic_tpModeloDev">
...
<span id="el<?php echo $metodologia_delete->RowCnt ?>_metodologia_ic_ativo" class="control-group metodologia_ic_ativo">
...
<span id="el<?php echo $metodologia_delete->RowCnt ?>_metodologia_nu_ordem" class="control-group metodologia_nu_ordem">
...
<?php
	$metodologia_delete->Recordset->MoveNext();
}
$metodologia_delete->Recordset->Close();
?>
</tbody>
...
<script type="text/javascript">
fmetodologiadelete.Init();
</script>
<?php
$metodologia_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$metodologia_delete->Page_Terminate();
?>
