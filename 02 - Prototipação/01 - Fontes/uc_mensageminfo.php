...
//
// Table class for uc_mensagem
//
class cuc_mensagem extends cTable {
	var $nu_uc;
	var $nu_mensagem;
	var $ds_acao;
	...
	function __construct() {
		...
		$this->TableVar = 'uc_mensagem';
		$this->TableName = 'uc_mensagem';
		...
		// nu_uc
		$this->nu_uc = new cField('uc_mensagem', 'uc_mensagem', 'x_nu_uc', 'nu_uc', '[nu_uc]', 'CAST([nu_uc] AS NVARCHAR)', 3, -1, FALSE, '[EV__nu_uc]', TRUE, TRUE, TRUE, 'FORMATTED TEXT');
		$this->nu_uc->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['nu_uc'] = &$this->nu_uc;

		// nu_mensagem
		$this->nu_mensagem = new cField('uc_mensagem', 'uc_mensagem', 'x_nu_mensagem', 'nu_mensagem', '[nu_mensagem]', 'CAST([nu_mensagem] AS NVARCHAR)', 3, -1, FALSE, '[EV__nu_mensagem]', TRUE, TRUE, TRUE, 'FORMATTED TEXT');
		$this->nu_mensagem->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['nu_mensagem'] = &$this->nu_mensagem;

		// ds_acao
		$this->ds_acao = new cField('uc_mensagem', 'uc_mensagem', 'x_ds_acao', 'ds_acao', '[ds_acao]', '[ds_acao]', 201, -1, FALSE, '[ds_acao]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['ds_acao'] = &$this->ds_acao;
	}
	...
	function SqlSelectList() { // Select for List page
		return "SELECT * FROM (" .
			"SELECT *, (SELECT DISTINCT TOP 1 [co_alternativo] + '" . ew_ValueSeparator(1, $this->nu_uc) . "' + [no_uc] FROM [uc] [EW_TMP_LOOKUPTABLE] WHERE [EW_TMP_LOOKUPTABLE].[nu_uc] = [uc_mensagem].[nu_uc]) AS [EV__nu_uc], (SELECT DISTINCT TOP 1 [co_alternativo] + '" . ew_ValueSeparator(1, $this->nu_mensagem) . "' + [no_mensagem] FROM [mensagem] [EW_TMP_LOOKUPTABLE] WHERE [EW_TMP_LOOKUPTABLE].[nu_mensagem] = [uc_mensagem].[nu_mensagem]) AS [EV__nu_mensagem] FROM [dbo].[uc_mensagem]" .
			") [EW_TMP_TABLE]";
	}
	...
	function SqlOrderBy() { // Order By
		return "[nu_uc] ASC,[nu_mensagem] ASC";
	}
	...
	function UseVirtualFields() {
		...
		if (strpos($sOrderBy, " " . $this->nu_uc->FldVirtualExpression . " ") !== FALSE)
			return TRUE;
		if ($this->nu_mensagem->AdvancedSearch->SearchValue <> "" ||
			$this->nu_mensagem->AdvancedSearch->SearchValue2 <> "" ||
			strpos($sWhere, " " . $this->nu_mensagem->FldVirtualExpression . " ") !== FALSE)
			return TRUE;
		if (strpos($sOrderBy, " " . $this->nu_mensagem->FldVirtualExpression . " ") !== FALSE)
			return TRUE;
		return FALSE;
	}
	...
	// Update Table
	var $UpdateTable = "[dbo].[uc_mensagem]";
	...
	function SqlKeyFilter() {
		return "[nu_uc] = @nu_uc@ AND [nu_mensagem] = @nu_mensagem@";
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("uc_mensagemview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("uc_mensagemview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl() {
		return "uc_mensagemadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("uc_mensagemedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("uc_mensagemadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("uc_mensagemdelete.php", $this->UrlParm());
	}
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKey[] = @$_GET["nu_mensagem"]; // nu_mensagem
			$arKeys[] = $arKey;
		...
		foreach ($arKeys as $key) {
			...
			if (!is_numeric($key[0])) // nu_uc
				continue;
			if (!is_numeric($key[1])) // nu_mensagem
				continue;
			$ar[] = $key;
		}
		return $ar;
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->nu_mensagem->CurrentValue = $key[1];
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				...
				if ($this->nu_mensagem->Exportable) $Doc->ExportCaption($this->nu_mensagem);
				if ($this->ds_acao->Exportable) $Doc->ExportCaption($this->ds_acao);
			} else {
				if ($this->nu_uc->Exportable) $Doc->ExportCaption($this->nu_uc);
				if ($this->nu_mensagem->Exportable) $Doc->ExportCaption($this->nu_mensagem);
			}
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					...
					if ($this->nu_mensagem->Exportable) $Doc->ExportField($this->nu_mensagem);
					if ($this->ds_acao->Exportable) $Doc->ExportField($this->ds_acao);
				} else {
					if ($this->nu_uc->Exportable) $Doc->ExportField($this->nu_uc);
					if ($this->nu_mensagem->Exportable) $Doc->ExportField($this->nu_mensagem);
				}
