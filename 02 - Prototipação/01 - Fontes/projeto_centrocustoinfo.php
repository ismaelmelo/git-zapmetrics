...
$projeto_centrocusto = NULL;

//
// Table class for projeto_centrocusto
//
class cprojeto_centrocusto extends cTable {
	var $nu_projeto;
	var $nu_centroCusto_;
	var $pc_participacao;
	...
	function __construct() {
		...
		$this->TableVar = 'projeto_centrocusto';
		$this->TableName = 'projeto_centrocusto';
		...
		// nu_projeto
		$this->nu_projeto = new cField('projeto_centrocusto', 'projeto_centrocusto', 'x_nu_projeto', 'nu_projeto', '[nu_projeto]', 'CAST([nu_projeto] AS NVARCHAR)', 3, -1, FALSE, '[nu_projeto]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->nu_projeto->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['nu_projeto'] = &$this->nu_projeto;

		// nu_centroCusto 
		$this->nu_centroCusto_ = new cField('projeto_centrocusto', 'projeto_centrocusto', 'x_nu_centroCusto_', 'nu_centroCusto ', '[nu_centroCusto ]', 'CAST([nu_centroCusto ] AS NVARCHAR)', 3, -1, FALSE, '[nu_centroCusto ]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->nu_centroCusto_->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['nu_centroCusto '] = &$this->nu_centroCusto_;

		// pc_participacao
		$this->pc_participacao = new cField('projeto_centrocusto', 'projeto_centrocusto', 'x_pc_participacao', 'pc_participacao', '[pc_participacao]', 'CAST([pc_participacao] AS NVARCHAR)', 131, -1, FALSE, '[pc_participacao]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->pc_participacao->FldDefaultErrMsg = $Language->Phrase("IncorrectFloat");
		$this->fields['pc_participacao'] = &$this->pc_participacao;
	}
	...
	function GetMasterFilter() {
		...
		// Master filter
		$sMasterFilter = "";
		if ($this->getCurrentMasterTable() == "centrocusto") {
			if ($this->nu_centroCusto_->getSessionValue() <> "")
				$sMasterFilter .= "[nu_centroCusto]=" . ew_QuotedValue($this->nu_centroCusto_->getSessionValue(), EW_DATATYPE_NUMBER);
			else
				return "";
		}
	...
	function GetDetailFilter() {
		...
		if ($this->getCurrentMasterTable() == "centrocusto") {
			if ($this->nu_centroCusto_->getSessionValue() <> "")
				$sDetailFilter .= "[nu_centroCusto ]=" . ew_QuotedValue($this->nu_centroCusto_->getSessionValue(), EW_DATATYPE_NUMBER);
			else
				return "";
		}
		if ($this->getCurrentMasterTable() == "projeto") {
	...
	// Master filter
	function SqlMasterFilter_centrocusto() {
		return "[nu_centroCusto]=@nu_centroCusto@";
	}

	// Detail filter
	function SqlDetailFilter_centrocusto() {
		return "[nu_centroCusto ]=@nu_centroCusto_@";
	}
	...
	function SqlFrom() { // From
		return "[dbo].[projeto_centrocusto]";
	}
	...
	// Update Table
	var $UpdateTable = "[dbo].[projeto_centrocusto]";
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "projeto_centrocustolist.php";
		}
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("projeto_centrocustoview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("projeto_centrocustoview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl() {
		return "projeto_centrocustoadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("projeto_centrocustoedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("projeto_centrocustoadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("projeto_centrocustodelete.php", $this->UrlParm());
	}
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				...
				if ($this->nu_centroCusto_->Exportable) $Doc->ExportCaption($this->nu_centroCusto_);
				if ($this->pc_participacao->Exportable) $Doc->ExportCaption($this->pc_participacao);
			} else {
				if ($this->nu_projeto->Exportable) $Doc->ExportCaption($this->nu_projeto);
				if ($this->nu_centroCusto_->Exportable) $Doc->ExportCaption($this->nu_centroCusto_);
				if ($this->pc_participacao->Exportable) $Doc->ExportCaption($this->pc_participacao);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					...
					if ($this->nu_centroCusto_->Exportable) $Doc->ExportField($this->nu_centroCusto_);
					if ($this->pc_participacao->Exportable) $Doc->ExportField($this->pc_participacao);
				} else {
					if ($this->nu_projeto->Exportable) $Doc->ExportField($this->nu_projeto);
					if ($this->nu_centroCusto_->Exportable) $Doc->ExportField($this->nu_centroCusto_);
					if ($this->pc_participacao->Exportable) $Doc->ExportField($this->pc_participacao);
