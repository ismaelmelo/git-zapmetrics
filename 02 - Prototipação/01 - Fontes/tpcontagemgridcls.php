...
$tpcontagem_grid = NULL; // Initialize page object first

class ctpcontagem_grid extends ctpcontagem {
	...
	// Page object name
	var $PageObjName = 'tpcontagem_grid';

	// Grid form hidden field names
	var $FormName = 'ftpcontagemgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["tpcontagem"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function Page_Main() {
		...
		// Load master record
		if ($this->CurrentMode <> "add" && $this->GetMasterFilter() <> "" && $this->getCurrentMasterTable() == "tpmetrica") {
			global $tpmetrica;
			$rsmaster = $tpmetrica->LoadRs($this->DbMasterFilter);
			$this->MasterRecordExists = ($rsmaster && !$rsmaster->EOF);
			if (!$this->MasterRecordExists) {
				$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record found
				$this->Page_Terminate("tpmetricalist.php"); // Return to master page
			} else {
				$tpmetrica->LoadListRowValues($rsmaster);
				$tpmetrica->RowType = EW_ROWTYPE_MASTER; // Master row
				$tpmetrica->RenderListRow();
				$rsmaster->Close();
			}
		}
	...
	function SetupKeyValues($key) {
		...
		if (count($arrKeyFlds) >= 1) {
			$this->nu_tpContagem->setFormValue($arrKeyFlds[0]);
			if (!is_numeric($this->nu_tpContagem->FormValue))
				return FALSE;
		}
		return TRUE;
	}
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->nu_tpContagem->CurrentValue;
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_no_tpContagem") && $objForm->HasValue("o_no_tpContagem") && $this->no_tpContagem->CurrentValue <> $this->no_tpContagem->OldValue)
			return FALSE;
	...
	function LoadSortOrder() {
		...
		if ($sOrderBy == "") {
			if ($this->SqlOrderBy() <> "") {
				...
				$this->no_tpContagem->setSort("ASC");
			}
		}
	}
	...
	function ResetCmd() {
		...
		if (substr($this->Command,0,5) == "reset") {
			...
			if ($this->Command == "resetall") {
				...
				$this->DbMasterFilter = "";
				$this->DbDetailFilter = "";
				$this->nu_tpMetrica->setSessionValue("");
			}
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->nu_tpContagem->CurrentValue . "\">";
		}
	...
	function LoadFormValues() {
		...
		$this->no_tpContagem->setOldValue($objForm->GetValue("o_no_tpContagem"));
		...
		if (!$this->nu_tpContagem->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->nu_tpContagem->CurrentValue = strval($arKeys[0]); // nu_tpContagem
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function DeleteRows() {
		...
		// Check if records exist for detail table 'proposito'
		$rows = ($rs) ? $rs->GetRows() : array();
		if (!isset($GLOBALS["proposito"])) $GLOBALS["proposito"] = new cproposito();
		foreach ($rows as $row) {
			$rsdetail = $GLOBALS["proposito"]->LoadRs("[nu_tpContagem] = " . ew_QuotedValue($row['nu_tpContagem'], EW_DATATYPE_NUMBER));
			if ($rsdetail && !$rsdetail->EOF) {
				$sRelatedRecordMsg = str_replace("%t", "proposito", $Language->Phrase("RelatedRecordExists"));
	...
	function AddRow($rsold = NULL) {
		...
		if ($this->nu_tpMetrica->getSessionValue() <> "") {
			$rsnew['nu_tpMetrica'] = $this->nu_tpMetrica->getSessionValue();
		}
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "tpmetrica") {
			$this->nu_tpMetrica->Visible = FALSE;
			if ($GLOBALS["tpmetrica"]->EventCancelled) $this->EventCancelled = TRUE;
		}
