...
$processo_delete = NULL; // Initialize page object first

class cprocesso_delete extends cprocesso {
	...
	// Page object name
	var $PageObjName = 'processo_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("processolist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in processo class, processoinfo.php
...
// Create page object
if (!isset($processo_delete)) $processo_delete = new cprocesso_delete();

// Page init
$processo_delete->Page_Init();

// Page main
$processo_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$processo_delete->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var processo_delete = new ew_Page("processo_delete");
processo_delete.PageID = "delete"; // Page ID
var EW_PAGE_ID = processo_delete.PageID; // For backward compatibility

// Form object
var fprocessodelete = new ew_Form("fprocessodelete");

// Form_CustomValidate event
fprocessodelete.Form_CustomValidate = 
...
fprocessodelete.ValidateRequired = true;
<?php } else { ?>
fprocessodelete.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fprocessodelete.Lists["x_nu_divisao"] = {"LinkField":"x_nu_divisao","Ajax":null,"AutoFill":false,"DisplayFields":["x_no_divisao","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
if ($processo_delete->Recordset = $processo_delete->LoadRecordset())
	$processo_deleteTotalRecs = $processo_delete->Recordset->RecordCount(); // Get record count
if ($processo_deleteTotalRecs <= 0) { // No record found, exit
	if ($processo_delete->Recordset)
		$processo_delete->Recordset->Close();
	$processo_delete->Page_Terminate("processolist.php"); // Return to list
}
?>
<?php $Breadcrumb->Render(); ?>
<?php $processo_delete->ShowPageHeader(); ?>
<?php
$processo_delete->ShowMessage();
?>
<form name="fprocessodelete" id="fprocessodelete" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="processo">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($processo_delete->RecKeys as $key) { ?>
<?php $keyvalue = is_array($key) ? implode($EW_COMPOSITE_KEY_SEPARATOR, $key) : $key; ?>
<input type="hidden" name="key_m[]" value="<?php echo ew_HtmlEncode($keyvalue) ?>">
<?php } ?>
<table cellspacing="0" class="ewGrid"><tr><td class="ewGridContent">
<div class="ewGridMiddlePanel">
<table id="tbl_processodelete" class="ewTable ewTableSeparate">
<?php echo $processo->TableCustomInnerHtml ?>
	<thead>
	<tr class="ewTableHeader">
		<td><span id="elh_processo_nu_divisao" class="processo_nu_divisao"><?php echo $processo->nu_divisao->FldCaption() ?></span></td>
		<td><span id="elh_processo_no_processo" class="processo_no_processo"><?php echo $processo->no_processo->FldCaption() ?></span></td>
		<td><span id="elh_processo_ic_ativo" class="processo_ic_ativo"><?php echo $processo->ic_ativo->FldCaption() ?></span></td>
	</tr>
	</thead>
	<tbody>
<?php
$processo_delete->RecCnt = 0;
$i = 0;
while (!$processo_delete->Recordset->EOF) {
	$processo_delete->RecCnt++;
	$processo_delete->RowCnt++;

	// Set row properties
	$processo->ResetAttrs();
	$processo->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$processo_delete->LoadRowValues($processo_delete->Recordset);

	// Render row
	$processo_delete->RenderRow();
?>
	<tr<?php echo $processo->RowAttributes() ?>>
		<td<?php echo $processo->nu_divisao->CellAttributes() ?>>
<span id="el<?php echo $processo_delete->RowCnt ?>_processo_nu_divisao" class="control-group processo_nu_divisao">
...
<span id="el<?php echo $processo_delete->RowCnt ?>_processo_no_processo" class="control-group processo_no_processo">
...
<span id="el<?php echo $processo_delete->RowCnt ?>_processo_ic_ativo" class="control-group processo_ic_ativo">
...
<?php
	$processo_delete->Recordset->MoveNext();
}
$processo_delete->Recordset->Close();
?>
</tbody>
...
<script type="text/javascript">
fprocessodelete.Init();
</script>
<?php
$processo_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$processo_delete->Page_Terminate();
?>
