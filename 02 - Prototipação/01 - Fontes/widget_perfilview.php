...
$widget_perfil_view = NULL; // Initialize page object first

class cwidget_perfil_view extends cwidget_perfil {
	...
	// Page object name
	var $PageObjName = 'widget_perfil_view';
	...
	function __construct() {
		...
		$KeyUrl = "";
		if (@$_GET["nu_perfil"] <> "") {
			$this->RecKey["nu_perfil"] = $_GET["nu_perfil"];
			$KeyUrl .= "&nu_perfil=" . urlencode($this->RecKey["nu_perfil"]);
		}
		if (@$_GET["nu_widget"] <> "") {
			$this->RecKey["nu_widget"] = $_GET["nu_widget"];
			$KeyUrl .= "&nu_widget=" . urlencode($this->RecKey["nu_widget"]);
		}
	...
	function Page_Init() {
		...
		if (@$_GET["nu_perfil"] <> "") {
			...
			$gsExportFile .= ew_StripSlashes($_GET["nu_perfil"]);
		}
		if (@$_GET["nu_widget"] <> "") {
			if ($gsExportFile <> "") $gsExportFile .= "_";
			$gsExportFile .= ew_StripSlashes($_GET["nu_widget"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["nu_perfil"] <> "") {
				...
				$this->RecKey["nu_perfil"] = $this->nu_perfil->QueryStringValue;
			} else {
				$bLoadCurrentRecord = TRUE;
			}
			if (@$_GET["nu_widget"] <> "") {
				$this->nu_widget->setQueryStringValue($_GET["nu_widget"]);
				$this->RecKey["nu_widget"] = $this->nu_widget->QueryStringValue;
			} else {
				$bLoadCurrentRecord = TRUE;
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					...
					if ($this->TotalRecs <= 0) { // No record found
						...
						$this->Page_Terminate("widget_perfillist.php"); // Return to list page
						...
						while (!$this->Recordset->EOF) {
							if (strval($this->nu_perfil->CurrentValue) == strval($this->Recordset->fields('nu_perfil')) && strval($this->nu_widget->CurrentValue) == strval($this->Recordset->fields('nu_widget'))) {
					...
					if (!$bMatchRecord) {
						if ($this->getSuccessMessage() == "" && $this->getFailureMessage() == "")
							$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record message
						$sReturnUrl = "widget_perfillist.php"; // No matching record, return to list
					} else {
			...
			$sReturnUrl = "widget_perfillist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a id=\"emf_widget_perfil\" href=\"javascript:void(0);\" class=\"ewExportLink ewEmail\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_widget_perfil',hdr:ewLanguage.Phrase('ExportToEmail'),f:document.fwidget_perfilview,key:" . ew_ArrayToJsonAttr($this->RecKey) . ",sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
...
// Create page object
if (!isset($widget_perfil_view)) $widget_perfil_view = new cwidget_perfil_view();

// Page init
$widget_perfil_view->Page_Init();

// Page main
$widget_perfil_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$widget_perfil_view->Page_Render();
?>
...
// Page object
var widget_perfil_view = new ew_Page("widget_perfil_view");
widget_perfil_view.PageID = "view"; // Page ID
var EW_PAGE_ID = widget_perfil_view.PageID; // For backward compatibility

// Form object
var fwidget_perfilview = new ew_Form("fwidget_perfilview");

// Form_CustomValidate event
fwidget_perfilview.Form_CustomValidate = 
...
fwidget_perfilview.ValidateRequired = true;
<?php } else { ?>
fwidget_perfilview.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fwidget_perfilview.Lists["x_nu_perfil"] = {"LinkField":"x_nu_level","Ajax":null,"AutoFill":false,"DisplayFields":["x_no_level","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fwidget_perfilview.Lists["x_nu_widget"] = {"LinkField":"x_nu_widget","Ajax":null,"AutoFill":false,"DisplayFields":["x_no_widget","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $widget_perfil_view->ExportOptions->Render("body") ?>
<?php if (!$widget_perfil_view->ExportOptions->UseDropDownButton) { ?>
</div>
<div class="ewViewOtherOptions">
<?php } ?>
<?php
	foreach ($widget_perfil_view->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<?php } ?>
<?php $widget_perfil_view->ShowPageHeader(); ?>
<?php
$widget_perfil_view->ShowMessage();
?>
<form name="fwidget_perfilview" id="fwidget_perfilview" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="widget_perfil">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_widget_perfilview" class="table table-bordered table-striped">
...
<?php echo $widget_perfil->nu_perfil->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php echo $widget_perfil->nu_widget->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($widget_perfil->no_titulo->Visible) { // no_titulo ?>
	<tr id="r_no_titulo">
		<td><span id="elh_widget_perfil_no_titulo"><?php echo $widget_perfil->no_titulo->FldCaption() ?></span></td>
...
<?php echo $widget_perfil->no_titulo->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php echo $widget_perfil->no_legTexto->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php echo $widget_perfil->no_legValores->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($widget_perfil->nu_posicao->Visible) { // nu_posicao ?>
	<tr id="r_nu_posicao">
		<td><span id="elh_widget_perfil_nu_posicao"><?php echo $widget_perfil->nu_posicao->FldCaption() ?></span></td>
...
<?php echo $widget_perfil->nu_posicao->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php echo $widget_perfil->vr_larguraEmPx->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php echo $widget_perfil->vr_alturaEmPx->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($widget_perfil->ic_ativo->Visible) { // ic_ativo ?>
	<tr id="r_ic_ativo">
		<td><span id="elh_widget_perfil_ic_ativo"><?php echo $widget_perfil->ic_ativo->FldCaption() ?></span></td>
...
<?php echo $widget_perfil->ic_ativo->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<table class="ewPager">
<tr><td>
<?php if (!isset($widget_perfil_view->Pager)) $widget_perfil_view->Pager = new cNumericPager($widget_perfil_view->StartRec, $widget_perfil_view->DisplayRecs, $widget_perfil_view->TotalRecs, $widget_perfil_view->RecRange) ?>
<?php if ($widget_perfil_view->Pager->RecordCount > 0) { ?>
<table cellspacing="0" class="ewStdTable"><tbody><tr><td>
<div class="pagination"><ul>
	<?php if ($widget_perfil_view->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $widget_perfil_view->PageUrl() ?>start=<?php echo $widget_perfil_view->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($widget_perfil_view->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $widget_perfil_view->PageUrl() ?>start=<?php echo $widget_perfil_view->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($widget_perfil_view->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $widget_perfil_view->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($widget_perfil_view->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $widget_perfil_view->PageUrl() ?>start=<?php echo $widget_perfil_view->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($widget_perfil_view->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $widget_perfil_view->PageUrl() ?>start=<?php echo $widget_perfil_view->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</td>
</tr></tbody></table>
...
<script type="text/javascript">
fwidget_perfilview.Init();
</script>
<?php
$widget_perfil_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($widget_perfil->Export == "") { ?>
...
<?php
$widget_perfil_view->Page_Terminate();
?>
