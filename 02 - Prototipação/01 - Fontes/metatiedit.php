...
$metati_edit = NULL; // Initialize page object first

class cmetati_edit extends cmetati {
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->nu_metati->CurrentValue == "")
			$this->Page_Terminate("metatilist.php"); // Invalid key, return to list
	...
	function LoadFormValues() {
		...
		// Load from form
		global $objForm;
		if (!$this->nu_metati->FldIsDetailKey)
			$this->nu_metati->setFormValue($objForm->GetValue("x_nu_metati"));
	...
	function RestoreFormValues() {
		global $objForm;
		$this->LoadRow();
		$this->nu_metati->CurrentValue = $this->nu_metati->FormValue;
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			// nu_metati
			$this->nu_metati->EditCustomAttributes = "";
			$this->nu_metati->EditValue = $this->nu_metati->CurrentValue;
			$this->nu_metati->ViewCustomAttributes = "";
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// no_metati
			$this->no_metati->SetDbValueDef($rsnew, $this->no_metati->CurrentValue, NULL, $this->no_metati->ReadOnly);

			// ds_metati
			$this->ds_metati->SetDbValueDef($rsnew, $this->ds_metati->CurrentValue, NULL, $this->ds_metati->ReadOnly);
...
// Create page object
if (!isset($metati_edit)) $metati_edit = new cmetati_edit();

// Page init
$metati_edit->Page_Init();

// Page main
$metati_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$metati_edit->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var metati_edit = new ew_Page("metati_edit");
metati_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = metati_edit.PageID; // For backward compatibility

// Form object
var fmetatiedit = new ew_Form("fmetatiedit");

// Validate form
fmetatiedit.Validate = function() {
...
fmetatiedit.Form_CustomValidate = 
...
fmetatiedit.ValidateRequired = true;
<?php } else { ?>
fmetatiedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fmetatiedit.Lists["x_nu_periodoPei"] = {"LinkField":"x_nu_periodoPei","Ajax":null,"AutoFill":false,"DisplayFields":["x_nu_anoInicio","x_nu_anoFim","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fmetatiedit.Lists["x_nu_periodoPdti"] = {"LinkField":"x_nu_periodo","Ajax":null,"AutoFill":false,"DisplayFields":["x_no_periodo","","",""],"ParentFields":["x_nu_periodoPei"],"FilterFields":["x_nu_periodoPei"],"Options":[]};
fmetatiedit.Lists["x_nu_metaneg"] = {"LinkField":"x_nu_metaneg","Ajax":null,"AutoFill":false,"DisplayFields":["x_no_metaneg","","",""],"ParentFields":["x_nu_periodoPei"],"FilterFields":["x_nu_periodoPei"],"Options":[]};
...
<?php $metati_edit->ShowPageHeader(); ?>
<?php
$metati_edit->ShowMessage();
?>
<form name="fmetatiedit" id="fmetatiedit" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="metati">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_metatiedit" class="table table-bordered table-striped">
<?php if ($metati->nu_metati->Visible) { // nu_metati ?>
	<tr id="r_nu_metati">
		<td><span id="elh_metati_nu_metati"><?php echo $metati->nu_metati->FldCaption() ?></span></td>
		<td<?php echo $metati->nu_metati->CellAttributes() ?>>
<span id="el_metati_nu_metati" class="control-group">
<span<?php echo $metati->nu_metati->ViewAttributes() ?>>
<?php echo $metati->nu_metati->EditValue ?></span>
</span>
<input type="hidden" data-field="x_nu_metati" name="x_nu_metati" id="x_nu_metati" value="<?php echo ew_HtmlEncode($metati->nu_metati->CurrentValue) ?>">
<?php echo $metati->nu_metati->CustomMsg ?></td>
	</tr>
<?php } ?>
...
<script type="text/javascript">
fmetatiedit.Lists["x_nu_periodoPei"].Options = <?php echo (is_array($metati->nu_periodoPei->EditValue)) ? ew_ArrayToJson($metati->nu_periodoPei->EditValue, 1) : "[]" ?>;
</script>
</span>
...
<script type="text/javascript">
fmetatiedit.Lists["x_nu_periodoPdti"].Options = <?php echo (is_array($metati->nu_periodoPdti->EditValue)) ? ew_ArrayToJson($metati->nu_periodoPdti->EditValue, 1) : "[]" ?>;
</script>
</span>
...
<script type="text/javascript">
fmetatiedit.Lists["x_nu_metaneg"].Options = <?php echo (is_array($metati->nu_metaneg->EditValue)) ? ew_ArrayToJson($metati->nu_metaneg->EditValue, 1) : "[]" ?>;
</script>
</span>
