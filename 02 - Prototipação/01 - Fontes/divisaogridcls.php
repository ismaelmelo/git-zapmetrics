...
$divisao_grid = NULL; // Initialize page object first

class cdivisao_grid extends cdivisao {
	...
	// Page object name
	var $PageObjName = 'divisao_grid';

	// Grid form hidden field names
	var $FormName = 'fdivisaogrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["divisao"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->nu_divisao->CurrentValue;
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_no_divisao") && $objForm->HasValue("o_no_divisao") && $this->no_divisao->CurrentValue <> $this->no_divisao->OldValue)
			return FALSE;
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->nu_divisao->CurrentValue . "\">";
		}
	...
	function LoadFormValues() {
		...
		$this->no_divisao->setOldValue($objForm->GetValue("o_no_divisao"));
		...
		if (!$this->nu_divisao->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->nu_divisao->CurrentValue = strval($arKeys[0]); // nu_divisao
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function AddRow($rsold = NULL) {
		...
		if ($this->nu_area->getSessionValue() <> "") {
			$sMasterFilter = str_replace("@nu_area@", ew_AdjustSql($this->nu_area->getSessionValue()), $sMasterFilter);
		} else {
			$bValidMasterRecord = FALSE;
		}
		if ($bValidMasterRecord) {
		...
		// nu_area
		if ($this->nu_area->getSessionValue() <> "") {
			$rsnew['nu_area'] = $this->nu_area->getSessionValue();
		}
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "area") {
			$this->nu_area->Visible = FALSE;
			if ($GLOBALS["area"]->EventCancelled) $this->EventCancelled = TRUE;
		}
