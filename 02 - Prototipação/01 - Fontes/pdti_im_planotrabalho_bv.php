...
$pdti_im_planoTrabalho_blobview = NULL; // Initialize page object first

class cpdti_im_planoTrabalho_blobview extends cpdti {
	...
	// Page object name
	var $PageObjName = 'pdti_im_planoTrabalho_blobview';
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel(CurrentProjectID() . 'pdti');
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('pdti', 'x_im_planoTrabalho');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (!EW_DEBUG_ENABLED && ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('im_planoTrabalho');
				...
				if (trim(strval($this->Recordset->fields('im_planoTrabalho'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('im_planoTrabalho') . "\"");
				}
...
// Create page object
if (!isset($pdti_im_planoTrabalho_blobview)) $pdti_im_planoTrabalho_blobview = new cpdti_im_planoTrabalho_blobview();

// Page init
$pdti_im_planoTrabalho_blobview->Page_Init();

// Page main
$pdti_im_planoTrabalho_blobview->Page_Main();
?>
<?php
$pdti_im_planoTrabalho_blobview->Page_Terminate();
?>
