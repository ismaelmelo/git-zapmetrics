...
$gc_ata_delete = NULL; // Initialize page object first

class cgc_ata_delete extends cgc_ata {
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("gc_atalist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in gc_ata class, gc_atainfo.php
...
// Create page object
if (!isset($gc_ata_delete)) $gc_ata_delete = new cgc_ata_delete();

// Page init
$gc_ata_delete->Page_Init();

// Page main
$gc_ata_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$gc_ata_delete->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var gc_ata_delete = new ew_Page("gc_ata_delete");
gc_ata_delete.PageID = "delete"; // Page ID
var EW_PAGE_ID = gc_ata_delete.PageID; // For backward compatibility

// Form object
var fgc_atadelete = new ew_Form("fgc_atadelete");

// Form_CustomValidate event
fgc_atadelete.Form_CustomValidate = 
...
fgc_atadelete.ValidateRequired = true;
<?php } else { ?>
fgc_atadelete.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fgc_atadelete.Lists["x_nu_grupoOuComite"] = {"LinkField":"x_nu_gpComite","Ajax":null,"AutoFill":false,"DisplayFields":["x_no_gpComite","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fgc_atadelete.Lists["x_nu_reuniao"] = {"LinkField":"x_nu_reuniao","Ajax":null,"AutoFill":false,"DisplayFields":["x_nu_reuniao","x_dt_reuniao","x_no_local","x_hh_inicio"],"ParentFields":[],"FilterFields":[],"Options":[]};
...
if ($gc_ata_delete->Recordset = $gc_ata_delete->LoadRecordset())
	$gc_ata_deleteTotalRecs = $gc_ata_delete->Recordset->RecordCount(); // Get record count
if ($gc_ata_deleteTotalRecs <= 0) { // No record found, exit
	if ($gc_ata_delete->Recordset)
		$gc_ata_delete->Recordset->Close();
	$gc_ata_delete->Page_Terminate("gc_atalist.php"); // Return to list
}
?>
<?php $Breadcrumb->Render(); ?>
<?php $gc_ata_delete->ShowPageHeader(); ?>
<?php
$gc_ata_delete->ShowMessage();
?>
<form name="fgc_atadelete" id="fgc_atadelete" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="gc_ata">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($gc_ata_delete->RecKeys as $key) { ?>
<?php $keyvalue = is_array($key) ? implode($EW_COMPOSITE_KEY_SEPARATOR, $key) : $key; ?>
<input type="hidden" name="key_m[]" value="<?php echo ew_HtmlEncode($keyvalue) ?>">
<?php } ?>
<table cellspacing="0" class="ewGrid"><tr><td class="ewGridContent">
<div class="ewGridMiddlePanel">
<table id="tbl_gc_atadelete" class="ewTable ewTableSeparate">
<?php echo $gc_ata->TableCustomInnerHtml ?>
	<thead>
	<tr class="ewTableHeader">
		<td><span id="elh_gc_ata_nu_ata" class="gc_ata_nu_ata"><?php echo $gc_ata->nu_ata->FldCaption() ?></span></td>
		<td><span id="elh_gc_ata_nu_grupoOuComite" class="gc_ata_nu_grupoOuComite"><?php echo $gc_ata->nu_grupoOuComite->FldCaption() ?></span></td>
		<td><span id="elh_gc_ata_nu_reuniao" class="gc_ata_nu_reuniao"><?php echo $gc_ata->nu_reuniao->FldCaption() ?></span></td>
		<td><span id="elh_gc_ata_dt_publicacao" class="gc_ata_dt_publicacao"><?php echo $gc_ata->dt_publicacao->FldCaption() ?></span></td>
		<td><span id="elh_gc_ata_ic_situacao" class="gc_ata_ic_situacao"><?php echo $gc_ata->ic_situacao->FldCaption() ?></span></td>
	</tr>
	</thead>
	<tbody>
<?php
$gc_ata_delete->RecCnt = 0;
$i = 0;
while (!$gc_ata_delete->Recordset->EOF) {
	$gc_ata_delete->RecCnt++;
	$gc_ata_delete->RowCnt++;

	// Set row properties
	$gc_ata->ResetAttrs();
	$gc_ata->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$gc_ata_delete->LoadRowValues($gc_ata_delete->Recordset);

	// Render row
	$gc_ata_delete->RenderRow();
?>
	<tr<?php echo $gc_ata->RowAttributes() ?>>
		<td<?php echo $gc_ata->nu_ata->CellAttributes() ?>>
<span id="el<?php echo $gc_ata_delete->RowCnt ?>_gc_ata_nu_ata" class="control-group gc_ata_nu_ata">
...
<span id="el<?php echo $gc_ata_delete->RowCnt ?>_gc_ata_nu_grupoOuComite" class="control-group gc_ata_nu_grupoOuComite">
...
<span id="el<?php echo $gc_ata_delete->RowCnt ?>_gc_ata_nu_reuniao" class="control-group gc_ata_nu_reuniao">
...
<span id="el<?php echo $gc_ata_delete->RowCnt ?>_gc_ata_dt_publicacao" class="control-group gc_ata_dt_publicacao">
...
<span id="el<?php echo $gc_ata_delete->RowCnt ?>_gc_ata_ic_situacao" class="control-group gc_ata_ic_situacao">
...
<?php
	$gc_ata_delete->Recordset->MoveNext();
}
$gc_ata_delete->Recordset->Close();
?>
</tbody>
