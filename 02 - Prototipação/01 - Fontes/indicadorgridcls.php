...
$indicador_grid = NULL; // Initialize page object first

class cindicador_grid extends cindicador {
	...
	// Page object name
	var $PageObjName = 'indicador_grid';

	// Grid form hidden field names
	var $FormName = 'findicadorgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["indicador"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->nu_indicador->CurrentValue;
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_no_indicador") && $objForm->HasValue("o_no_indicador") && $this->no_indicador->CurrentValue <> $this->no_indicador->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_ds_indicador") && $objForm->HasValue("o_ds_indicador") && $this->ds_indicador->CurrentValue <> $this->ds_indicador->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_ic_tpIndicador") && $objForm->HasValue("o_ic_tpIndicador") && $this->ic_tpIndicador->CurrentValue <> $this->ic_tpIndicador->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_nu_processoCobit5") && $objForm->HasValue("o_nu_processoCobit5") && $this->nu_processoCobit5->CurrentValue <> $this->nu_processoCobit5->OldValue)
			return FALSE;
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->nu_indicador->CurrentValue . "\">";
		}
	...
	function LoadFormValues() {
		...
		if (!$this->nu_indicador->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->nu_indicador->CurrentValue = strval($arKeys[0]); // nu_indicador
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "indicadorversao") {
			$this->nu_indicador->Visible = FALSE;
			if ($GLOBALS["indicadorversao"]->EventCancelled) $this->EventCancelled = TRUE;
		}
