...
$indicadorversao_grid = NULL; // Initialize page object first

class cindicadorversao_grid extends cindicadorversao {
	...
	// Page object name
	var $PageObjName = 'indicadorversao_grid';

	// Grid form hidden field names
	var $FormName = 'findicadorversaogrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["indicadorversao"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_nu_indicador") && $objForm->HasValue("o_nu_indicador") && $this->nu_indicador->CurrentValue <> $this->nu_indicador->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_nu_versao") && $objForm->HasValue("o_nu_versao") && $this->nu_versao->CurrentValue <> $this->nu_versao->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_ic_periodicidadeGeracao") && $objForm->HasValue("o_ic_periodicidadeGeracao") && $this->ic_periodicidadeGeracao->CurrentValue <> $this->ic_periodicidadeGeracao->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_ic_reponsavelColetaCtrl") && $objForm->HasValue("o_ic_reponsavelColetaCtrl") && $this->ic_reponsavelColetaCtrl->CurrentValue <> $this->ic_reponsavelColetaCtrl->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->nu_indicador->CurrentValue . $GLOBALS["EW_COMPOSITE_KEY_SEPARATOR"] . $this->nu_versao->CurrentValue . "\">";
		}
	...
	function LoadFormValues() {
		...
		$this->nu_indicador->setOldValue($objForm->GetValue("o_nu_indicador"));
		...
		$this->ic_periodicidadeGeracao->setOldValue($objForm->GetValue("o_ic_periodicidadeGeracao"));
		...
		$this->ic_reponsavelColetaCtrl->setOldValue($objForm->GetValue("o_ic_reponsavelColetaCtrl"));
		...
		$this->dh_versao->setOldValue($objForm->GetValue("o_dh_versao"));
	}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			$this->ic_periodicidadeGeracao->SetDbValueDef($rsnew, $this->ic_periodicidadeGeracao->CurrentValue, NULL, $this->ic_periodicidadeGeracao->ReadOnly);

			// ic_reponsavelColetaCtrl
			$this->ic_reponsavelColetaCtrl->SetDbValueDef($rsnew, $this->ic_reponsavelColetaCtrl->CurrentValue, NULL, $this->ic_reponsavelColetaCtrl->ReadOnly);
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "indicadorvalor") {
			...
			if ($GLOBALS["indicadorvalor"]->EventCancelled) $this->EventCancelled = TRUE;
			$this->nu_versao->Visible = FALSE;
			if ($GLOBALS["indicadorvalor"]->EventCancelled) $this->EventCancelled = TRUE;
		}
