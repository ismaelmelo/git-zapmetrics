...
$os_edit = NULL; // Initialize page object first

class cos_edit extends cos {
	...
	function Page_Init() {
		...
		$this->nu_os->Visible = !$this->IsAdd() && !$this->IsCopy() && !$this->IsGridAdd();
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->nu_os->CurrentValue == "")
			$this->Page_Terminate("oslist.php"); // Invalid key, return to list
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			$this->nu_os->EditValue = $this->nu_os->CurrentValue;
			$this->nu_os->ViewCustomAttributes = "";
			...
			if (!$GLOBALS["os"]->UserIDAllow("edit")) $sWhereWrk = $GLOBALS["usuario"]->AddUserIDFilter($sWhereWrk);
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// co_os
			$this->co_os->SetDbValueDef($rsnew, $this->co_os->CurrentValue, NULL, $this->co_os->ReadOnly);

			// no_titulo
			$this->no_titulo->SetDbValueDef($rsnew, $this->no_titulo->CurrentValue, "", $this->no_titulo->ReadOnly);

			// nu_contrato
			$this->nu_contrato->SetDbValueDef($rsnew, $this->nu_contrato->CurrentValue, NULL, $this->nu_contrato->ReadOnly);

			// nu_itemContratado
			$this->nu_itemContratado->SetDbValueDef($rsnew, $this->nu_itemContratado->CurrentValue, NULL, $this->nu_itemContratado->ReadOnly);

			// nu_areaSolicitante
			$this->nu_areaSolicitante->SetDbValueDef($rsnew, $this->nu_areaSolicitante->CurrentValue, 0, $this->nu_areaSolicitante->ReadOnly);
			...
			// dt_entrega
			$this->dt_entrega->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->dt_entrega->CurrentValue, 7), NULL, $this->dt_entrega->ReadOnly);

			// nu_stOs
			$this->nu_stOs->SetDbValueDef($rsnew, $this->nu_stOs->CurrentValue, NULL, $this->nu_stOs->ReadOnly);
			...
			// nu_usuarioAnalista
			$this->nu_usuarioAnalista->SetDbValueDef($rsnew, $this->nu_usuarioAnalista->CurrentValue, NULL, $this->nu_usuarioAnalista->ReadOnly);

			// ds_observacoes
			$this->ds_observacoes->SetDbValueDef($rsnew, $this->ds_observacoes->CurrentValue, NULL, $this->ds_observacoes->ReadOnly);

			// vr_os
			$this->vr_os->SetDbValueDef($rsnew, $this->vr_os->CurrentValue, NULL, $this->vr_os->ReadOnly);
...
// Create page object
if (!isset($os_edit)) $os_edit = new cos_edit();
...
// Page object
var os_edit = new ew_Page("os_edit");
os_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = os_edit.PageID; // For backward compatibility

// Form object
var fosedit = new ew_Form("fosedit");

// Validate form
fosedit.Validate = function() {
...
fosedit.ValidateRequired = true;
<?php } else { ?>
fosedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fosedit.Lists["x_nu_contrato"] = {"LinkField":"x_nu_contrato","Ajax":null,"AutoFill":false,"DisplayFields":["x_nu_contrato","x_no_contrato","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fosedit.Lists["x_nu_itemContratado"] = {"LinkField":"x_nu_itemContratado","Ajax":null,"AutoFill":false,"DisplayFields":["x_nu_itemOc","x_no_itemContratado","",""],"ParentFields":["x_nu_contrato"],"FilterFields":["x_nu_contrato"],"Options":[]};
fosedit.Lists["x_nu_areaSolicitante"] = {"LinkField":"x_nu_area","Ajax":null,"AutoFill":false,"DisplayFields":["x_no_area","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fosedit.Lists["x_nu_projeto"] = {"LinkField":"x_nu_projeto","Ajax":null,"AutoFill":false,"DisplayFields":["x_no_projeto","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fosedit.Lists["x_nu_stOs"] = {"LinkField":"x_nu_stOs","Ajax":null,"AutoFill":false,"DisplayFields":["x_no_stUc","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fosedit.Lists["x_nu_usuarioAnalista"] = {"LinkField":"x_nu_usuario","Ajax":null,"AutoFill":false,"DisplayFields":["x_no_usuario","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $os_edit->ShowPageHeader(); ?>
<?php
$os_edit->ShowMessage();
?>
<form name="fosedit" id="fosedit" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="os">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_osedit" class="table table-bordered table-striped">
<?php if ($os->nu_os->Visible) { // nu_os ?>
	<tr id="r_nu_os">
		<td><span id="elh_os_nu_os"><?php echo $os->nu_os->FldCaption() ?></span></td>
		<td<?php echo $os->nu_os->CellAttributes() ?>>
<span id="el_os_nu_os" class="control-group">
<span<?php echo $os->nu_os->ViewAttributes() ?>>
<?php echo $os->nu_os->EditValue ?></span>
</span>
<input type="hidden" data-field="x_nu_os" name="x_nu_os" id="x_nu_os" value="<?php echo ew_HtmlEncode($os->nu_os->CurrentValue) ?>">
<?php echo $os->nu_os->CustomMsg ?></td>
	</tr>
<?php } ?>
...
<script type="text/javascript">
fosedit.Lists["x_nu_contrato"].Options = <?php echo (is_array($os->nu_contrato->EditValue)) ? ew_ArrayToJson($os->nu_contrato->EditValue, 1) : "[]" ?>;
</script>
</span>
...
<script type="text/javascript">
fosedit.Lists["x_nu_itemContratado"].Options = <?php echo (is_array($os->nu_itemContratado->EditValue)) ? ew_ArrayToJson($os->nu_itemContratado->EditValue, 1) : "[]" ?>;
</script>
</span>
...
<script type="text/javascript">
fosedit.Lists["x_nu_areaSolicitante"].Options = <?php echo (is_array($os->nu_areaSolicitante->EditValue)) ? ew_ArrayToJson($os->nu_areaSolicitante->EditValue, 1) : "[]" ?>;
</script>
</span>
...
<?php } ?>
<script type="text/javascript">
fosedit.Lists["x_nu_projeto"].Options = <?php echo (is_array($os->nu_projeto->EditValue)) ? ew_ArrayToJson($os->nu_projeto->EditValue, 1) : "[]" ?>;
</script>
</span>
...
ew_CreateCalendar("fosedit", "x_dt_entrega", "%d/%m/%Y");
</script>
<?php } ?>
</span>
...
<script type="text/javascript">
fosedit.Lists["x_nu_stOs"].Options = <?php echo (is_array($os->nu_stOs->EditValue)) ? ew_ArrayToJson($os->nu_stOs->EditValue, 1) : "[]" ?>;
</script>
</span>
...
<script type="text/javascript">
fosedit.Lists["x_nu_usuarioAnalista"].Options = <?php echo (is_array($os->nu_usuarioAnalista->EditValue)) ? ew_ArrayToJson($os->nu_usuarioAnalista->EditValue, 1) : "[]" ?>;
</script>
</span>
