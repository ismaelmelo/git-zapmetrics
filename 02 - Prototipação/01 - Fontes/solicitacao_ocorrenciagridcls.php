...
$solicitacao_ocorrencia_grid = NULL; // Initialize page object first

class csolicitacao_ocorrencia_grid extends csolicitacao_ocorrencia {
	...
	// Page object name
	var $PageObjName = 'solicitacao_ocorrencia_grid';

	// Grid form hidden field names
	var $FormName = 'fsolicitacao_ocorrenciagrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["solicitacao_ocorrencia"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_ic_tpOcorrencia") && $objForm->HasValue("o_ic_tpOcorrencia") && $this->ic_tpOcorrencia->CurrentValue <> $this->ic_tpOcorrencia->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_ic_exibirNoLaudo") && $objForm->HasValue("o_ic_exibirNoLaudo") && $this->ic_exibirNoLaudo->CurrentValue <> $this->ic_exibirNoLaudo->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->nu_solicitacao->CurrentValue . $GLOBALS["EW_COMPOSITE_KEY_SEPARATOR"] . $this->nu_ocorrencia->CurrentValue . "\">";
		}
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 2) {
			if (strval($arKeys[0]) <> "")
				...
				$bValidKey = FALSE;
			if (strval($arKeys[1]) <> "")
				$this->nu_ocorrencia->CurrentValue = strval($arKeys[1]); // nu_ocorrencia
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function AddRow($rsold = NULL) {
		...
		if ($this->nu_solicitacao->getSessionValue() <> "") {
			$sMasterFilter = str_replace("@nu_solMetricas@", ew_AdjustSql($this->nu_solicitacao->getSessionValue()), $sMasterFilter);
		} else {
			$bValidMasterRecord = FALSE;
		}
		if ($bValidMasterRecord) {
		...
		// nu_solicitacao
		if ($this->nu_solicitacao->getSessionValue() <> "") {
			$rsnew['nu_solicitacao'] = $this->nu_solicitacao->getSessionValue();
		}
