...
//
class cprocesso extends cTable {
	var $nu_processo;
	var $nu_area;
	var $nu_divisao;
	...
	function __construct() {
		...
		// nu_processo
		$this->nu_processo = new cField('processo', 'processo', 'x_nu_processo', 'nu_processo', '[nu_processo]', 'CAST([nu_processo] AS NVARCHAR)', 3, -1, FALSE, '[nu_processo]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->nu_processo->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['nu_processo'] = &$this->nu_processo;

		// nu_area
		$this->nu_area = new cField('processo', 'processo', 'x_nu_area', 'nu_area', '[nu_area]', 'CAST([nu_area] AS NVARCHAR)', 3, -1, FALSE, '[nu_area]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->nu_area->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['nu_area'] = &$this->nu_area;

		// nu_divisao
		$this->nu_divisao = new cField('processo', 'processo', 'x_nu_divisao', 'nu_divisao', '[nu_divisao]', 'CAST([nu_divisao] AS NVARCHAR)', 3, -1, FALSE, '[nu_divisao]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->nu_divisao->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['nu_divisao'] = &$this->nu_divisao;

		// nu_processoCobit5
		$this->nu_processoCobit5 = new cField('processo', 'processo', 'x_nu_processoCobit5', 'nu_processoCobit5', '[nu_processoCobit5]', 'CAST([nu_processoCobit5] AS NVARCHAR)', 3, -1, FALSE, '[nu_processoCobit5]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		...
		// nu_processoPai
		$this->nu_processoPai = new cField('processo', 'processo', 'x_nu_processoPai', 'nu_processoPai', '[nu_processoPai]', 'CAST([nu_processoPai] AS NVARCHAR)', 3, -1, FALSE, '[nu_processoPai]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->nu_processoPai->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['nu_processoPai'] = &$this->nu_processoPai;

		// no_processo
		$this->no_processo = new cField('processo', 'processo', 'x_no_processo', 'no_processo', '[no_processo]', '[no_processo]', 200, -1, FALSE, '[no_processo]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['no_processo'] = &$this->no_processo;

		// ds_processo
		$this->ds_processo = new cField('processo', 'processo', 'x_ds_processo', 'ds_processo', '[ds_processo]', '[ds_processo]', 201, -1, FALSE, '[ds_processo]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['ds_processo'] = &$this->ds_processo;

		// ic_ativo
		$this->ic_ativo = new cField('processo', 'processo', 'x_ic_ativo', 'ic_ativo', '[ic_ativo]', '[ic_ativo]', 129, -1, FALSE, '[ic_ativo]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function GetDetailUrl() {
		...
		// Detail url
		$sDetailUrl = "";
		if ($this->getCurrentDetailTable() == "atividade") {
			$sDetailUrl = $GLOBALS["atividade"]->GetListUrl() . "?showmaster=" . $this->TableVar;
			$sDetailUrl .= "&nu_processo=" . $this->nu_processo->CurrentValue;
		}
		if ($sDetailUrl == "") {
			$sDetailUrl = "processolist.php";
		}
		return $sDetailUrl;
	}
	...
	function Update(&$rs, $where = "", $rsold = NULL) {
		...
		// Cascade update detail field 'nu_processo'
		if (!is_null($rsold) && (isset($rs['nu_processo']) && $rsold['nu_processo'] <> $rs['nu_processo'])) {
			if (!isset($GLOBALS["atividade"])) $GLOBALS["atividade"] = new catividade();
			$rscascade = array();
			$rscascade['nu_processo'] = $rs['nu_processo']; 
			$GLOBALS["atividade"]->Update($rscascade, "[nu_processo] = " . ew_QuotedValue($rsold['nu_processo'], EW_DATATYPE_NUMBER));
		}
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			if (array_key_exists('nu_processo', $rs))
				ew_AddFilter($where, ew_QuotedName('nu_processo') . '=' . ew_QuotedValue($rs['nu_processo'], $this->nu_processo->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function Delete(&$rs, $where = "") {
		...
		// Cascade delete detail table 'atividade'
		if (!isset($GLOBALS["atividade"])) $GLOBALS["atividade"] = new catividade();
		$rscascade = array();
		$GLOBALS["atividade"]->Delete($rscascade, "[nu_processo] = " . ew_QuotedValue($rs['nu_processo'], EW_DATATYPE_NUMBER));
		return $conn->Execute($this->DeleteSQL($rs, $where));
	}

	// Key filter WHERE clause
	function SqlKeyFilter() {
		return "[nu_processo] = @nu_processo@";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		if (!is_numeric($this->nu_processo->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@nu_processo@", ew_AdjustSql($this->nu_processo->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("processoview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("processoview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl() {
		return "processoadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("processoedit.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("processoedit.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}
	...
	function GetCopyUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("processoadd.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("processoadd.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("processodelete.php", $this->UrlParm());
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->nu_processo->CurrentValue)) {
			$sUrl .= "nu_processo=" . urlencode($this->nu_processo->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKeys[] = @$_GET["nu_processo"]; // nu_processo
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->nu_processo->CurrentValue = $key;
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				...
				if ($this->nu_divisao->Exportable) $Doc->ExportCaption($this->nu_divisao);
				if ($this->nu_processoCobit5->Exportable) $Doc->ExportCaption($this->nu_processoCobit5);
				if ($this->nu_processoPai->Exportable) $Doc->ExportCaption($this->nu_processoPai);
				if ($this->no_processo->Exportable) $Doc->ExportCaption($this->no_processo);
				if ($this->ds_processo->Exportable) $Doc->ExportCaption($this->ds_processo);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					...
					if ($this->nu_divisao->Exportable) $Doc->ExportField($this->nu_divisao);
					if ($this->nu_processoCobit5->Exportable) $Doc->ExportField($this->nu_processoCobit5);
					if ($this->nu_processoPai->Exportable) $Doc->ExportField($this->nu_processoPai);
					if ($this->no_processo->Exportable) $Doc->ExportField($this->no_processo);
					if ($this->ds_processo->Exportable) $Doc->ExportField($this->ds_processo);
