...
//
// Table class for vwrdm_members
//
class cvwrdm_members extends cTable {
	var $id;
	var $user_id;
	var $project_id;
	var $created_on;
	...
	function __construct() {
		...
		$this->TableVar = 'vwrdm_members';
		$this->TableName = 'vwrdm_members';
		$this->TableType = 'VIEW';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new cField('vwrdm_members', 'vwrdm_members', 'x_id', 'id', '[id]', 'CAST([id] AS NVARCHAR)', 3, -1, FALSE, '[id]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// user_id
		$this->user_id = new cField('vwrdm_members', 'vwrdm_members', 'x_user_id', 'user_id', '[user_id]', 'CAST([user_id] AS NVARCHAR)', 3, -1, FALSE, '[user_id]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->user_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['user_id'] = &$this->user_id;

		// project_id
		$this->project_id = new cField('vwrdm_members', 'vwrdm_members', 'x_project_id', 'project_id', '[project_id]', 'CAST([project_id] AS NVARCHAR)', 3, -1, FALSE, '[project_id]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->project_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['project_id'] = &$this->project_id;

		// created_on
		$this->created_on = new cField('vwrdm_members', 'vwrdm_members', 'x_created_on', 'created_on', '[created_on]', '(REPLACE(STR(DAY([created_on]),2,0),\' \',\'0\') + \'/\' + REPLACE(STR(MONTH([created_on]),2,0),\' \',\'0\') + \'/\' + STR(YEAR([created_on]),4,0))', 135, 7, FALSE, '[created_on]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->created_on->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateDMY"));
		$this->fields['created_on'] = &$this->created_on;

		// mail_notification
		$this->mail_notification = new cField('vwrdm_members', 'vwrdm_members', 'x_mail_notification', 'mail_notification', '[mail_notification]', 'CAST([mail_notification] AS NVARCHAR)', 131, -1, FALSE, '[mail_notification]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->mail_notification->FldDefaultErrMsg = $Language->Phrase("IncorrectFloat");
	...
	// Update Table
	var $UpdateTable = "[db_owner].[vwrdm_members]";
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("vwrdm_membersview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("vwrdm_membersview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl() {
		return "vwrdm_membersadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("vwrdm_membersedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("vwrdm_membersadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("vwrdm_membersdelete.php", $this->UrlParm());
	}
