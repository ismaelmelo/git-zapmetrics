...
//
// Table class for indicadorvalor
//
class cindicadorvalor extends cTable {
	var $nu_indicador;
	var $nu_versao;
	var $dh_geracao;
	var $vr_indicadorNumerico;
	var $vr_indicadorTexto;
	...
	function __construct() {
		...
		$this->TableVar = 'indicadorvalor';
		$this->TableName = 'indicadorvalor';
		...
		// nu_indicador
		$this->nu_indicador = new cField('indicadorvalor', 'indicadorvalor', 'x_nu_indicador', 'nu_indicador', '[nu_indicador]', 'CAST([nu_indicador] AS NVARCHAR)', 3, -1, FALSE, '[nu_indicador]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->nu_indicador->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['nu_indicador'] = &$this->nu_indicador;

		// nu_versao
		$this->nu_versao = new cField('indicadorvalor', 'indicadorvalor', 'x_nu_versao', 'nu_versao', '[nu_versao]', 'CAST([nu_versao] AS NVARCHAR)', 3, -1, FALSE, '[nu_versao]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->nu_versao->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['nu_versao'] = &$this->nu_versao;

		// dh_geracao
		$this->dh_geracao = new cField('indicadorvalor', 'indicadorvalor', 'x_dh_geracao', 'dh_geracao', '[dh_geracao]', '(REPLACE(STR(DAY([dh_geracao]),2,0),\' \',\'0\') + \'/\' + REPLACE(STR(MONTH([dh_geracao]),2,0),\' \',\'0\') + \'/\' + STR(YEAR([dh_geracao]),4,0))', 135, 11, FALSE, '[dh_geracao]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->dh_geracao->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateDMY"));
		$this->fields['dh_geracao'] = &$this->dh_geracao;

		// vr_indicadorNumerico
		$this->vr_indicadorNumerico = new cField('indicadorvalor', 'indicadorvalor', 'x_vr_indicadorNumerico', 'vr_indicadorNumerico', '[vr_indicadorNumerico]', 'CAST([vr_indicadorNumerico] AS NVARCHAR)', 131, -1, FALSE, '[vr_indicadorNumerico]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->vr_indicadorNumerico->FldDefaultErrMsg = $Language->Phrase("IncorrectFloat");
		$this->fields['vr_indicadorNumerico'] = &$this->vr_indicadorNumerico;

		// vr_indicadorTexto
		$this->vr_indicadorTexto = new cField('indicadorvalor', 'indicadorvalor', 'x_vr_indicadorTexto', 'vr_indicadorTexto', '[vr_indicadorTexto]', '[vr_indicadorTexto]', 200, -1, FALSE, '[vr_indicadorTexto]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['vr_indicadorTexto'] = &$this->vr_indicadorTexto;
	}
	...
	function GetDetailUrl() {
		...
		// Detail url
		$sDetailUrl = "";
		if ($this->getCurrentDetailTable() == "indicadorversao") {
			$sDetailUrl = $GLOBALS["indicadorversao"]->GetListUrl() . "?showmaster=" . $this->TableVar;
			$sDetailUrl .= "&nu_indicador=" . $this->nu_indicador->CurrentValue;
			$sDetailUrl .= "&nu_versao=" . $this->nu_versao->CurrentValue;
		}
		if ($sDetailUrl == "") {
			$sDetailUrl = "indicadorvalorlist.php";
		}
		return $sDetailUrl;
	}
	...
	// Update Table
	var $UpdateTable = "[dbo].[indicadorvalor]";
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			...
			if (array_key_exists('dh_geracao', $rs))
				ew_AddFilter($where, ew_QuotedName('dh_geracao') . '=' . ew_QuotedValue($rs['dh_geracao'], $this->dh_geracao->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function SqlKeyFilter() {
		return "[nu_indicador] = @nu_indicador@ AND [nu_versao] = @nu_versao@ AND [dh_geracao] = '@dh_geracao@'";
	}
	...
	function KeyFilter() {
		...
		$sKeyFilter = str_replace("@dh_geracao@", ew_AdjustSql(ew_UnFormatDateTime($this->dh_geracao->CurrentValue,11)), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("indicadorvalorview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("indicadorvalorview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl() {
		return "indicadorvaloradd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("indicadorvaloredit.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("indicadorvaloredit.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}
	...
	function GetCopyUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("indicadorvaloradd.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("indicadorvaloradd.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("indicadorvalordelete.php", $this->UrlParm());
	}
	...
	function KeyUrl($url, $parm = "") {
		...
		if (!is_null($this->dh_geracao->CurrentValue)) {
			$sUrl .= "&dh_geracao=" . urlencode($this->dh_geracao->CurrentValue);
		} else {
			return "javascript:alert(ewLanguage.Phrase('InvalidRecord'));";
		}
		return $sUrl;
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKey[] = @$_GET["dh_geracao"]; // dh_geracao
			$arKeys[] = $arKey;
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->dh_geracao->CurrentValue = $key[2];
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				...
				if ($this->dh_geracao->Exportable) $Doc->ExportCaption($this->dh_geracao);
				if ($this->vr_indicadorNumerico->Exportable) $Doc->ExportCaption($this->vr_indicadorNumerico);
				if ($this->vr_indicadorTexto->Exportable) $Doc->ExportCaption($this->vr_indicadorTexto);
			} else {
				if ($this->nu_indicador->Exportable) $Doc->ExportCaption($this->nu_indicador);
				if ($this->nu_versao->Exportable) $Doc->ExportCaption($this->nu_versao);
				if ($this->dh_geracao->Exportable) $Doc->ExportCaption($this->dh_geracao);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					...
					if ($this->dh_geracao->Exportable) $Doc->ExportField($this->dh_geracao);
					if ($this->vr_indicadorNumerico->Exportable) $Doc->ExportField($this->vr_indicadorNumerico);
					if ($this->vr_indicadorTexto->Exportable) $Doc->ExportField($this->vr_indicadorTexto);
				} else {
					if ($this->nu_indicador->Exportable) $Doc->ExportField($this->nu_indicador);
					if ($this->nu_versao->Exportable) $Doc->ExportField($this->nu_versao);
					if ($this->dh_geracao->Exportable) $Doc->ExportField($this->dh_geracao);
