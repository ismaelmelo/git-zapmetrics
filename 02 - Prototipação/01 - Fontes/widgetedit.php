...
$widget_edit = NULL; // Initialize page object first

class cwidget_edit extends cwidget {
	...
	function Page_Main() {
		...
		if ($this->nu_widget->CurrentValue == "")
			$this->Page_Terminate("widgetlist.php"); // Invalid key, return to list
		...
		switch ($this->CurrentAction) {
			case "I": // Get a record to display
				if (!$this->LoadRow()) { // Load record based on key
					...
					$this->Page_Terminate("widgetlist.php"); // No matching record, return to list
				}
				break;
			Case "U": // Update
				...
				if ($this->EditRow()) { // Update record based on key
					...
					if (ew_GetPageName($sReturnUrl) == "widgetview.php")
	...
	function LoadFormValues() {
		...
		if (!$this->no_widget->FldIsDetailKey) {
			$this->no_widget->setFormValue($objForm->GetValue("x_no_widget"));
		}
	...
	function RestoreFormValues() {
		...
		$this->no_widget->CurrentValue = $this->no_widget->FormValue;
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// no_widget
			$this->no_widget->SetDbValueDef($rsnew, $this->no_widget->CurrentValue, NULL, $this->no_widget->ReadOnly);

			// im_arquivo
			if (!($this->im_arquivo->ReadOnly) && !$this->im_arquivo->Upload->KeepFile) {
				$this->im_arquivo->Upload->DbValue = $rs->fields('im_arquivo'); // Get original value
...
// Create page object
if (!isset($widget_edit)) $widget_edit = new cwidget_edit();

// Page init
$widget_edit->Page_Init();

// Page main
$widget_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$widget_edit->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var widget_edit = new ew_Page("widget_edit");
widget_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = widget_edit.PageID; // For backward compatibility

// Form object
var fwidgetedit = new ew_Form("fwidgetedit");

// Validate form
fwidgetedit.Validate = function() {
...
fwidgetedit.Form_CustomValidate = 
...
fwidgetedit.ValidateRequired = true;
<?php } else { ?>
fwidgetedit.ValidateRequired = false; 
<?php } ?>
...
<?php $widget_edit->ShowPageHeader(); ?>
<?php
$widget_edit->ShowMessage();
?>
<form name="fwidgetedit" id="fwidgetedit" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="widget">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_widgetedit" class="table table-bordered table-striped">
...
<?php echo $widget->no_widget->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($widget->im_arquivo->Visible) { // im_arquivo ?>
	<tr id="r_im_arquivo">
		<td><span id="elh_widget_im_arquivo"><?php echo $widget->im_arquivo->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
...
<?php if (@$_POST["fa_x_im_arquivo"] == "0") { ?>
<input type="hidden" name="fa_x_im_arquivo" id= "fa_x_im_arquivo" value="0">
<?php } else { ?>
<input type="hidden" name="fa_x_im_arquivo" id= "fa_x_im_arquivo" value="1">
<?php } ?>
...
<?php echo $widget->im_arquivo->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($widget->ic_ativo->Visible) { // ic_ativo ?>
	<tr id="r_ic_ativo">
		<td><span id="elh_widget_ic_ativo"><?php echo $widget->ic_ativo->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
...
<?php echo $widget->ic_ativo->CustomMsg ?></td>
	</tr>
<?php } ?>
</table>
</td></tr></table>
<input type="hidden" data-field="x_nu_widget" name="x_nu_widget" id="x_nu_widget" value="<?php echo ew_HtmlEncode($widget->nu_widget->CurrentValue) ?>">
