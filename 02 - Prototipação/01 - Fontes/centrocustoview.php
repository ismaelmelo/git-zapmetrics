...
$centrocusto_view = NULL; // Initialize page object first

class ccentrocusto_view extends ccentrocusto {
	...
	// Page object name
	var $PageObjName = 'centrocusto_view';
	...
	function __construct() {
		...
		if (@$_GET["nu_centroCusto"] <> "") {
			$this->RecKey["nu_centroCusto"] = $_GET["nu_centroCusto"];
			$KeyUrl .= "&nu_centroCusto=" . urlencode($this->RecKey["nu_centroCusto"]);
		}
	...
	function Page_Init() {
		...
		if (@$_GET["nu_centroCusto"] <> "") {
			...
			$gsExportFile .= ew_StripSlashes($_GET["nu_centroCusto"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["nu_centroCusto"] <> "") {
				...
				$this->RecKey["nu_centroCusto"] = $this->nu_centroCusto->QueryStringValue;
			} else {
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					...
					if ($this->TotalRecs <= 0) { // No record found
						...
						$this->Page_Terminate("centrocustolist.php"); // Return to list page
						...
						while (!$this->Recordset->EOF) {
							if (strval($this->nu_centroCusto->CurrentValue) == strval($this->Recordset->fields('nu_centroCusto'))) {
					...
					if (!$bMatchRecord) {
						if ($this->getSuccessMessage() == "" && $this->getFailureMessage() == "")
							$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record message
						$sReturnUrl = "centrocustolist.php"; // No matching record, return to list
					} else {
			...
			$sReturnUrl = "centrocustolist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupOtherOptions() {
		...
		$body = "<a class=\"ewAction ewDetailList\" href=\"" . ew_HtmlEncode("projeto_centrocustolist.php?" . EW_TABLE_SHOW_MASTER . "=centrocusto&nu_centroCusto=" . strval($this->nu_centroCusto->CurrentValue) . "") . "\">" . $body . "</a>";
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a id=\"emf_centrocusto\" href=\"javascript:void(0);\" class=\"ewExportLink ewEmail\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_centrocusto',hdr:ewLanguage.Phrase('ExportToEmail'),f:document.fcentrocustoview,key:" . ew_ArrayToJsonAttr($this->RecKey) . ",sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
...
// Create page object
if (!isset($centrocusto_view)) $centrocusto_view = new ccentrocusto_view();

// Page init
$centrocusto_view->Page_Init();

// Page main
$centrocusto_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$centrocusto_view->Page_Render();
?>
...
// Page object
var centrocusto_view = new ew_Page("centrocusto_view");
centrocusto_view.PageID = "view"; // Page ID
var EW_PAGE_ID = centrocusto_view.PageID; // For backward compatibility

// Form object
var fcentrocustoview = new ew_Form("fcentrocustoview");

// Form_CustomValidate event
fcentrocustoview.Form_CustomValidate = 
...
fcentrocustoview.ValidateRequired = true;
<?php } else { ?>
fcentrocustoview.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fcentrocustoview.Lists["x_nu_area"] = {"LinkField":"x_nu_area","Ajax":null,"AutoFill":false,"DisplayFields":["x_no_area","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fcentrocustoview.Lists["x_nu_divisao"] = {"LinkField":"x_nu_divisao","Ajax":null,"AutoFill":false,"DisplayFields":["x_no_divisao","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fcentrocustoview.Lists["x_nu_centroCustoPai"] = {"LinkField":"x_nu_centroCusto","Ajax":null,"AutoFill":false,"DisplayFields":["x_co_alternativo","x_no_centroCusto","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $centrocusto_view->ExportOptions->Render("body") ?>
<?php if (!$centrocusto_view->ExportOptions->UseDropDownButton) { ?>
</div>
<div class="ewViewOtherOptions">
<?php } ?>
<?php
	foreach ($centrocusto_view->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<?php } ?>
<?php $centrocusto_view->ShowPageHeader(); ?>
<?php
$centrocusto_view->ShowMessage();
?>
<form name="fcentrocustoview" id="fcentrocustoview" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="centrocusto">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_centrocustoview" class="table table-bordered table-striped">
...
<?php echo $centrocusto->nu_area->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<span<?php echo $centrocusto->nu_divisao->ViewAttributes() ?>>
<?php echo $centrocusto->nu_divisao->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<span<?php echo $centrocusto->nu_centroCustoPai->ViewAttributes() ?>>
<?php echo $centrocusto->nu_centroCustoPai->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($centrocusto->co_alternativo->Visible) { // co_alternativo ?>
	<tr id="r_co_alternativo">
		<td><span id="elh_centrocusto_co_alternativo"><?php echo $centrocusto->co_alternativo->FldCaption() ?></span></td>
...
<?php echo $centrocusto->co_alternativo->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($centrocusto->no_centroCusto->Visible) { // no_centroCusto ?>
	<tr id="r_no_centroCusto">
		<td><span id="elh_centrocusto_no_centroCusto"><?php echo $centrocusto->no_centroCusto->FldCaption() ?></span></td>
...
<?php echo $centrocusto->no_centroCusto->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($centrocusto->ic_ativo->Visible) { // ic_ativo ?>
	<tr id="r_ic_ativo">
		<td><span id="elh_centrocusto_ic_ativo"><?php echo $centrocusto->ic_ativo->FldCaption() ?></span></td>
...
<?php echo $centrocusto->ic_ativo->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<table class="ewPager">
<tr><td>
<?php if (!isset($centrocusto_view->Pager)) $centrocusto_view->Pager = new cNumericPager($centrocusto_view->StartRec, $centrocusto_view->DisplayRecs, $centrocusto_view->TotalRecs, $centrocusto_view->RecRange) ?>
<?php if ($centrocusto_view->Pager->RecordCount > 0) { ?>
<table cellspacing="0" class="ewStdTable"><tbody><tr><td>
<div class="pagination"><ul>
	<?php if ($centrocusto_view->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $centrocusto_view->PageUrl() ?>start=<?php echo $centrocusto_view->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($centrocusto_view->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $centrocusto_view->PageUrl() ?>start=<?php echo $centrocusto_view->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($centrocusto_view->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $centrocusto_view->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($centrocusto_view->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $centrocusto_view->PageUrl() ?>start=<?php echo $centrocusto_view->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($centrocusto_view->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $centrocusto_view->PageUrl() ?>start=<?php echo $centrocusto_view->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</td>
</tr></tbody></table>
...
<?php } ?>
<?php
	if (in_array("projeto_centrocusto", explode(",", $centrocusto->getCurrentDetailTable())) && $projeto_centrocusto->DetailView) {
?>
<?php include_once "projeto_centrocustogrid.php" ?>
<?php } ?>
</form>
<script type="text/javascript">
fcentrocustoview.Init();
</script>
<?php
$centrocusto_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($centrocusto->Export == "") { ?>
...
<?php
$centrocusto_view->Page_Terminate();
?>
