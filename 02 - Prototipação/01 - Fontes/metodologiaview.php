...
$metodologia_view = NULL; // Initialize page object first

class cmetodologia_view extends cmetodologia {
	...
	// Page object name
	var $PageObjName = 'metodologia_view';
	...
	function __construct() {
		...
		if (@$_GET["nu_metodologia"] <> "") {
			$this->RecKey["nu_metodologia"] = $_GET["nu_metodologia"];
			$KeyUrl .= "&nu_metodologia=" . urlencode($this->RecKey["nu_metodologia"]);
		}
	...
	function Page_Init() {
		...
		if (@$_GET["nu_metodologia"] <> "") {
			...
			$gsExportFile .= ew_StripSlashes($_GET["nu_metodologia"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["nu_metodologia"] <> "") {
				...
				$this->RecKey["nu_metodologia"] = $this->nu_metodologia->QueryStringValue;
			} else {
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					...
					if ($this->TotalRecs <= 0) { // No record found
						...
						$this->Page_Terminate("metodologialist.php"); // Return to list page
						...
						while (!$this->Recordset->EOF) {
							if (strval($this->nu_metodologia->CurrentValue) == strval($this->Recordset->fields('nu_metodologia'))) {
					...
					if (!$bMatchRecord) {
						if ($this->getSuccessMessage() == "" && $this->getFailureMessage() == "")
							$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record message
						$sReturnUrl = "metodologialist.php"; // No matching record, return to list
					} else {
			...
			$sReturnUrl = "metodologialist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupOtherOptions() {
		...
		// Detail table 'roteiro'
		$body = $Language->TablePhrase("roteiro", "TblCaption");
		$body = "<a class=\"ewAction ewDetailList\" href=\"" . ew_HtmlEncode("roteirolist.php?" . EW_TABLE_SHOW_MASTER . "=metodologia&nu_metodologia=" . strval($this->nu_metodologia->CurrentValue) . "") . "\">" . $body . "</a>";
		$item = &$option->Add("detail_roteiro");
		$item->Body = $body;
		$item->Visible = $Security->AllowList(CurrentProjectID() . 'roteiro');
		if ($item->Visible) {
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a id=\"emf_metodologia\" href=\"javascript:void(0);\" class=\"ewExportLink ewEmail\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_metodologia',hdr:ewLanguage.Phrase('ExportToEmail'),f:document.fmetodologiaview,key:" . ew_ArrayToJsonAttr($this->RecKey) . ",sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("roteiro", $DetailTblVar)) {
				...
				if ($GLOBALS["roteiro_grid"]->DetailView) {
					$GLOBALS["roteiro_grid"]->CurrentMode = "view";
...
// Create page object
if (!isset($metodologia_view)) $metodologia_view = new cmetodologia_view();

// Page init
$metodologia_view->Page_Init();

// Page main
$metodologia_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$metodologia_view->Page_Render();
?>
...
// Page object
var metodologia_view = new ew_Page("metodologia_view");
metodologia_view.PageID = "view"; // Page ID
var EW_PAGE_ID = metodologia_view.PageID; // For backward compatibility

// Form object
var fmetodologiaview = new ew_Form("fmetodologiaview");

// Form_CustomValidate event
fmetodologiaview.Form_CustomValidate = 
...
fmetodologiaview.ValidateRequired = true;
<?php } else { ?>
fmetodologiaview.ValidateRequired = false; 
<?php } ?>
...
<?php $metodologia_view->ExportOptions->Render("body") ?>
<?php if (!$metodologia_view->ExportOptions->UseDropDownButton) { ?>
</div>
<div class="ewViewOtherOptions">
<?php } ?>
<?php
	foreach ($metodologia_view->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<?php } ?>
<?php $metodologia_view->ShowPageHeader(); ?>
<?php
$metodologia_view->ShowMessage();
?>
<form name="fmetodologiaview" id="fmetodologiaview" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="metodologia">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_metodologiaview" class="table table-bordered table-striped">
<?php if ($metodologia->no_metodologia->Visible) { // no_metodologia ?>
	<tr id="r_no_metodologia">
		<td><span id="elh_metodologia_no_metodologia"><?php echo $metodologia->no_metodologia->FldCaption() ?></span></td>
...
<?php echo $metodologia->no_metodologia->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<span<?php echo $metodologia->ds_metodologia->ViewAttributes() ?>>
<?php echo $metodologia->ds_metodologia->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($metodologia->ic_tpModeloDev->Visible) { // ic_tpModeloDev ?>
	<tr id="r_ic_tpModeloDev">
		<td><span id="elh_metodologia_ic_tpModeloDev"><?php echo $metodologia->ic_tpModeloDev->FldCaption() ?></span></td>
...
<?php echo $metodologia->ic_tpModeloDev->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($metodologia->ic_ativo->Visible) { // ic_ativo ?>
	<tr id="r_ic_ativo">
		<td><span id="elh_metodologia_ic_ativo"><?php echo $metodologia->ic_ativo->FldCaption() ?></span></td>
...
<?php echo $metodologia->ic_ativo->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php echo $metodologia->nu_ordem->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<table class="ewPager">
<tr><td>
<?php if (!isset($metodologia_view->Pager)) $metodologia_view->Pager = new cNumericPager($metodologia_view->StartRec, $metodologia_view->DisplayRecs, $metodologia_view->TotalRecs, $metodologia_view->RecRange) ?>
<?php if ($metodologia_view->Pager->RecordCount > 0) { ?>
<table cellspacing="0" class="ewStdTable"><tbody><tr><td>
<div class="pagination"><ul>
	<?php if ($metodologia_view->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $metodologia_view->PageUrl() ?>start=<?php echo $metodologia_view->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($metodologia_view->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $metodologia_view->PageUrl() ?>start=<?php echo $metodologia_view->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($metodologia_view->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $metodologia_view->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($metodologia_view->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $metodologia_view->PageUrl() ?>start=<?php echo $metodologia_view->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($metodologia_view->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $metodologia_view->PageUrl() ?>start=<?php echo $metodologia_view->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</td>
</tr></tbody></table>
...
<?php } ?>
<?php
	if (in_array("roteiro", explode(",", $metodologia->getCurrentDetailTable())) && $roteiro->DetailView) {
?>
<?php include_once "roteirogrid.php" ?>
<?php } ?>
</form>
<script type="text/javascript">
fmetodologiaview.Init();
</script>
<?php
$metodologia_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($metodologia->Export == "") { ?>
...
<?php
$metodologia_view->Page_Terminate();
?>
