...
$parsisp_versao_view = NULL; // Initialize page object first

class cparsisp_versao_view extends cparsisp_versao {
	...
	// Page object name
	var $PageObjName = 'parsisp_versao_view';
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					...
					if ($this->TotalRecs <= 0) { // No record found
						...
						$this->Page_Terminate("parsisp_versaolist.php"); // Return to list page
						...
						while (!$this->Recordset->EOF) {
							if (strval($this->nu_parSisp->CurrentValue) == strval($this->Recordset->fields('nu_parSisp')) && strval($this->nu_versao->CurrentValue) == strval($this->Recordset->fields('nu_versao'))) {
					...
					if (!$bMatchRecord) {
						if ($this->getSuccessMessage() == "" && $this->getFailureMessage() == "")
							$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record message
						$sReturnUrl = "parsisp_versaolist.php"; // No matching record, return to list
					} else {
			...
			$sReturnUrl = "parsisp_versaolist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a id=\"emf_parsisp_versao\" href=\"javascript:void(0);\" class=\"ewExportLink ewEmail\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_parsisp_versao',hdr:ewLanguage.Phrase('ExportToEmail'),f:document.fparsisp_versaoview,key:" . ew_ArrayToJsonAttr($this->RecKey) . ",sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
...
// Create page object
if (!isset($parsisp_versao_view)) $parsisp_versao_view = new cparsisp_versao_view();

// Page init
$parsisp_versao_view->Page_Init();

// Page main
$parsisp_versao_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$parsisp_versao_view->Page_Render();
?>
...
// Page object
var parsisp_versao_view = new ew_Page("parsisp_versao_view");
parsisp_versao_view.PageID = "view"; // Page ID
var EW_PAGE_ID = parsisp_versao_view.PageID; // For backward compatibility

// Form object
var fparsisp_versaoview = new ew_Form("fparsisp_versaoview");

// Form_CustomValidate event
fparsisp_versaoview.Form_CustomValidate = 
...
fparsisp_versaoview.ValidateRequired = true;
<?php } else { ?>
fparsisp_versaoview.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fparsisp_versaoview.Lists["x_nu_parSisp"] = {"LinkField":"x_nu_parSisp","Ajax":null,"AutoFill":false,"DisplayFields":["x_no_parSisp","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fparsisp_versaoview.Lists["x_nu_usuarioResp"] = {"LinkField":"x_nu_usuario","Ajax":null,"AutoFill":false,"DisplayFields":["x_no_usuario","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $parsisp_versao_view->ExportOptions->Render("body") ?>
<?php if (!$parsisp_versao_view->ExportOptions->UseDropDownButton) { ?>
</div>
<div class="ewViewOtherOptions">
<?php } ?>
<?php
	foreach ($parsisp_versao_view->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<?php } ?>
<?php $parsisp_versao_view->ShowPageHeader(); ?>
<?php
$parsisp_versao_view->ShowMessage();
?>
<form name="fparsisp_versaoview" id="fparsisp_versaoview" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="parsisp_versao">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_parsisp_versaoview" class="table table-bordered table-striped">
<?php if ($parsisp_versao->nu_parSisp->Visible) { // nu_parSisp ?>
	<tr id="r_nu_parSisp">
		<td><span id="elh_parsisp_versao_nu_parSisp"><?php echo $parsisp_versao->nu_parSisp->FldCaption() ?></span></td>
		<td<?php echo $parsisp_versao->nu_parSisp->CellAttributes() ?>>
<span id="el_parsisp_versao_nu_parSisp" class="control-group">
...
<?php if ($parsisp_versao->nu_versao->Visible) { // nu_versao ?>
	<tr id="r_nu_versao">
		<td><span id="elh_parsisp_versao_nu_versao"><?php echo $parsisp_versao->nu_versao->FldCaption() ?></span></td>
...
<?php if ($parsisp_versao->vr_parSisp->Visible) { // vr_parSisp ?>
	<tr id="r_vr_parSisp">
		<td><span id="elh_parsisp_versao_vr_parSisp"><?php echo $parsisp_versao->vr_parSisp->FldCaption() ?></span></td>
...
<span<?php echo $parsisp_versao->ds_codigoSql->ViewAttributes() ?>>
<?php echo $parsisp_versao->ds_codigoSql->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($parsisp_versao->nu_usuarioResp->Visible) { // nu_usuarioResp ?>
	<tr id="r_nu_usuarioResp">
		<td><span id="elh_parsisp_versao_nu_usuarioResp"><?php echo $parsisp_versao->nu_usuarioResp->FldCaption() ?></span></td>
		<td<?php echo $parsisp_versao->nu_usuarioResp->CellAttributes() ?>>
<span id="el_parsisp_versao_nu_usuarioResp" class="control-group">
...
<span<?php echo $parsisp_versao->ds_versao->ViewAttributes() ?>>
<?php echo $parsisp_versao->ds_versao->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($parsisp_versao->dh_inclusao->Visible) { // dh_inclusao ?>
	<tr id="r_dh_inclusao">
		<td><span id="elh_parsisp_versao_dh_inclusao"><?php echo $parsisp_versao->dh_inclusao->FldCaption() ?></span></td>
		<td<?php echo $parsisp_versao->dh_inclusao->CellAttributes() ?>>
<span id="el_parsisp_versao_dh_inclusao" class="control-group">
...
<table class="ewPager">
<tr><td>
<?php if (!isset($parsisp_versao_view->Pager)) $parsisp_versao_view->Pager = new cNumericPager($parsisp_versao_view->StartRec, $parsisp_versao_view->DisplayRecs, $parsisp_versao_view->TotalRecs, $parsisp_versao_view->RecRange) ?>
<?php if ($parsisp_versao_view->Pager->RecordCount > 0) { ?>
<table cellspacing="0" class="ewStdTable"><tbody><tr><td>
<div class="pagination"><ul>
	<?php if ($parsisp_versao_view->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $parsisp_versao_view->PageUrl() ?>start=<?php echo $parsisp_versao_view->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($parsisp_versao_view->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $parsisp_versao_view->PageUrl() ?>start=<?php echo $parsisp_versao_view->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($parsisp_versao_view->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $parsisp_versao_view->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($parsisp_versao_view->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $parsisp_versao_view->PageUrl() ?>start=<?php echo $parsisp_versao_view->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($parsisp_versao_view->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $parsisp_versao_view->PageUrl() ?>start=<?php echo $parsisp_versao_view->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</td>
</tr></tbody></table>
...
<script type="text/javascript">
fparsisp_versaoview.Init();
</script>
<?php
$parsisp_versao_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($parsisp_versao->Export == "") { ?>
...
<?php
$parsisp_versao_view->Page_Terminate();
?>
