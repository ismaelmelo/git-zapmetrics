...
class cpei extends cTable {
	...
	function __construct() {
		...
		// nu_pei
		$this->nu_pei = new cField('pei', 'pei', 'x_nu_pei', 'nu_pei', '[nu_pei]', 'CAST([nu_pei] AS NVARCHAR)', 3, -1, FALSE, '[nu_pei]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->nu_pei->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['nu_pei'] = &$this->nu_pei;

		// nu_periodoPei
		$this->nu_periodoPei = new cField('pei', 'pei', 'x_nu_periodoPei', 'nu_periodoPei', '[nu_periodoPei]', 'CAST([nu_periodoPei] AS NVARCHAR)', 3, -1, FALSE, '[EV__nu_periodoPei]', TRUE, TRUE, TRUE, 'FORMATTED TEXT');
		$this->nu_periodoPei->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['nu_periodoPei'] = &$this->nu_periodoPei;

		// no_capaPei
		$this->no_capaPei = new cField('pei', 'pei', 'x_no_capaPei', 'no_capaPei', '[no_capaPei]', '[no_capaPei]', 200, -1, FALSE, '[no_capaPei]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['no_capaPei'] = &$this->no_capaPei;

		// ds_introducao
		$this->ds_introducao = new cField('pei', 'pei', 'x_ds_introducao', 'ds_introducao', '[ds_introducao]', '[ds_introducao]', 201, -1, FALSE, '[ds_introducao]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['ds_introducao'] = &$this->ds_introducao;

		// ds_missao
		$this->ds_missao = new cField('pei', 'pei', 'x_ds_missao', 'ds_missao', '[ds_missao]', '[ds_missao]', 201, -1, FALSE, '[ds_missao]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['ds_missao'] = &$this->ds_missao;

		// ds_visao
		$this->ds_visao = new cField('pei', 'pei', 'x_ds_visao', 'ds_visao', '[ds_visao]', '[ds_visao]', 201, -1, FALSE, '[ds_visao]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['ds_visao'] = &$this->ds_visao;

		// ds_valores
		$this->ds_valores = new cField('pei', 'pei', 'x_ds_valores', 'ds_valores', '[ds_valores]', '[ds_valores]', 201, -1, FALSE, '[ds_valores]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['ds_valores'] = &$this->ds_valores;

		// ic_situacao
		$this->ic_situacao = new cField('pei', 'pei', 'x_ic_situacao', 'ic_situacao', '[ic_situacao]', '[ic_situacao]', 129, -1, FALSE, '[ic_situacao]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function SqlSelectList() { // Select for List page
		return "SELECT * FROM (" .
			"SELECT *, (SELECT TOP 1 [nu_anoInicio] + '" . ew_ValueSeparator(1, $this->nu_periodoPei) . "' + [nu_anoFim] FROM [periodopei] [EW_TMP_LOOKUPTABLE] WHERE [EW_TMP_LOOKUPTABLE].[nu_periodoPei] = [pei].[nu_periodoPei]) AS [EV__nu_periodoPei] FROM [dbo].[pei]" .
			") [EW_TMP_TABLE]";
	}
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			if (array_key_exists('nu_pei', $rs))
				ew_AddFilter($where, ew_QuotedName('nu_pei') . '=' . ew_QuotedValue($rs['nu_pei'], $this->nu_pei->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function SqlKeyFilter() {
		return "[nu_pei] = @nu_pei@";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		if (!is_numeric($this->nu_pei->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@nu_pei@", ew_AdjustSql($this->nu_pei->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("peiview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("peiview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}
	...
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("peiedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("peiadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("peidelete.php", $this->UrlParm());
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->nu_pei->CurrentValue)) {
			$sUrl .= "nu_pei=" . urlencode($this->nu_pei->CurrentValue);
		} else {
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				if ($this->nu_pei->Exportable) $Doc->ExportCaption($this->nu_pei);
				if ($this->nu_periodoPei->Exportable) $Doc->ExportCaption($this->nu_periodoPei);
				if ($this->no_capaPei->Exportable) $Doc->ExportCaption($this->no_capaPei);
				if ($this->ds_introducao->Exportable) $Doc->ExportCaption($this->ds_introducao);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					if ($this->nu_pei->Exportable) $Doc->ExportField($this->nu_pei);
					if ($this->nu_periodoPei->Exportable) $Doc->ExportField($this->nu_periodoPei);
					if ($this->no_capaPei->Exportable) $Doc->ExportField($this->no_capaPei);
					if ($this->ds_introducao->Exportable) $Doc->ExportField($this->ds_introducao);
