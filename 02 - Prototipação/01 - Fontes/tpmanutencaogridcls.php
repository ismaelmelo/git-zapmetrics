...
$tpmanutencao_grid = NULL; // Initialize page object first

class ctpmanutencao_grid extends ctpmanutencao {
	...
	// Page object name
	var $PageObjName = 'tpmanutencao_grid';

	// Grid form hidden field names
	var $FormName = 'ftpmanutencaogrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["tpmanutencao"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->nu_tpManutencao->CurrentValue;
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_no_tpManutencao") && $objForm->HasValue("o_no_tpManutencao") && $this->no_tpManutencao->CurrentValue <> $this->no_tpManutencao->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_ic_modeloCalculo") && $objForm->HasValue("o_ic_modeloCalculo") && $this->ic_modeloCalculo->CurrentValue <> $this->ic_modeloCalculo->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_ic_utilizaFaseRoteiroCalculo") && $objForm->HasValue("o_ic_utilizaFaseRoteiroCalculo") && $this->ic_utilizaFaseRoteiroCalculo->CurrentValue <> $this->ic_utilizaFaseRoteiroCalculo->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_nu_parametro") && $objForm->HasValue("o_nu_parametro") && $this->nu_parametro->CurrentValue <> $this->nu_parametro->OldValue)
			return FALSE;
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->nu_tpManutencao->CurrentValue . "\">";
		}
	...
	function LoadDefaultValues() {
		...
		$this->ic_utilizaFaseRoteiroCalculo->OldValue = $this->ic_utilizaFaseRoteiroCalculo->CurrentValue;
	...
	function LoadFormValues() {
		...
		$this->no_tpManutencao->setOldValue($objForm->GetValue("o_no_tpManutencao"));
		if (!$this->ic_modeloCalculo->FldIsDetailKey) {
			$this->ic_modeloCalculo->setFormValue($objForm->GetValue("x_ic_modeloCalculo"));
		}
		$this->ic_modeloCalculo->setOldValue($objForm->GetValue("o_ic_modeloCalculo"));
		...
		$this->ic_utilizaFaseRoteiroCalculo->setOldValue($objForm->GetValue("o_ic_utilizaFaseRoteiroCalculo"));
		if (!$this->nu_parametro->FldIsDetailKey) {
			$this->nu_parametro->setFormValue($objForm->GetValue("x_nu_parametro"));
		}
		$this->nu_parametro->setOldValue($objForm->GetValue("o_nu_parametro"));
		...
		if (!$this->nu_tpManutencao->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->nu_tpManutencao->CurrentValue = strval($arKeys[0]); // nu_tpManutencao
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
