...
$criterioavaliacao_edit = NULL; // Initialize page object first

class ccriterioavaliacao_edit extends ccriterioavaliacao {
	...
	// Page object name
	var $PageObjName = 'criterioavaliacao_edit';
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->nu_alternativaAvaliacao->CurrentValue == "")
			$this->Page_Terminate("criterioavaliacaolist.php"); // Invalid key, return to list
	...
	function LoadFormValues() {
		...
		// Load from form
		global $objForm;
		if (!$this->nu_alternativaAvaliacao->FldIsDetailKey)
			$this->nu_alternativaAvaliacao->setFormValue($objForm->GetValue("x_nu_alternativaAvaliacao"));
	...
	function RestoreFormValues() {
		global $objForm;
		$this->LoadRow();
		$this->nu_alternativaAvaliacao->CurrentValue = $this->nu_alternativaAvaliacao->FormValue;
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			// nu_alternativaAvaliacao
			$this->nu_alternativaAvaliacao->EditCustomAttributes = "";
			$this->nu_alternativaAvaliacao->EditValue = $this->nu_alternativaAvaliacao->CurrentValue;
			$this->nu_alternativaAvaliacao->ViewCustomAttributes = "";
			...
			$sSqlWrk .= " ORDER BY [no_criterioPrioridade] ASC";
				$rswrk = $conn->Execute($sSqlWrk);
				if ($rswrk && !$rswrk->EOF) { // Lookup values found
					$this->nu_criterioPrioridade->EditValue = $rswrk->fields('DispFld');
					$rswrk->Close();
				} else {
					$this->nu_criterioPrioridade->EditValue = $this->nu_criterioPrioridade->CurrentValue;
				}
			} else {
				$this->nu_criterioPrioridade->EditValue = NULL;
			}
			$this->nu_criterioPrioridade->ViewCustomAttributes = "";
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// no_alternativa
			$this->no_alternativa->SetDbValueDef($rsnew, $this->no_alternativa->CurrentValue, "", $this->no_alternativa->ReadOnly);

			// vr_alternativa
			$this->vr_alternativa->SetDbValueDef($rsnew, $this->vr_alternativa->CurrentValue, 0, $this->vr_alternativa->ReadOnly);
...
// Create page object
if (!isset($criterioavaliacao_edit)) $criterioavaliacao_edit = new ccriterioavaliacao_edit();

// Page init
$criterioavaliacao_edit->Page_Init();

// Page main
$criterioavaliacao_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$criterioavaliacao_edit->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var criterioavaliacao_edit = new ew_Page("criterioavaliacao_edit");
criterioavaliacao_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = criterioavaliacao_edit.PageID; // For backward compatibility

// Form object
var fcriterioavaliacaoedit = new ew_Form("fcriterioavaliacaoedit");

// Validate form
fcriterioavaliacaoedit.Validate = function() {
...
fcriterioavaliacaoedit.Form_CustomValidate = 
...
fcriterioavaliacaoedit.ValidateRequired = true;
<?php } else { ?>
fcriterioavaliacaoedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fcriterioavaliacaoedit.Lists["x_nu_criterioPrioridade"] = {"LinkField":"x_nu_criterioPrioridade","Ajax":null,"AutoFill":false,"DisplayFields":["x_no_criterioPrioridade","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $criterioavaliacao_edit->ShowPageHeader(); ?>
<?php
$criterioavaliacao_edit->ShowMessage();
?>
<form name="fcriterioavaliacaoedit" id="fcriterioavaliacaoedit" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="criterioavaliacao">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_criterioavaliacaoedit" class="table table-bordered table-striped">
...
<?php echo $criterioavaliacao->nu_alternativaAvaliacao->EditValue ?></span>
</span>
<input type="hidden" data-field="x_nu_alternativaAvaliacao" name="x_nu_alternativaAvaliacao" id="x_nu_alternativaAvaliacao" value="<?php echo ew_HtmlEncode($criterioavaliacao->nu_alternativaAvaliacao->CurrentValue) ?>">
<?php echo $criterioavaliacao->nu_alternativaAvaliacao->CustomMsg ?></td>
	</tr>
<?php } ?>
...
<?php echo $criterioavaliacao->nu_criterioPrioridade->EditValue ?></span>
</span>
<input type="hidden" data-field="x_nu_criterioPrioridade" name="x_nu_criterioPrioridade" id="x_nu_criterioPrioridade" value="<?php echo ew_HtmlEncode($criterioavaliacao->nu_criterioPrioridade->CurrentValue) ?>">
...
<script type="text/javascript">
fcriterioavaliacaoedit.Init();
<?php if (EW_MOBILE_REFLOW && ew_IsMobile()) { ?>
ew_Reflow();
<?php } ?>
</script>
<?php
$criterioavaliacao_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$criterioavaliacao_edit->Page_Terminate();
?>
