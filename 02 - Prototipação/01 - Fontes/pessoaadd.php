...
$pessoa_add = NULL; // Initialize page object first

class cpessoa_add extends cpessoa {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["nu_pessoa"] != "") {
				...
				$this->setKey("nu_pessoa", $this->nu_pessoa->CurrentValue); // Set up key
			} else {
				$this->setKey("nu_pessoa", ""); // Clear key
...
// Create page object
if (!isset($pessoa_add)) $pessoa_add = new cpessoa_add();

// Page init
$pessoa_add->Page_Init();

// Page main
$pessoa_add->Page_Main();
...
// Page object
var pessoa_add = new ew_Page("pessoa_add");
pessoa_add.PageID = "add"; // Page ID
var EW_PAGE_ID = pessoa_add.PageID; // For backward compatibility

// Form object
var fpessoaadd = new ew_Form("fpessoaadd");
...
fpessoaadd.Form_CustomValidate = 
...
fpessoaadd.ValidateRequired = true;
<?php } else { ?>
fpessoaadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fpessoaadd.Lists["x_nu_cargo"] = {"LinkField":"x_nu_cargo","Ajax":null,"AutoFill":false,"DisplayFields":["x_no_cargo","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fpessoaadd.Lists["x_nu_areaLotacao"] = {"LinkField":"x_nu_area","Ajax":null,"AutoFill":false,"DisplayFields":["x_no_area","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
?>
<form name="fpessoaadd" id="fpessoaadd" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="pessoa">
<input type="hidden" name="a_add" id="a_add" value="A">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_pessoaadd" class="table table-bordered table-striped">
...
<script type="text/javascript">
fpessoaadd.Lists["x_nu_cargo"].Options = <?php echo (is_array($pessoa->nu_cargo->EditValue)) ? ew_ArrayToJson($pessoa->nu_cargo->EditValue, 1) : "[]" ?>;
</script>
</span>
...
<script type="text/javascript">
fpessoaadd.Lists["x_nu_areaLotacao"].Options = <?php echo (is_array($pessoa->nu_areaLotacao->EditValue)) ? ew_ArrayToJson($pessoa->nu_areaLotacao->EditValue, 1) : "[]" ?>;
</script>
</span>
