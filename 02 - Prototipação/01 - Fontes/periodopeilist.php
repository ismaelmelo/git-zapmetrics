...
$periodopei_list = NULL; // Initialize page object first

class cperiodopei_list extends cperiodopei {
	...
	// Page object name
	var $PageObjName = 'periodopei_list';

	// Grid form hidden field names
	var $FormName = 'fperiodopeilist';
	...
	function __construct() {
		...
		$this->AddUrl = "periodopeiadd.php";
		$this->InlineAddUrl = $this->PageUrl() . "a=add";
		$this->GridAddUrl = $this->PageUrl() . "a=gridadd";
		$this->GridEditUrl = $this->PageUrl() . "a=gridedit";
		$this->MultiDeleteUrl = "periodopeidelete.php";
		$this->MultiUpdateUrl = "periodopeiupdate.php";
	...
	function SetupKeyValues($key) {
		...
		if (count($arrKeyFlds) >= 1) {
			$this->nu_periodoPei->setFormValue($arrKeyFlds[0]);
			if (!is_numeric($this->nu_periodoPei->FormValue))
				return FALSE;
		}
		return TRUE;
	}
	...
	function SetUpSortOrder() {
		...
		if (@$_GET["order"] <> "") {
			...
			$this->UpdateSort($this->no_periodo, $bCtrl); // no_periodo
	...
	function RenderOtherOptions() {
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->CustomActions as $action => $name) {
				...
				$item->Body = "<a class=\"ewAction ewCustomAction\" href=\"\" onclick=\"ew_SubmitSelected(document.fperiodopeilist, '" . ew_CurrentUrl() . "', null, '" . $action . "');return false;\">" . $name . "</a>";
			}
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a id=\"emf_periodopei\" href=\"javascript:void(0);\" class=\"ewExportLink ewEmail\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_periodopei',hdr:ewLanguage.Phrase('ExportToEmail'),f:document.fperiodopeilist,sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
...
// Create page object
if (!isset($periodopei_list)) $periodopei_list = new cperiodopei_list();

// Page init
$periodopei_list->Page_Init();

// Page main
$periodopei_list->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$periodopei_list->Page_Render();
?>
<?php include_once "header.php" ?>
<?php if ($periodopei->Export == "") { ?>
<script type="text/javascript">

// Page object
var periodopei_list = new ew_Page("periodopei_list");
periodopei_list.PageID = "list"; // Page ID
var EW_PAGE_ID = periodopei_list.PageID; // For backward compatibility

// Form object
var fperiodopeilist = new ew_Form("fperiodopeilist");
fperiodopeilist.FormKeyCountName = '<?php echo $periodopei_list->FormKeyCountName ?>';

// Form_CustomValidate event
fperiodopeilist.Form_CustomValidate = 
...
fperiodopeilist.ValidateRequired = true;
<?php } else { ?>
fperiodopeilist.ValidateRequired = false; 
<?php } ?>
...
<?php } ?>
<?php if ($periodopei_list->ExportOptions->Visible()) { ?>
<div class="ewListExportOptions"><?php $periodopei_list->ExportOptions->Render("body") ?></div>
<?php } ?>
<?php
	$bSelectLimit = EW_SELECT_LIMIT;
	if ($bSelectLimit) {
		$periodopei_list->TotalRecs = $periodopei->SelectRecordCount();
	} else {
		if ($periodopei_list->Recordset = $periodopei_list->LoadRecordset())
			$periodopei_list->TotalRecs = $periodopei_list->Recordset->RecordCount();
	}
	$periodopei_list->StartRec = 1;
	if ($periodopei_list->DisplayRecs <= 0 || ($periodopei->Export <> "" && $periodopei->ExportAll)) // Display all records
		$periodopei_list->DisplayRecs = $periodopei_list->TotalRecs;
	if (!($periodopei->Export <> "" && $periodopei->ExportAll))
		$periodopei_list->SetUpStartRec(); // Set up start record position
	if ($bSelectLimit)
		$periodopei_list->Recordset = $periodopei_list->LoadRecordset($periodopei_list->StartRec-1, $periodopei_list->DisplayRecs);
$periodopei_list->RenderOtherOptions();
?>
<?php $periodopei_list->ShowPageHeader(); ?>
<?php
$periodopei_list->ShowMessage();
?>
<table cellspacing="0" class="ewGrid"><tr><td class="ewGridContent">
<form name="fperiodopeilist" id="fperiodopeilist" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="periodopei">
<div id="gmp_periodopei" class="ewGridMiddlePanel">
<?php if ($periodopei_list->TotalRecs > 0) { ?>
<table id="tbl_periodopeilist" class="ewTable ewTableSeparate">
...
// Render list options
$periodopei_list->RenderListOptions();

// Render list options (header, left)
$periodopei_list->ListOptions->Render("header", "left");
?>
<?php if ($periodopei->nu_periodoPei->Visible) { // nu_periodoPei ?>
	<?php if ($periodopei->SortUrl($periodopei->nu_periodoPei) == "") { ?>
		<td><div id="elh_periodopei_nu_periodoPei" class="periodopei_nu_periodoPei"><div class="ewTableHeaderCaption"><?php echo $periodopei->nu_periodoPei->FldCaption() ?></div></div></td>
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $periodopei->SortUrl($periodopei->nu_periodoPei) ?>',2);"><div id="elh_periodopei_nu_periodoPei" class="periodopei_nu_periodoPei">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $periodopei->nu_periodoPei->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($periodopei->nu_periodoPei->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($periodopei->nu_periodoPei->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></td>
	<?php } ?>
<?php } ?>		
<?php if ($periodopei->nu_anoInicio->Visible) { // nu_anoInicio ?>
	<?php if ($periodopei->SortUrl($periodopei->nu_anoInicio) == "") { ?>
		<td><div id="elh_periodopei_nu_anoInicio" class="periodopei_nu_anoInicio"><div class="ewTableHeaderCaption"><?php echo $periodopei->nu_anoInicio->FldCaption() ?></div></div></td>
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $periodopei->SortUrl($periodopei->nu_anoInicio) ?>',2);"><div id="elh_periodopei_nu_anoInicio" class="periodopei_nu_anoInicio">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $periodopei->nu_anoInicio->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($periodopei->nu_anoInicio->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($periodopei->nu_anoInicio->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></td>
	<?php } ?>
<?php } ?>		
<?php if ($periodopei->nu_anoFim->Visible) { // nu_anoFim ?>
	<?php if ($periodopei->SortUrl($periodopei->nu_anoFim) == "") { ?>
		<td><div id="elh_periodopei_nu_anoFim" class="periodopei_nu_anoFim"><div class="ewTableHeaderCaption"><?php echo $periodopei->nu_anoFim->FldCaption() ?></div></div></td>
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $periodopei->SortUrl($periodopei->nu_anoFim) ?>',2);"><div id="elh_periodopei_nu_anoFim" class="periodopei_nu_anoFim">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $periodopei->nu_anoFim->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($periodopei->nu_anoFim->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($periodopei->nu_anoFim->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></td>
	<?php } ?>
<?php } ?>		
<?php if ($periodopei->no_periodo->Visible) { // no_periodo ?>
	<?php if ($periodopei->SortUrl($periodopei->no_periodo) == "") { ?>
		<td><div id="elh_periodopei_no_periodo" class="periodopei_no_periodo"><div class="ewTableHeaderCaption"><?php echo $periodopei->no_periodo->FldCaption() ?></div></div></td>
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $periodopei->SortUrl($periodopei->no_periodo) ?>',2);"><div id="elh_periodopei_no_periodo" class="periodopei_no_periodo">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $periodopei->no_periodo->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($periodopei->no_periodo->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($periodopei->no_periodo->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></td>
	<?php } ?>
<?php } ?>		
<?php if ($periodopei->ic_situacao->Visible) { // ic_situacao ?>
	<?php if ($periodopei->SortUrl($periodopei->ic_situacao) == "") { ?>
		<td><div id="elh_periodopei_ic_situacao" class="periodopei_ic_situacao"><div class="ewTableHeaderCaption"><?php echo $periodopei->ic_situacao->FldCaption() ?></div></div></td>
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $periodopei->SortUrl($periodopei->ic_situacao) ?>',2);"><div id="elh_periodopei_ic_situacao" class="periodopei_ic_situacao">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $periodopei->ic_situacao->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($periodopei->ic_situacao->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($periodopei->ic_situacao->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></td>
	<?php } ?>
<?php } ?>		
<?php
...
$periodopei_list->ListOptions->Render("header", "right");
?>
	</tr>
</thead>
<tbody>
<?php
if ($periodopei->ExportAll && $periodopei->Export <> "") {
	$periodopei_list->StopRec = $periodopei_list->TotalRecs;
} else {

	// Set the last record to display
	if ($periodopei_list->TotalRecs > $periodopei_list->StartRec + $periodopei_list->DisplayRecs - 1)
		$periodopei_list->StopRec = $periodopei_list->StartRec + $periodopei_list->DisplayRecs - 1;
	else
		$periodopei_list->StopRec = $periodopei_list->TotalRecs;
}
$periodopei_list->RecCnt = $periodopei_list->StartRec - 1;
if ($periodopei_list->Recordset && !$periodopei_list->Recordset->EOF) {
	$periodopei_list->Recordset->MoveFirst();
	if (!$bSelectLimit && $periodopei_list->StartRec > 1)
		$periodopei_list->Recordset->Move($periodopei_list->StartRec - 1);
} elseif (!$periodopei->AllowAddDeleteRow && $periodopei_list->StopRec == 0) {
	$periodopei_list->StopRec = $periodopei->GridAddRowCount;
}

// Initialize aggregate
$periodopei->RowType = EW_ROWTYPE_AGGREGATEINIT;
$periodopei->ResetAttrs();
$periodopei_list->RenderRow();
while ($periodopei_list->RecCnt < $periodopei_list->StopRec) {
	$periodopei_list->RecCnt++;
	if (intval($periodopei_list->RecCnt) >= intval($periodopei_list->StartRec)) {
		$periodopei_list->RowCnt++;

		// Set up key count
		$periodopei_list->KeyCount = $periodopei_list->RowIndex;

		// Init row class and style
		$periodopei->ResetAttrs();
		$periodopei->CssClass = "";
		if ($periodopei->CurrentAction == "gridadd") {
		} else {
			$periodopei_list->LoadRowValues($periodopei_list->Recordset); // Load row values
		}
		$periodopei->RowType = EW_ROWTYPE_VIEW; // Render view

		// Set up row id / data-rowindex
		$periodopei->RowAttrs = array_merge($periodopei->RowAttrs, array('data-rowindex'=>$periodopei_list->RowCnt, 'id'=>'r' . $periodopei_list->RowCnt . '_periodopei', 'data-rowtype'=>$periodopei->RowType));
...
$periodopei_list->ListOptions->Render("body", "left", $periodopei_list->RowCnt);
?>
	<?php if ($periodopei->nu_periodoPei->Visible) { // nu_periodoPei ?>
...
<a id="<?php echo $periodopei_list->PageObjName . "_row_" . $periodopei_list->RowCnt ?>"></a></td>
	<?php } ?>
	<?php if ($periodopei->nu_anoInicio->Visible) { // nu_anoInicio ?>
...
$periodopei_list->ListOptions->Render("body", "right", $periodopei_list->RowCnt);
?>
	</tr>
<?php
	}
	if ($periodopei->CurrentAction <> "gridadd")
		$periodopei_list->Recordset->MoveNext();
}
?>
</tbody>
</table>
<?php } ?>
<?php if ($periodopei->CurrentAction == "") { ?>
...
// Close recordset
if ($periodopei_list->Recordset)
	$periodopei_list->Recordset->Close();
?>
<?php if ($periodopei->Export == "") { ?>
<div class="ewGridLowerPanel">
<?php if ($periodopei->CurrentAction <> "gridadd" && $periodopei->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>">
<table class="ewPager">
<tr><td>
<?php if (!isset($periodopei_list->Pager)) $periodopei_list->Pager = new cNumericPager($periodopei_list->StartRec, $periodopei_list->DisplayRecs, $periodopei_list->TotalRecs, $periodopei_list->RecRange) ?>
<?php if ($periodopei_list->Pager->RecordCount > 0) { ?>
<table cellspacing="0" class="ewStdTable"><tbody><tr><td>
<div class="pagination"><ul>
	<?php if ($periodopei_list->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $periodopei_list->PageUrl() ?>start=<?php echo $periodopei_list->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($periodopei_list->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $periodopei_list->PageUrl() ?>start=<?php echo $periodopei_list->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($periodopei_list->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $periodopei_list->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($periodopei_list->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $periodopei_list->PageUrl() ?>start=<?php echo $periodopei_list->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($periodopei_list->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $periodopei_list->PageUrl() ?>start=<?php echo $periodopei_list->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</td>
<td>
	<?php if ($periodopei_list->Pager->ButtonCount > 0) { ?>&nbsp;&nbsp;&nbsp;&nbsp;<?php } ?>
	<?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $periodopei_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $periodopei_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $periodopei_list->Pager->RecordCount ?>
</td>
</tr></tbody></table>
<?php } else { ?>
	<?php if ($Security->CanList()) { ?>
	<?php if ($periodopei_list->SearchWhere == "0=101") { ?>
...
<?php
	foreach ($periodopei_list->OtherOptions as &$option)
...
<?php } ?>
<?php
$periodopei_list->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($periodopei->Export == "") { ?>
...
<?php
$periodopei_list->Page_Terminate();
?>
