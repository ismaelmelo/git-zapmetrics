...
$pei_view = NULL; // Initialize page object first

class cpei_view extends cpei {
	...
	function __construct() {
		...
		$KeyUrl = "";
		if (@$_GET["nu_pei"] <> "") {
			$this->RecKey["nu_pei"] = $_GET["nu_pei"];
			$KeyUrl .= "&nu_pei=" . urlencode($this->RecKey["nu_pei"]);
		}
	...
	function Page_Init() {
		...
		if (@$_GET["nu_pei"] <> "") {
			...
			$gsExportFile .= ew_StripSlashes($_GET["nu_pei"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["nu_pei"] <> "") {
				...
				$this->RecKey["nu_pei"] = $this->nu_pei->QueryStringValue;
			} else {
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					...
					if ($this->TotalRecs <= 0) { // No record found
						...
						$this->Page_Terminate("peilist.php"); // Return to list page
						...
						while (!$this->Recordset->EOF) {
							if (strval($this->nu_pei->CurrentValue) == strval($this->Recordset->fields('nu_pei'))) {
					...
					if (!$bMatchRecord) {
						if ($this->getSuccessMessage() == "" && $this->getFailureMessage() == "")
							$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record message
						$sReturnUrl = "peilist.php"; // No matching record, return to list
					} else {
			...
			$sReturnUrl = "peilist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a id=\"emf_pei\" href=\"javascript:void(0);\" class=\"ewExportLink ewEmail\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_pei',hdr:ewLanguage.Phrase('ExportToEmail'),f:document.fpeiview,key:" . ew_ArrayToJsonAttr($this->RecKey) . ",sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
...
// Create page object
if (!isset($pei_view)) $pei_view = new cpei_view();
...
// Page object
var pei_view = new ew_Page("pei_view");
pei_view.PageID = "view"; // Page ID
var EW_PAGE_ID = pei_view.PageID; // For backward compatibility

// Form object
var fpeiview = new ew_Form("fpeiview");

// Form_CustomValidate event
fpeiview.Form_CustomValidate = 
...
fpeiview.ValidateRequired = true;
<?php } else { ?>
fpeiview.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fpeiview.Lists["x_nu_periodoPei"] = {"LinkField":"x_nu_periodoPei","Ajax":null,"AutoFill":false,"DisplayFields":["x_nu_anoInicio","x_nu_anoFim","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $pei_view->ExportOptions->Render("body") ?>
<?php if (!$pei_view->ExportOptions->UseDropDownButton) { ?>
</div>
<div class="ewViewOtherOptions">
<?php } ?>
<?php
	foreach ($pei_view->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<?php } ?>
<?php $pei_view->ShowPageHeader(); ?>
<?php
$pei_view->ShowMessage();
?>
<form name="fpeiview" id="fpeiview" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="pei">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_peiview" class="table table-bordered table-striped">
...
<?php echo $pei->nu_pei->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($pei->nu_periodoPei->Visible) { // nu_periodoPei ?>
	<tr id="r_nu_periodoPei">
		<td><span id="elh_pei_nu_periodoPei"><?php echo $pei->nu_periodoPei->FldCaption() ?></span></td>
...
<?php echo $pei->nu_periodoPei->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($pei->no_capaPei->Visible) { // no_capaPei ?>
	<tr id="r_no_capaPei">
		<td><span id="elh_pei_no_capaPei"><?php echo $pei->no_capaPei->FldCaption() ?></span></td>
...
<?php echo $pei->no_capaPei->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<span<?php echo $pei->ds_introducao->ViewAttributes() ?>>
<?php echo $pei->ds_introducao->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<span<?php echo $pei->ds_missao->ViewAttributes() ?>>
<?php echo $pei->ds_missao->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<span<?php echo $pei->ds_visao->ViewAttributes() ?>>
<?php echo $pei->ds_visao->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<span<?php echo $pei->ds_valores->ViewAttributes() ?>>
<?php echo $pei->ds_valores->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($pei->ic_situacao->Visible) { // ic_situacao ?>
	<tr id="r_ic_situacao">
		<td><span id="elh_pei_ic_situacao"><?php echo $pei->ic_situacao->FldCaption() ?></span></td>
...
<?php echo $pei->ic_situacao->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<table class="ewPager">
<tr><td>
<?php if (!isset($pei_view->Pager)) $pei_view->Pager = new cNumericPager($pei_view->StartRec, $pei_view->DisplayRecs, $pei_view->TotalRecs, $pei_view->RecRange) ?>
<?php if ($pei_view->Pager->RecordCount > 0) { ?>
<table cellspacing="0" class="ewStdTable"><tbody><tr><td>
<div class="pagination"><ul>
	<?php if ($pei_view->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $pei_view->PageUrl() ?>start=<?php echo $pei_view->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($pei_view->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $pei_view->PageUrl() ?>start=<?php echo $pei_view->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($pei_view->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $pei_view->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($pei_view->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $pei_view->PageUrl() ?>start=<?php echo $pei_view->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($pei_view->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $pei_view->PageUrl() ?>start=<?php echo $pei_view->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</td>
</tr></tbody></table>
