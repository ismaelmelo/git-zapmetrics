...
$mensagem_view = NULL; // Initialize page object first

class cmensagem_view extends cmensagem {
	...
	function Page_Init() {
		...
		if (@$_GET["nu_mensagem"] <> "") {
			...
			$gsExportFile .= ew_StripSlashes($_GET["nu_mensagem"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					...
					if ($this->TotalRecs <= 0) { // No record found
						...
						$this->Page_Terminate("mensagemlist.php"); // Return to list page
						...
						while (!$this->Recordset->EOF) {
							if (strval($this->nu_mensagem->CurrentValue) == strval($this->Recordset->fields('nu_mensagem'))) {
					...
					if (!$bMatchRecord) {
						if ($this->getSuccessMessage() == "" && $this->getFailureMessage() == "")
							$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record message
						$sReturnUrl = "mensagemlist.php"; // No matching record, return to list
					} else {
			...
			$sReturnUrl = "mensagemlist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a id=\"emf_mensagem\" href=\"javascript:void(0);\" class=\"ewExportLink ewEmail\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_mensagem',hdr:ewLanguage.Phrase('ExportToEmail'),f:document.fmensagemview,key:" . ew_ArrayToJsonAttr($this->RecKey) . ",sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
...
// Create page object
if (!isset($mensagem_view)) $mensagem_view = new cmensagem_view();

// Page init
$mensagem_view->Page_Init();

// Page main
$mensagem_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$mensagem_view->Page_Render();
?>
...
// Page object
var mensagem_view = new ew_Page("mensagem_view");
mensagem_view.PageID = "view"; // Page ID
var EW_PAGE_ID = mensagem_view.PageID; // For backward compatibility

// Form object
var fmensagemview = new ew_Form("fmensagemview");

// Form_CustomValidate event
fmensagemview.Form_CustomValidate = 
...
fmensagemview.ValidateRequired = true;
<?php } else { ?>
fmensagemview.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fmensagemview.Lists["x_nu_stMensagem"] = {"LinkField":"x_nu_stMensagem","Ajax":null,"AutoFill":false,"DisplayFields":["x_no_stMensagem","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $mensagem_view->ExportOptions->Render("body") ?>
<?php if (!$mensagem_view->ExportOptions->UseDropDownButton) { ?>
</div>
<div class="ewViewOtherOptions">
<?php } ?>
<?php
	foreach ($mensagem_view->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<?php } ?>
<?php $mensagem_view->ShowPageHeader(); ?>
<?php
$mensagem_view->ShowMessage();
?>
<form name="fmensagemview" id="fmensagemview" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="mensagem">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_mensagemview" class="table table-bordered table-striped">
<?php if ($mensagem->co_alternativo->Visible) { // co_alternativo ?>
	<tr id="r_co_alternativo">
		<td><span id="elh_mensagem_co_alternativo"><?php echo $mensagem->co_alternativo->FldCaption() ?></span></td>
...
<?php echo $mensagem->co_alternativo->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($mensagem->no_mensagem->Visible) { // no_mensagem ?>
	<tr id="r_no_mensagem">
		<td><span id="elh_mensagem_no_mensagem"><?php echo $mensagem->no_mensagem->FldCaption() ?></span></td>
...
<?php echo $mensagem->no_mensagem->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php echo $mensagem->ds_mensagem->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<span<?php echo $mensagem->ds_ajuda->ViewAttributes() ?>>
<?php echo $mensagem->ds_ajuda->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($mensagem->nu_stMensagem->Visible) { // nu_stMensagem ?>
	<tr id="r_nu_stMensagem">
		<td><span id="elh_mensagem_nu_stMensagem"><?php echo $mensagem->nu_stMensagem->FldCaption() ?></span></td>
...
<?php echo $mensagem->nu_stMensagem->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<table class="ewPager">
<tr><td>
<?php if (!isset($mensagem_view->Pager)) $mensagem_view->Pager = new cNumericPager($mensagem_view->StartRec, $mensagem_view->DisplayRecs, $mensagem_view->TotalRecs, $mensagem_view->RecRange) ?>
<?php if ($mensagem_view->Pager->RecordCount > 0) { ?>
<table cellspacing="0" class="ewStdTable"><tbody><tr><td>
<div class="pagination"><ul>
	<?php if ($mensagem_view->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $mensagem_view->PageUrl() ?>start=<?php echo $mensagem_view->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($mensagem_view->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $mensagem_view->PageUrl() ?>start=<?php echo $mensagem_view->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($mensagem_view->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $mensagem_view->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($mensagem_view->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $mensagem_view->PageUrl() ?>start=<?php echo $mensagem_view->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($mensagem_view->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $mensagem_view->PageUrl() ?>start=<?php echo $mensagem_view->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</td>
</tr></tbody></table>
