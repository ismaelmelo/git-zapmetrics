...
class carea_list extends carea {
	...
	function __construct() {
		...
		$this->AddUrl = "areaadd.php?" . EW_TABLE_SHOW_DETAIL . "=";
	...
	function SetUpSortOrder() {
		...
		if (@$_GET["order"] <> "") {
			...
			$this->UpdateSort($this->no_area, $bCtrl); // no_area
			$this->UpdateSort($this->nu_tpArea, $bCtrl); // nu_tpArea
	...
	function ResetCmd() {
		...
		if (substr($this->Command,0,5) == "reset") {
			...
			if ($this->Command == "resetsort") {
				$sOrderBy = "";
				$this->setSessionOrderBy($sOrderBy);
				$this->no_area->setSort("");
				$this->nu_tpArea->setSort("");
	...
	function SetupListOptions() {
		...
		// "detail_divisao"
		$item = &$this->ListOptions->Add("detail_divisao");
		$item->CssStyle = "white-space: nowrap;";
		$item->Visible = $Security->AllowList(CurrentProjectID() . 'divisao') && !$this->ShowMultipleDetails;
		$item->OnLeft = FALSE;
		$item->ShowInButtonGroup = FALSE;
		if (!isset($GLOBALS["divisao_grid"])) $GLOBALS["divisao_grid"] = new cdivisao_grid;
	...
	function RenderListOptions() {
		...
		// "detail_divisao"
		$oListOpt = &$this->ListOptions->Items["detail_divisao"];
		if ($Security->AllowList(CurrentProjectID() . 'divisao')) {
			$body = $Language->Phrase("DetailLink") . $Language->TablePhrase("divisao", "TblCaption");
			$body = "<a class=\"btn btn-small ewRowLink ewDetailList\" data-action=\"list\" href=\"" . ew_HtmlEncode("divisaolist.php?" . EW_TABLE_SHOW_MASTER . "=area&nu_area=" . strval($this->nu_area->CurrentValue) . "") . "\">" . $body . "</a>";
			$links = "";
			if ($GLOBALS["divisao_grid"]->DetailView && $Security->CanView() && $Security->AllowView(CurrentProjectID() . 'divisao')) {
				$links .= "<li><a class=\"ewRowLink ewDetailView\" data-action=\"view\" href=\"" . ew_HtmlEncode($this->GetViewUrl(EW_TABLE_SHOW_DETAIL . "=divisao")) . "\">" . $Language->Phrase("MasterDetailViewLink") . "</a></li>";
				if ($DetailViewTblVar <> "") $DetailViewTblVar .= ",";
				$DetailViewTblVar .= "divisao";
			}
			if ($GLOBALS["divisao_grid"]->DetailEdit && $Security->CanEdit() && $Security->AllowEdit(CurrentProjectID() . 'divisao')) {
				$links .= "<li><a class=\"ewRowLink ewDetailEdit\" data-action=\"edit\" href=\"" . ew_HtmlEncode($this->GetEditUrl(EW_TABLE_SHOW_DETAIL . "=divisao")) . "\">" . $Language->Phrase("MasterDetailEditLink") . "</a></li>";
				if ($DetailEditTblVar <> "") $DetailEditTblVar .= ",";
				$DetailEditTblVar .= "divisao";
			}
			if ($links <> "") {
	...
	function SetupOtherOptions() {
		...
		$DetailTableLink = "";
		$item = &$option->Add("detailadd_divisao");
		$item->Body = "<a class=\"ewDetailAddGroup ewDetailAdd\" href=\"" . ew_HtmlEncode($this->GetAddUrl() . "?" . EW_TABLE_SHOW_DETAIL . "=divisao") . "\">" . $Language->Phrase("AddLink") . "&nbsp;" . $this->TableCaption() . "/" . $GLOBALS["divisao"]->TableCaption() . "</a>";
		$item->Visible = ($GLOBALS["divisao"]->DetailAdd && $Security->AllowAdd(CurrentProjectID() . 'divisao') && $Security->CanAdd());
		if ($item->Visible) {
	...
	function RenderOtherOptions() {
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->CustomActions as $action => $name) {
				...
				$item->Body = "<a class=\"ewAction ewCustomAction\" href=\"\" onclick=\"ew_SubmitSelected(document.farealist, '" . ew_CurrentUrl() . "', null, '" . $action . "');return false;\">" . $name . "</a>";
			}
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a id=\"emf_area\" href=\"javascript:void(0);\" class=\"ewExportLink ewEmail\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_area',hdr:ewLanguage.Phrase('ExportToEmail'),f:document.farealist,sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
	...
	function ExportData() {
		...
		if (EW_EXPORT_MASTER_RECORD && $this->GetMasterFilter() <> "" && $this->getCurrentMasterTable() == "tparea") {
			global $tparea;
			$rsmaster = $tparea->LoadRs($this->DbMasterFilter); // Load master record
			if ($rsmaster && !$rsmaster->EOF) {
				$ExportStyle = $ExportDoc->Style;
				$ExportDoc->SetStyle("v"); // Change to vertical
				if ($this->Export <> "csv" || EW_EXPORT_MASTER_RECORD_FOR_CSV) {
					$tparea->ExportDocument($ExportDoc, $rsmaster, 1, 1);
...
// Form object
var farealist = new ew_Form("farealist");
...
farealist.ValidateRequired = true;
<?php } else { ?>
farealist.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
farealist.Lists["x_nu_tpArea"] = {"LinkField":"x_nu_tpArea","Ajax":null,"AutoFill":false,"DisplayFields":["x_no_tpArea","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
farealist.Lists["x_nu_pessoaResp"] = {"LinkField":"x_nu_pessoa","Ajax":null,"AutoFill":false,"DisplayFields":["x_no_pessoa","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php } ?>
<?php if ($area->getCurrentMasterTable() == "" && $area_list->ExportOptions->Visible()) { ?>
<div class="ewListExportOptions"><?php $area_list->ExportOptions->Render("body") ?></div>
<?php } ?>
<?php if (($area->Export == "") || (EW_EXPORT_MASTER_RECORD && $area->Export == "print")) { ?>
<?php
$gsMasterReturnUrl = "tparealist.php";
if ($area_list->DbMasterFilter <> "" && $area->getCurrentMasterTable() == "tparea") {
	if ($area_list->MasterRecordExists) {
		if ($area->getCurrentMasterTable() == $area->TableVar) $gsMasterReturnUrl .= "?" . EW_TABLE_SHOW_MASTER . "=";
?>
<?php if ($area_list->ExportOptions->Visible()) { ?>
<div class="ewListExportOptions"><?php $area_list->ExportOptions->Render("body") ?></div>
<?php } ?>
<?php include_once "tpareamaster.php" ?>
<?php
	}
}
?>
...
if ($area_list->DbMasterFilter <> "" && $area->getCurrentMasterTable() == "organizacao") {
	if ($area_list->MasterRecordExists) {
		if ($area->getCurrentMasterTable() == $area->TableVar) $gsMasterReturnUrl .= "?" . EW_TABLE_SHOW_MASTER . "=";
?>
<?php if ($area_list->ExportOptions->Visible()) { ?>
...
<form name="farealist" id="farealist" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="area">
<div id="gmp_area" class="ewGridMiddlePanel">
<?php if ($area_list->TotalRecs > 0) { ?>
<table id="tbl_arealist" class="ewTable ewTableSeparate">
...
<?php if ($area->no_area->Visible) { // no_area ?>
	...
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $area->SortUrl($area->no_area) ?>',2);"><div id="elh_area_no_area" class="area_no_area">
...
<?php if ($area->nu_tpArea->Visible) { // nu_tpArea ?>
	...
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $area->SortUrl($area->nu_tpArea) ?>',2);"><div id="elh_area_nu_tpArea" class="area_nu_tpArea">
...
<?php if ($area->nu_pessoaResp->Visible) { // nu_pessoaResp ?>
	...
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $area->SortUrl($area->nu_pessoaResp) ?>',2);"><div id="elh_area_nu_pessoaResp" class="area_nu_pessoaResp">
...
<?php if ($area->ic_ativo->Visible) { // ic_ativo ?>
	...
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $area->SortUrl($area->ic_ativo) ?>',2);"><div id="elh_area_ic_ativo" class="area_ic_ativo">
...
<a id="<?php echo $area_list->PageObjName . "_row_" . $area_list->RowCnt ?>"></a></td>
	<?php } ?>
	<?php if ($area->nu_tpArea->Visible) { // nu_tpArea ?>
...
<table class="ewPager">
<tr><td>
<?php if (!isset($area_list->Pager)) $area_list->Pager = new cNumericPager($area_list->StartRec, $area_list->DisplayRecs, $area_list->TotalRecs, $area_list->RecRange) ?>
<?php if ($area_list->Pager->RecordCount > 0) { ?>
<table cellspacing="0" class="ewStdTable"><tbody><tr><td>
<div class="pagination"><ul>
	<?php if ($area_list->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $area_list->PageUrl() ?>start=<?php echo $area_list->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($area_list->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $area_list->PageUrl() ?>start=<?php echo $area_list->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($area_list->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $area_list->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($area_list->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $area_list->PageUrl() ?>start=<?php echo $area_list->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($area_list->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $area_list->PageUrl() ?>start=<?php echo $area_list->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</td>
<td>
	<?php if ($area_list->Pager->ButtonCount > 0) { ?>&nbsp;&nbsp;&nbsp;&nbsp;<?php } ?>
	<?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $area_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $area_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $area_list->Pager->RecordCount ?>
</td>
</tr></tbody></table>
<?php } else { ?>
