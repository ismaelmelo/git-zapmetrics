...
$parsisp_versao_grid = NULL; // Initialize page object first

class cparsisp_versao_grid extends cparsisp_versao {
	...
	// Page object name
	var $PageObjName = 'parsisp_versao_grid';

	// Grid form hidden field names
	var $FormName = 'fparsisp_versaogrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["parsisp_versao"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->nu_parSisp->CurrentValue;
					if ($sKey <> "") $sKey .= $GLOBALS["EW_COMPOSITE_KEY_SEPARATOR"];
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_nu_parSisp") && $objForm->HasValue("o_nu_parSisp") && $this->nu_parSisp->CurrentValue <> $this->nu_parSisp->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_nu_versao") && $objForm->HasValue("o_nu_versao") && $this->nu_versao->CurrentValue <> $this->nu_versao->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_vr_parSisp") && $objForm->HasValue("o_vr_parSisp") && $this->vr_parSisp->CurrentValue <> $this->vr_parSisp->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->nu_parSisp->CurrentValue . $GLOBALS["EW_COMPOSITE_KEY_SEPARATOR"] . $this->nu_versao->CurrentValue . "\">";
		}
	...
	function LoadFormValues() {
		...
		$this->nu_parSisp->setOldValue($objForm->GetValue("o_nu_parSisp"));
		...
		$this->vr_parSisp->setOldValue($objForm->GetValue("o_vr_parSisp"));
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 2) {
			if (strval($arKeys[0]) <> "")
				$this->nu_parSisp->CurrentValue = strval($arKeys[0]); // nu_parSisp
			else
				$bValidKey = FALSE;
			if (strval($arKeys[1]) <> "")
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			if (strval($this->vr_parSisp->EditValue) <> "" && is_numeric($this->vr_parSisp->EditValue)) {
			$this->vr_parSisp->EditValue = ew_FormatNumber($this->vr_parSisp->EditValue, -2, -1, -2, 0);
			$this->vr_parSisp->OldValue = $this->vr_parSisp->EditValue;
			}
			...
			$sSqlWrk .= " ORDER BY [nu_parSisp] ASC";
				$rswrk = $conn->Execute($sSqlWrk);
				if ($rswrk && !$rswrk->EOF) { // Lookup values found
					$this->nu_parSisp->EditValue = $rswrk->fields('DispFld');
					$rswrk->Close();
				} else {
					$this->nu_parSisp->EditValue = $this->nu_parSisp->CurrentValue;
				}
			} else {
				$this->nu_parSisp->EditValue = NULL;
			}
			$this->nu_parSisp->ViewCustomAttributes = "";
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// vr_parSisp
			$this->vr_parSisp->SetDbValueDef($rsnew, $this->vr_parSisp->CurrentValue, NULL, $this->vr_parSisp->ReadOnly);
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "parSisp") {
			$this->nu_parSisp->Visible = FALSE;
			if ($GLOBALS["parSisp"]->EventCancelled) $this->EventCancelled = TRUE;
		}
