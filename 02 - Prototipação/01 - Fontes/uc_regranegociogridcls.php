...
$uc_regranegocio_grid = NULL; // Initialize page object first

class cuc_regranegocio_grid extends cuc_regranegocio {
	...
	// Page object name
	var $PageObjName = 'uc_regranegocio_grid';

	// Grid form hidden field names
	var $FormName = 'fuc_regranegociogrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["uc_regranegocio"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->nu_uc->CurrentValue;
					if ($sKey <> "") $sKey .= $GLOBALS["EW_COMPOSITE_KEY_SEPARATOR"];
					$sKey .= $this->co_rn->CurrentValue;
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_co_rn") && $objForm->HasValue("o_co_rn") && $this->co_rn->CurrentValue <> $this->co_rn->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->nu_uc->CurrentValue . $GLOBALS["EW_COMPOSITE_KEY_SEPARATOR"] . $this->co_rn->CurrentValue . "\">";
		}
	...
	function LoadFormValues() {
		...
		$this->co_rn->setOldValue($objForm->GetValue("o_co_rn"));
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 2) {
			if (strval($arKeys[0]) <> "")
				$this->nu_uc->CurrentValue = strval($arKeys[0]); // nu_uc
			else
				$bValidKey = FALSE;
			if (strval($arKeys[1]) <> "")
				$this->co_rn->CurrentValue = strval($arKeys[1]); // co_rn
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			if (!$GLOBALS["uc_regranegocio"]->UserIDAllow("gridcls")) $sWhereWrk = $GLOBALS["regranegocio"]->AddUserIDFilter($sWhereWrk);
			...
			$sSqlWrk .= " ORDER BY [co_alternativo] ASC";
				$rswrk = $conn->Execute($sSqlWrk);
				if ($rswrk && !$rswrk->EOF) { // Lookup values found
					$this->co_rn->EditValue = $rswrk->fields('DispFld');
					$this->co_rn->EditValue .= ew_ValueSeparator(1,$this->co_rn) . $rswrk->fields('Disp2Fld');
					$rswrk->Close();
				} else {
					$this->co_rn->EditValue = $this->co_rn->CurrentValue;
				}
			} else {
				$this->co_rn->EditValue = NULL;
			}
			}
			$this->co_rn->ViewCustomAttributes = "";
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			$KeyValue = isset($rsnew['co_rn']) ? $rsnew['co_rn'] : $rsold['co_rn'];
			if (strval($KeyValue) <> "") {
				$sMasterFilter = str_replace("@co_rn@", ew_AdjustSql($KeyValue), $sMasterFilter);
			} else {
				$bValidMasterRecord = FALSE;
			}
			if ($bValidMasterRecord) {
	...
	function AddRow($rsold = NULL) {
		...
		if ($this->nu_uc->getSessionValue() <> "") {
			$sMasterFilter = str_replace("@nu_uc@", ew_AdjustSql($this->nu_uc->getSessionValue()), $sMasterFilter);
		} else {
			$bValidMasterRecord = FALSE;
		}
		if ($bValidMasterRecord) {
		...
		// nu_uc
		if ($this->nu_uc->getSessionValue() <> "") {
			$rsnew['nu_uc'] = $this->nu_uc->getSessionValue();
		}
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "uc") {
			$this->nu_uc->Visible = FALSE;
			if ($GLOBALS["uc"]->EventCancelled) $this->EventCancelled = TRUE;
		}
		if ($sMasterTblVar == "corn") {
			$this->co_rn->Visible = FALSE;
			if ($GLOBALS["corn"]->EventCancelled) $this->EventCancelled = TRUE;
		}
