...
$tpprojeto_edit = NULL; // Initialize page object first

class ctpprojeto_edit extends ctpprojeto {
	...
	// Page object name
	var $PageObjName = 'tpprojeto_edit';
	...
	function Page_Main() {
		...
		if (@$_GET["nu_tpProjeto"] <> "") {
			$this->nu_tpProjeto->setQueryStringValue($_GET["nu_tpProjeto"]);
		}
		...
		// Check if valid key
		if ($this->nu_tpProjeto->CurrentValue == "")
			$this->Page_Terminate("tpprojetolist.php"); // Invalid key, return to list
		...
		switch ($this->CurrentAction) {
			case "I": // Get a record to display
				if (!$this->LoadRow()) { // Load record based on key
					...
					$this->Page_Terminate("tpprojetolist.php"); // No matching record, return to list
				}
				break;
			Case "U": // Update
				...
				if ($this->EditRow()) { // Update record based on key
					...
					if (ew_GetPageName($sReturnUrl) == "tpprojetoview.php")
	...
	function LoadFormValues() {
		...
		if (!$this->ic_tpProjDem->FldIsDetailKey) {
			$this->ic_tpProjDem->setFormValue($objForm->GetValue("x_ic_tpProjDem"));
		}
		if (!$this->no_tpProjeto->FldIsDetailKey) {
			$this->no_tpProjeto->setFormValue($objForm->GetValue("x_no_tpProjeto"));
		}
	...
	function RestoreFormValues() {
		...
		$this->ic_tpProjDem->CurrentValue = $this->ic_tpProjDem->FormValue;
		$this->no_tpProjeto->CurrentValue = $this->no_tpProjeto->FormValue;
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			if (strval($this->ic_tpProjDem->CurrentValue) <> "") {
				switch ($this->ic_tpProjDem->CurrentValue) {
					case $this->ic_tpProjDem->FldTagValue(1):
						$this->ic_tpProjDem->EditValue = $this->ic_tpProjDem->FldTagCaption(1) <> "" ? $this->ic_tpProjDem->FldTagCaption(1) : $this->ic_tpProjDem->CurrentValue;
						break;
					case $this->ic_tpProjDem->FldTagValue(2):
						$this->ic_tpProjDem->EditValue = $this->ic_tpProjDem->FldTagCaption(2) <> "" ? $this->ic_tpProjDem->FldTagCaption(2) : $this->ic_tpProjDem->CurrentValue;
						break;
					case $this->ic_tpProjDem->FldTagValue(3):
						$this->ic_tpProjDem->EditValue = $this->ic_tpProjDem->FldTagCaption(3) <> "" ? $this->ic_tpProjDem->FldTagCaption(3) : $this->ic_tpProjDem->CurrentValue;
						break;
					default:
						$this->ic_tpProjDem->EditValue = $this->ic_tpProjDem->CurrentValue;
				}
			} else {
				$this->ic_tpProjDem->EditValue = NULL;
			}
			$this->ic_tpProjDem->ViewCustomAttributes = "";

			// no_tpProjeto
			$this->no_tpProjeto->EditCustomAttributes = "";
			$this->no_tpProjeto->EditValue = ew_HtmlEncode($this->no_tpProjeto->CurrentValue);
	...
	function ValidateForm() {
		...
		if (!EW_SERVER_VALIDATE)
			return ($gsFormError == "");
		if (!$this->no_tpProjeto->FldIsDetailKey && !is_null($this->no_tpProjeto->FormValue) && $this->no_tpProjeto->FormValue == "") {
			ew_AddMessage($gsFormError, $Language->Phrase("EnterRequiredField") . " - " . $this->no_tpProjeto->FldCaption());
		}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// no_tpProjeto
			$this->no_tpProjeto->SetDbValueDef($rsnew, $this->no_tpProjeto->CurrentValue, "", $this->no_tpProjeto->ReadOnly);
...
// Create page object
if (!isset($tpprojeto_edit)) $tpprojeto_edit = new ctpprojeto_edit();

// Page init
$tpprojeto_edit->Page_Init();

// Page main
$tpprojeto_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$tpprojeto_edit->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var tpprojeto_edit = new ew_Page("tpprojeto_edit");
tpprojeto_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = tpprojeto_edit.PageID; // For backward compatibility

// Form object
var ftpprojetoedit = new ew_Form("ftpprojetoedit");

// Validate form
ftpprojetoedit.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			elm = this.GetElements("x" + infix + "_no_tpProjeto");
			if (elm && !ew_HasValue(elm))
				return this.OnError(elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($tpprojeto->no_tpProjeto->FldCaption()) ?>");
			elm = this.GetElements("x" + infix + "_ic_ativo");
			if (elm && !ew_HasValue(elm))
				return this.OnError(elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($tpprojeto->ic_ativo->FldCaption()) ?>");
...
ftpprojetoedit.Form_CustomValidate = 
...
ftpprojetoedit.ValidateRequired = true;
<?php } else { ?>
ftpprojetoedit.ValidateRequired = false; 
<?php } ?>
...
<?php $tpprojeto_edit->ShowPageHeader(); ?>
<?php
$tpprojeto_edit->ShowMessage();
?>
<form name="ftpprojetoedit" id="ftpprojetoedit" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="tpprojeto">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_tpprojetoedit" class="table table-bordered table-striped">
<?php if ($tpprojeto->nu_tpProjeto->Visible) { // nu_tpProjeto ?>
	<tr id="r_nu_tpProjeto">
		<td><span id="elh_tpprojeto_nu_tpProjeto"><?php echo $tpprojeto->nu_tpProjeto->FldCaption() ?></span></td>
		<td<?php echo $tpprojeto->nu_tpProjeto->CellAttributes() ?>>
<span id="el_tpprojeto_nu_tpProjeto" class="control-group">
<span<?php echo $tpprojeto->nu_tpProjeto->ViewAttributes() ?>>
<?php echo $tpprojeto->nu_tpProjeto->EditValue ?></span>
</span>
<input type="hidden" data-field="x_nu_tpProjeto" name="x_nu_tpProjeto" id="x_nu_tpProjeto" value="<?php echo ew_HtmlEncode($tpprojeto->nu_tpProjeto->CurrentValue) ?>">
<?php echo $tpprojeto->nu_tpProjeto->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($tpprojeto->ic_tpProjDem->Visible) { // ic_tpProjDem ?>
	<tr id="r_ic_tpProjDem">
		<td><span id="elh_tpprojeto_ic_tpProjDem"><?php echo $tpprojeto->ic_tpProjDem->FldCaption() ?></span></td>
		<td<?php echo $tpprojeto->ic_tpProjDem->CellAttributes() ?>>
<span id="el_tpprojeto_ic_tpProjDem" class="control-group">
<span<?php echo $tpprojeto->ic_tpProjDem->ViewAttributes() ?>>
<?php echo $tpprojeto->ic_tpProjDem->EditValue ?></span>
</span>
<input type="hidden" data-field="x_ic_tpProjDem" name="x_ic_tpProjDem" id="x_ic_tpProjDem" value="<?php echo ew_HtmlEncode($tpprojeto->ic_tpProjDem->CurrentValue) ?>">
<?php echo $tpprojeto->ic_tpProjDem->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($tpprojeto->no_tpProjeto->Visible) { // no_tpProjeto ?>
	<tr id="r_no_tpProjeto">
		<td><span id="elh_tpprojeto_no_tpProjeto"><?php echo $tpprojeto->no_tpProjeto->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
		<td<?php echo $tpprojeto->no_tpProjeto->CellAttributes() ?>>
<span id="el_tpprojeto_no_tpProjeto" class="control-group">
...
<?php echo $tpprojeto->no_tpProjeto->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($tpprojeto->ic_ativo->Visible) { // ic_ativo ?>
	<tr id="r_ic_ativo">
		<td><span id="elh_tpprojeto_ic_ativo"><?php echo $tpprojeto->ic_ativo->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
		<td<?php echo $tpprojeto->ic_ativo->CellAttributes() ?>>
<span id="el_tpprojeto_ic_ativo" class="control-group">
...
if (is_array($arwrk)) {
	...
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($tpprojeto->ic_ativo->CurrentValue) == strval($arwrk[$rowcntwrk][0])) ? " checked=\"checked\"" : "";
...
<?php echo $tpprojeto->ic_ativo->CustomMsg ?></td>
	</tr>
<?php } ?>
</table>
</td></tr></table>
