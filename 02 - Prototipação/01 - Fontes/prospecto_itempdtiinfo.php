...
$prospecto_itempdti = NULL;

//
// Table class for prospecto_itempdti
//
class cprospecto_itempdti extends cTable {
	var $nu_prospecto;
	var $nu_itemPdti;
	...
	function __construct() {
		...
		$this->TableVar = 'prospecto_itempdti';
		$this->TableName = 'prospecto_itempdti';
		...
		// nu_prospecto
		$this->nu_prospecto = new cField('prospecto_itempdti', 'prospecto_itempdti', 'x_nu_prospecto', 'nu_prospecto', '[nu_prospecto]', 'CAST([nu_prospecto] AS NVARCHAR)', 3, -1, FALSE, '[nu_prospecto]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->nu_prospecto->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['nu_prospecto'] = &$this->nu_prospecto;

		// nu_itemPdti
		$this->nu_itemPdti = new cField('prospecto_itempdti', 'prospecto_itempdti', 'x_nu_itemPdti', 'nu_itemPdti', '[nu_itemPdti]', 'CAST([nu_itemPdti] AS NVARCHAR)', 3, -1, FALSE, '[nu_itemPdti]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	// Update Table
	var $UpdateTable = "[dbo].[prospecto_itempdti]";
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			...
			if (array_key_exists('nu_itemPdti', $rs))
				ew_AddFilter($where, ew_QuotedName('nu_itemPdti') . '=' . ew_QuotedValue($rs['nu_itemPdti'], $this->nu_itemPdti->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function SqlKeyFilter() {
		return "[nu_prospecto] = @nu_prospecto@ AND [nu_itemPdti] = @nu_itemPdti@";
	}
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->nu_itemPdti->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@nu_itemPdti@", ew_AdjustSql($this->nu_itemPdti->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "prospecto_itempdtilist.php";
		}
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("prospecto_itempdtiview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("prospecto_itempdtiview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl() {
		return "prospecto_itempdtiadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("prospecto_itempdtiedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("prospecto_itempdtiadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("prospecto_itempdtidelete.php", $this->UrlParm());
	}
	...
	function KeyUrl($url, $parm = "") {
		...
		if (!is_null($this->nu_itemPdti->CurrentValue)) {
			$sUrl .= "&nu_itemPdti=" . urlencode($this->nu_itemPdti->CurrentValue);
		} else {
			return "javascript:alert(ewLanguage.Phrase('InvalidRecord'));";
		}
		return $sUrl;
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKey[] = @$_GET["nu_prospecto"]; // nu_prospecto
			$arKey[] = @$_GET["nu_itemPdti"]; // nu_itemPdti
			$arKeys[] = $arKey;
		...
		foreach ($arKeys as $key) {
			...
			if (!is_numeric($key[0])) // nu_prospecto
				continue;
			if (!is_numeric($key[1])) // nu_itemPdti
				continue;
			$ar[] = $key;
		}
		return $ar;
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->nu_prospecto->CurrentValue = $key[0];
			$this->nu_itemPdti->CurrentValue = $key[1];
