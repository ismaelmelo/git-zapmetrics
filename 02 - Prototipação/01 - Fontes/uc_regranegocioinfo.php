...
$uc_regranegocio = NULL;

//
// Table class for uc_regranegocio
//
class cuc_regranegocio extends cTable {
	var $nu_uc;
	var $co_rn;
	...
	function __construct() {
		...
		$this->TableVar = 'uc_regranegocio';
		$this->TableName = 'uc_regranegocio';
		...
		// nu_uc
		$this->nu_uc = new cField('uc_regranegocio', 'uc_regranegocio', 'x_nu_uc', 'nu_uc', '[nu_uc]', 'CAST([nu_uc] AS NVARCHAR)', 3, -1, FALSE, '[EV__nu_uc]', TRUE, TRUE, TRUE, 'FORMATTED TEXT');
		$this->nu_uc->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['nu_uc'] = &$this->nu_uc;

		// co_rn
		$this->co_rn = new cField('uc_regranegocio', 'uc_regranegocio', 'x_co_rn', 'co_rn', '[co_rn]', '[co_rn]', 200, -1, FALSE, '[EV__co_rn]', TRUE, TRUE, TRUE, 'FORMATTED TEXT');
		$this->fields['co_rn'] = &$this->co_rn;
	}
	...
	function GetMasterFilter() {
		...
		// Master filter
		$sMasterFilter = "";
		if ($this->getCurrentMasterTable() == "uc") {
			if ($this->nu_uc->getSessionValue() <> "")
				$sMasterFilter .= "[nu_uc]=" . ew_QuotedValue($this->nu_uc->getSessionValue(), EW_DATATYPE_NUMBER);
			else
				return "";
		}
		if ($this->getCurrentMasterTable() == "corn") {
			if ($this->co_rn->getSessionValue() <> "")
				$sMasterFilter .= "[co_rn]=" . ew_QuotedValue($this->co_rn->getSessionValue(), EW_DATATYPE_STRING);
			else
				return "";
		}
	...
	function GetDetailFilter() {
		...
		if ($this->getCurrentMasterTable() == "uc") {
			if ($this->nu_uc->getSessionValue() <> "")
				$sDetailFilter .= "[nu_uc]=" . ew_QuotedValue($this->nu_uc->getSessionValue(), EW_DATATYPE_NUMBER);
			else
				return "";
		}
		if ($this->getCurrentMasterTable() == "corn") {
			if ($this->co_rn->getSessionValue() <> "")
				$sDetailFilter .= "[co_rn]=" . ew_QuotedValue($this->co_rn->getSessionValue(), EW_DATATYPE_STRING);
			else
				return "";
		}
	...
	// Master filter
	function SqlMasterFilter_uc() {
		return "[nu_uc]=@nu_uc@";
	}

	// Detail filter
	function SqlDetailFilter_uc() {
		return "[nu_uc]=@nu_uc@";
	}
	...
	function SqlSelectList() { // Select for List page
		return "SELECT * FROM (" .
			"SELECT *, (SELECT DISTINCT TOP 1 [co_alternativo] + '" . ew_ValueSeparator(1, $this->nu_uc) . "' + [no_uc] FROM [uc] [EW_TMP_LOOKUPTABLE] WHERE [EW_TMP_LOOKUPTABLE].[nu_uc] = [uc_regranegocio].[nu_uc]) AS [EV__nu_uc], (SELECT DISTINCT TOP 1 [co_alternativo] + '" . ew_ValueSeparator(1, $this->co_rn) . "' + [no_regraNegocio] FROM [regranegocio] [EW_TMP_LOOKUPTABLE] WHERE [EW_TMP_LOOKUPTABLE].[co_alternativo] = [uc_regranegocio].[co_rn]) AS [EV__co_rn] FROM [dbo].[uc_regranegocio]" .
			") [EW_TMP_TABLE]";
	}
	...
	function UseVirtualFields() {
		...
		if ($this->BasicSearch->getKeyword() <> "")
			return TRUE;
		if ($this->co_rn->AdvancedSearch->SearchValue <> "" ||
			$this->co_rn->AdvancedSearch->SearchValue2 <> "" ||
			strpos($sWhere, " " . $this->co_rn->FldVirtualExpression . " ") !== FALSE)
			return TRUE;
		if (strpos($sOrderBy, " " . $this->co_rn->FldVirtualExpression . " ") !== FALSE)
			return TRUE;
		return FALSE;
	}
	...
	// Update Table
	var $UpdateTable = "[dbo].[uc_regranegocio]";
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			...
			if (array_key_exists('co_rn', $rs))
				ew_AddFilter($where, ew_QuotedName('co_rn') . '=' . ew_QuotedValue($rs['co_rn'], $this->co_rn->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function SqlKeyFilter() {
		return "[nu_uc] = @nu_uc@ AND [co_rn] = '@co_rn@'";
	}
	...
	function KeyFilter() {
		...
		$sKeyFilter = str_replace("@co_rn@", ew_AdjustSql($this->co_rn->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "uc_regranegociolist.php";
		}
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("uc_regranegocioview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("uc_regranegocioview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl() {
		return "uc_regranegocioadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("uc_regranegocioedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("uc_regranegocioadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("uc_regranegociodelete.php", $this->UrlParm());
	}
	...
	function KeyUrl($url, $parm = "") {
		...
		if (!is_null($this->co_rn->CurrentValue)) {
			$sUrl .= "&co_rn=" . urlencode($this->co_rn->CurrentValue);
		} else {
			return "javascript:alert(ewLanguage.Phrase('InvalidRecord'));";
		}
		return $sUrl;
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKey[] = @$_GET["nu_uc"]; // nu_uc
			$arKey[] = @$_GET["co_rn"]; // co_rn
			$arKeys[] = $arKey;
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->nu_uc->CurrentValue = $key[0];
			$this->co_rn->CurrentValue = $key[1];
   ...
   // Common render codes
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				...
				if ($this->co_rn->Exportable) $Doc->ExportCaption($this->co_rn);
			} else {
				if ($this->nu_uc->Exportable) $Doc->ExportCaption($this->nu_uc);
				if ($this->co_rn->Exportable) $Doc->ExportCaption($this->co_rn);
			}
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					...
					if ($this->co_rn->Exportable) $Doc->ExportField($this->co_rn);
				} else {
					if ($this->nu_uc->Exportable) $Doc->ExportField($this->nu_uc);
					if ($this->co_rn->Exportable) $Doc->ExportField($this->co_rn);
				}
	...
	function Row_Rendered() {
		...
		if ($this->co_rn->ViewValue == 1) {               
			$this->co_rn->EditValue = "aaa";
		}                           
	}                 
