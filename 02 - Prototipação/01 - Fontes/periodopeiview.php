...
$periodopei_view = NULL; // Initialize page object first

class cperiodopei_view extends cperiodopei {
	...
	// Page object name
	var $PageObjName = 'periodopei_view';
	...
	function __construct() {
		...
		if (@$_GET["nu_periodoPei"] <> "") {
			$this->RecKey["nu_periodoPei"] = $_GET["nu_periodoPei"];
			$KeyUrl .= "&nu_periodoPei=" . urlencode($this->RecKey["nu_periodoPei"]);
		}
	...
	function Page_Init() {
		...
		if (@$_GET["nu_periodoPei"] <> "") {
			...
			$gsExportFile .= ew_StripSlashes($_GET["nu_periodoPei"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["nu_periodoPei"] <> "") {
				...
				$this->RecKey["nu_periodoPei"] = $this->nu_periodoPei->QueryStringValue;
			} else {
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					...
					if ($this->TotalRecs <= 0) { // No record found
						...
						$this->Page_Terminate("periodopeilist.php"); // Return to list page
						...
						while (!$this->Recordset->EOF) {
							if (strval($this->nu_periodoPei->CurrentValue) == strval($this->Recordset->fields('nu_periodoPei'))) {
					...
					if (!$bMatchRecord) {
						if ($this->getSuccessMessage() == "" && $this->getFailureMessage() == "")
							$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record message
						$sReturnUrl = "periodopeilist.php"; // No matching record, return to list
					} else {
			...
			$sReturnUrl = "periodopeilist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a id=\"emf_periodopei\" href=\"javascript:void(0);\" class=\"ewExportLink ewEmail\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_periodopei',hdr:ewLanguage.Phrase('ExportToEmail'),f:document.fperiodopeiview,key:" . ew_ArrayToJsonAttr($this->RecKey) . ",sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
...
// Create page object
if (!isset($periodopei_view)) $periodopei_view = new cperiodopei_view();

// Page init
$periodopei_view->Page_Init();

// Page main
$periodopei_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$periodopei_view->Page_Render();
?>
...
// Page object
var periodopei_view = new ew_Page("periodopei_view");
periodopei_view.PageID = "view"; // Page ID
var EW_PAGE_ID = periodopei_view.PageID; // For backward compatibility

// Form object
var fperiodopeiview = new ew_Form("fperiodopeiview");

// Form_CustomValidate event
fperiodopeiview.Form_CustomValidate = 
...
fperiodopeiview.ValidateRequired = true;
<?php } else { ?>
fperiodopeiview.ValidateRequired = false; 
<?php } ?>
...
<?php $periodopei_view->ExportOptions->Render("body") ?>
<?php if (!$periodopei_view->ExportOptions->UseDropDownButton) { ?>
</div>
<div class="ewViewOtherOptions">
<?php } ?>
<?php
	foreach ($periodopei_view->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<?php } ?>
<?php $periodopei_view->ShowPageHeader(); ?>
<?php
$periodopei_view->ShowMessage();
?>
<form name="fperiodopeiview" id="fperiodopeiview" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="periodopei">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_periodopeiview" class="table table-bordered table-striped">
...
<?php echo $periodopei->nu_periodoPei->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($periodopei->nu_anoInicio->Visible) { // nu_anoInicio ?>
	<tr id="r_nu_anoInicio">
		<td><span id="elh_periodopei_nu_anoInicio"><?php echo $periodopei->nu_anoInicio->FldCaption() ?></span></td>
...
<?php echo $periodopei->nu_anoInicio->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($periodopei->nu_anoFim->Visible) { // nu_anoFim ?>
	<tr id="r_nu_anoFim">
		<td><span id="elh_periodopei_nu_anoFim"><?php echo $periodopei->nu_anoFim->FldCaption() ?></span></td>
...
<?php echo $periodopei->nu_anoFim->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php echo $periodopei->no_periodo->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($periodopei->ic_situacao->Visible) { // ic_situacao ?>
	<tr id="r_ic_situacao">
		<td><span id="elh_periodopei_ic_situacao"><?php echo $periodopei->ic_situacao->FldCaption() ?></span></td>
...
<?php echo $periodopei->ic_situacao->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<table class="ewPager">
<tr><td>
<?php if (!isset($periodopei_view->Pager)) $periodopei_view->Pager = new cNumericPager($periodopei_view->StartRec, $periodopei_view->DisplayRecs, $periodopei_view->TotalRecs, $periodopei_view->RecRange) ?>
<?php if ($periodopei_view->Pager->RecordCount > 0) { ?>
<table cellspacing="0" class="ewStdTable"><tbody><tr><td>
<div class="pagination"><ul>
	<?php if ($periodopei_view->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $periodopei_view->PageUrl() ?>start=<?php echo $periodopei_view->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($periodopei_view->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $periodopei_view->PageUrl() ?>start=<?php echo $periodopei_view->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($periodopei_view->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $periodopei_view->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($periodopei_view->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $periodopei_view->PageUrl() ?>start=<?php echo $periodopei_view->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($periodopei_view->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $periodopei_view->PageUrl() ?>start=<?php echo $periodopei_view->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</td>
</tr></tbody></table>
...
<script type="text/javascript">
fperiodopeiview.Init();
</script>
<?php
$periodopei_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($periodopei->Export == "") { ?>
...
<?php
$periodopei_view->Page_Terminate();
?>
