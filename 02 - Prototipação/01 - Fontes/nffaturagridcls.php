...
$nffatura_grid = NULL; // Initialize page object first

class cnffatura_grid extends cnffatura {
	...
	// Page object name
	var $PageObjName = 'nffatura_grid';

	// Grid form hidden field names
	var $FormName = 'fnffaturagrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["nffatura"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->nu_nfFatura->CurrentValue;
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_nu_notaFiscal") && $objForm->HasValue("o_nu_notaFiscal") && $this->nu_notaFiscal->CurrentValue <> $this->nu_notaFiscal->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->nu_nfFatura->CurrentValue . "\">";
		}
	...
	function LoadDefaultValues() {
		$this->nu_notaFiscal->CurrentValue = NULL;
		$this->nu_notaFiscal->OldValue = $this->nu_notaFiscal->CurrentValue;
	}
	...
	function LoadFormValues() {
		...
		$this->nu_notaFiscal->setOldValue($objForm->GetValue("o_nu_notaFiscal"));
		if (!$this->nu_nfFatura->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->nu_nfFatura->CurrentValue = strval($arKeys[0]); // nu_nfFatura
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			if (strval($this->nu_notaFiscal->EditValue) <> "" && is_numeric($this->nu_notaFiscal->EditValue)) {
			$this->nu_notaFiscal->EditValue = ew_FormatNumber($this->nu_notaFiscal->EditValue, -2, -1, -2, 0);
			$this->nu_notaFiscal->OldValue = $this->nu_notaFiscal->EditValue;
			}
	...
	function AddRow($rsold = NULL) {
		...
		// nu_notaFiscal
		$this->nu_notaFiscal->SetDbValueDef($rsnew, $this->nu_notaFiscal->CurrentValue, 0, FALSE);
		...
		if ($AddRow) {
			$this->nu_nfFatura->setDbValue($conn->Insert_ID());
			$rsnew['nu_nfFatura'] = $this->nu_nfFatura->DbValue;
		}
		if ($AddRow) {
