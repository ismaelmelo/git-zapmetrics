...
$regranegocio_grid = NULL; // Initialize page object first

class cregranegocio_grid extends cregranegocio {
	...
	// Page object name
	var $PageObjName = 'regranegocio_grid';

	// Grid form hidden field names
	var $FormName = 'fregranegociogrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["regranegocio"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->co_alternativo->CurrentValue;
					if ($sKey <> "") $sKey .= $GLOBALS["EW_COMPOSITE_KEY_SEPARATOR"];
	...
	function EmptyRow() {
		...
		if ($objForm->HasValue("x_nu_versao") && $objForm->HasValue("o_nu_versao") && $this->nu_versao->CurrentValue <> $this->nu_versao->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_no_regraNegocio") && $objForm->HasValue("o_no_regraNegocio") && $this->no_regraNegocio->CurrentValue <> $this->no_regraNegocio->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_nu_projeto") && $objForm->HasValue("o_nu_projeto") && $this->nu_projeto->CurrentValue <> $this->nu_projeto->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_nu_stRegraNegocio") && $objForm->HasValue("o_nu_stRegraNegocio") && $this->nu_stRegraNegocio->CurrentValue <> $this->nu_stRegraNegocio->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->co_alternativo->CurrentValue . $GLOBALS["EW_COMPOSITE_KEY_SEPARATOR"] . $this->nu_versao->CurrentValue . "\">";
		}
	...
	function LoadFormValues() {
		...
		$this->no_regraNegocio->setOldValue($objForm->GetValue("o_no_regraNegocio"));
		...
		$this->nu_stRegraNegocio->setOldValue($objForm->GetValue("o_nu_stRegraNegocio"));
		if (!$this->nu_usuario->FldIsDetailKey) {
			$this->nu_usuario->setFormValue($objForm->GetValue("x_nu_usuario"));
		}
		$this->nu_usuario->setOldValue($objForm->GetValue("o_nu_usuario"));
		...
		$this->dt_versao->setOldValue($objForm->GetValue("o_dt_versao"));
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 2) {
			if (strval($arKeys[0]) <> "")
				$this->co_alternativo->CurrentValue = strval($arKeys[0]); // co_alternativo
			else
				$bValidKey = FALSE;
			if (strval($arKeys[1]) <> "")
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// no_regraNegocio
			$this->no_regraNegocio->SetDbValueDef($rsnew, $this->no_regraNegocio->CurrentValue, NULL, $this->no_regraNegocio->ReadOnly);

			// nu_projeto
			$this->nu_projeto->SetDbValueDef($rsnew, $this->nu_projeto->CurrentValue, NULL, $this->nu_projeto->ReadOnly);

			// nu_stRegraNegocio
			$this->nu_stRegraNegocio->SetDbValueDef($rsnew, $this->nu_stRegraNegocio->CurrentValue, NULL, $this->nu_stRegraNegocio->ReadOnly);
	...
	function AddRow($rsold = NULL) {
		...
		if ($this->co_alternativo->getSessionValue() <> "") {
			$rsnew['co_alternativo'] = $this->co_alternativo->getSessionValue();
		}
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "corn") {
			$this->co_alternativo->Visible = FALSE;
