...
$oc_delete = NULL; // Initialize page object first

class coc_delete extends coc {
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("oclist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in oc class, ocinfo.php
...
// Create page object
if (!isset($oc_delete)) $oc_delete = new coc_delete();
...
$oc_delete->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var oc_delete = new ew_Page("oc_delete");
oc_delete.PageID = "delete"; // Page ID
var EW_PAGE_ID = oc_delete.PageID; // For backward compatibility

// Form object
var focdelete = new ew_Form("focdelete");

// Form_CustomValidate event
focdelete.Form_CustomValidate = 
...
focdelete.ValidateRequired = true;
<?php } else { ?>
focdelete.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
focdelete.Lists["x_nu_stOc"] = {"LinkField":"x_nu_stOc","Ajax":null,"AutoFill":false,"DisplayFields":["x_no_stOc","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
if ($oc_delete->Recordset = $oc_delete->LoadRecordset())
	$oc_deleteTotalRecs = $oc_delete->Recordset->RecordCount(); // Get record count
if ($oc_deleteTotalRecs <= 0) { // No record found, exit
	if ($oc_delete->Recordset)
		$oc_delete->Recordset->Close();
	$oc_delete->Page_Terminate("oclist.php"); // Return to list
}
?>
<?php $Breadcrumb->Render(); ?>
<?php $oc_delete->ShowPageHeader(); ?>
<?php
$oc_delete->ShowMessage();
?>
<form name="focdelete" id="focdelete" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="oc">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($oc_delete->RecKeys as $key) { ?>
<?php $keyvalue = is_array($key) ? implode($EW_COMPOSITE_KEY_SEPARATOR, $key) : $key; ?>
<input type="hidden" name="key_m[]" value="<?php echo ew_HtmlEncode($keyvalue) ?>">
<?php } ?>
<table cellspacing="0" class="ewGrid"><tr><td class="ewGridContent">
<div class="ewGridMiddlePanel">
<table id="tbl_ocdelete" class="ewTable ewTableSeparate">
<?php echo $oc->TableCustomInnerHtml ?>
	<thead>
	<tr class="ewTableHeader">
		<td><span id="elh_oc_ic_tpOc" class="oc_ic_tpOc"><?php echo $oc->ic_tpOc->FldCaption() ?></span></td>
		<td><span id="elh_oc_co_alternativo" class="oc_co_alternativo"><?php echo $oc->co_alternativo->FldCaption() ?></span></td>
		<td><span id="elh_oc_dt_oc" class="oc_dt_oc"><?php echo $oc->dt_oc->FldCaption() ?></span></td>
		<td><span id="elh_oc_nu_stOc" class="oc_nu_stOc"><?php echo $oc->nu_stOc->FldCaption() ?></span></td>
	</tr>
	</thead>
	<tbody>
<?php
$oc_delete->RecCnt = 0;
$i = 0;
while (!$oc_delete->Recordset->EOF) {
	$oc_delete->RecCnt++;
	$oc_delete->RowCnt++;

	// Set row properties
	$oc->ResetAttrs();
	$oc->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$oc_delete->LoadRowValues($oc_delete->Recordset);
...
<span id="el<?php echo $oc_delete->RowCnt ?>_oc_ic_tpOc" class="control-group oc_ic_tpOc">
...
<span id="el<?php echo $oc_delete->RowCnt ?>_oc_co_alternativo" class="control-group oc_co_alternativo">
...
<span id="el<?php echo $oc_delete->RowCnt ?>_oc_dt_oc" class="control-group oc_dt_oc">
<span<?php echo $oc->dt_oc->ViewAttributes() ?>>
<?php echo $oc->dt_oc->ListViewValue() ?></span>
</span>
</td>
		<td<?php echo $oc->nu_stOc->CellAttributes() ?>>
<span id="el<?php echo $oc_delete->RowCnt ?>_oc_nu_stOc" class="control-group oc_nu_stOc">
