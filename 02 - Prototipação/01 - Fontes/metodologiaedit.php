...
$metodologia_edit = NULL; // Initialize page object first

class cmetodologia_edit extends cmetodologia {
	...
	// Page object name
	var $PageObjName = 'metodologia_edit';
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->nu_metodologia->CurrentValue == "")
			$this->Page_Terminate("metodologialist.php"); // Invalid key, return to list
	...
	function ValidateForm() {
		...
		if (in_array("roteiro", $DetailTblVar) && $GLOBALS["roteiro"]->DetailEdit) {
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// no_metodologia
			$this->no_metodologia->SetDbValueDef($rsnew, $this->no_metodologia->CurrentValue, "", $this->no_metodologia->ReadOnly);

			// ds_metodologia
			$this->ds_metodologia->SetDbValueDef($rsnew, $this->ds_metodologia->CurrentValue, NULL, $this->ds_metodologia->ReadOnly);

			// ic_tpModeloDev
			$this->ic_tpModeloDev->SetDbValueDef($rsnew, $this->ic_tpModeloDev->CurrentValue, NULL, $this->ic_tpModeloDev->ReadOnly);
			...
			if ($bUpdateRow) {
				...
				if ($EditRow) {
					...
					if (in_array("roteiro", $DetailTblVar) && $GLOBALS["roteiro"]->DetailEdit) {
						if (!isset($GLOBALS["roteiro_grid"])) $GLOBALS["roteiro_grid"] = new croteiro_grid(); // Get detail page object
						$EditRow = $GLOBALS["roteiro_grid"]->GridUpdate();
					}
				}
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("roteiro", $DetailTblVar)) {
				...
				if ($GLOBALS["roteiro_grid"]->DetailEdit) {
					$GLOBALS["roteiro_grid"]->CurrentMode = "edit";
					$GLOBALS["roteiro_grid"]->CurrentAction = "gridedit";
...
// Create page object
if (!isset($metodologia_edit)) $metodologia_edit = new cmetodologia_edit();

// Page init
$metodologia_edit->Page_Init();

// Page main
$metodologia_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$metodologia_edit->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var metodologia_edit = new ew_Page("metodologia_edit");
metodologia_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = metodologia_edit.PageID; // For backward compatibility

// Form object
var fmetodologiaedit = new ew_Form("fmetodologiaedit");

// Validate form
fmetodologiaedit.Validate = function() {
...
fmetodologiaedit.Form_CustomValidate = 
...
fmetodologiaedit.ValidateRequired = true;
<?php } else { ?>
fmetodologiaedit.ValidateRequired = false; 
<?php } ?>
...
<?php $metodologia_edit->ShowPageHeader(); ?>
<?php
$metodologia_edit->ShowMessage();
?>
<form name="fmetodologiaedit" id="fmetodologiaedit" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="metodologia">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_metodologiaedit" class="table table-bordered table-striped">
...
<input type="hidden" data-field="x_nu_metodologia" name="x_nu_metodologia" id="x_nu_metodologia" value="<?php echo ew_HtmlEncode($metodologia->nu_metodologia->CurrentValue) ?>">
<?php
	if (in_array("roteiro", explode(",", $metodologia->getCurrentDetailTable())) && $roteiro->DetailEdit) {
?>
...
<script type="text/javascript">
fmetodologiaedit.Init();
<?php if (EW_MOBILE_REFLOW && ew_IsMobile()) { ?>
ew_Reflow();
<?php } ?>
</script>
<?php
$metodologia_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$metodologia_edit->Page_Terminate();
?>
