...
$ativroteiro_grid = NULL; // Initialize page object first

class cativroteiro_grid extends cativroteiro {
	...
	// Page object name
	var $PageObjName = 'ativroteiro_grid';

	// Grid form hidden field names
	var $FormName = 'fativroteirogrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["ativroteiro"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->nu_ativRoteiro->CurrentValue;
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_no_ativRoteiro") && $objForm->HasValue("o_no_ativRoteiro") && $this->no_ativRoteiro->CurrentValue <> $this->no_ativRoteiro->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_pc_distribuicao") && $objForm->HasValue("o_pc_distribuicao") && $this->pc_distribuicao->CurrentValue <> $this->pc_distribuicao->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_ic_ativo") && $objForm->HasValue("o_ic_ativo") && $this->ic_ativo->CurrentValue <> $this->ic_ativo->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_nu_ordem") && $objForm->HasValue("o_nu_ordem") && $this->nu_ordem->CurrentValue <> $this->nu_ordem->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->nu_ativRoteiro->CurrentValue . "\">";
		}
	...
	function LoadFormValues() {
		...
		$this->no_ativRoteiro->setOldValue($objForm->GetValue("o_no_ativRoteiro"));
		if (!$this->pc_distribuicao->FldIsDetailKey) {
			$this->pc_distribuicao->setFormValue($objForm->GetValue("x_pc_distribuicao"));
		}
		$this->pc_distribuicao->setOldValue($objForm->GetValue("o_pc_distribuicao"));
		...
		$this->nu_ordem->setOldValue($objForm->GetValue("o_nu_ordem"));
		if (!$this->nu_ativRoteiro->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->nu_ativRoteiro->CurrentValue = strval($arKeys[0]); // nu_ativRoteiro
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			if (strval($this->pc_distribuicao->EditValue) <> "" && is_numeric($this->pc_distribuicao->EditValue)) {
			$this->pc_distribuicao->EditValue = ew_FormatNumber($this->pc_distribuicao->EditValue, -2, 0, 0, 0);
			$this->pc_distribuicao->OldValue = $this->pc_distribuicao->EditValue;
			}
	...
	function AddRow($rsold = NULL) {
		...
		if ($this->nu_faseRoteiro->getSessionValue() <> "") {
			$sMasterFilter = str_replace("@nu_faseRoteiro@", ew_AdjustSql($this->nu_faseRoteiro->getSessionValue()), $sMasterFilter);
		} else {
			$bValidMasterRecord = FALSE;
		}
		if ($bValidMasterRecord) {
		...
		// nu_faseRoteiro
		if ($this->nu_faseRoteiro->getSessionValue() <> "") {
			$rsnew['nu_faseRoteiro'] = $this->nu_faseRoteiro->getSessionValue();
		}
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "faseroteiro") {
			$this->nu_faseRoteiro->Visible = FALSE;
			if ($GLOBALS["faseroteiro"]->EventCancelled) $this->EventCancelled = TRUE;
		}
