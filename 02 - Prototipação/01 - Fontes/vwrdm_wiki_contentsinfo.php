...
$vwrdm_wiki_contents = NULL;

//
// Table class for vwrdm_wiki_contents
//
class cvwrdm_wiki_contents extends cTable {
	var $id;
	var $page_id;
	var $author_id;
	var $text;
	...
	function __construct() {
		...
		$this->TableVar = 'vwrdm_wiki_contents';
		$this->TableName = 'vwrdm_wiki_contents';
		$this->TableType = 'VIEW';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new cField('vwrdm_wiki_contents', 'vwrdm_wiki_contents', 'x_id', 'id', '[id]', 'CAST([id] AS NVARCHAR)', 3, -1, FALSE, '[id]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// page_id
		$this->page_id = new cField('vwrdm_wiki_contents', 'vwrdm_wiki_contents', 'x_page_id', 'page_id', '[page_id]', 'CAST([page_id] AS NVARCHAR)', 3, -1, FALSE, '[page_id]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->page_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['page_id'] = &$this->page_id;

		// author_id
		$this->author_id = new cField('vwrdm_wiki_contents', 'vwrdm_wiki_contents', 'x_author_id', 'author_id', '[author_id]', 'CAST([author_id] AS NVARCHAR)', 3, -1, FALSE, '[author_id]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->author_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['author_id'] = &$this->author_id;

		// text
		$this->text = new cField('vwrdm_wiki_contents', 'vwrdm_wiki_contents', 'x_text', 'text', '[text]', '[text]', 203, -1, FALSE, '[text]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['text'] = &$this->text;

		// comments
		$this->comments = new cField('vwrdm_wiki_contents', 'vwrdm_wiki_contents', 'x_comments', 'comments', '[comments]', '[comments]', 202, -1, FALSE, '[comments]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['comments'] = &$this->comments;

		// updated_on
		$this->updated_on = new cField('vwrdm_wiki_contents', 'vwrdm_wiki_contents', 'x_updated_on', 'updated_on', '[updated_on]', '(REPLACE(STR(DAY([updated_on]),2,0),\' \',\'0\') + \'/\' + REPLACE(STR(MONTH([updated_on]),2,0),\' \',\'0\') + \'/\' + STR(YEAR([updated_on]),4,0))', 135, 7, FALSE, '[updated_on]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->updated_on->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateDMY"));
		$this->fields['updated_on'] = &$this->updated_on;

		// version
		$this->version = new cField('vwrdm_wiki_contents', 'vwrdm_wiki_contents', 'x_version', 'version', '[version]', 'CAST([version] AS NVARCHAR)', 3, -1, FALSE, '[version]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function SqlFrom() { // From
		return "[db_owner].[vwrdm_wiki_contents]";
	}
	...
	// Update Table
	var $UpdateTable = "[db_owner].[vwrdm_wiki_contents]";
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "vwrdm_wiki_contentslist.php";
		}
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("vwrdm_wiki_contentsview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("vwrdm_wiki_contentsview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl() {
		return "vwrdm_wiki_contentsadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("vwrdm_wiki_contentsedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("vwrdm_wiki_contentsadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("vwrdm_wiki_contentsdelete.php", $this->UrlParm());
	}
