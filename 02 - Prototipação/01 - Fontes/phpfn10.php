...
function CurrentProjectID() {
	...
	return "{F59C7BF3-F287-4BE0-9F86-FCB94F808AF8}";
}
...
class cBreadcrumb {
	...
	function __construct() {
		global $Language;
		$this->Links[] = array("home", $Language->Phrase("HomePage"), "custom.php", ""); // Home
...
class cAdvancedSecurity {
	...
	function ValidateUser(&$usr, &$pwd, $autologin) {
		...
		if (!$ValidateUser) {
			...
			if ($rs = $conn->Execute($sSql)) {
				if (!$rs->EOF) {
					$ValidateUser = $CustomValidateUser || ew_ComparePassword($rs->fields('no_senha'), $pwd);
					...
					if ($ValidateUser) {
						...
						$this->setCurrentUserName($rs->fields('no_login')); // Load user name
						$this->setSessionUserID($rs->fields('nu_usuario')); // Load User ID
						if (is_null($rs->fields('nu_perfil'))) {
							$this->setSessionUserLevelID(0);
						} else {
							$this->setSessionUserLevelID(intval($rs->fields('nu_perfil'))); // Load User Level
						}
						$this->SetUpUserLevel();
	...
	function SetUpUserLevelEx() {
		...
		if (count($this->UserLevelPriv) == 0 && $this->IsAdmin() && $Page != NULL && @$_SESSION[EW_SESSION_USER_LEVEL_MSG] == "") {
			...
			$Page->Page_Terminate("usuario_permissoeslist.php");
		}
		return TRUE;
	}
	...
	function CurrentUserEmail() {
		return $this->CurrentUserInfo("no_email");
	}
	...
	function GetUserIDByUserName($UserName) {
		...
		if (strval($UserName) <> "") {
			...
			$sSql = $usuario->GetSQL($sFilter, '');
			if (($RsUser = $conn->Execute($sSql)) && !$RsUser->EOF) {
				$UserID = $RsUser->fields('nu_usuario');
				$RsUser->Close();
				return $UserID;
			}
		}
	...
	function LoadUserID() {
		...
		if (strval($this->CurrentUserID) == "") {
			...
			$sFilter = $usuario->UserIDFilter($this->CurrentUserID);
			$sSql = $usuario->GetSQL($sFilter, '');
			if ($RsUser = $conn->Execute($sSql)) {
				while (!$RsUser->EOF) {
					$this->AddUserID($RsUser->fields('nu_usuario'));
					$RsUser->MoveNext();
				}
				$RsUser->Close();
			}
...
function &ew_Connect($info = NULL) {
	...
	if (!$info) {
		$info = "Provider=SQLNCLI11;Persist Security Info=False;Data Source=NOTE-HPDV2135\\ZAPMETRICS;Initial Catalog=cdoc;User Id=badadm;Password=eufrabad01;DataTypeCompatibility=80"; // ADO connection string
	}
...
class cUserProfile {
	...
	function SaveProfileToDatabase($usr) {
		...
		if ($usr == "" || $usr == EW_ADMIN_USER_NAME) // Ignore hard code admin
			return FALSE;
		$sFilter = str_replace("%u", ew_AdjustSql($usr), EW_USER_NAME_FILTER);
		$sSql = "UPDATE " . ew_QuotedName($usuario->TableName) .
