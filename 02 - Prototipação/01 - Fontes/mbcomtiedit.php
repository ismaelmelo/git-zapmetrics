...
$mbcomti_edit = NULL; // Initialize page object first

class cmbcomti_edit extends cmbcomti {
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->nu_recursoHum->CurrentValue == "")
			$this->Page_Terminate("mbcomtilist.php"); // Invalid key, return to list
	...
	function LoadFormValues() {
		...
		// Load from form
		global $objForm;
		if (!$this->nu_recursoHum->FldIsDetailKey)
			$this->nu_recursoHum->setFormValue($objForm->GetValue("x_nu_recursoHum"));
	...
	function RestoreFormValues() {
		global $objForm;
		$this->LoadRow();
		$this->nu_recursoHum->CurrentValue = $this->nu_recursoHum->FormValue;
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			// nu_recursoHum
			$this->nu_recursoHum->EditCustomAttributes = "";
			$this->nu_recursoHum->EditValue = $this->nu_recursoHum->CurrentValue;
			$this->nu_recursoHum->ViewCustomAttributes = "";
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// nu_organizacao
			$this->nu_organizacao->SetDbValueDef($rsnew, $this->nu_organizacao->CurrentValue, NULL, $this->nu_organizacao->ReadOnly);

			// nu_papelMembro
			$this->nu_papelMembro->SetDbValueDef($rsnew, $this->nu_papelMembro->CurrentValue, NULL, $this->nu_papelMembro->ReadOnly);
...
// Create page object
if (!isset($mbcomti_edit)) $mbcomti_edit = new cmbcomti_edit();

// Page init
$mbcomti_edit->Page_Init();

// Page main
$mbcomti_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$mbcomti_edit->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var mbcomti_edit = new ew_Page("mbcomti_edit");
mbcomti_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = mbcomti_edit.PageID; // For backward compatibility

// Form object
var fmbcomtiedit = new ew_Form("fmbcomtiedit");

// Validate form
fmbcomtiedit.Validate = function() {
...
fmbcomtiedit.Form_CustomValidate = 
...
fmbcomtiedit.ValidateRequired = true;
<?php } else { ?>
fmbcomtiedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fmbcomtiedit.Lists["x_nu_pessoa"] = {"LinkField":"x_nu_pessoa","Ajax":null,"AutoFill":false,"DisplayFields":["x_no_pessoa","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fmbcomtiedit.Lists["x_nu_organizacao"] = {"LinkField":"x_nu_organizacao","Ajax":null,"AutoFill":false,"DisplayFields":["x_no_organizacao","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fmbcomtiedit.Lists["x_nu_papelMembro"] = {"LinkField":"x_co_papel","Ajax":null,"AutoFill":false,"DisplayFields":["x_no_papel","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $mbcomti_edit->ShowPageHeader(); ?>
<?php
$mbcomti_edit->ShowMessage();
?>
<form name="fmbcomtiedit" id="fmbcomtiedit" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="mbcomti">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_mbcomtiedit" class="table table-bordered table-striped">
...
<?php echo $mbcomti->nu_recursoHum->EditValue ?></span>
</span>
<input type="hidden" data-field="x_nu_recursoHum" name="x_nu_recursoHum" id="x_nu_recursoHum" value="<?php echo ew_HtmlEncode($mbcomti->nu_recursoHum->CurrentValue) ?>">
<?php echo $mbcomti->nu_recursoHum->CustomMsg ?></td>
	</tr>
<?php } ?>
...
<?php } ?>
<script type="text/javascript">
fmbcomtiedit.Lists["x_nu_pessoa"].Options = <?php echo (is_array($mbcomti->nu_pessoa->EditValue)) ? ew_ArrayToJson($mbcomti->nu_pessoa->EditValue, 1) : "[]" ?>;
</script>
</span>
...
<script type="text/javascript">
fmbcomtiedit.Lists["x_nu_organizacao"].Options = <?php echo (is_array($mbcomti->nu_organizacao->EditValue)) ? ew_ArrayToJson($mbcomti->nu_organizacao->EditValue, 1) : "[]" ?>;
</script>
</span>
...
<?php } ?>
<script type="text/javascript">
fmbcomtiedit.Lists["x_nu_papelMembro"].Options = <?php echo (is_array($mbcomti->nu_papelMembro->EditValue)) ? ew_ArrayToJson($mbcomti->nu_papelMembro->EditValue, 1) : "[]" ?>;
</script>
</span>
...
ew_CreateCalendar("fmbcomtiedit", "x_dt_inicio", "%d/%m/%Y");
</script>
<?php } ?>
</span>
...
ew_CreateCalendar("fmbcomtiedit", "x_dt_fim", "%d/%m/%Y");
</script>
<?php } ?>
</span>
